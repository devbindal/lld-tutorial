...
const QUESTIONS = [
  { q: 'The Decorator pattern\'s intent is…',
    o: ['To guarantee one instance', 'To translate one interface into another', 'To attach additional responsibilities to an object DYNAMICALLY, keeping the same interface — a flexible alternative to subclassing', 'To copy expensive objects'], a: 2,
    ...
    w: { 0: 'One instance is Singleton.',
         1: 'Interface translation is Adapter (Day 26).',
  ...
  { q: 'Why is "same interface as the thing it wraps" the load-bearing rule?',
    o: ['It improves performance', 'Java requires it', 'It saves typing', 'It makes decorators STACKABLE: a decorator can wrap a decorator can wrap the component — any depth, assembled at runtime'], a: 3,
    ...
    w: { 0: 'Each layer adds a tiny call cost, if anything.',
         1: 'No language rule — a design necessity.',
         2: 'Typing is irrelevant.' },
    r: { id: 's6', label: 'Section 6 — why it stacks' } },
  { q: 'The abstract decorator base class typically…',
    o: ['has no relation to Component', 'is a static utility', 'implements the Component interface AND holds a Component field, forwarding calls to it', 'extends the concrete component'], a: 2,
    ...
    w: { 0: 'Both relations to Component are the whole trick.',
         1: 'Static utilities transform values; decorators stand in as objects.',
         3: 'Extending Espresso would tie decorators to ONE concrete drink.' },
    r: { id: 's4', label: 'Section 4 — the structure' } },
  { q: 'How does cost() compute ₹155 for Whip(Mocha(Espresso))?',
    o: ['Each decorator returns its own delta PLUS inner.cost() — the call recurses inward, bottoms out at Espresso, and sums on the way back', 'The outermost layer stores the total', 'The JVM adds them', 'A static price table'], a: 0,
    e: '25 + (30 + (80)) — a recursive unwind down the wrapper chain. No layer knows the total; each knows only its delta and its neighbor.',
    w: { 1: 'No layer knows the whole — that\'s the elegance.',
         2: 'The JVM executes; YOUR delegation chain adds.',
         3: 'A table would have to enumerate every combo — the explosion again.' },
  ...
  { q: 'In new BufferedReader(new InputStreamReader(socket.getInputStream())) — identify the patterns.',
    o: ['Two adapters', 'InputStreamReader is an ADAPTER (bytes→chars interface change); BufferedReader is a DECORATOR (same Reader interface, adds buffering + readLine)', 'Two decorators', 'A builder and a factory'], a: 1,
    ...
    w: { 0: 'BufferedReader keeps Reader and adds powers — decoration.',
         2: 'InputStreamReader CHANGES the interface — that\'s adaptation.',
  ...
  { q: 'Decorator vs Proxy — both wrap with the SAME interface. The difference?',
    o: ['Intent: decorator ADDS capabilities and clients knowingly stack them; proxy CONTROLS ACCESS (guard/cache/lazy) and clients usually don\'t know it\'s there', 'No difference', 'Decorators are only for streams', 'Proxies are faster'], a: 0,
    ...
    w: { 1: 'GoF lists them separately for intent reasons.',
         2: 'Streams are just the famous showcase.',
         3: 'Speed isn\'t the axis.' },
    r: { id: 's8', label: 'Section 8 — the family table' } },
  { q: 'After Coffee c = new Mocha(espresso); — what breaks about identity?',
    o: ['Nothing ever', 'c != espresso, c.equals(espresso) is false by default, and instanceof Espresso fails — wrapped objects are DIFFERENT objects, so identity-based lookups miss', 'Mocha and Espresso merge', 'The GC collects espresso'], a: 1,
    ...
    w: { 0: 'Identity is THE classic decorator gotcha.',
         2: 'Objects never merge.',
         3: 'espresso lives on — referenced by the wrapper.' },
  ...
  { q: 'Collections.unmodifiableList(list) is a decorator that THROWS on add(). Healthy or a Day 13 crime?',
    o: ['It silently ignores add()', 'A crime — delete it from the JDK', 'Not a decorator at all', 'A documented trade-off: it\'s an intentional read-only VIEW, with the same optional-operations escape hatch as Arrays.asList — know both the utility and the LSP criticism'], a: 3,
    ...
    w: { 0: 'It throws loudly — silent ignoring would be far worse.',
         1: 'It\'s enormously useful — defensive views everywhere (Day 2!).',

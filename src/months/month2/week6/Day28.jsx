...
const QUESTIONS = [
  { q: 'The Composite pattern\'s intent is…',
    o: ['To compose objects into TREE structures (part-whole hierarchies) and let clients treat single objects and whole trees uniformly', 'To convert interfaces', 'To limit a class to one instance', 'To add behavior to objects dynamically'], a: 0,
    ...
    w: { 1: 'Interface conversion is Adapter (Day 26).',
         2: 'One instance is Singleton (Day 21).',
         3: 'Dynamic behavior-adding is Decorator (Day 27).' },
    r: { id: 's1', label: 'Section 1 — the folder question' } },
  { q: 'Name the three GoF participants and the composite\'s defining field.',
    o: ['Target, Adaptee, Adapter', 'Builder, Director, Product', 'Component (the shared interface), Leaf (no children), Composite — which holds List<Component>, so it can contain leaves AND other composites', 'Subject, Observer, Event'], a: 2,
    ...
    w: { 0: 'That\'s Adapter\'s cast (Day 26).',
         1: 'That\'s Builder\'s cast (Day 24).',
         3: 'That\'s Observer — Week 7.' },
    r: { id: 's3', label: 'Section 3 — the structure' } },
  { q: 'Where does the recursion in folder.size() actually live?',
    o: ['In the JVM', 'Inside the Composite\'s method: it calls size() on each child — and any child that is itself a composite recurses naturally', 'In the client, which loops the tree', 'In a static TreeUtils helper'], a: 1,
    ...
    w: { 0: 'The JVM just executes your dispatch.',
         2: 'Client-side walking is the BEFORE picture the pattern erases.',
         3: 'A TreeUtils helper would need instanceof checks — the disease itself.' },
    r: { id: 's4', label: 'Section 4 — the explorer trace' } },
  { q: 'What dies in client code the moment Composite is applied?',
    o: ['The instanceof-and-recurse boilerplate: if (node is file) … else walk children — repeated in every operation, in every client', 'Constructors', 'Interfaces', 'All loops'], a: 0,
    ...
    w: { 1: 'Constructors are untouched.',
         2: 'An interface is the pattern\'s heart.',
         3: 'One loop survives — inside the composite, written once.' },
    r: { id: 's2', label: 'Section 2 — the naive world' } },
  { q: 'The transparency-vs-safety debate is about…',
    o: ['Public vs private fields', 'Where add()/remove() live: on Component (uniform, but leaf.add() must throw — LSP pain) or only on Composite (type-safe, but clients must know types to mutate)', 'Recursion depth limits', 'Encryption of trees'], a: 1,
    ...
    w: { 0: 'Field visibility is Day 1 — different debate.',
         2: 'Depth is a runtime concern, not this design choice.',
         3: 'No cryptography involved. 🙂' },
    r: { id: 's7', label: 'Section 7 — transparency vs safety' } },
  { q: 'Which famous APIs are Composite trees?',
    o: ['java.lang.Math', 'java.util.Random', 'Swing (Container extends Component and holds Components), the HTML DOM, org charts, menus with submenus, arithmetic expression trees', 'System.out'], a: 2,
  ...
  { q: 'folder.add(itsOwnGrandparent) — what happens to the naive composite, and the defense?',
    o: ['Nothing, trees self-heal', 'The compiler rejects it', 'The GC removes the cycle', 'The "tree" becomes a cycle: every recursive operation loops until StackOverflowError. Defense: at add(), walk the would-be child\'s ancestors (or your own) and reject cycles'], a: 3,
    ...
    w: { 0: 'No self-healing exists — size() just never returns.',
         1: 'Perfectly legal Java to the compiler.',
         2: 'GC collects garbage, not design errors (and everything here is still referenced!).' },
    r: { id: 's8', label: 'Section 8 — the cycle lab' } },
  { q: '(3 + 4) × 2 as a Composite — what are the leaves and composites?',
    o: ['Leaves are operators', 'Impossible — math is not a tree', 'Only binary trees allowed', 'Leaves = Number(3), Number(4), Number(2); composites = Add and Multiply, each holding child Expressions; evaluate() recurses exactly like folder.size()'], a: 3,
    ...
    w: { 0: 'Operators hold children → they\'re the composites; numbers are the leaves.',
         1: 'Expression TREES are the oldest tree in computing.',
         2: 'Composites take any number of children (sum(1,2,3,4…)).' },

...
const QUESTIONS = [
  { q: 'The Flyweight pattern\'s intent is…',
    o: ['To lazily create objects', 'To make objects faster', 'To support HUGE numbers of fine-grained objects by SHARING their common (intrinsic) state instead of copying it into every instance', 'To compress files'], a: 2,
    ...
    w: { 0: 'Lazy creation is the virtual Proxy (Day 29).',
         1: 'Memory, not speed, is the target (speed sometimes improves via cache locality — a bonus).',
  ...
  { q: 'Intrinsic vs extrinsic state — define both.',
    o: ['Static vs instance fields', 'Private vs public fields', 'Intrinsic = the shareable, context-free part stored IN the flyweight (mesh, glyph shape); extrinsic = the per-use part (x,y, color-this-time) kept OUTSIDE and passed in by the context', 'Primitive vs object fields'], a: 2,
    ...
    w: { 0: 'Static sharing is class-wide — flyweights share per-KEY (3 tree types, not 1 global).',
         1: 'Visibility is unrelated.',
  ...
  { q: 'Why MUST flyweights be immutable?',
    o: ['Hundreds of contexts share one instance — mutate it and every sharer changes at once (paint one oak, the forest turns red)', 'Java requires it', 'Immutable objects are smaller', 'To avoid garbage collection'], a: 0,
    ...
    w: { 1: 'No language rule — a survival rule.',
  ...
  { q: 'Which JDK features ARE flyweights you met weeks ago?',
    o: ['ArrayList and HashMap', 'Threads', 'Streams', 'The String pool ("hi" shared by every literal) and Integer.valueOf\'s −128..127 cache — shared immutable instances handed out by a factory'], a: 3,
    ...
    w: { 0: 'Collections copy nothing for you.',
         1: 'Threads are heavyweight by definition. 🙂',
         2: 'Streams are pipelines, not shared instances.' },
    r: { id: 's6', label: 'Section 6 — flyweights in the JDK' } },
  { q: 'The Bridge pattern\'s intent is…',
    o: ['To share heavy objects', 'To connect two networks', 'To translate interfaces', 'To DECOUPLE an abstraction from its implementation so the TWO hierarchies can grow independently — composition instead of a multiplying inheritance grid'], a: 3,
    ...
    w: { 0: 'Sharing is Flyweight — this morning.',
         1: 'Networking bridges are a different industry. 🙂',
  ...
  { q: 'The Bridge math: m remote types and n device types need ___ classes with inheritance, ___ with bridge.',
    o: ['m×n and m+n — every combo-subclass collapses into "abstraction HOLDS an implementor"', 'always 2', 'm² and n²', 'm+n and m×n'], a: 0,
    ...
    w: { 1: 'Both sides keep their variety — it\'s the COMBINATIONS that stop being classes.',
         2: 'No squaring anywhere.',
         3: 'Backwards — inheritance multiplies, bridge adds.' },
  ...
  { q: 'Bridge vs Strategy — the structures look IDENTICAL (object holds interface). The difference?',
    o: ['Strategy is compile-time', 'Intent and scale: Strategy swaps ONE algorithm behind one interface (behavioral, often per-call); Bridge splits TWO WHOLE HIERARCHIES that each keep growing (structural, architectural)', 'Bridge uses abstract classes only', 'No difference, same pattern'], a: 1,
    ...
    w: { 0: 'Both are runtime-composed.',
         2: 'Either side can use interfaces or abstract classes.',
         3: 'GoF separates them — by intent, as always with the look-alikes.' },
    r: { id: 's10', label: 'Section 10 — bridge\'s twins' } },
  { q: 'Bridge vs Adapter in one line (the timing answer)?',
    o: ['Bridge is for hardware', 'Adapter is RETROFIT glue for incompatibles that already exist; Bridge is DESIGNED-IN up front so two dimensions never fuse in the first place', 'Adapter only works on streams', 'Identical'], a: 1,
    ...
    w: { 0: 'The remote/device example is a metaphor, not a requirement. 🙂',
         2: 'Streams showcase adapters/decorators; bridges live in your architecture.',
         3: 'Different birth moments, different intents.' },

...
const QUESTIONS = [
  { q: 'The Adapter pattern\'s intent is…',
    o: ['To convert the interface of an existing class into the interface clients expect — making incompatibles work together without modifying either', 'To create families of objects', 'To hide a whole subsystem', 'To add new features to an object'], a: 0,
    ...
    w: { 1: 'Families are Abstract Factory (Day 23).',
         2: 'Hiding a subsystem behind one door is Facade (Day 29).',
         3: 'Adding features with the SAME interface is Decorator (tomorrow).' },
    r: { id: 's1', label: 'Section 1 — the travel plug' } },
  { q: 'When is Adapter the RIGHT tool (vs just changing the code)?',
    o: ['Only for streams', 'When both classes are yours and editable', 'When you cannot or should not modify either side: vendor jars, legacy frozen code, or interfaces with many existing clients', 'Whenever two classes talk'], a: 2,
    ...
    w: { 0: 'Streams are one famous example, not the definition.',
         1: 'Owning both sides = just refactor; an adapter there is ceremony.',
         3: 'Most collaborations need no translation at all.' },
    r: { id: 's2', label: 'Section 2 — the boundary problem' } },
  { q: 'The OBJECT adapter\'s structure is…',
    o: ['A subclass of the client', 'extends the adaptee and overrides everything', 'A static utility class of conversion methods', 'implements the TARGET interface and HOLDS the adaptee as a field, delegating with translation'], a: 3,
    ...
    w: { 0: 'Clients are never subclassed — they stay untouched.',
         1: 'Pure extension is the class-adapter flavor — and it can\'t also BE the target in Java unless the target is an interface.',
  ...
  { q: 'Why does Java favor the object adapter over the GoF class adapter (which inherits from the adaptee)?',
    o: ['Java has single class inheritance — and composition keeps the adaptee\'s full API hidden, swappable, and lets one adapter wrap any subclass of the adaptee', 'Class adapters are deprecated', 'Objects are faster than classes', 'The JVM forbids inheritance across jars'], a: 0,
    ...
    w: { 1: 'Nothing deprecated — just disadvantaged.',
  ...
  { q: 'Which JDK class is the canonical Adapter?',
    o: ['System.out', 'HashMap', 'ArrayList', 'InputStreamReader — it makes a byte InputStream usable wherever a character Reader is expected'], a: 3,
    ...
    w: { 0: 'System.out is a PrintStream field.',
         1: 'HashMap stores pairs — wraps nothing.',
         2: 'ArrayList is a plain collection.' },
    r: { id: 's7', label: 'Section 7 — JDK sightings' } },
  { q: 'Adapter vs Decorator — the one-line distinction?',
    o: ['Decorator only works on streams', 'Adapter CHANGES the interface (translation); Decorator KEEPS the interface and adds behavior', 'They are identical', 'Adapter is newer'], a: 1,
    ...
    w: { 0: 'Streams just happen to showcase both.',
         2: 'Same shape, different intent — the family table exists because people confuse them.',
         3: 'Both are 1994 GoF patterns.' },
    r: { id: 's8', label: 'Section 8 — the wrapper family' } },
  { q: 'How much logic belongs INSIDE an adapter?',
    o: ['None — adapters must be empty', 'As much as needed — it\'s a class like any other', 'Translation only: mapping names, formats, units, defaults. Business rules in an adapter hide policy in a place nobody looks', 'All validation for the whole app'], a: 2,
    ...
    w: { 0: 'Empty would mean nothing translated — defaults and format mapping ARE its job.',
         1: '"Any logic anywhere" is how policy gets lost in plumbing.',
  ...
  { q: 'How do Adapter and ISP (Day 14) combine at a messy boundary?',
    o: ['Copy the vendor\'s code into your repo', 'Define YOUR OWN narrow role interface for what you actually need, then write a small adapter from the fat/alien API to it — clients depend only on the lean contract', 'They conflict — pick one', 'Implement the vendor\'s interface everywhere'], a: 1,
    ...
    w: { 0: 'Vendoring code copies the problem in.',
         2: 'They\'re a famous duo, not rivals.',
         3: 'Spreading the vendor\'s interface couples your whole app to it.' },

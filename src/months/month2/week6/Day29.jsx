...
const QUESTIONS = [
  { q: 'The Facade pattern\'s intent is…',
    o: ['To convert interfaces', 'To stack behaviors', 'To provide ONE unified, simplified interface to a whole SUBSYSTEM of classes — an easy door, not a locked one', 'To control access to one object'], a: 2,
    ...
    w: { 0: 'Interface conversion is Adapter (26).',
         1: 'Stacking behaviors is Decorator (27).',
         3: 'Controlling access to ONE object is Proxy — today\'s second act.' },
    r: { id: 's3', label: 'Section 3 — facade structure & rules' } },
  { q: 'How much LOGIC may a healthy facade contain?',
    o: ['Database access only', 'All the business rules of the subsystem', 'None — it must be empty', 'Thin orchestration only: call order, wiring results between subsystem calls — the real work stays in the subsystem classes'], a: 3,
    ...
    w: { 0: 'Persistence belongs in the subsystem it fronts.',
         1: 'Absorbing the rules creates the god class (Day 11) with a pattern\'s blessing.',
  ...
  { q: 'The Proxy pattern\'s intent is…',
    o: ['To simplify many classes', 'To copy objects', 'To add condiments', 'To provide a SURROGATE with the SAME interface that CONTROLS access to the real object — guarding, delaying, caching, or remoting it'], a: 3,
    ...
    w: { 0: 'Simplifying many is Facade — this morning\'s pattern.',
         1: 'Copying is Prototype (25).',
         2: 'Adding powers knowingly is Decorator.' },
    r: { id: 's6', label: 'Section 6 — proxy intent & kinds' } },
  { q: 'A VIRTUAL proxy earns its keep by…',
    o: ['Making more instances', 'Encrypting calls', 'Deferring creation of an expensive real object until a call truly needs it — answering cheap questions (metadata, previews) from its own cache meanwhile', 'Running faster bytecode'], a: 2,
    ...
    w: { 0: 'Fewer/later real instances is the point.',
         1: 'Encryption isn\'t the job (a protection proxy guards by AUTHORIZATION, not crypto).',
  ...
  { q: 'Proxy vs Decorator — same interface, both wrap. The deepest differences?',
    o: ['Decorator ADDS features, client stacks them knowingly, always delegates inward; Proxy CONTROLS access, is usually invisible to the client, may answer or REFUSE without ever calling the real object — and often manages the real object\'s lifecycle itself', 'Proxies are compile-time only', 'Decorators need reflection', 'None'], a: 0,
    ...
    w: { 1: 'Both are runtime objects (proxies often GENERATED at runtime!).',
         2: 'Reflection is the proxy\'s trick (java.lang.reflect.Proxy), not the decorator\'s.',
         3: 'GoF separates them by intent — and interviews live on this question.' },
    r: { id: 's10', label: 'Section 10 — the family table' } },
  { q: 'The famous Spring @Transactional self-invocation trap is…',
    o: ['this.otherMethod() inside the same class BYPASSES the proxy — the transaction advice lives on the wrapper, and internal calls never cross it, so @Transactional silently does nothing', 'Transactions are slow', 'A compiler error', 'Spring forbids two transactions'], a: 0,
    ...
    w: { 1: 'Speed is not the trap.',
         2: 'It compiles — and fails SILENTLY, which is why it\'s famous.',
         3: 'Nested transactions are configurable.' },
    r: { id: 's9', label: 'Section 9 — proxies in the wild' } },
  { q: 'Hibernate throws LazyInitializationException when…',
    o: ['Too many rows return', 'You touch a lazy proxy AFTER its session closed — the stand-in finally needs to fetch, but the database connection that could do it is gone', 'The entity has no id', 'SQL has a typo'], a: 1,
    ...
    w: { 0: 'No rows were fetched at all.',
         2: 'Ids are fine.',
         3: 'SQL never ran — that\'s the point.' },

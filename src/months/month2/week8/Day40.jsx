...
const QUESTIONS = [
  { q: 'The Visitor pattern\'s intent is…',
    o: ['To iterate a collection', 'To represent an OPERATION on an object structure as its own class — letting you add NEW operations without modifying the element classes', 'To snapshot state', 'To chain handlers'], a: 1,
    ...
    w: { 0: 'Walking elements is Iterator (37) — visitors often RIDE an iteration.',
         2: 'Snapshots are Memento (39).',
         3: 'Handler chains are CoR (36).' },
    r: { id: 's1', label: 'Section 1 — the inspector’s rounds' } },
  { q: 'Why doesn\'t plain overloading — visitor.visit(shape) — work without accept()?',
    o: ['Overloads are slow', 'Overload selection is COMPILE-TIME, by the STATIC type: with Shape s, visit(s) binds to visit(Shape) forever, never to visit(Circle) — the runtime type is invisible to overload resolution (Day 4!)', 'visit is a keyword', 'It does work'], a: 1,
    ...
    w: { 0: 'Speed is irrelevant.',
         2: 'No such keyword.',
         3: 'Try it: visit(Shape) wins every time — the classic confusion.' },
    r: { id: 's3', label: 'Section 3 — why accept exists' } },
  { q: 'Explain the accept() one-liner\'s magic.',
    o: ['It logs the visit', 'Inside Circle.accept(v), the call v.visit(this) sees this with COMPILE-TIME type Circle — so the right overload binds. Dispatch #1 (dynamic, picks WHOSE accept) + dispatch #2 (overload on the now-known this) = simulated double dispatch', 'It uses reflection', 'It clones the visitor'], a: 1,
    ...
    w: { 0: 'No logging.',
         2: 'Zero reflection — that\'s the elegance.',
         3: 'Nothing is cloned.' },
    r: { id: 's4', label: 'Section 4 — the dispatch machine' } },
  { q: 'Visitor\'s trade-off axes — state them like Day 23\'s table.',
    o: ['Open everywhere', 'OPEN on the operation axis (new visitor = one file, elements untouched) but CLOSED on the type axis (new element = edit the Visitor interface AND every visitor) — the inverse of classic OOP, and a named dilemma: the Expression Problem', 'Closed everywhere', 'Only about speed'], a: 1,
    ...
    w: { 0: 'No design is open on both axes in plain Java.',
         2: 'It IS open — on one axis.',
         3: 'Speed never enters it.' },
    r: { id: 's5', label: 'Section 5 — the two-axis blast' } },
  { q: 'Visitor + Composite (Day 28) — why are they soulmates?',
    o: ['Both use recursion keywords', 'Composite gives a stable TREE of heterogeneous nodes; Visitor adds unlimited OPERATIONS over it without touching node classes — compilers do exactly this: one AST, dozens of visitor passes', 'They share an interface', 'GoF requires pairing them'], a: 1,
  ...
  { q: 'Is a visitor that ACCUMULATES state (running total, list of matches) doing it wrong?',
    o: ['Yes — visitors must be stateless', 'No — collecting results across visits is the NORMAL visitor style: the visitor is a per-traversal object, born for one walk, queried at the end (total(), matches())', 'Only with synchronized', 'State belongs in the elements'], a: 1,
    ...
    w: { 0: 'That rule belongs to SHARED objects; visitors are usually single-use.',
         2: 'Concurrency is orthogonal.',
         3: 'Pushing op-results into elements re-pollutes them — the disease again.' },
    r: { id: 's8', label: 'Section 8 — the auditor demo' } },
  { q: 'The MODERN Java alternative to Visitor — and the catch?',
    o: ['There is none', 'sealed interface + pattern-matching switch (Java 21): exhaustive over a closed type set, no accept() boilerplate — but it requires owning/sealing the hierarchy, and "add a type" still edits every switch (same axis, compiler-found)', 'Reflection visitors', 'instanceof chains are fine now'], a: 1,
    ...
    w: { 0: 'Java 17–21 changed this story significantly.',
         2: 'Reflection trades type safety for pain.',
         3: 'Unchecked instanceof chains still rot — sealing + exhaustiveness is what redeems the switch.' },
  ...
  { q: 'GoF lists 23 patterns; this course taught 22. The missing one — and why skipping it is defensible?',
    o: ['Singleton — too famous', 'Interpreter: define a grammar and evaluate sentences via a class per rule — rarely used directly, and structurally it is Composite (an expression tree, Day 28\'s homework!) plus per-node evaluate/visitors. You have effectively built it', 'Bridge — too hard', 'Observer — deprecated'], a: 1,
    ...
    w: { 0: 'Day 21 says hello.',
         2: 'Day 30 says hello.',
         3: 'Day 32 is very alive.' },

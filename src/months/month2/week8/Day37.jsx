...
const QUESTIONS = [
  { q: 'The Iterator pattern\'s intent is…',
    o: ['To copy collections', 'To access an aggregate\'s elements SEQUENTIALLY without exposing its underlying representation', 'To sort elements', 'To make collections immutable'], a: 1,
    ...
    w: { 0: 'No copying — a view that walks.',
         2: 'Order is whatever the aggregate offers; sorting is Comparator\'s job (31).',
         3: 'Mutability is orthogonal (iterators can even remove).' },
    r: { id: 's1', label: 'Section 1 — the next button' } },
  { q: 'Iterable vs Iterator — why TWO interfaces?',
    o: ['Historical accident', 'Iterable = "you can ask me for a cursor" (iterator() — a factory method!); Iterator = one single-use cursor with its own position. Each call to iterator() births a fresh, independent walk', 'They are aliases', 'Iterator extends Iterable'], a: 1,
    ...
    w: { 0: 'Deliberate design — restartability and multiple cursors.',
         2: 'Different jobs: source vs position.',
         3: 'Neither extends the other.' },
    r: { id: 's3', label: 'Section 3 — the two interfaces' } },
  { q: 'What does for (Song s : playlist) actually compile to?',
    o: ['An index loop with get(i)', 'Iterator it = playlist.iterator(); while (it.hasNext()) { Song s = it.next(); … } — the pattern is the language feature\'s engine', 'A stream pipeline', 'Recursion'], a: 1,
    ...
    w: { 0: 'Index loops only work on indexed things — for-each works on Sets too.',
  ...
  { q: 'ConcurrentModificationException in a single-threaded loop — the mechanics?',
    o: ['A JVM bug', 'The list\'s modCount changed (direct remove/add) while the iterator still holds the old expectedModCount — the next() tripwire compares them and throws. No threads required, despite the name', 'Too many elements', 'The GC interfered'], a: 1,
    ...
    w: { 0: 'Working exactly as designed.',
  ...
  { q: 'The THREE sanctioned ways to remove while iterating?',
    o: ['You simply cannot', 'iterator.remove() (updates both counters) · collection.removeIf(predicate) (internal iteration) · CopyOnWriteArrayList (iterate a snapshot — Day 32\'s listener trick)', 'Only streams', 'Reflection'], a: 1,
  ...
  { q: 'Why is for(i=0; i<n; i++) list.get(i) a performance LANDMINE?',
    o: ['It always allocates', 'On a LinkedList, get(i) walks from the head every call → the loop becomes O(n²); the iterator walks node-to-node in O(n). The client coupled itself to a representation assumption', 'Index loops are deprecated', 'get() is synchronized'], a: 1,
    ...
    w: { 0: 'Allocation is not the issue.',
         ...
         3: 'No locking involved.' },
    r: { id: 's4', label: 'Section 4 — the two players' } },
  { q: 'External vs INTERNAL iteration?',
    o: ['Same thing', 'External: YOU pull values (while hasNext/next) and control the loop. Internal: you hand a lambda to the collection (forEach, streams) and IT runs the loop — less control, fewer cursor bugs', 'Internal is single-threaded only', 'External is deprecated'], a: 1,
    ...
    w: { 0: 'Who drives the loop is the whole distinction.',
         2: 'parallelStream is internal iteration ACROSS threads.',
         3: 'Both alive; choose per job.' },

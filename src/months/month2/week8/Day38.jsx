...
const QUESTIONS = [
  { q: 'The Mediator pattern\'s intent is…',
    o: ['To simplify a subsystem for outsiders', 'To define ONE object that encapsulates HOW a set of colleagues interact — they hold only the mediator, never each other, and it runs the conversation rules', 'To broadcast to unknown subscribers', 'To iterate participants'], a: 1,
    ...
    w: { 0: 'One-way simplification for outsiders is Facade (29).',
         2: 'Anonymous broadcast is Observer (32) — mediator coordinates KNOWN colleagues, both directions.',
         3: 'Walking elements is Iterator (37).' },
    r: { id: 's1', label: 'Section 1 — the control tower' } },
  { q: 'The math that motivates the tower?',
    o: ['No difference for small N', 'N connections either way', 'Mediators double connections', 'Pairwise: N colleagues need N(N−1)/2 channels and each knows N−1 others; with a mediator: N channels, each colleague knows exactly ONE name. Growth: +N per newcomer vs +1'], a: 3,
    ...
    w: { 0: 'Even at N=4 the web is 6 vs 4 — and growing differently.',
         1: 'Pairwise is quadratic, not linear.',
         2: 'It collapses them.' },
    r: { id: 's4', label: 'Section 4 — web vs tower' } },
  { q: 'What does a COLLEAGUE know and do in this pattern?',
    o: ['It holds one mediator reference, reports its own events (mediator.notify(this, event)), and obeys instructions — it never names, queries or commands a sibling', 'It maintains the rule table', 'It subscribes to all peers', 'Everything about its peers'], a: 0,
    ...
    w: { 1: 'Rules live in the MEDIATOR — that locality is the point.',
         2: 'Peer subscriptions rebuild the web with observer syntax.',
         3: 'Peer knowledge is the disease being cured.' },
    r: { id: 's3', label: 'Section 3 — the structure' } },
  { q: 'Mediator vs Observer — the Day 32 question, settled?',
    o: ['Mediator has no events', 'Observer: ONE subject broadcasts outward to an anonymous, changing list — no coordination rules. Mediator: a hub that KNOWS its colleagues and encodes multi-party rules ("if checkbox then disable that") — two-way switchboard vs one-way megaphone', 'Identical', 'Observer needs more classes'], a: 1,
    ...
    w: { 0: 'Events are usually the transport INTO the mediator.',
         2: 'Megaphone vs switchboard.',
         3: 'Class count is incidental.' },
    r: { id: 's8', label: 'Section 8 — the family table' } },
  { q: 'Mediator vs Facade — the Day 29 question, settled?',
    o: ['Mediator is for files', 'Facade is bidirectional', 'Facade: one-way — OUTSIDERS get a simple door; subsystem classes neither know the facade nor talk through it. Mediator: INSIDERS route their mutual conversation through the hub, and they know it', 'Same pattern'], a: 2,
    ...
    w: { 0: 'No. 🙂',
         1: 'Facade\'s subsystem never calls back through it.',
         3: 'Different direction, different clients.' },
    r: { id: 's8', label: 'Section 8 — vs facade' } },
  { q: 'The honest COST of Mediator?',
    o: ['It prevents adding colleagues', 'There is none', 'The interaction complexity does not vanish — it RELOCATES into the mediator, which can bloat into a god object; the win is ONE auditable, testable home instead of a smear across N colleagues', 'It slows messages'], a: 2,
    ...
    w: { 0: 'Adding colleagues is exactly what gets EASIER.',
         1: 'Every centralization has a bill.',
         3: 'A method hop is nothing.' },
    r: { id: 's9', label: 'Section 9 — the god-mediator' } },
  { q: 'In the chat room, what made the "no passwords" rule elegant?',
    o: ['Users vote on it', 'The compiler enforced it', 'Each user filters their own messages', 'It lives ONCE in the room: every present and future user is covered automatically — centralized rules are the mediator\'s superpower (and DRY, Day 16)'], a: 3,
    ...
    w: { 0: 'Democracy is not a pattern. 🙂',
         1: 'It\'s runtime logic, not types.',
         2: 'Per-user filtering = N copies of policy drifting apart.' },
    r: { id: 's7', label: 'Section 7 — the chat room' } },
  { q: 'The architecture-scale version of mediator-vs-observer?',
    o: ['ORCHESTRATION (a workflow service explicitly directs each step — mediator) vs CHOREOGRAPHY (services react to each other\'s events — observer). Same trade-offs: central auditable rules vs decentralized evolution', 'REST vs GraphQL', 'Monolith vs microservices', 'SQL vs NoSQL'], a: 0,

...
const QUESTIONS = [
  { q: 'Chain of Responsibility\'s intent is…',
    o: ['To iterate collections', 'To freeze requests', 'To notify all subscribers', 'To DECOUPLE sender from receiver by giving MULTIPLE handlers a chance at the request — it travels the chain until one handles it'], a: 3,
    ...
    w: { 0: 'Iteration is tomorrow (37).',
         1: 'Freezing requests is Command (33) — though the traveling request is often a command object!',
         2: 'All-subscribers-react is Observer (32); here typically ONE handles and the rest never hear of it.' },
  ...
  { q: 'The standard handler structure is…',
    o: ['A list the client loops over', 'A map of types', 'A switch in the sender', 'Each handler holds a NEXT reference; a base-class template runs: if canHandle → handle, else next.handle(request) — and concrete handlers fill the blanks'], a: 3,
    ...
    w: { 0: 'A dispatcher loop is a valid VARIANT — but then the loop owner becomes a coupling point.',
         1: 'Type-maps are registries (31) — no chance for ordered fallthrough.',
         2: 'The sender switch is the disease.' },
    r: { id: 's3', label: 'Section 3 — the structure' } },
  { q: 'Why does ORDER in the chain matter so much?',
    o: ['Alphabetical is required', 'Only the last handler matters', 'The FIRST capable handler wins — put the CFO first and lunch receipts get C-suite approval; put broad handlers early and specific ones never run', 'It doesn\'t'], a: 2,
    ...
    w: { 0: 'No such rule. 🙂',
         1: 'The last handler often never runs.',
         3: 'The demo says otherwise — loudly.' },
    r: { id: 's6', label: 'Section 6 — the sabotaged chain' } },
  { q: 'A request reaches the end with NO handler. The right engineering response?',
    o: ['Decide a POLICY: a terminal catch-all handler, an explicit UnhandledRequestException, or a logged drop — anything but silent disappearance', 'Restart the application', 'Impossible by design', 'The JVM retries the chain'], a: 0,
    ...
    w: { 1: 'Drastic and useless.',
         2: 'Falling off the end is always possible — that\'s the design\'s honesty.',
         3: 'No auto-retry exists.' },
    r: { id: 's5', label: 'Section 5 — the end of the chain' } },
  { q: 'Classic CoR vs the PIPELINE (middleware) variant?',
    o: ['Classic: handlers either claim the request or pass it UNTOUCHED — one winner. Pipeline: EVERY stage does its work (auth, count, log) and forwards; stopping = not forwarding. Servlet filters express both via chain.doFilter()', 'Classic is asynchronous', 'Pipeline is deprecated', 'Identical'], a: 0,
    ...
    w: { 1: 'Sync/async is an independent axis.',
         2: 'Middleware pipelines run the modern web.',
         3: 'The contract difference changes everything downstream.' },
    r: { id: 's7', label: 'Section 7 — two semantics' } },
  { q: 'Which Java LANGUAGE feature is Chain of Responsibility built in?',
    o: ['Autoboxing', 'Exception propagation: a throw climbs the call stack, offering itself to each enclosing try/catch until one catches — handlers in a chain, unhandled = falls off the end (thread dies)', 'Generics', 'Garbage collection'], a: 1,
    ...
    w: { 0: 'Boxing converts values.',
         2: 'Generics are compile-time types.',
         3: 'GC frees memory, routes nothing.' },
  ...
  { q: 'Chain of Responsibility vs Decorator — both are linked wrappers. The contract difference?',
    o: ['Decorator is for streams only', 'None', 'Decorator layers ALL run — each adds its delta and ALWAYS forwards inward. CoR handlers MAY stop the journey — handling is exclusive, forwarding is conditional', 'CoR cannot be nested'], a: 2,
    ...
    w: { 0: 'Streams are just the famous decorator demo.',
         1: 'The everyone-runs guarantee is the dividing line.',
         3: 'Both nest happily.' },

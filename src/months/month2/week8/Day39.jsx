...
const QUESTIONS = [
  { q: 'The Memento pattern\'s intent — including the clause people forget?',
    o: ['Copy objects cheaply', 'WITHOUT VIOLATING ENCAPSULATION, capture an object\'s internal state externally so it can be restored later — the snapshot exists, yet the private fields stay private', 'Log every change', 'Compress object state'], a: 1,
    ...
    w: { 0: 'Copying for a NEW independent object is Prototype (25) — same mechanics, different purpose.',
         2: 'Logging changes is the Command-log idea (33).',
         3: 'Compression is an optimization, not the intent.' },
    r: { id: 's1', label: 'Section 1 — the save slot' } },
  { q: 'Name the three roles and the caretaker\'s defining limitation.',
    o: ['Subject, Observer, Event', 'Originator (creates & restores the memento), Memento (the sealed state bottle), Caretaker (STORES bottles but can never open them — it manages WHEN, never WHAT)', 'Client, Invoker, Receiver', 'Context, State, Transition'], a: 1,
  ...
  { q: 'How does Java ENFORCE memento opacity (not just promise it)?',
    o: ['With a comment saying please', 'A private-fields inner class: the OUTER class (originator) can read its inner class\'s private fields — a unique Java visibility rule — while caretakers hold the token and get compile errors on any peek', 'Reflection guards', 'Encrypting the fields'], a: 1,
    ...
    w: { 0: 'Hope is not access control.',
         2: 'Reflection can pierce anything (Day 21) — the design intent is compile-time honesty.',
         3: 'No crypto needed; visibility suffices.' },
    r: { id: 's5', label: 'Section 5 — the opaque box lab' } },
  { q: 'The Day 33 connection: in undo systems, Command and Memento divide labor how?',
    o: ['They are rivals', 'The command represents the ACTION; the memento is the bottled BEFORE-STATE the command stores so undo() can restore — Day 33\'s "snapshot" column finally has its name and rules', 'Memento replaces Command', 'Commands cannot use snapshots'], a: 1,
    ...
    w: { 0: 'They are the classic marriage.',
         2: 'You still need the action object to queue/log/redo.',
  ...
  { q: 'Why must a memento DEEP-copy mutable fields?',
    o: ['For speed', 'A shallow memento shares live mutable objects — later mutations REWRITE the "saved" state (the cursed amulet survived the restore). Deep-copy mutables; share immutables freely', 'Java requires it', 'To save memory'], a: 1,
    ...
    w: { 0: 'Deep copies are SLOWER — correctness is the purchase.',
         2: 'No language rule; a survival rule.',
         3: 'It costs MORE memory, deliberately.' },
    r: { id: 's7', label: 'Section 7 — the shallow disaster' } },
  { q: 'A text editor snapshots a 10 MB document on every keystroke. The fixes?',
    o: ['Buy more RAM forever', 'Cap or window the history · snapshot at meaningful boundaries (word, command) · store DIFFS between states instead of full copies · or flyweight-share the unchanged parts', 'Disable undo', 'Compress the JVM'], a: 1,
  ...
  { q: 'Which database feature is Memento speaking SQL?',
    o: ['CREATE INDEX', 'SAVEPOINT my_point … ROLLBACK TO my_point — bottle the transaction state, restore on demand; the DB is originator AND enforcer of opacity', 'SELECT DISTINCT', 'Foreign keys'], a: 1,
    ...
    w: { 0: 'Indexes accelerate reads.',
         2: 'A query modifier.',
         3: 'Integrity constraints.' },
    r: { id: 's8', label: 'Section 8 — mementos in the wild' } },
  { q: 'A "memento" class with public getters for every field is…',
    o: ['Perfectly standard', 'A leaky memento: the caretaker (and everyone else) can now read the originator\'s entire private state — you shipped Day 2\'s encapsulation out in a box. Opacity is the pattern; without it this is just a DTO', 'Required for testing', 'Faster'], a: 1,

...
const QUESTIONS = [
  { q: 'The Prototype pattern\'s intent is…',
    o: ['To prevent copying', 'To make all classes abstract', 'To create new objects by COPYING a pre-configured example, instead of constructing from scratch', 'To test prototypes before release'], a: 2,
    ...
    w: { 0: 'Copying is the MECHANISM, not the forbidden thing.',
         1: 'Abstractness is unrelated.',
         3: '"Prototype" here is the GoF pattern, not product prototyping.' },
    r: { id: 's1', label: 'Section 1 — the photocopier' } },
  { q: 'Prototype shines when…',
    o: ['Objects have two fields', 'Construction is costly (loading, parsing, computing) or you need many variants of a tuned base object at runtime', 'Objects are immutable', 'You need exactly one instance'], a: 1,
    ...
    w: { 0: 'Two-field objects are new\'s home turf.',
         2: 'Immutables need no copying at all — share them (the day\'s deepest idea).',
         3: '"Exactly one" is Singleton — Prototype makes MANY.' },
    r: { id: 's2', label: 'Section 2 — spawn the army' } },
  { q: 'A SHALLOW copy of an object with a List field copies…',
    o: ['Nothing', 'Only the list', 'The primitive fields by value, but the list REFERENCE — both objects now share ONE list', 'The list and its contents'], a: 2,
    ...
    w: { 0: 'Something is copied — primitives by value, references by address.',
         1: 'Everything copies — just at different depths than you hoped.',
         3: 'Copying contents too is DEEP copy — the upgrade, not the default.' },
    r: { id: 's4', label: 'Section 4 — shallow vs deep lab' } },
  { q: 'A DEEP copy fixes that by…',
    o: ['Locking the list', 'Making the list static', 'Using == instead of equals', 'Also copying the mutable objects the fields point to (new list, copied contents) — recursively, as deep as mutability goes'], a: 3,
    ...
    w: { 0: 'Locking is concurrency machinery — copying is the fix here.',
         1: 'static would SHARE state — backwards.',
         2: '== vs equals is comparison, not copying.' },
    r: { id: 's5', label: 'Section 5 — copy constructors' } },
  { q: 'Why do Java experts (Effective Java) say AVOID Cloneable/clone()?',
    o: ['The mechanism is broken-by-design: a marker interface with no method, protected clone(), checked exception, NO constructor runs, final fields can\'t be set — fragile and error-prone', 'It was removed in Java 17', 'It is too fast', 'It only works on arrays'], a: 0,
    ...
    w: { 1: 'Still present in every Java version — just discouraged.',
         2: 'Speed is fine — design is the problem.',
         3: 'Arrays clone happily (shallow!) — and so can any class.' },
    r: { id: 's3', label: 'Section 3 — the Cloneable minefield' } },
  { q: 'The recommended Java way to implement Prototype is…',
    o: ['A copy constructor — public Order(Order other) — or a copy()/static factory that explicitly copies fields (deep where needed)', 'Always implements Cloneable', 'Reflection', 'Serialization round-trips everywhere'], a: 0,
    ...
    w: { 1: 'Cloneable is exactly what Effective Java tells you to avoid.',
         2: 'Reflection-copying is fragile framework territory.',
         3: 'Serialization round-trips are slow, restrictive last resorts.' },
    r: { id: 's5', label: 'Section 5 — the cleaner Java way' } },
  { q: 'Why do IMMUTABLE objects make the whole copying question vanish?',
    o: ['They have no fields', 'They can\'t change, so sharing one instance is always safe — you never need a copy at all, just hand out the same reference', 'The JVM clones them automatically', 'They copy themselves'], a: 1,
    ...
    w: { 0: 'Field count is irrelevant.',
         2: 'The JVM clones nothing automatically.',
         3: 'Objects never copy themselves spontaneously.' },
    r: { id: 's7', label: 'Section 7 — the immutability shortcut' } },
  { q: 'A prototype REGISTRY is…',
    o: ['A type of Singleton', 'A list of all classes in the JVM', 'A database table', 'A map of named, pre-configured prototypes ("goblin" → tuned Goblin) — clients spawn(key) to get an independent copy'], a: 3,
    ...
    w: { 0: 'The registry itself MAY be a singleton — but that\'s incidental.',
         1: 'The JVM\'s class list is reflection territory.',
         2: 'It\'s in-memory design, not persistence.' },
    r: { id: 's6', label: 'Section 6 — the template drawer' } },
]

...
const QUESTIONS = [
  { q: 'The Singleton pattern guarantees…',
    o: ['A class with one method', 'Exactly ONE instance of a class exists, with one global access point to it', 'One class per file', 'A class that cannot be tested'], a: 1,
    ...
    w: { 0: 'Method count is irrelevant to the pattern.',
         2: 'File organization has nothing to do with it.',
         3: 'Untestability is a CRITICISM of it, not its definition. 🙂' },
    r: { id: 's1', label: 'Section 1 — the president' } },
  { q: 'Which Day-1 trick is the load-bearing wall of every singleton?',
    o: ['Method overloading', 'The PRIVATE constructor — outsiders cannot say new, so the class controls how many of itself exist', 'The this keyword', 'A static block'], a: 1,
    ...
    w: { 0: 'Overloading multiplies doors — the opposite move.',
         2: 'this is bookkeeping, not access control.',
         3: 'Static blocks initialize; they don\'t restrict creation.' },
    r: { id: 's2', label: 'Section 2 — the three ingredients' } },
  { q: 'Why does the unsynchronized lazy getInstance() create two instances under load?',
    o: ['Because the field is private', 'Java has a bug', 'A thread can be paused BETWEEN the null-check and the creation — a second thread then also sees null, and both create', 'The garbage collector duplicates it'], a: 2,
    ...
    w: { 0: 'Visibility doesn\'t affect interleaving.',
         1: 'The JVM behaves exactly as specified — the CODE made an unwarranted atomicity assumption.',
         3: 'GC never duplicates objects.' },
    r: { id: 's5', label: 'Section 5 — the thread race' } },
  { q: 'In double-checked locking, what is the volatile keyword for?',
    o: ['Without it, a thread may observe a HALF-CONSTRUCTED instance due to instruction reordering — volatile forbids that visibility problem', 'It makes the field private', 'It is decorative', 'Speed'], a: 0,
    ...
    w: { 1: 'Visibility modifiers are unrelated to memory ordering.',
         2: 'Nothing in concurrency code is decorative.',
         3: 'volatile actually costs a little speed — it BUYS correctness.' },
    r: { id: 's6', label: 'Section 6 — the implementation picker (DCL)' } },
  { q: 'Why is the static holder idiom considered the sweet spot for lazy singletons?',
    o: ['It allows multiple instances', 'It avoids static entirely', 'It uses the least memory', 'The inner class loads only on first use (lazy) and JVM class-initialization is guaranteed single-threaded (safe) — with zero locking code to get wrong'], a: 3,
    ...
    w: { 0: 'It allows exactly one — via class-init guarantees.',
         1: 'It uses static heavily — just cleverly nested.',
         2: 'Memory footprint is identical.' },
    r: { id: 's6', label: 'Section 6 — the holder idiom' } },
  { q: 'Why does Effective Java call the single-element ENUM the best singleton?',
    o: ['It is immune to the reflection and serialization attacks that can mint second instances of all class-based singletons', 'Enums are faster', 'It supports inheritance', 'It is the only lazy one'], a: 0,
    ...
    w: { 1: 'Enum constant access is no faster.',
         2: 'Enums can\'t extend — true, but that\'s their COST, not the crowning reason.',
         3: 'Enum singletons are eager-ish — laziness is not their selling point.' },
    r: { id: 's7', label: 'Section 7 — breaking singletons' } },
  { q: 'The MAIN design criticism of Singleton is…',
    o: ['It is slow', 'It cannot be thread-safe', 'It is global mutable state with hidden dependencies: classes secretly call getInstance() instead of declaring needs in constructors — coupling everyone (Day 17) and making fakes impossible to inject (Day 15)', 'It uses too much memory'], a: 2,
    ...
    w: { 0: 'Singleton access is fast — speed is never the complaint.',
         1: 'It absolutely can be thread-safe — that part is solved.',
         3: 'One instance typically SAVES memory.' },
    r: { id: 's8', label: 'Section 8 — the dark side' } },
  { q: 'The modern alternative for "I need exactly one of this service" is…',
    o: ['Clone it everywhere', 'Use global variables', 'Make every method static', 'Create ONE instance at the composition root (main) and INJECT it wherever needed — single by wiring, not by self-enforcement'], a: 3,
    ...
    w: { 0: 'Cloning multiplies instances — opposite goal.',
         1: 'Global variables are the disease itself.',
         2: 'All-static loses interfaces, laziness and substitutability.' },
    r: { id: 's8', label: 'Section 8 — the modern resolution' } },
]

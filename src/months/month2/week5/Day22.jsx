...
const QUESTIONS = [
  { q: 'Why is a scattered `new EmailNotification(...)` in ten files a design liability?',
    o: ['Each new couples that file to a concrete class and its constructor details — adding/changing products means hunting and editing every site', 'new is slow', 'Objects leak memory', 'Constructors are deprecated'], a: 0,
    ...
    w: { 1: 'new is as fast as it gets — knowledge, not speed, is the liability.',
  ...
  { q: 'A SIMPLE FACTORY is…',
    o: ['A GoF pattern with subclasses', 'A factory with no methods', 'One class/static method that centralizes creation: clients pass a key, the factory owns the switch and returns the interface type', 'Another name for Singleton'], a: 2,
    ...
    w: { 0: 'Subclass-based creation is the GoF Factory METHOD — the next level up.',
         1: 'A factory with no methods creates nothing. 🙂',
  ...
  { q: 'The GoF FACTORY METHOD pattern differs from a simple factory because…',
    o: ['It can only create one object', 'Creation is deferred to SUBCLASSES: an abstract creator defines an abstract createX() hook; each subclass overrides it to choose the product — selection by inheritance, not by switch', 'It has more ifs', 'It requires reflection'], a: 1,
    ...
    w: { 0: 'It creates one product per call — but any number of times.',
         2: 'if-count is not a pattern criterion.',
  ...
  { q: 'Inside Logistics.planDelivery(), the call createTransport() reaches RoadLogistics\'s version via…',
    o: ['Static binding', 'Reflection', 'A switch statement', 'Dynamic dispatch (Day 4): the base class calls an overridable method on this, and the runtime routes to the actual subclass'], a: 3,
    ...
    w: { 0: 'Static binding would always run the parent\'s version — the opposite of the trick.',
         1: 'No reflection involved — ordinary overriding.',
  ...
  { q: 'Integer.valueOf(42) and List.of(...) are examples of…',
    o: ['Static factory methods — named creation that can cache, return subtypes, and refuse', 'Abstract factories', 'Constructors', 'Builders'], a: 0,
    ...
    w: { 1: 'Abstract Factory is an OBJECT with several create methods (tomorrow).',
         2: 'Constructors are called WITH new — these methods replace that call.',
  ...
  { q: 'Which is NOT one of the classic advantages of a static factory over a constructor?',
    o: ['It has a meaningful NAME (Money.zero(INR) vs new Money(…))', 'It can return a cached instance instead of always allocating', 'It runs faster than constructors in all cases', 'It can return any SUBTYPE of the declared return type'], a: 2,
    ...
    w: { 0: 'Naming IS a classic advantage — readable creation.',
         ...
         3: 'Subtype returns IS the third (List.of\'s hidden classes).' },
    r: { id: 's7', label: 'Section 7 — Effective Java Item 1' } },
  { q: '"But the factory still contains a switch — isn\'t that an OCP violation?"',
    o: ['Yes — factories are anti-patterns', 'No — Day 12\'s rule was that the cartridge list must live in ONE contained place; the factory IS that place. Adding a product edits one case line, not N client files', 'Switches are illegal in Java', 'Yes, unless you use reflection'], a: 1,
    ...
    w: { 0: 'Factories are among the most-used patterns in existence — hardly anti.',
         2: 'Switches are core Java — and got nicer with switch expressions.',
         3: 'Reflection/registries can remove even that switch, but they\'re an escalation, not a requirement.' },
    r: { id: 's2', label: 'Section 2 — the contained switch' } },
  { q: 'What is "factory-itis"?',
    o: ['Using two factories', 'A naming convention', 'A JVM error', 'Wrapping EVERY new in a factory, even for simple value objects with one obvious implementation — ceremony without a varying axis (YAGNI, Day 16)'], a: 3,
    ...
    w: { 0: 'Two factories can be fine; ZERO varying axes is the disease.',
         1: 'It\'s an anti-pattern name, not a convention.',
         2: 'No JVM errors here — only design ones. 🙂' },

...
const QUESTIONS = [
  { q: 'The "telescoping constructor" problem is…',
    o: ['Constructors that call themselves', 'A JVM limitation', 'A chain of overloads (2, 3, 4… params) that still can\'t express arbitrary combinations — forcing monster calls full of positional flags', 'Constructors that are too long to read'], a: 2,
    ...
    w: { 0: 'Constructors calling each other is healthy this(...) chaining (Day 1).',
         1: 'No JVM limit involved.',
         3: 'Length alone isn\'t it — the overload-chain structure is.' },
  ...
  { q: 'The JavaBeans alternative (empty constructor + setters) trades the telescope for which two sins?',
    o: ['Too many imports', 'A window where the object EXISTS but is half-built/invalid, and permanent mutability — anyone can setX() forever', 'It cannot compile', 'Slowness and memory'], a: 1,
    ...
    w: { 0: 'Imports are fine. 🙂',
         2: 'It compiles beautifully — that\'s exactly the danger.',
         3: 'Setters are no slower — speed isn\'t the sin.' },
    r: { id: 's4', label: 'Section 4 — the setter "fix"' } },
  { q: 'In a fluent builder, each method like .cheese() ends with `return this;` — why?',
    o: ['It creates a copy each time', 'Java requires it', 'It returns the burger', 'It returns the SAME builder so calls chain: builder.size("M").cheese().tomato() — the Day 1 method-chaining trick, applied to construction'], a: 3,
    ...
    w: { 0: 'Copies each step would be wasteful (though some functional builders do!).',
         1: 'Java requires no return convention — fluency is a design choice.',
         2: 'Only build() returns the burger — steps return the COUNTER.' },
  ...
  { q: 'Where do REQUIRED vs OPTIONAL parameters go in an Effective-Java-style builder?',
    o: ['Required → the builder\'s constructor/entry method (can\'t start without them); optional → fluent methods you may skip', 'All optional', 'All in build()', 'Required as setters on the product'], a: 0,
    ...
    w: { 1: 'All-optional means nothing is guaranteed — invalid by default.',
         2: 'All-in-build() works but finds the gap at runtime, late.',
  ...
  { q: 'What makes build() the most important line of the pattern?',
    o: ['It is the single validation gate: cross-field checks run there, and only a COMPLETE, immutable product is ever released (fail fast, Day 19)', 'It clears memory', 'It prints the object', 'It calls super()'], a: 0,
    ...
    w: { 1: 'Memory is the GC\'s business.',
         2: 'Printing is incidental.',
  ...
  { q: 'Which JDK lines are builders you have already used?',
    o: ['Math.max and Math.min', 'System.out.println', 'Integer.parseInt', 'new StringBuilder().append(…).append(…).toString() and HttpRequest.newBuilder(uri).timeout(…).build()'], a: 3,
    ...
    w: { 0: 'Math methods are pure functions, no accumulation.',
         1: 'println is output.',
         2: 'parseInt converts — single-shot, no steps.' },
  ...
  { q: 'The original GoF Builder (with a Director) differs from the everyday Effective-Java builder because…',
    o: ['It is newer', 'GoF aims at building DIFFERENT REPRESENTATIONS via a builder interface driven by a Director; the everyday version just tames many-parameter construction of one class', 'It has no methods', 'It cannot validate'], a: 1,
    ...
    w: { 0: 'Age isn\'t the difference; intent is (representations vs parameter taming).',
         2: 'It has the same step methods — plus a Director orchestrating them.',
         3: 'Both flavors can validate.' },
    r: { id: 's7', label: 'Section 7 — the GoF footnote' } },
  { q: 'When is a builder OVERKILL?',
    o: ['More than 5 fields', 'Whenever records exist', 'Few required params, no optionals — new Point(3,4) or Money.of(…) say it all; builders earn their keep on OPTIONALITY, not on mere existence of rules', 'Never'], a: 2,
    ...
    w: { 0: 'Field COUNT isn\'t the trigger — five required fields can still suit a constructor.',
         1: 'Records and builders compose, not compete.',

...
const QUESTIONS = [
  { q: 'The Abstract Factory pattern\'s intent is…',
    o: ['To create one object lazily', 'To create FAMILIES of related objects that are guaranteed to match, without clients naming concrete classes', 'To make all factories abstract classes', 'To replace constructors with static methods'], a: 1,
    ...
    w: { 0: 'Lazy single creation is Singleton/lazy-init territory.',
         2: '"Abstract" describes the factory INTERFACE, not a class-kind rule.',
         3: 'Static factories are Day 22\'s tool — different animal.' },
  ...
  { q: 'How does Abstract Factory make a Victorian chair + Modern sofa mix IMPOSSIBLE (not just unlikely)?',
    o: ['Runtime style checks on every product', 'Every product the client gets comes from the SAME factory object — and each concrete factory only knows how to make its own family', 'A linter rule', 'Products throw if mismatched'], a: 1,
    ...
    w: { 0: 'Runtime checks catch mistakes LATE — the pattern prevents them from being writable.',
         2: 'Linters are discipline with tooling — still weaker than structure.',
         3: 'Products never check each other — they never need to.' },
    r: { id: 's3', label: 'Section 3 — the room builder' } },
  { q: 'The structure of Abstract Factory is…',
    o: ['One static create() with a big switch', 'A factory INTERFACE with one createX() per product type, plus one concrete factory per family producing that family\'s products', 'A singleton holding all products', 'A subclass per product'], a: 1,
    ...
    w: { 0: 'A big switch is the simple factory shape — families need a factory PER family.',
         2: 'A singleton holding products is a cache, not a factory.',
  ...
  { q: 'Factory Method vs Abstract Factory in one line?',
    o: ['They are identical', 'FM: one hook creating ONE product (often via subclassing). AF: an object with several creation methods producing a MATCHING FAMILY', 'AF is just FM with static methods', 'FM is newer'], a: 1,
    ...
    w: { 0: 'They differ in structure AND intent.',
         2: 'Static-ness is orthogonal.',
         3: 'Both date to the same 1994 book.' },
    r: { id: 's4', label: 'Section 4 — kit of factory methods' } },
  { q: 'Where is the ONE place that decides which family the whole app uses?',
    o: ['Every client checks the OS', 'The composition root (main/config): GuiFactory f = new MacFactory(); — injected everywhere (Day 15), so the theme swap is a one-line change', 'A global variable', 'Each product decides'], a: 1,
    ...
    w: { 0: 'Per-client OS checks re-scatter the choice the pattern centralized.',
         ...
         3: 'Products choosing their family is circular — the factory IS the choice.' },
  ...
  { q: 'The honest trade-off of Abstract Factory: which growth is cheap, and which hurts?',
    o: ['Both are cheap', 'New FAMILY = cheap (add classes, edit nothing). New PRODUCT TYPE = expensive (the factory interface grows, so every family must change)', 'New product type is cheap, new family hurts', 'Neither is possible'], a: 1,
    ...
    w: { 0: 'Every pattern pays somewhere — this one\'s bill arrives with new product types.',
         2: 'Exactly reversed — the most common exam slip on this pattern.',
         3: 'Both growths are possible; only their COSTS differ.' },
    r: { id: 's6', label: 'Section 6 — the trade-off axis' } },
  { q: 'Which is a real-world Abstract Factory sighting?',
    o: ['String.valueOf', 'Swing\'s pluggable Look-and-Feel — switching it re-skins every widget from one consistent family (also: a FakeRepositoryFactory wiring matching fakes for tests)', 'System.out.println', 'ArrayList'], a: 1,
    ...
    w: { 0: 'String.valueOf is a static factory method (Day 22).',
         2: 'println is plain output.',
         3: 'ArrayList is a product, not a factory.' },

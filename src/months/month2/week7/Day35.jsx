...
const QUESTIONS = [
  { q: 'The Template Method pattern\'s intent is…',
    o: ['To swap whole algorithms at runtime', 'To define an algorithm\'s SKELETON in a base-class method, deferring specific STEPS to subclasses — they redefine steps without changing the structure', 'To freeze requests as objects', 'To give objects lifecycles'], a: 1,
    ...
    w: { 0: 'Whole-algorithm swap is Strategy (31) — here only STEPS vary inside a fixed frame.',
         2: 'Frozen requests are Command (33).',
         3: 'Lifecycles are State (34).' },
    r: { id: 's1', label: 'Section 1 — the franchise manual' } },
  { q: 'The Hollywood Principle — meaning?',
    o: ['Make code dramatic', '"Don\'t call us, we\'ll call you": the PARENT calls the child\'s overrides at the right moments — inversion of control via inheritance; children never orchestrate', 'Always use callbacks', 'Never use inheritance'], a: 1,
    ...
    w: { 0: 'Tempting, but no. 🙂',
         2: 'Callbacks are ONE other way to be called; the principle is broader.',
  ...
  { q: 'Why should the template method itself be FINAL?',
    o: ['final runs faster', 'The skeleton IS the contract — order and invariants (mask before write!). If subclasses could override it, the one guarantee the pattern sells evaporates', 'Java requires it', 'To prevent lambdas'], a: 1,
    ...
    w: { 0: 'JIT speed is a footnote.',
         2: 'No language rule — a design rule.',
         3: 'Lambdas are unrelated.' },
    r: { id: 's4', label: 'Section 4 — three kinds of methods' } },
  { q: 'Abstract step vs HOOK?',
    o: ['Synonyms', 'Abstract step: subclass MUST implement. Hook: optional override with a sensible default (often empty, or a boolean like customerWantsCondiments) — subclasses MAY plug in', 'Hooks are static', 'Abstract steps return void only'], a: 1,
    ...
    w: { 0: 'Must vs may IS the design decision.',
         2: 'static cannot be overridden (Day 4).',
  ...
  { q: 'Why is calling an OVERRIDABLE method from a CONSTRUCTOR a famous bug?',
    o: ['Compile error', 'The parent constructor runs BEFORE child field initializers (Day 3) — the child\'s override executes against null/zero fields: a half-born object answering the phone', 'It deadlocks', 'It skips the parent constructor'], a: 1,
    ...
    w: { 0: 'It compiles cheerfully — that is the danger.',
         2: 'No threads needed.',
  ...
  { q: 'Template Method vs Strategy — the decision guide?',
    o: ['Always Strategy', 'TM (inheritance): variants share skeleton + protected state, known at compile time, vary in 1–2 steps. Strategy (composition): runtime swap, free combination, isolated testing — the modern default; TM still owns framework base classes', 'Always TM', 'They cannot do the same jobs'], a: 1,
    ...
    w: { 0: '"Always" ignores AbstractList and every framework you use.',
         2: '"Always TM" re-buys the inheritance taxes daily.',
         3: 'They overlap heavily — hence the comparison.' },
    r: { id: 's7', label: 'Section 7 — the showdown' } },
  { q: 'What is the "template CALLBACK" (Spring JdbcTemplate) twist?',
    o: ['A deprecated API', 'The skeleton stays (connect, loop, close, translate errors) but the varying step arrives as a LAMBDA parameter instead of a subclass override — Template Method rebuilt with composition', 'A new thread per query', 'An XML feature'], a: 1,
    ...
    w: { 0: 'Alive in every Spring app.',
         2: 'Threading is unrelated.',
         3: 'No XML anywhere. 🙂' },

...
const QUESTIONS = [
  { q: 'The Command pattern\'s intent is…',
    o: ['To choose among algorithms', 'To ENCAPSULATE A REQUEST AS AN OBJECT — so it can be passed around, queued, logged, scheduled, and undone', 'To notify many objects', 'To simplify a subsystem'], a: 1,
    ...
    w: { 0: 'Choosing among ways to do ONE job is Strategy (31).',
         ...
         3: 'One simple door is Facade (29).' },
  ...
  { q: 'In standard undo/redo, what happens to the REDO stack when a brand-new command executes?',
    o: ['Nothing', 'It is CLEARED — you forked the timeline, and the old future is no longer reachable', 'It is copied to undo', 'It doubles'], a: 1,
    ...
    w: { 0: 'Leaving stale redos would let you "redo" into a state that no longer follows.',
         2: 'No copying — discarding.',
         3: 'Nothing doubles.' },
    r: { id: 's6', label: 'Section 6 — undo mechanics' } },
  { q: 'Runnable and ExecutorService, in today\'s vocabulary?',
    o: ['Strategy and Context', 'Runnable = a Command (execute() renamed run(), no undo); the executor = an Invoker with a queue and worker threads — you have been queuing command objects since your first thread pool', 'Observer and Subject', 'Adapter and Adaptee'], a: 1,
    ...
    w: { 0: 'The executor doesn\'t pick HOW — it runs WHATEVER arrives, later.',
         2: 'Nothing subscribes here.',
  ...
  { q: 'A MacroCommand (MovieNight = dim + AC + TV) is which two patterns shaking hands?',
    o: ['Command + Singleton', 'Command + Composite: a command that HOLDS a list of commands and executes them in order — undo runs the list in REVERSE', 'Command + Flyweight', 'Command + Proxy'], a: 1,
    ...
    w: { 0: 'Instance count is irrelevant.',
         2: 'No shared-state economics here.',
         3: 'Nothing is access-controlled.' },
    r: { id: 's3', label: 'Section 3 — the remote & the macro' } },
  { q: 'Command vs Strategy — both are "an object with one method". The distinction?',
    o: ['None', 'Strategy = HOW to perform one job, chosen among interchangeable ways, context supplies the data per call. Command = A COMPLETE THING TO DO, self-contained with its receiver and arguments — many heterogeneous ones can sit in one queue', 'Command is older', 'Strategy cannot use lambdas'], a: 1,
  ...
  { q: 'undo() for "send email" — what is the honest answer?',
    o: ['Store the email and delete it from their inbox', 'Some effects are NOT invertible: you cannot unsend. Real systems use COMPENSATION (send a correction) or confirm-before-execute — and an honest design marks such commands non-undoable', 'All commands can be undone', 'Throw UnsupportedOperationException and call it undo'], a: 1,
    ...
    w: { 0: 'You do not control their inbox. 🙂',
         2: 'Pleasant fiction.',
         3: 'A throwing undo() is Day 13\'s penguin in a new costume.' },
    r: { id: 's9', label: 'Section 9 — Trap 2: the impossible undo' } },
  { q: 'Where should the BUSINESS LOGIC of a command live?',
    o: ['Inside execute(), all of it', 'In the RECEIVER — the command binds and forwards (a few lines); a fat execute() full of rules is a god-method hiding in a pattern costume', 'In the invoker', 'In the undo stack'], a: 1,
    ...
    w: { 0: 'That builds unreachable, untestable logic blobs per action.',
         2: 'The invoker\'s ignorance is sacred — that\'s its whole value.',
         3: 'The stack stores history, not behavior.' },

...
const QUESTIONS = [
  { q: 'The State pattern\'s intent — including GoF\'s strange phrase?',
    o: ['To let an object ALTER ITS BEHAVIOR when its internal state changes — "the object will appear to change its class"', 'To notify dependents', 'To choose an algorithm', 'To freeze requests'], a: 0,
    ...
    w: { 1: 'Broadcasting is Observer (32).',
         2: 'Algorithm choice is Strategy (31) — the twin, not the same.',
         3: 'Frozen requests are Command (33).' },
    r: { id: 's1', label: 'Section 1 — the phone button' } },
  { q: 'What exact disease does State cure?',
    o: ['Memory leaks', 'Slow switches', 'Too many classes', 'The same switch-on-status duplicated across EVERY method — a methods × states grid where each new state edits every method and one missed case fails silently'], a: 3,
    ...
    w: { 0: 'No leak involved.',
         1: 'Switch speed is fine; switch SPRAWL is the disease.',
  ...
  { q: 'In the canonical pattern, who performs the transition to the next state?',
    o: ['A background thread', 'The garbage collector', 'The current STATE object itself — it handles the event, then calls context.setState(next); states know their successors', 'The client'], a: 2,
    ...
    w: { 0: 'No threads required.',
         1: 'GC collects, never decides. 🙂',
         3: 'Clients just fire events (insertCoin) — they never steer the lifecycle.' },
    r: { id: 's4', label: 'Section 4 — the structure' } },
  { q: 'State vs Strategy — settle Day 31\'s teaser precisely.',
    o: ['Strategy has more classes', 'Identical patterns', 'State is compile-time', 'Strategy: client/config picks ONE brain, strategies ignore each other, swap is rare and external. State: the object cycles through MANY states over its lifecycle, states know and CREATE their successors, transitions are internal and constant'], a: 3,
    ...
    w: { 0: 'Class count is incidental.',
         1: 'The most-asked GoF comparison exists because they are NOT the same.',
  ...
  { q: 'When should you GRADUATE from Day 19\'s enum state machine to state CLASSES?',
    o: ['Always use classes', 'Never — enums always win', 'When per-state BEHAVIOR grows beyond transition checks: states need their own fields, dependencies (injected services), timers, or substantial logic — an enum constant gets cramped', 'When you have more than 2 states'], a: 2,
    ...
    w: { 0: 'A 3-state label-only lifecycle in classes is ceremony.',
         1: '"Always enums" caps you when states need DI or fields.',
  ...
  { q: 'Which JDK lifecycle is a state machine you already debug?',
    o: ['Thread states: NEW → RUNNABLE ⇄ BLOCKED/WAITING/TIMED_WAITING → TERMINATED — Thread.getState() returns the current state, and legal transitions are fixed', 'Integer caching', 'String concatenation', 'ArrayList growth'], a: 0,
    ...
    w: { 1: 'That\'s the flyweight from Day 30.',
         2: 'Strings just concatenate.',
         3: 'Capacity growth is an array trick.' },

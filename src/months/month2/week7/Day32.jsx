...
const QUESTIONS = [
  { q: 'The Observer pattern\'s intent is…',
    o: ['To define a ONE-TO-MANY dependency: when the subject changes, ALL registered dependents are notified automatically — without the subject knowing who they are', 'To swap algorithms', 'To wrap objects', 'To queue requests'], a: 0,
    ...
    w: { 1: 'Swappable algorithms = Strategy (31).',
         2: 'Wrapping = Week 6 territory.',
         3: 'Requests-as-objects = Command (33).' },
    r: { id: 's1', label: 'Section 1 — the channel' } },
  { q: 'What EXACTLY does the subject know about its observers?',
    o: ['Everything, via reflection', 'Their memory addresses only', 'Only that each implements the Observer interface and is currently on the list — nothing about who they are or what they do with the news', 'Their concrete classes and purposes'], a: 2,
    ...
    w: { 0: 'No reflection involved.',
         1: 'It holds references, but the design point is the interface contract.',
         3: 'Knowing concretes would re-couple the subject to every reaction.' },
    r: { id: 's2', label: 'Section 2 — the structure' } },
  { q: 'Push vs pull notification models — define both.',
    o: ['Synchronous vs asynchronous', 'Push: the subject sends the event DATA in the call (onOrderPlaced(order)); pull: it sends only "something changed" and observers query back for what they need', 'Public vs private', 'TCP vs UDP'], a: 1,
    ...
    w: { 0: 'Threading is an independent axis — both models can be sync or async.',
         2: 'Visibility is unrelated.',
         3: 'Networking terms, different layer.' },
    r: { id: 's5', label: 'Section 5 — push vs pull' } },
  { q: 'The "lapsed listener" problem is…',
    o: ['A compile error', 'Too many listeners', 'Observers that were never unsubscribed: the subject\'s strong references keep dead objects (closed screens!) alive forever — pointless notifications plus a memory leak', 'Listeners that respond slowly'], a: 2,
    ...
    w: { 0: 'It compiles beautifully and leaks silently.',
         1: 'Many LIVE listeners is fine.',
         3: 'Slowness is a different trap (blocking the notify loop).' },
    r: { id: 's8', label: 'Section 8 — the chaos lab' } },
  { q: 'Observer #2 throws inside update(). In a naive notify loop, what happens?',
    o: ['Only #2 is affected', 'Nothing, exceptions are ignored', 'The JVM retries', 'The exception rips up through notify() into the SUBJECT\'s caller, and every observer after #2 is silently never notified — one bad listener sinks the broadcast'], a: 3,
    ...
    w: { 0: 'That\'s the ROBUST behavior — only after you isolate.',
         1: 'Unhandled exceptions never self-ignore.',
  ...
  { q: 'Why were java.util.Observer/Observable deprecated (Java 9)?',
    o: ['Observable is a CLASS (steals your one extends — Day 7!), notifies with a typeless Object payload, and its setChanged/notify protocol is clunky — interfaces + your own typed events are simply better', 'Lambdas replaced all OOP', 'Observers became illegal', 'They were too fast'], a: 0,
    ...
    w: { 1: 'Lambdas plug INTO listener interfaces happily.',
         2: 'The PATTERN thrives; that implementation died.',
         3: 'Performance was never the issue.' },
    r: { id: 's6', label: 'Section 6 — Java history' } },
  { q: 'button.addActionListener(e -> save()) — which pattern, and who plays whom?',
    o: ['Command only', 'Adapter', 'Strategy: the button swaps algorithms', 'Observer: the button is the SUBJECT (click = state change), your lambda is an observer on its listener list — every GUI toolkit runs on this'], a: 3,
    ...
    w: { 0: 'Command describes the action object; the subscription mechanism is observer.',
         1: 'Nothing is translated.',
         2: 'The button doesn\'t compute differently — it ANNOUNCES.' },

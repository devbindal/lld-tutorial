...
const QUESTIONS = [
  { q: 'The Strategy pattern\'s intent is…',
    o: ['To give an object states', 'To create object families', 'To wrap objects with features', 'To define a FAMILY of interchangeable algorithms, encapsulate each one, and let the algorithm vary independently of the clients that use it'], a: 3,
    ...
    w: { 0: 'Self-changing behavior over a lifecycle is State (34) — the twin, next week.',
         1: 'Families of PRODUCTS are Abstract Factory (23).',
         2: 'Feature-wrapping is Decorator (27).' },
    r: { id: 's1', label: 'Section 1 — the navigator' } },
  { q: 'GoF participants: name the three.',
    o: ['Subject, Observer, Event', 'Target, Adaptee, Adapter', 'Component, Leaf, Composite', 'Strategy (the interface), ConcreteStrategies (one per algorithm), and Context (holds a Strategy reference and delegates to it)'], a: 3,
  ...
  { q: 'Why is strategy-by-composition stronger than algorithm-by-subclassing (PricingCheckout extends Checkout)?',
    o: ['It uses less memory', 'The algorithm becomes swappable at RUNTIME, combinable with any context, testable alone — and the context stays closed (Day 7 + Day 12 in one move)', 'It compiles faster', 'Subclassing is deprecated'], a: 1,
    ...
    w: { 0: 'Memory is a wash (often better — see sharing).',
         2: 'Compilation is irrelevant.',
         3: 'Subclassing is fine for true is-a — this is not that.' },
    r: { id: 's3', label: 'Section 3 — why composition' } },
  { q: 'The "who picks the strategy?" problem is best solved by…',
    o: ['ONE contained selector — a registry Map (code → strategy) or factory — so selection knowledge has a single home and clients just look up', 'Random selection', 'Always hardcoding one', 'Every client writing its own if/else over types'], a: 0,
    ...
    w: { 1: 'Comedy, not architecture. 🙂',
         2: 'Hardcoding one forever means you never needed Strategy (fair YAGNI outcome!).',
         3: 'Scattered selection re-creates the disease the pattern cured.' },
    r: { id: 's5', label: 'Section 5 — the coupon registry' } },
  { q: 'Why is Comparator the world\'s most-used Strategy?',
    o: ['sort() is a closed context written decades ago; every Comparator you pass is a pluggable comparison ALGORITHM — millions of them invented since, zero edits to sort()', 'It only sorts numbers', 'It is not a strategy', 'It uses inheritance'], a: 0,
    ...
    w: { 1: 'It compares anything you define.',
         2: 'It is the canonical one.',
         3: 'Pure composition — you pass an object/lambda in.' },
    r: { id: 's7', label: 'Section 7 — Comparator lab' } },
  { q: 'What did lambdas (Java 8) change about Strategy?',
    o: ['Removed the interface', 'Collapsed the ceremony: a one-method strategy interface is a functional interface, so a strategy is now a lambda or method reference — same pattern, one line', 'Made strategies stateful', 'Made it obsolete'], a: 1,
    ...
    w: { 0: 'The functional interface IS the strategy interface.',
         2: 'Lambdas push the OPPOSITE way — toward stateless.',
         3: 'More alive than ever — every lambda you pass to a method is one.' },
    r: { id: 's6', label: 'Section 6 — the lambda era' } },
  { q: 'Strategy vs State — same UML diagram. The real differences?',
    o: ['Strategy needs enums', 'None, same pattern', 'WHO drives change: clients/config choose a strategy (and rarely mid-flow); a State object TRANSITIONS ITSELF to the next state as events occur — and states know about each other, strategies are strangers', 'State is compile-time'], a: 2,
    ...
    w: { 0: 'Enums are one implementation option, never a requirement.',
         1: 'The structure twins are distinguished by intent — the recurring GoF exam move.',
         3: 'Both are runtime mechanisms.' },
    r: { id: 's8', label: 'Section 8 — the twins table' } },
  { q: 'Why should strategies (usually) be STATELESS — and what does that buy?',
    o: ['It prevents lambdas', 'Statelessness is required by Java', 'No per-use mutable fields → one instance is safely shared by every cart and every thread (flyweight, Day 30) — per-call data arrives as parameters instead', 'It makes them faster to write'], a: 2,
    ...
    w: { 0: 'Lambdas are naturally stateless — they encourage it.',
         1: 'No language rule — a sharing-safety rule.',
         3: 'Writing effort is similar.' },

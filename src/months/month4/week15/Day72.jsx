...
const QUESTIONS = [
  {
    ...
    w: {
      ...
      1: 'Blocking bids in the last 5 minutes is too restrictive and not the standard approach. The solution is to extend time, not block bids.',
  ...
  },
  {
    ...
    w: {
      0: 'The highest bidder does NOT win if the reserve is not met. The reserve exists precisely to protect the seller from selling below their minimum.',
      1: 'The system cannot force a bidder to pay ₹8,000 when they bid ₹7,500. That would be a breach of contract.',
      3: 'Sellers set reserve prices specifically because they are NOT obligated to sell at any price — the reserve is a binding minimum.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'volatile ensures visibility (changes are seen immediately) but NOT atomicity. Two threads can still both read the old value, validate, and both write — causing a lost-update race.',
      2: 'AtomicInteger handles a single numeric CAS, but bid placement involves multiple steps (read highest, validate, add to list, check snipe). A single atomic integer is not enough.',
      3: 'Modern JVMs do NOT make concurrent writes safe by default. The Java Memory Model requires explicit synchronization for shared mutable state.',
    },
    r: { id: 's6', label: 'Section 6 — Bid validation rules' },
  },
  {
    ...
    w: {
      ...
      2: 'Jumping to the max (₹10,000) wastes money. The proxy system bids the minimum needed: just one increment above the competitor.',
  ...
  },
  {
    ...
    o: ['State', 'Command', 'Observer', 'Prototype'],
    a: 1,
    ...
    w: {
      0: 'State models the auction\'s lifecycle (OPEN, CLOSING, CLOSED). A Bid is not a state — it is a recorded action.',
  ...
  },
  {
    ...
    w: {
      0: 'This path skips CLOSING — it only applies when the auction ends with no bids in the snipe window. Once any bid triggers snipe prevention, CLOSING is mandatory.',
      2: 'CLOSING never transitions back to OPEN. The state machine is one-directional — each state only knows its valid successors going forward.',
      3: 'DRAFT cannot jump to CLOSING — the auction must first be opened (OPEN) before any bids can arrive to trigger the extension.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'BigDecimal is actually SLOWER than double for CPU arithmetic. The reason to use it is correctness, not performance.',
      1: 'double can represent values up to ~1.8 × 10^308 — far beyond any bid amount. Its problem is decimal PRECISION, not range.',
  ...
  },
  {
    ...
    w: {
      ...
      1: '₹9,500 would be correct in proxy bidding within a first-price system (one increment above the runner-up). In a pure Vickrey auction, the exact second-highest bid is the price — no increment.',

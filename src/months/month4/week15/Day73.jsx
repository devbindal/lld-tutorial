...
const QUESTIONS = [
  {
    ...
    w: {
      1: 'Pull-on-read is for celebrities. For small follower counts, you want the read to be fast (O(1)), which requires prebuilding the feed on write.',
      2: 'Feeds are never built from search — that would require scanning all posts in the system on every page load.',
      3: 'The hybrid strategy skips fan-out only above the celebrity threshold (e.g. >1M followers). 3 followers does not qualify.',
    },
  ...
  },
  {
    ...
    o: ['O(followers) = O(5M)', 'O(1)', 'O(log N)', 'O(followers²)'],
    a: 1,
    ...
    w: {
      0: 'O(followers) is exactly the celebrity problem — writing to 5M queues would take too long and generate enormous write traffic. That\'s why fan-out is skipped.',
  ...
  },
  {
    ...
    w: {
      0: 'Celebrity posts must still appear in the feed — they are just delivered via pull-on-read instead of push.',
      2: 'Retroactive fan-out is not standard — it would be extremely expensive (5M writes triggered by a user opening a feed is worse than the original celebrity problem).',
      3: 'Querying all 200 followees defeats the purpose of prebuilding the queue for regular users. Only celebrities require pull.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Concurrent inserts are a separate concern. Even a single insert by one user breaks the offset for another user mid-browse.',
      1: 'Offset pagination supports any sort order. The problem is purely about positional stability when the underlying data changes.',
      2: 'Offsets are plain integers — no floating point involved. The bug is positional, not numeric.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      1: 'Caching the entire result set is a separate optimization; the cursor itself is just a stable reference point.',
  ...
  },
  {
    ...
    w: {
      0: 'Composition means the child cannot exist without the parent (Day 6). Users exist independently of who follows them.',
      1: 'A Follower is not a different type of User — it is the same User entity seen from a different angle. IS-A inheritance would mean a separate class, which is wrong.',
  ...
  },
  {
    ...
    o: ['Iterator (Day 37)', 'Composite (Day 28)', 'Chain of Responsibility (Day 36)', 'Memento (Day 39)'],
    a: 0,
    ...
    w: {
      ...
      2: 'Chain of Responsibility passes a request along handlers — it\'s about processing pipelines, not sequential traversal of a dataset.',
      3: 'Memento saves and restores state snapshots. A cursor does point into state, but the pattern here is iteration (advance through a sequence), not state restoration.',
    },
    r: { id: 's9', label: 'Section 9 — Cursor vs offset pagination' },
  },
  {
    ...
    w: {
      0: 'Full replay on every unfollow is catastrophically expensive — a user could follow/unfollow hundreds of accounts.',
      1: 'Immediate deletion requires scanning and deleting every FeedItem from Dave for Alice — potentially millions of rows at scale. This is done asynchronously in background jobs if at all.',

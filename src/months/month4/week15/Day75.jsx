...
const QUESTIONS = [
  { q: 'The drill clock starts. The FIRST thing you do?',
    o: ['Open a Java file and start the entity classes', 'Write 4–6 clarifying questions on paper and state exclusions out loud — the questions shape every decision and are graded', 'Draw a full UML class diagram', 'Name the patterns you plan to use'],
    a: 1,
    ...
    w: { 0: 'Entities before scope are entities for the wrong problem.', 2: 'A quick entity TABLE is all you need; full UML art wastes the clock.', 3: 'Pattern selection happens after scope — patterns are driven by constraints, not the other way.' },
  ...
  { q: 'Track A (Gym Membership): two members simultaneously call book() and both see capacity = 9/10. What happens without synchronization?',
    o: ['Nothing — the check is read-only so there is no race', 'Both bookings succeed — capacity becomes 11/10; classic check-then-act race; fix: synchronized block covering check AND insert', 'A ConcurrentModificationException is thrown', 'The second booking blocks until the first completes'],
    a: 1,
    ...
    w: { 0: 'Read-only checks are not atomic with the subsequent write — another thread can win the race between the read and the write.', 2: 'ConcurrentModificationException is a different issue (modifying a list while iterating).', 3: 'No synchronization means no blocking — both threads proceed in parallel.' },
  ...
  { q: 'Track B (Code Review): all assigned reviewers have APPROVED a PR, but two comments are still open (unresolved). Should the PR merge?',
    o: ['Yes — all reviewers approved, so it merges', 'No — merge eligibility is a conjunction: ALL approvals AND NO open comments; one open comment blocks merge even if all reviewers approved', 'Only if the author resolves one of the two comments', 'Only if a reviewer marks the PR as APPROVED again after comments are resolved'],
    a: 1,
    ...
    w: { 0: 'Approvals alone are not sufficient — open comments represent unaddressed review feedback that blocks the merge.', 2: 'Resolving one of two is still one unresolved — the PR still cannot merge.', 3: 'The approval is already recorded; re-approving is not how GitHub-style reviews work.' },
  ...
  { q: 'Do two hotel reservations overlap? Reservation A: Mon–Thu. Reservation B: Thu–Sat. (Half-open intervals [checkIn, checkOut))',
    o: ['Yes — they share Thursday', 'No — Thursday is the check-out day for A and check-in for B; half-open means A does not occupy Thursday; the next guest can check in the same day', 'It depends on the check-out time', 'They partially overlap but it is allowed'],
    a: 1,
    ...
    w: { 0: 'If intervals were closed [Mon, Thu] and [Thu, Sat], then Thu would be double-booked. Half-open solves exactly this.', 2: 'In this design, check-out is the exclusive end of the interval; time-of-day is out of scope.', 3: 'There is no partial overlap — the intervals are disjoint by design.' },
  ...
  { q: 'An auction is in OPEN state. A bid arrives 3 minutes before the auction closes. What should the CLOSING state do?',
    o: ['Reject the bid — the auction is almost over', 'Accept the bid and extend the endTime by 5 minutes, transitioning to CLOSING state to prevent snipe bidding', 'Accept the bid and close immediately', 'Transition directly to SETTLED'],
    a: 1,
    ...
    w: { 0: 'Rejecting a bid because the auction is nearly over is unfair — the auction is still OPEN.', 2: 'Closing immediately after a late bid is exactly snipe bidding — the system should prevent it.', 3: 'SETTLED requires CLOSED first; the auction must finish accepting bids before settlement.' },
    r: { id: 's2', label: 'Section 2 — skills table' } },
  { q: 'Why does offset-based feed pagination break when a new post is inserted?',
    o: ['Because offset is an int and can overflow', 'A new post at position 0 shifts every subsequent item by 1 — the user\'s "page 2" now re-shows the last item of page 1 as a duplicate', 'Because the feed is sorted in reverse and skip() counts from the wrong end', 'Because List.stream().skip() is lazy and not cached'],
    a: 1,
    ...
    w: { 0: 'int overflow would only happen with billions of posts — not a practical concern.', 2: 'The direction of sorting does not change the offset-shift problem.', 3: 'Laziness and caching are unrelated to the duplicate-on-insert bug.' },
    r: { id: 's2', label: 'Section 2 — skills table' } },
  { q: 'A task board user creates a new column called "Blocked". With the enum-column design, what happens?',
    o: ['The system adds a new enum constant at runtime', 'It is impossible — adding a column requires recompiling the code; the board is inflexible by design', 'The column is stored as a string but mapped to the nearest enum constant', 'The task.status field is set to null for that column'],
    a: 1,
    ...
    w: { 0: 'Java enums are compiled into bytecode — they cannot be extended or added at runtime.', 2: 'Mapping to the nearest enum constant loses information and produces wrong states.', 3: 'A null status would crash any code that reads the task\'s column.' },
    r: { id: 's2', label: 'Section 2 — skills table' } },
  { q: 'A user moves Task T from "In Progress" to "Review", then moves it to "Done". Then they click Undo. What happens to the redo stack?',
    o: ['The redo stack contains [move to Done]', 'Nothing — redo is cleared only by Undo, not by moves', 'The redo stack is cleared — it was cleared when the move to Done was performed as a new move', 'The redo stack contains [move to Review]'],
    a: 0,
    ...
    w: { 1: 'Redo is cleared when a NEW move is made, not when Undo is called — Undo is the operation that populates redo.', 2: 'Redo is cleared by a new move, not by Undo itself. At this point no new move has been made — only Undo.', 3: 'The redo stack holds the most recently undone operation, which is "move to Done", not "move to Review".' },
    r: { id: 's2', label: 'Section 2 — skills table' } },
]

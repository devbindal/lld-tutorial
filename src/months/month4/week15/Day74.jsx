...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'The exact opposite is true. Task board columns CANNOT be an enum because they change at runtime. State machines with fixed states use enums.',
      2: 'Columns are highly mutable — add, rename, reorder, and delete are all core features of a task board.',
      3: 'The number of states is irrelevant. The defining difference is whether the states are known at compile time (fixed enum) or created at runtime (data).',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Performance is not the reason. Commands add a tiny overhead over a direct field update. The reason is correctness and reversibility.',
      2: 'SRP is not violated by a direct update. The reason for Command is undo/redo and audit history, not SRP.',
      3: 'Command pattern is chosen specifically when you need undo/redo or history — not as a blanket rule for lists.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Keeping the redo stack after a new move would create a branching history. Virtually no real system implements this — linear history is the correct default.',
      1: 'You are never required to redo before moving. You can freely mix moves and undos.',
      3: 'New commands always go to the END (top of stack) of the history. History is LIFO — last-in is first-undone.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'Fractional indexing is a logical ordering strategy, not a compression algorithm. It has nothing to do with database storage size.',
      2: 'Alphabetical sorting by title is a different feature (TaskSortStrategy). Fractional indexing is purely about drag-and-drop position ordering.',
  ...
  },
  {
    ...
    o: ['Strategy', 'Decorator', 'Composite', 'Chain of Responsibility'],
    a: 2,
    ...
    w: {
      ...
      3: 'Chain of Responsibility passes a request along a handler chain. The task board tree is a data hierarchy, not a request-routing chain.',
    },
  ...
  },
  {
    ...
    o: ['Command', 'Observer', 'Template Method', 'Facade'],
    a: 1,
    ...
    w: {
      ...
      2: 'Template Method defines a skeleton algorithm with steps for subclasses to fill in. Notification dispatch is not a skeleton algorithm.',
      3: 'Facade simplifies a complex subsystem behind one interface. It does not model the publish-subscribe notification relationship.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Composite does not prevent deletion — it is a structural pattern, not an enforcement mechanism. You must add that guard yourself.',
      3: 'Java GC removes objects with no live references. In a database there is no GC; you must handle cascades explicitly.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Overwriting would delete a task — a catastrophic data loss bug. Position is an ordering field, not a unique key.',
      1: 'Duplicate positions break sorted order. The UI would render two tasks at the same spot, and the result of sorting by position would be non-deterministic.',

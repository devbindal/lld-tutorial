...
const QUESTIONS = [
  {
    ...
    w: {
      1: 'A <= C AND B >= D is the "contains" condition — one range fully contains the other. This misses cases where ranges partially overlap.',
  ...
  },
  {
    ...
    w: {
      1: '"Room" is not a reserved Java keyword at all. This is a distractor.',
      2: 'Performance is a database concern, not a domain-model concern. The right model is chosen for correctness first.',
      3: 'A room CAN be booked multiple times — just for non-overlapping date ranges. Assuming "once" is what leads to incorrect models.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Template Method defines a skeleton algorithm for subclasses — it has nothing to do with atomic claims or concurrency.',
      1: 'Observer notifies subscribers of events — it does not make a claim atomic. You could use it to notify guests after booking, but it does not prevent double-booking.',
      3: 'Strategy lets you swap HOW something is done — it does not address the thread-safety of a single reservation claim.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      1: 'This is the wrong model — hotels are designed to have sequential bookings. The whole point of availability checking is to allow back-to-back bookings.',
  ...
  },
  {
    ...
    o: ['Factory Method', 'Decorator', 'Strategy', 'Composite'],
    a: 1,
    ...
    w: {
      ...
      2: 'Strategy swaps one algorithm for another entirely. Add-ons do not replace pricing — they layer on top. Strategy would force you to re-implement the entire pricing logic for each combination.',
      3: 'Composite treats a tree of objects as a single unit — useful for hierarchies (like folder/file trees), not for composing incremental price additions onto a base.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'An average has no business or legal meaning. Neither guest nor hotel would agree to this calculation.',
      1: 'Retroactively changing the price on a confirmed reservation is a consumer protection violation. The guest agreed to ₹3,200 — charging ₹4,500 would be fraud.',
      2: 'Hotel rates change constantly (seasonal pricing, demand surges). The design must handle this gracefully by snapshotting at booking time.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Chain of Responsibility passes a request along a chain of handlers — it models request routing, not object lifecycle states.',
    },

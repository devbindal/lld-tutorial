...
const QUESTIONS = [
  { q: 'The drill clock starts. The FIRST thing you do?',
    o: ['Open a new Java file and start the Board class', 'Write 4–6 clarifying questions on paper and state exclusions out loud — the questions are graded and shape every decision after', 'Set up the project structure', 'Draw a full class diagram'],
    a: 1,
    ...
    w: { 0: 'Entities before scope are entities for the wrong problem.', 2: 'Project setup happens before the timer.', 3: 'A quick entity TABLE is all you need; full UML art wastes the clock.' },
  ...
  { q: 'Track A (Library Borrowing): a member has 2 active loans. Two threads simultaneously call borrowBook() and both pass the activeLoans.size() < 3 check. What happens?',
    o: ['The second borrow blocks until the first finishes', 'The member ends up with 4 loans — classic check-then-act race; fix: synchronize on the member object or use an AtomicInteger loanCount with compareAndSet', 'A ConcurrentModificationException is thrown', 'Nothing — size() is atomic'],
    a: 1,
    ...
    w: { 0: 'No synchronization → no blocking.', 2: 'ConcurrentModificationException is a different issue (modifying a collection while iterating).', 3: 'size() is not atomic with the subsequent add.' },
  ...
  { q: 'Track B (Event Stream): you commit the consumer offset BEFORE calling process(event). A crash occurs during processing. What breaks?',
    o: ['The consumer re-reads from the beginning', 'Nothing — the offset was saved', 'At-most-once delivery / data loss — the event is marked done but was never processed. Always process FIRST, then commit (at-least-once)', 'The dead-letter queue catches it'],
    a: 2,
    ...
    w: { 0: 'The consumer restarts from the COMMITTED offset, which already skipped the failed event.', 1: 'The offset was saved but the event was never processed — a silent data loss.', 3: 'Dead-letter is for exceptions inside process(), not for events that were never processed.' },
  ...
  { q: 'Chess: adding a new piece (Amazon piece — moves like a Queen + Knight). With the polymorphism design, how many files change?',
    o: ['Every file that mentions piece types', 'Game.java must add a case', 'One new file (Amazon.java extends Piece, implements legalMoves) + one wiring line in the factory/registry — zero other changes', 'Board.java must add special handling'],
    a: 2,
    ...
    w: { 0: 'There are no type-checks to update — that was the smell we removed.', 1: 'Game.java has no switch on piece type.', 3: 'Board.java works on squares, not piece types.' },
    r: { id: 's6', label: 'Section 6 — smell hunt (snippet 1)' } },
  { q: 'Which locker slot assignment strategy wastes least large-slot space?',
    o: ['Greedy smallest-fitting: filter slots where size.fits(pkg), sort by rank ASC, take the first — wastes least space, leaves larger slots available for larger packages', 'Assign randomly to balance load', 'Find the largest slot that fits (most space wasted)', 'Assign the newest slot'],
    a: 0,
    ...
    w: { 1: 'Random ignores size; a small package in a large slot wastes capacity.', 2: 'Largest-fitting is the opposite — it wastes the most precious space.', 3: 'Recency has no bearing on fitness.' },
    r: { id: 's2', label: 'Section 2 — Week 14 skills' } },
  { q: 'Food delivery: when is the delivery agent assigned to an order?',
    o: ['After the order is CONFIRMED — the restaurant might reject a PLACED order, so assigning an agent before confirmation wastes an agent on a doomed order', 'When the order is PLACED', 'When the agent arrives at the restaurant', 'When the customer pays'],
    a: 0,
    ...
    w: { 1: 'Assigning at PLACED wastes an agent if the restaurant rejects.', 2: 'Agent arrival is a physical event, not the assignment trigger.', 3: 'Payment is a separate flow; agent assignment is about logistics.' },
    r: { id: 's2', label: 'Section 2 — skills table' } },
  { q: 'Consumer group in a Pub-Sub system means…',
    o: ['Messages are batched per group', 'All consumers in the group receive every message (fan-out)', 'The group has higher priority than solo consumers', 'Each message is delivered to exactly ONE consumer in the group — the group collectively processes the topic at its own pace, enabling load balancing across instances'],
    a: 3,
    ...
    w: { 0: 'Batching is a read-size optimization, not the group semantics.', 1: 'That is broadcast / multiple independent subscribers, not a consumer group.', 2: 'Priority is not part of the consumer group concept.' },
    r: { id: 's5', label: 'Section 5 — Track B' } },
  { q: 'You scored 11/17 on the rubric, with all misses in Pub-Sub correctness. The review loop says…',
    o: ['Re-do all of Week 14', 'Lower the rubric threshold', 'Move on — 11/17 is passing', 'Restudy ONLY Day 69 (the flagged day), actively type the channel/consumer code, then re-drill Track B at full length to verify the gap closed'],
    a: 3,
    ...
    w: { 0: 'Re-studying only the gap days is more effective than blanket review.', 1: 'Adjusting the measuring stick guarantees no improvement.', 2: '10–13 is strong maybe — the Pub-Sub gap is real and will cost you on Day-69-shaped questions.' },

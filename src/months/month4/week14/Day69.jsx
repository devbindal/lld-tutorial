...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'Bytes on disk describes storage size, not position. The offset is a sequential index number, not a byte count.',
      2: 'Delay in milliseconds describes scheduling or latency, not the consumer\'s reading position in a log.',
      3: 'Retry count is a delivery-reliability concept, not what an offset tracks. The offset is purely about reading position.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Speed is not the concern here. Committing before is actually dangerous — it trades correctness for no real performance gain in this protocol.',
      2: 'The broker does not enforce this rule — it accepts a commit at any time. The ordering requirement is a consumer-side safety guarantee.',
      3: 'Offsets only go forward — there is no backwards counting. The bug from committing before is skipping messages, not counting errors.',
    },
    r: { id: 's6', label: 'Section 6 — Consumer offset tracking' },
  },
  {
  ...
  },
  {
    ...
    w: {
      0: 'Head-of-line blocking would defeat the purpose of multiple consumers. Partitions ensure independent progress — no consumer waits for another.',
      2: 'The broker only reassigns partitions during a rebalance (consumer join/leave/crash), not dynamically due to slowness. C1 keeps its partition.',
      3: 'The broker does not pause consumers because one is slow. Each consumer pulls at its own pace.',
    },
    r: { id: 's8', label: 'Section 8 — Consumer groups' },
  },
  {
    ...
    w: {
      0: 'Messages are published directly to the main topic — there is no pre-publish staging queue in standard pub-sub.',
      2: 'Consumers do not send messages back to producers in pub-sub. The DLQ is a one-way sink for failed messages, not a reverse channel.',
  ...
  },
  {
    ...
    w: {
      0: 'Global ordering across partitions would require a central sequencer — a scalability bottleneck. Partitioned brokers deliberately avoid this.',
      2: 'Using the same key does route to the same partition (which gives ordering), but the statement "ordering is guaranteed globally" is still false — ordering is only per-partition.',
      3: 'There IS an ordering guarantee — just per-partition, not globally. Saying "no ordering anywhere" is too strong.',
    },
    r: { id: 's8', label: 'Section 8 — Consumer groups' },
  },
  {
    ...
    w: {
      ...
      3: 'Committing a future offset is either ignored or treated as the current end — it does not trigger reassignment of partitions.',
    },
    r: { id: 's8', label: 'Section 8 — Consumer groups' },
  },
  {
    ...
    w: {
      0: 'Both queues and topics typically support multiple producers. The distinction is about how messages are consumed (one receiver vs many independent receivers).',
      2: 'Both queues and topics can store messages in memory or on disk. Kafka, for example, uses disk for topics, but this is an implementation choice, not the defining difference.',
      3: 'Traditional queues often ARE FIFO (ordered). The defining difference is how many consumers receive each message, not ordering.',
    },

...
const QUESTIONS = [
  ...
  },
  {
    ...
    w: {
      0: 'A midpoint ignores that the journey is sequential: restaurant first, then customer. You can\'t "half pick up" food.',
      2: 'City center has no relation to either the restaurant or the customer — it would maximize both wait times.',
      3: 'The customer\'s address is the final destination, not the pickup point. Matching there would put agents near customers but far from the food.',
    },
    r: { id: 's6', label: 'Section 6 — Delivery matching' },
  },
  {
    q: 'Which design pattern best models the Order\'s state progression (PLACED → CONFIRMED → PREPARING → …)?',
    o: ['Strategy', 'Observer', 'Command', 'State'],
    a: 3,
    ...
    w: {
      ...
      1: 'Observer notifies subscribers of events — it complements State here (each transition fires a notification) but doesn\'t model the lifecycle itself.',
      2: 'Command models operations you can undo — it could wrap individual order actions, but the lifecycle state machine is best captured by State.',
    },
    r: { id: 's4', label: 'Section 4 — The order state machine' },
  },
  {
    q: 'At which Order state can a customer NO LONGER cancel?',
    o: ['PREPARING', 'READY_FOR_PICKUP', 'CONFIRMED', 'PICKED_UP'],
    a: 3,
    ...
    w: {
      0: 'At PREPARING some apps allow cancellation with a fee, since the restaurant has invested effort but the food isn\'t yet on its way.',
      1: 'At READY_FOR_PICKUP the food is ready but still at the restaurant counter — technically still cancellable (food is wasted, but possible to reverse the delivery).',
      2: 'At CONFIRMED the restaurant has just accepted but not started cooking — cancellation is still low-cost.',
    },
    r: { id: 's4', label: 'Section 4 — The order state machine' },
  },
  {
    ...
    w: {
      0: 'double can store arbitrarily large values (up to ~10^308). Its problem is precision, not range.',
      1: 'There is no such Java interface — Money is a design pattern (Value Object) described in Day 20, not a built-in type.',
      3: 'double is actually faster than BigDecimal for CPU arithmetic. The reason to avoid it here is correctness, not performance.',
    },
  ...
  },
  {
    ...
    o: ['Decorator', 'Template Method', 'Composite', 'Factory Method'],
    a: 0,
    ...
    w: {
      1: 'Template Method defines a skeleton algorithm for subclasses to fill in — it doesn\'t model the concept of wrapping an existing calculator with a multiplier.',
      2: 'Composite treats a tree of objects as one — it\'s about hierarchies, not about adding a wrapping multiplier.',
      3: 'Factory Method decides WHICH object to create — it\'s about construction, not about adding behavior at runtime.',
    },
    r: { id: 's8', label: 'Section 8 — Pricing and the menu model' },
  },
  {
    ...
    w: {
      1: 'Using the current menu price would change the customer\'s bill retroactively — a major billing error and a violation of consumer trust.',
  ...
  },
  {
    ...
    w: {
      0: 'Throughput is a scaling concern, not a design structure difference. Both systems handle high order volumes.',
      1: 'Both systems typically need a mobile app — the technical infrastructure difference is not the core design challenge.',

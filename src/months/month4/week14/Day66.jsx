...
const QUESTIONS = [
  { q: 'How should the six piece types\' movement be modeled?',
    o: ['A switch on pieceType in the board', 'Six unrelated classes', 'Polymorphism / Strategy: an abstract Piece (or a MovementStrategy) where each concrete piece implements its OWN legalMoves(board, from) — the board treats them uniformly, no type switch', 'One Piece class with flags'], a: 2,
    ...
    w: { 0: 'A type switch scatters movement logic and reopens for every piece.', 1: 'Unrelated classes can\'t be handled uniformly by the board.', 3: 'Flags can\'t express six distinct move rules cleanly.' },
    r: { id: 's3', label: 'Section 3 — movement as strategy' } },
  { q: 'The four-stage move-validation pipeline, in order?',
    o: ['① the piece\'s own geometry → ② the path is clear (except Knight) → ③ the destination is empty or an enemy → ④ the move does NOT leave your own king in check', 'Random checks', 'Only check detection', 'Just "is the square empty?"'], a: 0,
    ...
    w: { 1: 'It\'s an ordered pipeline.', 2: 'Check is the LAST stage, atop the others.', 3: 'Empty-square is just stage 3.' },
    r: { id: 's5', label: 'Section 5 — validation pipeline' } },
  { q: 'Why is "does not leave your own king in check" the hardest validation rule?',
    o: ['It is not', 'It is GLOBAL, not local: you must simulate the move, then ask whether ANY enemy piece now attacks your king — so a geometrically legal move can still be illegal (a PINNED piece can\'t move)', 'Only the king obeys it', 'It needs the database'], a: 1,
    ...
    w: { 0: 'It\'s the layered, global rule.', 2: 'It constrains EVERY piece (you can\'t expose your king with any move).', 3: 'It\'s pure board logic, no DB.' },
    r: { id: 's5', label: 'Section 5 — the global rule' } },
  { q: 'Castling, en passant, and pawn promotion break which simplifying assumption?',
    o: ['That pieces have colors', 'That the board is 8×8', 'That a piece\'s move depends only on its current square — these depend on GAME HISTORY/CONTEXT (has the king/rook moved? did a pawn just double-step? did a pawn reach the last rank?), so you need move history', 'Nothing'], a: 2,
    ...
    w: { 0: 'Colors are always there.', 1: 'The board stays 8×8.', 3: 'They genuinely need history.' },
    r: { id: 's7', label: 'Section 7 — special moves' } },
  { q: 'A Move modeled as a Command object buys you…',
    o: ['Nothing', 'Faster rendering', 'Smaller memory', 'Undo/redo and a move history: each Move records what it did (piece, from, to, captured piece, flags), so it can be applied and reversed — enabling undo, replay, and the history that special moves and repetition rules need (Day 33)'], a: 3,
    ...
    w: { 0: 'It enables undo, history, replay.', 1: 'Rendering is unrelated.', 2: 'It adds state for the history, deliberately.' },
    r: { id: 's7', label: 'Section 7 — move as command' } },
  { q: 'Checkmate vs stalemate — the difference?',
    o: ['Both mean "no legal moves", but checkmate = your king IS in check (you lose), stalemate = your king is NOT in check (it\'s a DRAW). One boolean — is the king attacked — flips the outcome', 'Checkmate is a draw', 'Stalemate is a win', 'They are the same (a loss)'], a: 0,
    ...
    w: { 1: 'Checkmate is a loss, not a draw.', 2: 'Stalemate is a draw, not a win.', 3: 'Opposite outcomes.' },
    r: { id: 's8', label: 'Section 8 — check states' } },
  { q: 'The Knight\'s special property the path-check must respect?',
    o: ['It can\'t capture', 'It JUMPS — it is the only piece that ignores pieces in between, so the "path must be clear" stage (②) is skipped for the knight (only its destination square is checked)', 'It moves like a queen', 'It moves twice'], a: 1,
    ...
    w: { 0: 'It captures normally.', 2: 'Its move is the L-jump, not the queen\'s.', 3: 'It moves once, in an L.' },
    r: { id: 's4', label: 'Section 4 — piece movement' } },
  { q: 'The chess GAME loop is fundamentally…',
    o: ['Unique to chess', 'Event-driven', 'A state machine only', 'The turn-based game-loop skeleton (Week 9): whose turn → get/validate a move → apply it → check end conditions (checkmate/stalemate/draw) → switch player — chess just fills the validate + end-condition slots with rich rules'], a: 3,
    ...
    w: { 0: 'It reuses the Week-9 skeleton.', 1: 'It\'s turn-based, not event-driven.', 2: 'It has state, but the LOOP is the structure.' },

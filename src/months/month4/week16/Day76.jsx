...
const EXAM_QUESTIONS = [
  { id: 'q1', type: 'MCQ', text: 'Which keyword makes a Java variable constant?', options: ['static', 'final', 'const', 'immutable'], correct: 1, points: 2 },
  { id: 'q2', type: 'MCQ', text: 'What does the Observer pattern do?', options: ['It caches expensive results', 'It notifies dependents when state changes', 'It wraps objects to add behaviour', 'It creates objects without specifying their class'], correct: 1, points: 2 },
  { id: 'q3', type: 'MCQ', text: 'Which collection preserves insertion order in Java?', options: ['HashSet', 'TreeMap', 'ArrayList', 'HashMap'], correct: 2, points: 2 },
...
const QUESTIONS = [
  {
    ...
    w: { 0: 'Client-side timers are not trusted — a student can pause JavaScript or adjust their clock.', 2: 'Late answers violate the time-limit contract. There is no partial grace period in this design.', 3: 'The server should not crash — it should return a controlled error with AttemptExpiredException.' },
  ...
  },
  {
    ...
    w: { 0: 'Grading per save leaks whether each answer is right or wrong — the student can probe correct answers one-by-one.', 2: 'MCQ auto-grading requires no human involvement — it would be wasteful to wait for the teacher.', 3: 'Pre-grading is impossible without the student\'s answers.' },
  ...
  },
  {
    ...
    w: { 0: 'Starting a second attempt while one is still in progress would give the student extra time on a fresh attempt.', 2: 'An uncompleted attempt in flight is still occupying an attempt slot in spirit — it just has not been scored yet.', 3: 'The correct response is a controlled rejection, not a crash.' },
  ...
  },
  {
    ...
    w: { 0: 'Deleting the B record loses the fact that the student originally picked B — valuable audit data.', 2: 'Two Answer records for the same question cause a grading ambiguity — which one counts?', 3: 'Changing answers before submission is a normal exam behaviour.' },
    r: { id: 's4', label: 'Section 4 — Attempt state machine' },
  },
  {
    ...
    w: { 0: 'The question text must be shown — without it there is no exam.', 1: 'The options must be shown for MCQ — the student must see what to pick.', 3: 'Points can be shown — they help the student allocate time. They do not reveal correctness.' },
  ...
  },
  {
    ...
    w: { 0: 'Students in-progress would see a different question than they started with — potentially invalidating their work.', 2: 'Manual review does not fix the fairness problem — some students still saw the old wording.', 3: 'Future-only edits would mean the same exam ID serves two different question sets — violating exam integrity.' },
  ...
  },
  {
    ...
    w: { 0: 'Observer is for notifying dependents of state changes — not for structuring an algorithm skeleton.', 2: 'Decorator wraps an object to add behaviour — not applicable to a fixed-order algorithm.', 3: 'Command encapsulates a request as an object for undo/queue — the grading flow is not a command.' },
  ...
  },
  {
    ...
    w: { 0: 'Timer expiry relates to submission — it does not block the display of MCQ results.', 2: 'This scenario states the attempt is already submitted (auto-grading ran).', 3: 'Publishing results is not part of this design — the score is visible to the student, but only partial until essays are graded.' },
    r: { id: 's5', label: 'Section 5 — Auto-grading' },
  },
]

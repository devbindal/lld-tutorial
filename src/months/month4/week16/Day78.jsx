...
const QUESTIONS = [
  {
    ...
    w: { 0: 'try-catch catches exceptions, not races.', 2: 'volatile makes reads fresh but does not make compareAndSet atomic — volatile++ still races.', 3: 'A UI retry does nothing about the server-side race.' },
  ...
  },
  {
    ...
    w: { 0: 'The structure looks alike but intent and who-controls-transitions differ fundamentally.', 2: 'Both are general behavioral patterns used across many domains.', 3: 'Neither is a subtype of the other in any formal sense.' },
  ...
  },
  {
    ...
    w: { 0: 'TreeMap is O(log n) for all operations.', 2: 'LinkedHashMap.accessOrder is a valid shortcut but the question is about understanding the underlying structure.', 3: 'ArrayDeque gives O(n) lookup by key.' },
  ...
  },
  {
    ...
    w: { 0: 'Catching the exception on the client side does not help if the server already processed the request.', 2: 'The status code tells you what happened last time, not how to prevent double-execution.', 3: 'HTTPS is about transport security, not idempotency.' },
    r: { id: 's5', label: 'Section 5 — Concurrency cheat sheet' },
  },
  {
    ...
    w: { 0: 'Mathematical IS-A does not equal OOP IS-A. OOP IS-A requires behavioral substitutability.', 2: 'It compiles fine — that is the problem. The violation is invisible at compile time.', 3: 'LSP applies to any type hierarchy (classes and interfaces alike).' },
  ...
  },
  {
    ...
    w: { 0: 'This is not always true and is domain-specific, not a design principle.', 2: 'The customer location is known at order time.', 3: 'This is a design logic issue, not a legal one.' },
    r: { id: 's2', label: 'Section 2 — The 7 problem shapes' },
  },
  {
    ...
    w: { 0: 'At-least-once requires committing AFTER process — not before.', 2: 'The dead-letter queue only receives events that process() throws an exception on, not events that were skipped by an early commit.', 3: 'The broker does not know processing failed — it only sees the committed offset.' },
    r: { id: 's5', label: 'Section 5 — Concurrency cheat sheet' },
  },
  {
    ...
    w: { 0: 'double arithmetic is fast — speed is not the issue.', 2: 'double is not the problematic part for thread safety; shared mutable state is.', 3: 'Java has no such specification rule — this is a correctness concern, not a language rule.' },
    r: { id: 's4', label: 'Section 4 — SOLID quick reference' },
  },
  {
    ...
    w: { 0: 'Adding new methods is better done via composition or a new interface, not Decorator.', 2: 'Decorator is a general pattern; Java I/O streams, Collections.unmodifiableList, and surge pricing wrappers all use it.', 3: 'The whole point of Decorator is runtime-removable layering — if the behavior is permanent, a subclass or field may be simpler.' },
    r: { id: 's3', label: 'Section 3 — GoF frequency table' },
  },
  {
    ...
    w: { 0: 'Lockers have no turn structure or rules engine.', 1: 'Payment exists but the hard design problem is the atomic slot claim, not balance tracking.', 3: 'The system does not process a request through a chain of handlers.' },
    r: { id: 's2', label: 'Section 2 — The 7 problem shapes' },
  },
]

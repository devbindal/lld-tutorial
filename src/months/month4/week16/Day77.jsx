...
const QUESTIONS = [
  {
    ...
    w: {
      ...
      2: 'AtomicInteger works for the digital license counter because licenses are fungible. Physical copies have individual identity (Copy A vs Copy B) — you need to find a specific copy AND claim it atomically, not just decrement a count.',
      3: 'A DB constraint is a valid defence-in-depth layer, but the application layer must still prevent the race — otherwise you get a constraint violation exception instead of a clean "no copy available" error.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'synchronized would work correctly — it is just heavier than needed for a counter. AtomicInteger is the lighter, better fit.',
      2: 'AtomicInteger is specifically designed for thread-safe mutation — compareAndSet is its primary atomicity tool.',
      3: 'Digital licenses absolutely need atomicity — two concurrent borrows can both see activeBorrows < totalLicenses and both increment, giving N+1 active borrows.',
    },
    r: { id: 's5', label: 'Section 5 — Borrow flows' },
  },
  {
    ...
    w: {
      0: 'Making it AVAILABLE means anyone could borrow it before Dave picks it up — that defeats the purpose of the reservation queue.',
      1: 'Dave cannot be force-borrowed — he must come and pick it up. The system cannot know if Dave will arrive.',
      3: 'LOST is a real copy status (damaged/missing book) but has nothing to do with reservation fulfillment.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'A nightly sweeper would work but is unnecessary complexity. Lazy computation at return time is simpler and equally correct for the fine amount.',
      2: 'Fine tracking is a core requirement — overdue items incur daily fines per the problem statement.',
      3: 'Floating-point arithmetic (double) must never be used for money. 0.1 + 0.2 != 0.3 in double. Use BigDecimal.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Fines and borrowing are intentionally coupled — the library uses fine-blocking as the enforcement mechanism for overdue items.',
  ...
  },
  {
    ...
    w: {
      0: 'Indefinite holds starve other members who are waiting. The hold window exists precisely to prevent this.',
      2: 'RESERVED indefinitely is a leaked hold — the copy is effectively out of circulation for no one.',
      3: 'Force-borrowing on behalf of a member is never correct — the member must be present to borrow.',
    },
    r: { id: 's8', label: 'Section 8 — Reservation queue' },
  },
  {
    ...
    w: {
      0: 'Strategy (Day 31) would apply to the notification channel (email vs SMS vs push) — that is a swappable "how to notify" seam. The "when to notify" event is Observer.',
      1: 'Command (Day 33) encapsulates a request as an object for undo/redo or queuing. A notification event is not a command.',
      3: 'Template Method (Day 35) defines an algorithm skeleton. Sending a single notification is not a multi-step algorithm with fixed structure.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'A List<String> on DigitalLicense conflates the access-control concern with the file-serving concern. The license should not know about formats.',
      2: 'A DIGITAL type on PhysicalCopy would break the two-resource-model design — digital licenses are counters, not per-instance state machines.',
      3: 'Storing formats in Book violates SRP — Book models bibliographic data (title, author, ISBN), not file assets.',
    },

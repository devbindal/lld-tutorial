...
const QUESTIONS = [
  ...
  },
  {
    ...
    w: { 0: 'Sequence diagrams come later — they show flow, not structure. The entity model must come first so you have things to put on the lifelines.', 2: 'State machines are important but they apply to one entity. The full entity model shows all the classes and their relationships.', 3: 'A list of patterns without context is meaningless. Draw the entities; the patterns emerge naturally from the structure.' },
  ...
  },
  {
    ...
    w: { 0: '"I do not know" closes the conversation and signals lack of confidence. Even a partial answer with reasoning is better.', 2: '"Always" is almost never true in engineering. Saying it signals you have memorised rules but not understood them.', 3: 'Changing your design to avoid the question signals that you do not stand behind your choices. Defend it with reasoning instead.' },
  ...
  },
  {
    ...
    w: { 0: 'Erasing and restarting loses 3–5 minutes, breaks the flow, and signals panic under pressure.', 1: '3 minutes of silence is the second-worst thing you can do. Interviewers cannot grade silence.', 3: '"Please ignore this" sounds like you are giving up on correctness. Acknowledge it, flag it, plan to fix it.' },
  ...
  },
  {
    ...
    w: { 0: 'Edge cases are the lowest-weight rubric item — they are "nice to have" but do not define the hire signal.', 2: 'Lines of code have zero correlation with design quality. A 30-line solution with the right abstractions outscores a 200-line one that misses the state machine.', 3: 'Naming patterns is medium-weight. But using the most patterns without reasoning is worse than using one and explaining why.' },
  ...
  },
  {
    ...
    w: { 0: 'A database transaction may fix it at the persistence layer, but it does not show you understand the class-level race. The interviewer wants to hear you name it.', 2: '"A lock somewhere" is vague. Where? On what? Naming synchronized or CAS on the specific resource shows you know the tool.', 3: 'Saying "only in production" suggests you do not take concurrency seriously. It also happens in tests with thread pools.' },
    r: { id: 's3', label: 'Section 3 — talking out loud' },
  },
  {
    ...
    w: { 0: 'Redesigning for distributed at minute 10 means no code gets written and the core LLD never gets evaluated. The scale discussion is minute 38–45.', 2: 'Naming infrastructure tools without explaining what bottleneck they solve is a buzzword answer. Interviewers penalise it.', 3: 'Ignoring the question is rude and signals poor communication.' },
    r: { id: 's1', label: 'Section 1 — the 45-minute breakdown' },
  },
  {
    ...
    w: { 0: '1–3 classes is too simple. A Parking Lot with only ParkingLot and Car has nowhere to put the state machine, pricing, or concurrency logic.', 2: '10–15 classes in 10 minutes means 40 seconds per class — no field list, no cardinality, no reasoning. The model becomes noise.', 3: 'More classes mean more time. You have 10 minutes for the entity model. Every class beyond 7 is risk.' },

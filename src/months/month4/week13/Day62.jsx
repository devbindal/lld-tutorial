...
const QUESTIONS = [
  { q: 'The BEST way to make code thread-safe, before reaching for any lock?',
    o: ['Use more threads', 'Add volatile everywhere', 'synchronized everything', 'Remove the need: make state IMMUTABLE or CONFINE it to one thread — no shared mutable state means no coordination at all'], a: 3,
    ...
    w: { 0: 'More threads add contention, not safety.', 1: 'volatile gives visibility, not atomicity — and not everywhere.', 2: 'Over-locking is slow, deadlock-prone, and often still wrong.' },
    r: { id: 's1', label: 'Section 1 — the cure hierarchy' } },
  { q: 'What does synchronized provide?',
    o: ['BOTH mutual exclusion (atomicity — one thread in the block at a time) AND visibility/ordering (happens-before) — entering/exiting the monitor publishes writes', 'Only speed', 'Only atomicity', 'Only visibility'], a: 0,
    ...
    w: { 1: 'It costs some speed; it BUYS correctness.', 2: 'It gives visibility too.', 3: 'It gives atomicity too.' },
    r: { id: 's2', label: 'Section 2 — synchronized' } },
  { q: 'volatile count++ — is it thread-safe?',
    o: ['Yes', 'Yes if the field is also final', 'No — volatile gives visibility + ordering but NOT atomicity; count++ is still read-modify-write and can lose updates. Use AtomicInteger or a lock for compound operations', 'Only on 64-bit JVMs'], a: 2,
    ...
    w: { 0: 'Compound ops still race.', 1: 'final can\'t be reassigned, so ++ wouldn\'t compile.', 3: 'It\'s wrong on every JVM for compound ops.' },
    r: { id: 's3', label: 'Section 3 — volatile' } },
  { q: 'When is volatile the RIGHT tool?',
    o: ['Never', 'For everything', 'For counters', 'For a single field where you need VISIBILITY (and ordering) but no compound update — e.g. a stop/done flag, or safely publishing an immutable object reference'], a: 3,
    ...
    w: { 0: 'It\'s the right tool for flags/publication.', 1: 'Wrong tool for compound updates.', 2: 'Counters are compound — use atomics.' },
    r: { id: 's3', label: 'Section 3 — volatile' } },
  { q: 'How does AtomicInteger.incrementAndGet() work without locking?',
    o: ['It disables other threads', 'It uses synchronized internally', 'A compare-and-set (CAS) retry loop: read current, compute new, atomically set ONLY IF still equal to current; if another thread changed it, the CAS fails and it retries — lock-free, never blocks', 'Magic'], a: 2,
    ...
    w: { 0: 'It never stops other threads.', 1: 'It\'s lock-free, not synchronized.', 3: 'It\'s a hardware CAS instruction.' },
    r: { id: 's5', label: 'Section 5 — atomics & CAS' } },
  { q: 'ReentrantLock over synchronized buys you…',
    o: ['Nothing', 'tryLock with a TIMEOUT (deadlock avoidance), lockInterruptibly, fairness, and multiple Condition variables — at the cost of a mandatory try/finally unlock', 'Faster locking always', 'Automatic unlock'], a: 1,
    ...
    w: { 0: 'It adds real capabilities.', 2: 'Not always faster; it\'s about features.', 3: 'You must unlock manually (finally).' },
    r: { id: 's7', label: 'Section 7 — explicit locks' } },
  { q: 'The Month-3 "atomic claim" (book a seat / occupy a spot) is built with…',
    o: ['An atomic compare-and-set on the resource\'s state (e.g. AtomicReference.compareAndSet(AVAILABLE, BOOKED)) or a lock around check+act — so exactly one of two racers wins', 'A boolean check then set', 'A retry without atomicity', 'volatile'], a: 0,
    ...
    w: { 1: 'That\'s the racy check-then-act it replaces.', 2: 'The retry must wrap an ATOMIC set.', 3: 'volatile can\'t make two steps atomic.' },
    r: { id: 's9', label: 'Section 9 — building the atomic claim' } },
  { q: 'A lock-granularity rule that keeps concurrent code fast?',
    o: ['Lock the whole method always', 'Hold the lock for the SMALLEST critical section, and NEVER do slow I/O (network/disk) while holding it — minimize the serial part; consider lock striping for hot maps', 'Never unlock', 'Use one global lock for everything'], a: 1,
    ...
    w: { 0: 'Coarse locking serializes too much.', 2: 'Forgetting to unlock deadlocks everything.', 3: 'A global lock kills throughput.' },

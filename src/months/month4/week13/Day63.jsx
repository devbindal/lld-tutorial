...
const QUESTIONS = [
  { q: 'What problem does producer–consumer solve?',
    o: ['Removing all threads', 'DECOUPLING producers and consumers that work at different/variable rates, via a shared buffer — so neither waits on the other except at the buffer\'s limits, with backpressure built in', 'Sorting data', 'Making code faster'], a: 1,
    ...
    w: { 0: 'It uses multiple threads.', 2: 'Unrelated.', 3: 'It\'s about coordination, not raw speed.' },
    r: { id: 's1', label: 'Section 1 — the problem' } },
  { q: 'Why must the buffer be BOUNDED?',
    o: ['Bounded is faster', 'Backpressure: a bounded queue forces a fast producer to slow down (block) when consumers fall behind. An UNBOUNDED queue hides the imbalance until it exhausts memory (OOM)', 'Unbounded is illegal', 'To save typing'], a: 1,
    ...
    w: { 0: 'Speed isn\'t the reason.', 2: 'Unbounded is legal — and dangerous.', 3: 'It\'s about backpressure, not effort.' },
    r: { id: 's1', label: 'Section 1 — backpressure' } },
  { q: 'Why must wait() be called inside a WHILE loop, not an if?',
    o: ['if doesn\'t compile', 'Style', 'while is faster', 'To RE-CHECK the condition after waking — a thread can wake from a spurious wakeup, or another consumer may have already taken the item, so the condition might be false again; while re-checks, if would proceed wrongly'], a: 3,
    ...
    w: { 0: 'Both compile; if is just wrong.', 1: 'It\'s a correctness rule, not style.', 2: 'Speed is irrelevant.' },
    r: { id: 's4', label: 'Section 4 — while not if' } },
  { q: 'notify() vs notifyAll()?',
    o: ['notify wakes all', 'Identical', 'notifyAll is deprecated', 'notify() wakes ONE waiting thread (which may be waiting on a different condition → lost-wakeup risk); notifyAll() wakes all so the right one can proceed — prefer notifyAll unless you are certain all waiters are interchangeable'], a: 3,
    ...
    w: { 0: 'Reversed.', 1: 'They differ.', 2: 'Neither is deprecated.' },
    r: { id: 's4', label: 'Section 4 — notifyAll' } },
  { q: 'The right way to do producer–consumer in Java (instead of hand-rolling wait/notify)?',
    o: ['synchronized + wait/notify by hand', 'A volatile array', 'A BlockingQueue (ArrayBlockingQueue / LinkedBlockingQueue): put() blocks when full, take() blocks when empty — the buffer, the locking, and the signaling are all handled for you', 'A plain ArrayList'], a: 2,
    ...
    w: { 0: 'Hand-rolling is the error-prone way the library replaces.', 1: 'A volatile array doesn\'t coordinate put/take.', 3: 'A plain list isn\'t thread-safe or blocking.' },
    r: { id: 's6', label: 'Section 6 — BlockingQueue' } },
  { q: 'How is a thread pool (ExecutorService) a producer–consumer system?',
    o: ['It is not', 'It sorts tasks', 'submit(task) PRODUCES work onto an internal BlockingQueue; the pool\'s worker threads are the CONSUMERS that take and run tasks — it IS the pattern, packaged', 'It uses one thread'], a: 2,
    ...
    w: { 0: 'It is exactly this pattern.', 1: 'It runs tasks, doesn\'t sort.', 3: 'A pool has many workers.' },
  ...
  { q: 'A ThreadPoolExecutor with core=2, a bounded queue=3, max=4 receives 8 quick tasks. What happens to the last ones?',
    o: ['2 run on core threads, 3 wait in the queue, 2 more spin up extra threads (to max 4), and the remaining ones are REJECTED by the rejection (saturation) policy', 'They are dropped silently', 'They all queue', 'All 8 run at once'], a: 0,
    ...
    w: { 1: 'Rejection is explicit, via a policy — not silent.', 2: 'The queue is bounded (3).', 3: 'Concurrency is capped at max threads.' },
    r: { id: 's7', label: 'Section 7 — saturation' } },
  { q: 'How do you stop consumer threads gracefully when work is done?',
    o: ['A POISON PILL (a sentinel item that means "shut down" when taken) or ExecutorService.shutdown() + awaitTermination (and respond to interruption) — never the deprecated, unsafe Thread.stop()', 'Kill the JVM', 'An infinite loop', 'Thread.stop()'], a: 0,
    ...
    w: { 1: 'Killing the JVM isn\'t graceful.', 2: 'That never stops.', 3: 'Thread.stop() is unsafe/deprecated.' },

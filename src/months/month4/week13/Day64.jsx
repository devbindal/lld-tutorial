...
const QUESTIONS = [
  { q: 'The core difference between optimistic and pessimistic locking?',
    o: ['They are the same', 'They differ in their ASSUMPTION: pessimistic assumes conflict is LIKELY → lock before touching and hold; optimistic assumes conflict is RARE → don\'t lock, do the work, and CHECK on commit that nothing changed (retry if it did)', 'Optimistic uses more memory', 'Optimistic is faster, always'], a: 1,
    ...
    w: { 0: 'Opposite assumptions.', 2: 'Memory isn\'t the distinction.', 3: 'Only under LOW contention; high contention favors pessimistic.' },
    r: { id: 's1', label: 'Section 1 — two philosophies' } },
  { q: 'Pessimistic locking — what does it cost?',
    o: ['Nothing', 'Blocking (others WAIT while you hold the lock → lower concurrency) and DEADLOCK risk with multiple locks — and you must never hold it across slow I/O or user think-time', 'It can\'t prevent races', 'Lost updates'], a: 1,
    ...
    w: { 0: 'It has real costs.', 2: 'It does prevent races — that\'s its job.', 3: 'It PREVENTS lost updates.' },
    r: { id: 's2', label: 'Section 2 — pessimistic' } },
  { q: 'The optimistic version-number technique works how?',
    o: ['It uses a timestamp only', 'It locks the row first', 'Each row has a version; you read it, then commit with UPDATE … SET …, version=version+1 WHERE id=? AND version=<read>. If 0 rows change, someone else updated it first → conflict → re-read and retry', 'It ignores conflicts'], a: 2,
    ...
    w: { 0: 'A version counter (or a compatible timestamp) — the check is the WHERE clause.', 1: 'No lock — that\'s the point.', 3: 'It detects and handles them.' },
    r: { id: 's3', label: 'Section 3 — optimistic' } },
  { q: 'Low contention vs high contention — which strategy?',
    o: ['Contention does not matter', 'Always pessimistic', 'Always optimistic', 'LOW contention → optimistic (skip locking overhead, retries are rare); HIGH contention → pessimistic (optimistic retries pile up toward livelock; lock once and serialize)'], a: 3,
    ...
    w: { 0: 'Contention is THE deciding factor.', 1: 'Pessimistic over-serializes when conflicts are rare.', 2: 'Optimistic thrashes under heavy contention.' },
    r: { id: 's7', label: 'Section 7 — choosing' } },
  { q: 'A seat must stay reserved while the user spends 5 minutes paying. The right mechanism?',
    o: ['Pure optimistic, no reservation', 'Hold a DB row lock for 5 minutes', 'A global lock', 'A LEASE — an atomic hold with a TTL: claim it optimistically, mark it HELD with an expiry; pay → BOOKED, or it auto-expires back to AVAILABLE'], a: 3,
    ...
    w: { 0: 'No reservation means the seat can be taken mid-payment.', 1: 'A 5-minute DB lock is a throughput disaster.', 2: 'A global lock serializes the whole system.' },
    r: { id: 's8', label: 'Section 8 — leases' } },
  { q: 'Optimistic locking can\'t deadlock (no locks held). What can it suffer instead?',
    o: ['LIVELOCK / starvation under high contention — threads keep colliding and retrying without making progress; cap retries (with backoff) and consider switching to pessimistic when conflicts are frequent', 'Lost updates', 'Deadlock', 'Nothing'], a: 0,
    ...
    w: { 1: 'It PREVENTS lost updates.', 2: 'No locks held → no deadlock.', 3: 'Retries can starve under contention.' },
    r: { id: 's7', label: 'Section 7 — trade-offs' } },
  { q: 'CAS, a DB version column, and HTTP ETag/If-Match are…',
    o: ['Unrelated', 'All pessimistic', 'The SAME optimistic idea at three layers: "apply only if the value is unchanged since I read it" — CAS in memory, version column in the DB, ETag/If-Match over HTTP', 'All deadlock-prone'], a: 2,
    ...
    w: { 0: 'They\'re the same concept.', 1: 'All optimistic, not pessimistic.', 3: 'None holds locks, so none deadlocks.' },
    r: { id: 's3', label: 'Section 3 — optimistic everywhere' } },
  { q: 'Whichever strategy you pick, a retried operation must also be…',
    o: ['IDEMPOTENT — a retry (after an optimistic conflict, or a client re-send) must not double-apply; pair the atomic claim with an idempotency key, the recurring Month-3/4 lesson', 'Faster', 'Synchronous', 'Logged'], a: 0,
    ...
    w: { 1: 'Speed is unrelated to correctness on retry.', 2: 'Sync/async is orthogonal.', 3: 'Logging doesn\'t make it safe.' },

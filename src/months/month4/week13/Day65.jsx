...
const QUESTIONS = [
  { q: 'What is a rate limiter for, and what is its core API?',
    o: ['To speed up requests', 'To CAP requests per client per time window (protect a service from overload/abuse, enforce quotas/tiers) — core API: allow(clientId) → true/false, returning 429 (Too Many Requests) when over', 'To cache responses', 'To balance load across servers'], a: 1,
    ...
    w: { 0: 'It throttles, not speeds up.', 2: 'Caching is a different concern.', 3: 'That\'s a load balancer.' },
    r: { id: 's1', label: 'Section 1 — the problem' } },
  { q: 'The fixed-window counter\'s notorious flaw?',
    o: ['Too much memory', 'The BOUNDARY BURST: the counter resets at the window edge, so a client can send the full limit just before the boundary AND just after — up to 2× the limit in a short span', 'It can\'t reset', 'It\'s too slow'], a: 1,
    ...
    w: { 0: 'It\'s the most memory-efficient.', 2: 'Resetting at the boundary IS the flaw.', 3: 'It\'s O(1) and fast.' },
    r: { id: 's3', label: 'Section 3 — fixed window' } },
  { q: 'How does the token bucket algorithm work?',
    o: ['It queues requests and drains at a fixed rate', 'Tokens refill at a steady rate up to a CAPACITY; each request consumes one token; if the bucket is empty the request is rejected — so it ALLOWS bursts up to capacity but the long-run rate equals the refill rate', 'It counts requests per fixed minute', 'It stores every timestamp'], a: 1,
    ...
    w: { 0: 'That\'s leaky bucket.', 2: 'That\'s fixed window.', 3: 'That\'s sliding window log.' },
    r: { id: 's5', label: 'Section 5 — token bucket' } },
  { q: 'Token bucket vs leaky bucket?',
    o: ['Identical', 'Token bucket ALLOWS bursts (up to capacity) and caps the average; leaky bucket enforces a CONSTANT outflow (requests drain at a fixed rate, no bursts) — choose by whether bursts are acceptable', 'Leaky bucket allows bursts', 'Token bucket has no capacity'], a: 1,
    ...
    w: { 0: 'They differ on bursts.', 2: 'Leaky bucket forbids bursts.', 3: 'Token bucket\'s capacity IS the burst budget.' },
    r: { id: 's5', label: 'Section 5 — token vs leaky' } },
  { q: 'The sliding window COUNTER (vs the log) is popular in production because…',
    o: ['It is perfectly exact', 'It approximates a true sliding window by weight-blending the current and previous fixed-window counts — accurate ENOUGH with O(1) memory (no per-request timestamps), avoiding both the boundary burst and the log\'s memory cost', 'It uses no counters', 'It allows unlimited bursts'], a: 1,
    ...
    w: { 0: 'It\'s an approximation (the log is exact).', 2: 'It uses two counters.', 3: 'It limits, not unlimits.' },
    r: { id: 's4', label: 'Section 4 — sliding window' } },
  { q: 'Why is thread-safety central to a rate limiter (the Week-13 payoff)?',
    o: ['It isn\'t', 'It is SHARED MUTABLE STATE hit by every request thread; "check tokens, then take one" is check-then-act — without atomicity (a lock around refill+take, or a CAS loop) concurrent requests over-admit past the limit', 'Only the API matters', 'Single-threaded servers exist'], a: 1,
    ...
    w: { 0: 'It\'s the crux at scale.', 2: 'Correctness under concurrency matters as much as the API.', 3: 'Real services are highly concurrent.' },
    r: { id: 's8', label: 'Section 8 — thread-safety' } },
  { q: 'Across a fleet of servers, a single global limit per client needs…',
    o: ['Each node limiting independently', 'A SHARED store (e.g. Redis) holding the counter/bucket so all nodes enforce ONE global limit atomically — per-node limiting alone lets the real total be N× the limit; Redis does it with INCR+expiry or a Lua/token-bucket script', 'Nothing special', 'More threads'], a: 1,
    ...
    w: { 0: 'Independent nodes give N× the intended limit.', 2: 'It requires coordination.', 3: 'Threads don\'t span machines.' },
  ...
  { q: 'Different algorithms behind one interface, swappable per client tier (free vs paid) — that\'s…',
    o: ['Inheritance', 'A Strategy: a RateLimiter interface with TokenBucket/SlidingWindow implementations, configured per client — a new algorithm or tier is a new class / config, not an edit (Day 31)', 'A Singleton', 'Hardcoded ifs'], a: 1,
    ...
    w: { 0: 'Composition/Strategy, not a type hierarchy.', 2: 'Oneness isn\'t the point.', 3: 'The growing-if smell it replaces.' },

...
const QUESTIONS = [
  { q: 'The single root cause of (almost) every concurrency bug?',
    o: ['Using ExecutorService', 'Too many threads', 'SHARED MUTABLE STATE accessed by multiple threads without coordination — remove sharing OR mutability OR add coordination and the bug class disappears', 'Slow CPUs'], a: 2,
    ...
    w: { 0: 'Executors are the recommended tool, not the cause.', 1: 'Even two threads on one variable is enough.', 3: 'Speed doesn\'t cause races (it changes the odds).' },
    r: { id: 's1', label: 'Section 1 — the root cause' } },
  { q: 'Why is count++ NOT atomic?',
    o: ['Java makes it atomic', 'It\'s really THREE operations — read count, add 1, write back — and another thread can interleave between them, so two increments can both read the same value and one update is LOST', 'It is atomic', 'Only doubles have this problem'], a: 1,
    ...
    w: { 0: 'The JVM does not make ++ atomic.', 2: 'It compiles to multiple bytecodes; not atomic.', 3: 'It affects ints, longs, references — anything read-modify-write.' },
    r: { id: 's3', label: 'Section 3 — lost update' } },
  { q: 'Two threads both check "seat available?" then both book it. This race is called…',
    o: ['Check-then-act — the gap between the CHECK and the ACT lets both pass the check before either acts; it\'s the exact double-booking / two-gates-one-spot race from all of Month 3', 'Deadlock', 'Starvation', 'Lost update'], a: 0,
    ...
    w: { 1: 'Deadlock is mutual waiting, not this.', 2: 'Starvation is a thread never getting to run.', 3: 'Lost update is read-modify-write on a counter; related but distinct.' },
  ...
  { q: 'A worker loops while(!stop); main sets stop=true but the worker never exits. The hazard?',
    o: ['A lost update', 'VISIBILITY — without synchronization/volatile, the worker may read a cached copy of stop and never see main\'s write (CPU caches + JIT optimization under the Java Memory Model)', 'A deadlock', 'A logic bug'], a: 1,
    ...
    w: { 0: 'No update is lost — it\'s never seen.', 2: 'Nothing is mutually waiting.', 3: 'The code is logically fine; it\'s a memory-visibility issue.' },
    r: { id: 's7', label: 'Section 7 — visibility' } },
  { q: 'The three concurrency hazards under the Java Memory Model?',
    o: ['Only deadlock', 'Only race conditions', 'ATOMICITY (an operation interrupted mid-way), VISIBILITY (a write not seen by other threads), and ORDERING (the compiler/CPU reordering instructions) — synchronization addresses all three', 'Speed, memory, disk'], a: 2,
    ...
    w: { 0: 'Deadlock is a liveness failure, not the full set.', 1: 'Races are one symptom; the hazards are three.', 3: 'Those are performance, not correctness hazards.' },
    r: { id: 's7', label: 'Section 7 — the three hazards' } },
  { q: 'Best practice for running concurrent tasks in Java?',
    o: ['An ExecutorService / thread pool — reuse a bounded set of threads, submit tasks, get Futures; hand-rolling and unbounded threads exhaust resources and are hard to manage', 'One giant thread', 'Recursion', 'new Thread(...).start() everywhere'], a: 0,
    ...
    w: { 1: 'One thread isn\'t concurrent.', 2: 'Unrelated.', 3: 'Unbounded thread creation exhausts the machine.' },
    r: { id: 's2', label: 'Section 2 — threads in Java' } },
  { q: 'Why are concurrency bugs so notoriously hard to find?',
    o: ['They cause compile errors', 'They always crash immediately', 'They are not', 'They are NON-DETERMINISTIC ("Heisenbugs"): they depend on timing/interleaving, so they pass thousands of times, fail rarely in production, and often vanish under a debugger — you can\'t reliably reproduce them'], a: 3,
    ...
    w: { 0: 'They compile fine.', 1: 'They usually corrupt data silently, not crash.', 2: 'They are famously hard.' },
    r: { id: 's9', label: 'Section 9 — why so hard' } },
  { q: 'Two threads each grab two locks in OPPOSITE order and both wait forever. This is…',
    o: ['A lost update', 'A visibility bug', 'Starvation', 'DEADLOCK — T1 holds A waiting for B while T2 holds B waiting for A; neither proceeds. A liveness failure (cured in Part 2 by a consistent lock order / tryLock)'], a: 3,
    ...
    w: { 0: 'No update is lost; threads are stuck.', 1: 'It\'s a liveness, not visibility, failure.', 2: 'Starvation is unfair scheduling, not mutual waiting.' },

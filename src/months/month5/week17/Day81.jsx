...
const QUESTIONS = [
  {
    ...
    o: ['Delete (invalidate) the cache key', 'Write to cache first, then write to the database', 'Do nothing — the TTL will expire it eventually', 'Update the cache key with the new value immediately'],
    a: 0,
    ...
    w: {
      1: 'Write-cache-first is write-behind pattern; if the DB write fails, you have fresh data in cache but stale data in DB — the source of truth is wrong.',
      ...
      3: 'Updating the cache looks atomic but is not: between DB write and cache update, another thread can read a stale value from the DB and then overwrite your fresh cache entry.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Hot key with too many writes is a different problem — a write-amplification issue, not a stampede.',
      1: 'Mass eviction is a different problem (capacity/eviction policy). A stampede is specifically about concurrent misses hitting the DB.',
  ...
  },
  {
    ...
    o: ['At application startup (eager loading)', 'Only when a cache miss occurs (lazy loading)', 'On every write to the database', 'Redis populates it automatically by reading the DB'],
    a: 1,
    ...
    w: {
      0: 'Startup loading (cache warming) can be added on top, but the defining characteristic of cache-aside is the on-miss population during reads.',
      2: 'Populating on every DB write is write-through, not cache-aside. Cache-aside only populates on a read miss.',
  ...
  },
  {
    ...
    w: {
      0: 'Write-through affects write path. The hot-key problem is a read-path issue.',
      2: 'Longer TTL reduces miss-driven DB load but does nothing about the per-second Redis hit rate — the hot key is still accessed 200k times/s.',
  ...
  },
  {
    ...
    w: {
      0: 'The check is on existence, not on the current value. Checking the value atomically requires a Lua script or WATCH+MULTI+EXEC.',
      1: 'Without NX, SET is unconditional — multiple threads would all succeed, defeating the mutual exclusion. NX is the key ingredient.',
      3: 'GET is for reading. SET NX EX is a write with a conditional guard.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'LFU has more overhead (frequency counters) and is wrong for access patterns where recent = important (news feed, trending page). There is always a trade-off.',
      2: 'Equal frequency plus oldest key is more like FIFO — LFU would still track frequency and behave differently from FIFO in that case.',
      3: 'Evicting the key set longest ago regardless of access is FIFO, not LFU or LRU.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Write-through and cache-aside use the same amount of memory. Eviction policy — not the write strategy — controls memory.',
      1: 'Write-through is a legitimate pattern when read freshness matters more than write latency. Calling it "never use" is wrong.',
      2: 'Write-heavy read-rare workloads are a case AGAINST write-through — you pay write cost to populate a cache nobody reads (cold-write problem). Cache-aside would be cheaper.',
    },
    r: { id: 's3', label: 'Section 3 — caching patterns' },
  },
  {
    ...
    w: {
      0: 'Redis does not auto-scale memory. The `maxmemory` setting is a hard cap enforced by the policy you configure.',
      1: 'Silent drop of old sessions would be `allkeys-lru` or `volatile-lru`. `noeviction` does the opposite — it refuses to evict anything.',
      2: 'Evicting by shortest remaining TTL is `volatile-ttl`. `noeviction` does not evict anything regardless of TTL.',
    },
    r: { id: 's5', label: 'Section 5 — eviction policies' },
  },
]

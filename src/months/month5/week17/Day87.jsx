...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'That is the forward index (doc → words). It is the naive approach: searching "java" would mean scanning every document — O(N × doc_length). Too slow.',
      2: 'A ranked list is the output of a search query, not the index itself. The index is the structure that makes that ranking possible quickly.',
      3: 'Document hashing is used for deduplication in crawling, not for the core index structure.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Stop words and stemming are independent preprocessing steps. Stop words do not break stemming.',
      2: 'The tokenization split is done by regex on non-word characters, completely independent of which words are stop words.',
      3: 'Stop words affect all documents roughly equally. The problem is they appear everywhere — removing them reduces index size without hurting relevance.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'TF is a ratio (not a raw count) and IDF is a logarithm (not a raw document count). Raw counts make longer documents unfairly dominant.',
      ...
      3: 'TF divides by the total tokens in THIS document (100), not the total number of documents (10). These are different denominators.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'That is an AND query, not a phrase query. AND query finds docs with both words anywhere. "tutorial java" would also match — wrong for an exact phrase.',
      2: 'Scoring by frequency does not enforce that the words appear next to each other. "I like java. Tutorial later" would score high but is not the phrase "java tutorial".',
      3: 'Stemming helps match variants ("tutorials" → "tutorial") but you still need positional checking to ensure the stemmed forms appear adjacent in that order.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'A Bloom filter YES is not definite. It can produce false positives — the URL hash collides with a previously inserted URL. The correct phrasing is "might have been seen".',
      2: 'Bloom filters have NO false negatives. If a URL was never inserted, the filter will always say NO. The concern is false positives (YES when not actually seen), not false negatives.',
      3: 'In standard web crawl dedup, a Bloom filter YES means skip — there is no need for a special queue. The occasional false positive simply means a URL is skipped once.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Free disk space routing is dynamic and changes over time. Queries for "java" would need to find out where "java" ended up — there is no stable lookup. Consistent hashing solves this deterministically.',
      2: 'Full replication means every machine stores the full index — no sharding benefit. Updates must be propagated to all 5 machines. This is replication, not sharding.',
      3: 'Location of the requesting user is irrelevant to WHERE the postings live. Routing by proximity is for CDN caching of results, not index sharding.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Duplicate document removal is a separate step, typically done with document fingerprinting (SimHash, Shingling). Stemming is purely about word form normalization.',
      2: 'Splitting compound words is compound splitting or segmentation — a different NLP task. Stemming only removes suffixes from a single word (e.g., "running" → "run").',
      3: 'Measuring frequency relative to document length is Term Frequency (TF), one component of TF-IDF. That is a ranking signal, not a tokenization step.',
    },
    r: { id: 's3', label: 'Section 3 — Preprocessing pipeline' },
  },
  {
    ...
    w: {
      1: 'A nested loop comparing every element of A against every element of B is O(m×n). This is the naive approach. Since postings lists are sorted, the two-pointer merge is always better.',
      ...
      3: 'Binary search of each element of A in B would be O(m log n) — better than O(m×n) but worse than O(m+n). Sorted lists let us do better with two pointers since both lists move forward.',
    },

...
const QUESTIONS = [
  ...
  },
  {
    ...
    w: {
      0: 'Logs from each service describe events but are not correlated into a single timeline with accurate relative timing. You would have to manually reconstruct the sequence.',
      ...
      3: 'CPU dashboards show resource usage, not call durations between services. A service can be slow while CPU is normal (e.g. waiting on DB I/O).',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'A traceId is a random string generated per request by the calling code. It has nothing to do with the log storage system\'s database keys.',
  ...
  },
  {
    ...
    w: {
      0: 'Reliability, Efficiency, Durability is not an observability method — it describes system qualities.',
      1: 'Read rate and data volume are not standard RED metrics. RED specifically covers traffic rate, error rate, and latency.',
      3: 'Resources, Events, and Dependencies describe a different framing. RED specifically measures the behavior of requests through a service, not resource consumption.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Histogram bucket boundaries can use any numeric type. The integer/float distinction is not relevant here.',
  ...
  },
  {
    ...
    w: {
      0: 'traceId identifies a request, not a service. Each service gets its own spanId inside that request\'s trace.',
      2: 'They are not random with no relation. The structure (traceId shared, parentSpanId linking spans) is essential for reconstructing the call tree.',
  ...
  },
  {
    ...
    w: {
      0: 'OpenTelemetry is open-source and vendor-neutral. It is not a paid service and does not store data itself.',
      2: 'OpenTelemetry is an instrumentation library and protocol, not a microservice framework.',
  ...
  },
  {
    ...
    w: {
      0: 'Restarting services destroys in-memory state and does not explain what caused the failure. The problem may recur.',
      1: 'Average CPU is an aggregate metric. It says nothing about one specific request from one customer at one specific time.',
      3: 'Adding logging after the fact does not help with the current incident. Good observability means having the data already when you need it.',
    },

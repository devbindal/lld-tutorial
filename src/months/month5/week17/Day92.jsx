...
const QUESTIONS = [
  {
    ...
    w: {
      ...
      1: 'Gateways proxy requests; they never replace storage layers.',
      3: 'Business logic belongs inside the individual service, not the gateway. Putting it there creates a god-gateway.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'Forwarding the JWT unchanged means every service must know how to verify it — the same duplication problem.',
      2: 'Re-verifying in every service defeats the purpose of the gateway pattern — it spreads auth logic everywhere.',
      3: 'Direct DB queries from every service create tight coupling and are slow.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'Strategy selects one algorithm at a time — it does not pipe a request through multiple handlers in sequence.',
      2: 'Observer broadcasts an event to multiple listeners — there is no sequential pipeline or short-circuit.',
  ...
  },
  {
    ...
    w: {
      ...
      1: 'API Gateways reject excess requests with 429; they do not silently queue them (that would hide backpressure from the client).',
      3: 'The first requests should succeed; only the excess is rejected.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      1: 'TLS termination is a gateway responsibility, not a mesh-exclusive feature.',
      2: 'JWT verification is done at the gateway. The mesh typically uses mTLS (mutual TLS) for service-to-service authentication, not JWT.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Path-prefix routing does not require an exact match. It only requires the request path to start with the registered prefix.',
      3: 'Shortest prefix wins is the wrong heuristic. We want the most specific (longest) prefix so different services can share a path root.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'The sidecar handles service-to-service communication. Database connections are a separate concern inside the service.',
      1: 'The sidecar handles networking concerns, not business logic. The service still implements all its domain behavior.',
      2: 'The gateway and mesh serve different traffic layers. A mesh handles internal east-west calls; a gateway handles the external north-south entry point. Both are needed.',
    },

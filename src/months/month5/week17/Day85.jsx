...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'Hash speed is unrelated to the number of servers. The problem is correctness of mapping, not performance of hashing.',
      2: 'Collision rate depends on the hash function, not the number of servers.',
      3: 'Modulo hashing works fine with string keys; the issue is only when N changes.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      3: 'Consistent hashing is deterministic — the same key always goes to the same server. Random would defeat the whole point.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'HashMap is unordered — you cannot ask "what is the next key ≥ X?" without scanning everything.',
      2: 'Linear scan works but is O(N) per lookup. TreeMap gives O(log N), which is much better for large rings.',
  ...
  },
  {
    ...
    w: {
      0: 'Virtual nodes actually increase TreeMap size (V × N entries). They cost memory; the benefit is even distribution.',
      1: 'All keys use the same hash function. Virtual nodes are just multiple entries per server, not separate hash functions.',
      3: 'ceilingKey is O(log(total ring size)). More virtual nodes make it slightly slower, not faster.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      1: 'Half would only happen if the new server lands exactly at the midpoint of the largest arc. On average it is 1/N.',
      3: 'Some keys must move — the new server needs to serve something. Zero movement is only possible if the server is added but never used.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'MySQL replication copies data to a standby — it is about durability, not distributing keys across nodes.',
      1: 'Git has nothing to do with consistent hashing. Branches are version pointers, not distributed key-value mappings.',
      2: 'Kafka partitioning uses hash(key) % numPartitions — classic modulo hashing, not consistent hashing.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Multiplying the hash by i could overflow or produce poorly distributed results. The canonical approach hashes a unique string per virtual node.',
  ...
  },
  {
    ...
    w: {
      0: 'There is no ring size limit in bounded load. It is about per-server load caps at request time.',
      2: 'The number of virtual nodes is a separate configuration choice. Bounded load operates at request-routing time, not ring-construction time.',

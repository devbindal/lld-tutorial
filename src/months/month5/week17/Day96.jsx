...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'This is what happens WITHOUT a CDN, or on a cache miss. With a cache hit, the origin is never involved.',
      2: 'This describes a cache miss, not a cache hit. On a miss, the PoP fetches from origin and then caches it for next time.',
      3: 'DNS uses geo-DNS or anycast to route to the NEAREST PoP, not a random one.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'max-age=3600 is for browsers. s-maxage overrides it for CDNs specifically. CDNs use s-maxage when both are present.',
      2: '"public" only means CDNs are allowed to cache it. It says nothing about duration — that comes from max-age/s-maxage.',
      3: 'They do not conflict. s-maxage is designed to override max-age for shared caches like CDNs. This is intentional and defined in the HTTP spec.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'The old URL still works — it is still cached. It just is not referenced by any HTML anymore, so users never request it.',
  ...
  },
  {
    ...
    w: {
      0: 'DDoS mitigation is a separate CDN feature (rate limiting, anycast absorption). Origin shield is specifically about reducing origin load from PoP cache misses.',
      2: 'Encryption (TLS) is handled separately on each connection. Origin shield is about request routing, not security.',
      3: 'Compression is configured at the origin or CDN layer independently. Origin shield is about reducing the COUNT of origin requests, not their size.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'max-age=60 is far too short. The URL will never change (hash guarantees freshness), so caching for 60s throws away the benefit of URL versioning.',
      1: 'private, no-store is for sensitive user-specific data like bank balances. A JS bundle is public and should be aggressively cached.',
      3: 'stale-while-revalidate is useful for content that updates periodically (like an HTML page). A hashed bundle never updates at its URL — immutable is the right signal.',
    },
    r: { id: 's3', label: 'Section 3 — Cache-Control headers' },
  },
  {
    ...
    w: {
      0: 'Without Vary, yes. But with Vary: Accept-Language, the CDN creates separate entries per language. A French user must not get the German cached response.',
      2: 'Vary does not disable caching. It splits the cache key. CDNs fully support Vary — they just cache per (URL + Vary header value) combination.',
      3: 'The HTTP spec defines Vary behavior; all major CDNs implement it the same way. It is not provider-specific.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Product images are the same for all users — perfect for "public" CDN caching with a long TTL.',
      1: 'CSS stylesheets are public static assets — ideal for CDN caching with long TTLs and content hashing.',
      3: 'A public blog post is the same for all readers — it should be publicly cached at the CDN for performance.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Edge functions CAN make outbound HTTP requests (to origin, APIs, etc.). Network access is a primary use case.',
      1: 'CDNs serve static files as their core purpose. Edge functions can also intercept and modify static responses.',
      3: 'CDN edge networks have PoPs on every continent. Edge functions run in every region — that is why they have low latency globally.',
    },

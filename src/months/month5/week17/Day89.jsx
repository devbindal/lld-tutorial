...
const QUESTIONS = [
  ...
  },
  {
    ...
    o: ['Sort all N scores with Arrays.sort()', 'Use a min-heap of size K', 'Use a hash map keyed by score', 'Scan twice: once for max, once for second-max…'],
    a: 1,
    ...
    w: {
      0: 'Sorting all N scores costs O(N log N) and O(N) space — you process and keep far more than you need.',
      2: 'A hash map by score cannot efficiently answer "the top-K" — you would still need to extract and sort O(N) entries.',
      3: 'Scanning once per rank costs O(N × K) which is far worse than O(N log K) for large K.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'That would be ZADD NX — it only adds if the member is absent. ZINCRBY always updates the score.',
      2: 'That is ZREVRANK — it returns position, it does not change anything.',
      3: 'That is ZREM — it removes a member entirely.',
    },
    r: { id: 's2', label: 'Section 2 — Redis Sorted Set' },
  },
  {
    ...
    w: {
      0: 'SQL absolutely supports LIMIT — this is not the constraint.',
      1: 'SQL can sort by any type including numbers — not the issue.',
      3: 'With a proper index on the score column, SQL avoids a full table scan. The bottleneck is write throughput and lack of an atomic increment, not the query itself.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Encoding timestamps into scores breaks score arithmetic (you cannot ZINCRBY naturally) and makes range queries complex.',
      2: 'Replaying the full event log is accurate but far too slow for real-time query latency — used for rebuilding projections, not serving queries.',
      3: 'A SQL WHERE clause on date works for moderate traffic but does not scale to millions of real-time updates.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Count-Min Sketch does not change the top-K extraction speed. A heap is still used to extract the final top-K from the sketch.',
  ...
  },
  {
    ...
    w: {
      0: 'Write amplification would be a fan-out problem — writing to all regions on every event. The described pattern writes locally only.',
      2: 'Redis supports ZUNIONSTORE across keys on the same instance. Cross-server merges require a job that reads from each region and writes to the global instance.',
      3: 'ZINCRBY and ZREVRANK remain O(log N) on each individual sorted set — merging does not change per-set complexity.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'The heap is capped at size K — growing beyond K would defeat the purpose of the bounded structure.',

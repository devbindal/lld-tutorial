...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'Event Sourcing stores events (things that happened), not commands (things requested). Commands can be rejected; events already occurred.',
      1: 'That is traditional CRUD — Event Sourcing is the opposite: never update, always append.',
  ...
  },
  {
    ...
    w: {
      ...
      1: 'Querying the event store directly is possible but defeats the purpose — you would replay events on every read. The query side maintains ready-to-serve projections.',
      3: 'A cache is a performance trick on the same model. CQRS creates a genuinely separate model with different storage.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Two-Phase Commit (2PC) is exactly what Sagas avoid — 2PC requires locking across services and is impractical in microservices.',
      ...
      3: 'Database rollbacks are not available across services. Each service committed its own transaction. Compensation must be a new forward action.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'This has the definitions backwards: a command is the request, an event is the fact.',
      3: 'An event cannot be rejected because it already happened. You can issue a compensating event, but not reject the original.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'Both can be asynchronous. The distinction is about whether there is a central coordinator, not about sync/async.',
      2: 'This has the two definitions swapped. Orchestration has the coordinator, choreography is peer-to-peer.',
  ...
  },
  {
    ...
    w: {
      ...
      2: 'The event store is written first (by the command handler). Consumers read from it asynchronously afterward.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'Acknowledging prevents future delivery from the broker, but does not protect against the same event arriving before the ack is processed (the at-least-once window).',
      2: 'Throwing an exception would stop the consumer and may cause the broker to retry, making the problem worse.',
      3: 'Processing twice is rarely harmless — charging a payment twice, sending two emails, or decrementing inventory twice are all serious bugs.',
    },

...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'Opening a new TCP connection per message is what plain HTTP does — WebSocket specifically avoids this overhead by keeping one connection alive.',
      2: 'Polling every second is a naive workaround for real-time delivery, not WebSocket. WebSocket eliminates polling entirely.',
      3: 'The server cannot open a connection back to the client — it has no way to initiate TCP. The client always initiates, then upgrades.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Servers are separate processes, often on separate machines. Direct memory access is not possible across a network boundary.',
      2: 'Databases are for durable storage, not real-time signaling between servers. Using a DB as a notification bus is slow and causes polling on the DB.',
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Fan-out is a term borrowed from electronics (one signal going to many receivers) — nothing to do with physical cooling.',
      3: 'Time zones affect display formatting, not message ordering logic. Ordering is about server-assigned sequence numbers.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Re-broadcasting all history from the beginning would be enormous for active conversations. Only missed messages (since lastSeen) are delivered.',
      2: 'The whole point of the database is so the server can replay missed messages without relying on other clients.',
  ...
  },
  {
    ...
    w: {
      ...
      1: 'SENT means the server stored the message. DELIVERED means it reached the recipient\'s device. These are distinct states with different meaning.',
      3: 'Both statuses apply to both direct and group conversations. In group chats, DELIVERED and READ are tracked per recipient.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'SSE is fast for delivery — it is a persistent HTTP stream. The problem is direction, not speed.',
      2: 'SSE is a standard browser API supported in all modern browsers without plugins.',
      3: 'SSE carries UTF-8 text events. The limitation is direction (server→client only), not content type.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Random UUIDs have no ordering guarantee — sorting by UUID does not give chronological order.',
      1: 'Client timestamps are unreliable: clocks differ between devices, and clients can fake them. The server assigns the authoritative timestamp.',
      3: 'A global auto-increment across all conversations works but creates a single write bottleneck and leaks metadata (conversation volume can be inferred from gaps).',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'WebSocket session objects cannot be stored in a relational database — they are live in-memory objects holding an open TCP connection.',
      3: 'A single global static list shared across server instances is impossible — each server is a separate process, often on a different machine.',
    },

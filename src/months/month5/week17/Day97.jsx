...
const QUESTIONS = [
  {
    ...
    w: {
      1: 'L4 and L7 are very different OSI layers. L4 sees TCP connections; L7 sees HTTP messages. They have different routing capabilities.',
      2: 'L7 works with both HTTP and HTTPS (it does TLS termination). The distinction is about what it can read, not which protocol it supports.',
      3: 'L4/L7 do not distinguish between databases and web servers — these terms refer to which OSI layer the load balancer operates on.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Round-robin does not account for server capacity. It sends equal requests to all servers, overloading the weaker one.',
      1: 'IP hash is about session affinity, not capacity. It does not help when servers have different capacities.',
  ...
  },
  {
    ...
    w: {
      0: 'Round-robin is simple but ignores actual server load. Least-connections is genuinely better for uneven request durations.',
      1: 'It is the opposite: round-robin just counts turns (it does not count connections). Least-connections tracks active connections.',
      3: 'TLS termination is a feature of L7 load balancers, not of the least-connections algorithm. The algorithm only decides which server to send the next request to.',
    },
    r: { id: 's3', label: 'Section 3 — Load balancing algorithms' },
  },
  {
    ...
    w: {
      ...
      2: 'In active health checks, the load balancer probes the servers, not the other way around. Servers do not send heartbeats to the LB in this model.',
  ...
  },
  {
    ...
    w: {
      0: 'Redis is a cache/store, not a web server. Requests still go to application servers — Redis just stores the shared session data.',
  ...
  },
  {
    ...
    w: {
      0: 'That describes rate limiting or connection limits, not draining. Draining is about gracefully removing a server from the rotation.',
      1: 'Stopping all connections immediately drops in-flight requests — that is the opposite of graceful. Draining specifically avoids this.',
      3: 'Load balancers do not routinely flush connection tables on a timer. Draining is a per-server lifecycle operation during deployments.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      1: 'Weighted round-robin rotates with bias based on server weights, not client IPs. NAT does not affect it.',
  ...
  },
  {
    ...
    w: {
      0: 'TLS termination does not block non-HTTPS traffic — that is a firewall rule. Termination is about where decryption happens.',

...
const QUESTIONS = [
  {
    ...
    o: ['O(L) where L is the length of the prefix — you walk exactly L nodes', 'O(log N)', 'O(L × N)', 'O(N) where N is total words in the Trie'],
    a: 0,
    ...
    w: { 1: 'O(log N) is a binary search on a sorted array; Trie doesn\'t sort by whole words.', 2: 'O(L × N) would mean scanning all words per character — that\'s worse than O(N), not a Trie property.', 3: 'O(N) is the naive array-scan cost that Trie avoids — the whole point of the structure.' },
  ...
  },
  {
    ...
    o: ['Binary search the 500 results', 'Min-heap of size 5: O(500 log 5)', 'Stored top-5 at each Trie node: O(1) lookup', 'Sort all 500 and take the first 5: O(500 log 500)'],
    a: 2,
    ...
    w: { 0: 'You can only binary search a sorted structure — DFS results are not sorted by default, and this is not a standard approach.', 1: 'Min-heap is O(M log K) — better than sorting, but still O(M) work per query.', 3: 'Sorting is O(M log M) — acceptable but slowest of the three since you sort all matches.' },
  ...
  },
  {
    ...
    o: ['DFS is always faster than stored top-K', 'Stored top-K makes reads O(1) but makes writes more expensive — each insert updates top-K lists all the way up the path from root to the inserted word', 'Stored top-K uses less memory', 'Stored top-K is only useful when K = 1'],
    a: 1,
    ...
    w: { 0: 'DFS is O(M) per query where M = number of matching words; stored top-K is O(1). DFS is slower for large M.', 2: 'Stored top-K uses MORE memory — every node holds a list of K words. DFS uses no extra node memory.', 3: 'Stored top-K is useful for any K — K just controls list size and insert cost.' },
  ...
  },
  {
    ...
    o: ['Randomly distribute all Trie nodes', 'Replicate the full Trie to every machine', 'Shard by user ID', 'Shard by prefix — e.g. "a*" goes to shard A, "b*" goes to shard B — so a query for "jav" hits exactly one shard'],
    a: 3,
    ...
    w: { 0: 'Random distribution means a single prefix query must ask ALL shards and merge results — expensive and complex.', 1: 'Full replication to every machine works for small tries but is not scalable for the global web-search Trie.', 2: 'Sharding by user ID is for personalization data, not the global frequency Trie — user identity is irrelevant to prefix routing.' },
  ...
  },
  {
    ...
    o: ['Directly increment frequency at the leaf node and update top-K lists along the path from root to that leaf — O(L × K)', 'Never update frequencies — they are fixed at build time', 'Store the click in a log and rebuild the Trie offline every hour', 'Rebuild the entire Trie from scratch'],
    a: 2,
    ...
    w: { 0: 'This describes an in-memory online update — correct for a small system, but production systems prefer offline batch to avoid live-write bottlenecks.', 1: 'Fixed frequencies would mean popular new searches (like a breaking news term) never surface — unusable.', 3: 'Rebuilding from scratch per click is O(total words × avg length) — catastrophic at scale.' },
    r: { id: 's9', label: 'Section 9 — offline aggregation' },
  },
  {
    q: 'How does personalization fit on top of a global Trie result?',
    o: ['The Trie returns global top-K; a post-processing step re-ranks by the user\'s personal history — this is the Decorator pattern', 'The Trie itself stores per-user frequencies — separate Trie per user', 'Personalization requires a completely different data structure, not a Trie', 'You add a "user" field to each TrieNode'],
    a: 0,
  ...
  },
  {
    ...
    o: ['A stack', 'A max-heap of size 200', 'Sort the 200 words alphabetically and take the first 5', 'A min-heap of size 5 — as you scan the 200 matches, if a word\'s frequency exceeds the heap\'s minimum, swap it in; the heap always holds the current top-5'],
    a: 3,
    ...
    w: { 0: 'A stack has no ordering property — it does not help with top-K selection.', 1: 'A max-heap of size M (all 200 words) costs O(M log M) to build and doesn\'t save over sorting.', 2: 'Alphabetical sort has nothing to do with frequency ranking — you want the highest-frequency words, not the lexicographically first.' },

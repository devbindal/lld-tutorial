...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'File names are metadata in the metadata DB — they have nothing to do with how chunks are stored or addressed.',
      2: 'Random UUIDs are common for file IDs in the metadata layer, but chunk IDs in a CAS system are derived deterministically from content — that\'s the whole point.',
      3: 'Byte-offset addressing is how a naive file system works. CAS uses content hashes so the same data at different offsets still gets the same ID.',
    },
  ...
  },
  {
    ...
    o: ['10 (all chunks — always re-upload the whole file)', '6 (the ones the server has)', '4 (only the missing ones)', '0 (the server constructs the file from the hashes)'],
    a: 2,
    ...
    w: {
      0: 'That\'s what naive single-blob upload does. Chunked upload + CAS exists precisely to avoid this.',
      1: 'The 6 chunks the server already has are the ones it does NOT need — the client skips those.',
      3: 'The server can\'t reconstruct byte content from hashes alone — it needs the actual chunk bytes for any hash it doesn\'t already store.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Commit speed is unrelated. The reason is semantic: we don\'t know the final deduplicated cost until we know which chunks were already present.',
      2: 'Chunk sizes are tracked precisely — each Chunk record has a size field.',
  ...
  },
  {
    ...
    w: {
      0: 'A newly uploaded chunk gets its reference count set to 1 immediately at commit time. A count of 0 means all references have been removed.',
      1: 'A refCount of 0 has nothing to do with corruption — it purely means the chunk is unreachable from any live file version.',
      3: 'A count of 0 means the OPPOSITE — no files reference it, so it is safe to delete.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'Compression reduces size but still transfers all chunks. Delta sync is about only transferring chunks that actually changed — a fundamentally different saving.',
      2: 'Wi-Fi vs mobile data is a client-side network policy, not the delta sync algorithm.',
      3: 'Line-level diffs (like git) operate on text. Chunked delta sync works on binary chunks and doesn\'t require understanding file format — it\'s more general.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'The block store is content-addressed; there are no "slots" to copy into. The new version simply lists the same hashes — the bytes are never moved.',
      2: 'Cache size is irrelevant. The savings come from the fact that unchanged chunk hashes point to existing block-store entries.',
      3: 'Keeping only the latest version would be the naive approach. The whole point of versioning is to keep all versions efficiently by sharing chunks.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      1: 'Many databases support BLOB columns. The reason is not that it\'s impossible — it\'s that it\'s impractical at scale.',
      3: 'Encryption is orthogonal — you can encrypt data in either store. Separation is about access pattern and cost, not encryption.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'That would be a naive per-user silo with no deduplication. Content-addressable storage exists to avoid exactly this.',
      2: 'Compression is a separate concern and is not automatic in the base design. Even with compression, you\'d store one compressed copy, not half.',
      3: 'Deduplication happens at the chunk level, not the whole-file level. But because the files are identical, all chunks are identical, so the net effect is the same as whole-file dedup here.',
    },

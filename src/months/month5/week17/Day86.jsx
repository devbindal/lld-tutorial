...
const QUESTIONS = [
  {
    ...
    w: {
      ...
      1: 'UUID v4 is 128 random bits. Truncating to 8 chars drops most of the entropy, creating significant collision risk.',
      3: 'MD5 hashes can collide (birthday paradox), and truncating to 6 chars makes collisions even more likely. Same URL always produces the same code, losing per-user analytics.',
    },
  ...
  },
  {
    ...
    w: {
      0: '"62" looks like decimal notation. Base-62 has no digit "6" used that way — digits go 0-9, a-z, A-Z.',
      2: '"1Z" would mean 1×62 + 61 = 123, not 62.',
      3: '"Z" is the last single digit in base-62 (index 61). 61 encodes to "Z", not 62.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Both 301 and 302 can carry a Location header. The difference is cacheability, not header support.',
      1: '301 is actually what browsers cache, not 302. 302 is intentionally NOT cached to ensure every request passes through your server.',
      3: 'The HTTP spec has no such requirement. 302 vs 301 is a business decision, not a spec constraint.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Databases absolutely support string keys — that is a core use case.',
      1: 'Redis and most databases handle the same string lengths. URL length is not the reason.',
      2: 'Redis does not generate codes. The counter and encoding logic live in the application layer.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Async logging does not prevent duplicates — at-least-once delivery actually risks duplicates. Idempotency keys solve that, not async delivery.',
      2: 'The analytics DB is a write-heavy store (append-only events). It is not read-only.',
      3: 'GDPR governs data privacy, not how you write to analytics DBs. Async vs sync logging is a performance decision.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'The database constraint is on (shortCode) being unique, not on (longUrl) being unique. The same longUrl can have many short codes.',
      3: 'Returning the same code is a valid choice but loses analytics isolation — you cannot tell which user\'s audience clicked how many times.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'A distributed lock on a DB auto-increment creates a central bottleneck and a single point of failure — exactly what Snowflake-style IDs avoid.',
      1: 'Base-62 encoding is just a number representation. It does not prevent two servers from encoding the same number differently or from generating the same counter value.',
      2: 'Random numbers with retry is Option A (random codes), not a counter approach. It has collision risk.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'Cache TTL can evict expired entries, but the cache may also serve stale data if the TTL is longer than the link\'s expiresAt. The application must always check expiry, not rely on cache eviction timing.',
      2: 'Ignoring expiry on the redirect would break the product contract entirely. Users who share time-limited links expect them to stop working.',

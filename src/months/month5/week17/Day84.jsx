...
const QUESTIONS = [
  {
    ...
    w: {
      ...
      2: 'Waiting still consumes a thread and adds latency; OPEN means instant rejection, not waiting.',
      3: 'The transition to HALF_OPEN happens only after the reset timeout elapses, not immediately.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'The nesting order determines whether retries count as failures and whether retries even hit the network when the circuit is open.',
      2: 'Latency is not the reason — the semantics of what counts as a failure are.',
      3: 'Bypassing the circuit breaker would defeat its entire purpose of protecting the downstream.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'The problem is synchronization, not retry count. Jitter spreads timing, not adds retries.',
      2: 'The issue is TOO MANY requests at the same moment, not too few.',
      3: 'Jitter is about TIMING within a delay window, not endpoint selection.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Timeout matters, but the idempotency question is safety-critical and comes first.',
      2: 'Circuit breaker state affects whether the call goes through, but a closed circuit does not make a double-charge safe.',
      3: 'Delay strategy matters for timing but not for correctness of the operation itself.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'That describes the leaky bucket rate-limiting algorithm.',
  ...
  },
  {
    ...
    w: {
      0: 'A timeout can cause a failure that the circuit breaker counts, but they are distinct mechanisms.',
      2: 'Speed is not the distinction — scope (per-call vs across-calls) is.',
      3: 'This is the exact reversal of the truth.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'This is the outermost-to-innermost description — the execution order when a request arrives is the reverse of this list.',
  ...
  },
  {
    ...
    w: {
      ...
      3: 'Resetting the failure counter is a side effect, not the reason HALF_OPEN exists.',
    },
    r: { id: 's4', label: 'Section 4 — Circuit Breaker states' },
  },
]

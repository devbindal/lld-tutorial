...
const QUESTIONS = [
  {
    ...
    w: {
      ...
      1: 'The coordinator only sends COMMIT after all participants reply READY — that sequencing is the point of the prepare phase.',
      2: 'Participants can vote ABORT for any reason (insufficient funds, lock timeout, etc.) — that is expected and handled.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Calling the downstream service synchronously creates tight coupling and still does not solve atomicity between your DB write and the call.',
      1: 'Writing to Kafka first has the same problem in reverse — if the DB write fails, the event was already published.',
      3: 'Kafka does not support XA / two-phase commit with standard databases in practice, and even if it did, this pattern avoids that complexity.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'The outbox pattern does not inherently use optimistic locking — it uses a simple status column updated by the relay.',
      3: 'Kafka partitions do guarantee ordering within a partition; the duplication issue comes from the relay crash, not from ordering.',
    },
  ...
  },
  {
    ...
    w: {
      1: '2PC achieves strong consistency, not eventual consistency. Using a coordinator is how it achieves that.',
      2: 'Saga does not give ACID — it deliberately sacrifices isolation (I) for performance and fault tolerance.',
      3: 'This is backwards. Saga is the eventually consistent approach; 2PC is the strongly consistent one.',
    },
  ...
  },
  {
    ...
    w: {
      1: 'CDC is an alternative to polling the outbox table; it is not a commit protocol for Kafka.',
      2: 'Deduplication is handled by idempotency keys on the consumer side, not by CDC.',
      3: 'CDC has nothing to do with encryption — it is about change propagation, not security.',
    },
    r: { id: 's6', label: 'Section 6 — Outbox Pattern' },
  },
  {
    ...
    w: {
      0: 'JDBC absolutely supports autocommit=false — that is how single-service transactions work.',
      1: 'Commit speed is a performance concern, not the fundamental reason transactions cannot span services.',
      3: 'Java supports multi-threaded DB access fine; the issue is separate databases, not threads.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'High throughput across geographically distributed services is exactly where 2PC is the worst choice — network latency and locking make it impractical.',
      1: 'Avoiding compensating transaction work is not a technical reason to prefer 2PC — in fact 2PC has its own operational complexity.',
      2: '2PC is not strictly better — it blocks participants, requires a reliable coordinator, and does not scale to cross-service internet scenarios.',
    },
    r: { id: 's5', label: 'Section 5 — Saga vs 2PC' },
  },
  {
    ...
    w: {
      0: 'The consumer will not crash — it will simply not find the key and process the event again, which is the correctness risk.',
      2: 'Redis and Kafka are independent systems — deleting a key from Redis has no effect on Kafka messages.',

...
const QUESTIONS = [
  {
    ...
    w: {
      0: 'Speed is not the defining difference; Kafka trades some latency for durability and fan-out, and is not universally faster.',
      2: 'This describes traditional queues. Kafka keeps messages and lets each consumer group replay from any offset.',
      3: 'RabbitMQ does not inherently support more consumers; the models are fundamentally different (work-queue vs distributed-log).',
    },
  ...
  },
  {
    ...
    w: {
      0: 'enable.idempotence prevents duplicate writes from the producer but does not control which partition a message goes to.',
      1: 'A single partition works but kills horizontal scalability — the whole point of partitions is to parallelise.',
      3: 'auto.offset.reset controls where a new consumer group starts reading, not how the producer routes messages.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Kafka does not measure consumer speed for assignment; it distributes partitions as evenly as possible.',
      ...
      3: 'Partition assignment is determined during group coordination (rebalance), not per message.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Message loss is the risk of at-MOST-once (commit before processing). At-least-once errs on the side of replaying.',
      2: 'Messages are definitely replayed in at-least-once — that is the whole point of committing after processing.',
      3: 'Delivery semantics are per consumer group and any number of groups can use at-least-once.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      3: 'Time-based deletion is normal retention. Log compaction is key-based, not time-based.',
    },
  ...
  },
  {
    ...
    w: {
      0: 'Kafka does not delete uncommitted messages during rebalance; the log is immutable and offsets are just pointers.',
      2: 'No new topics are created; partitions are simply reassigned to the remaining consumers.',
      3: 'The consumer group persists; only partition assignments change.',
    },
  ...
  },
  {
    ...
    w: {
      ...
      2: 'Existing consumers do not need to stop. Consumer groups are independent; one reading does not block another.',
  ...
  },
  {
    ...
    o: ['Partition', 'Topic', 'Offset', 'Consumer Group'],
    a: 2,
    ...
    w: {
      ...
      1: 'A topic is the named category, like a table name — not a position.',

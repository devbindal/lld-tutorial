...
const QUESTIONS = [
  { q: 'DRY, properly stated, says every piece of ______ must have exactly one authoritative home.',
    o: ['comment', 'class', 'knowledge (a rule, a rate, a format)', 'code'], a: 2,
    ...
    w: { 0: 'Comments aren\'t the unit either (though duplicated knowledge IN comments also drifts!).',
         1: 'Classes are containers; knowledge is the unit.',
         3: '"Code" is the common misquote — and it leads to merging lookalikes that should stay apart.' },
    r: { id: 's2', label: 'Section 2 — DRY = one home per fact' } },
  { q: 'Why is copy-pasted business logic dangerous?',
    o: ['The compiler warns about it', 'It runs slower', 'It uses more disk space', 'When the rule changes, every copy must be found and updated — miss one and the system silently disagrees with itself'], a: 3,
    ...
    w: { 0: 'No compiler warning exists for semantic duplication.',
         1: 'Speed is identical — correctness over time is what decays.',
         2: 'Disk space is free; drift is expensive.' },
    r: { id: 's3', label: 'Section 3 — the GST change request' } },
  { q: 'Signup requires age ≥ 18 (contract law) and car rental requires age ≥ 18 (company policy). Merging them into one shared AgeCheck is…',
    o: ['A KISS violation', 'Wrong — false duplication: identical code, different knowledge, different reasons to change', 'Required by DRY', 'Required by SOLID'], a: 1,
    ...
    w: { 0: 'KISS is about machinery; this is about ownership of facts.',
         2: 'DRY requires same KNOWLEDGE — these are two different rules in matching costumes.',
         3: 'SOLID doesn\'t demand merging — SRP actually argues against it (two actors!).' },
    r: { id: 's4', label: 'Section 4 — false duplication' } },
  { q: 'The "rule of three" for duplication says…',
    o: ['Tolerate the 2nd copy with an itch; by the 3rd, the shared knowledge is proven — extract it then', 'Three interfaces per class', 'Delete every 3rd class', 'Never write a function under 3 lines'], a: 0,
    ...
    w: { 1: 'Interface counting belongs to Day 14, not here.',
         2: 'Nothing gets deleted — the rule times EXTRACTION.',
         3: 'Function length rules are a different style debate.' },
    r: { id: 's4', label: 'Section 4 — the rule of three' } },
  { q: 'KISS\'s main argument against "clever" code is…',
    o: ['Short code is always better', 'Code is read far more often than written — every decoding cost is paid by every future reader (including you in 6 months)', 'Java forbids bit tricks', 'Clever code always has bugs'], a: 1,
    ...
    w: { 0: 'Short but cryptic loses to slightly longer but instant to read.',
         2: 'No syntax is forbidden; opacity is.',
         3: 'Clever code can be perfectly correct — its cost is human, not mechanical.' },
    r: { id: 's5', label: 'Section 5 — the reader is the customer' } },
  { q: 'YAGNI says don\'t build the plugin system "we might need someday" because…',
    o: ['Unused flexibility still costs: it must be designed, tested, understood and maintained — and guesses about the future are usually wrong anyway', 'It would violate DRY', 'Java has no plugins', 'Plugins are bad design'], a: 0,
    ...
    w: { 1: 'DRY is unrelated; the engine duplicates nothing.',
         2: 'Java has plugin mechanisms galore — irrelevant.',
         3: 'Plugin systems are great WHEN plugins exist — the timing is the sin.' },
    r: { id: 's7', label: 'Section 7 — YAGNI' } },
  { q: 'Does YAGNI forbid Day 15\'s practice of putting interfaces at the database/email boundaries?',
    o: ['YAGNI only applies to UI code', 'Yes, unless using Spring', 'Yes — all abstraction is YAGNI', 'No — boundaries to volatile infrastructure are KNOWN change-points (and needed for testing today); YAGNI targets imagined features, not proven seams'], a: 3,
    ...
    w: { 0: 'YAGNI applies everywhere — UI included.',
         1: 'Frameworks don\'t change the principle.',
         2: '"All abstraction is YAGNI" throws out testability with the bathwater.' },
    r: { id: 's7', label: 'Section 7 — YAGNI vs proven seams' } },
  { q: 'The healthiest way to see DRY/KISS/YAGNI next to SOLID is…',
    o: ['They replace SOLID', 'They contradict each other, pick one', 'As counterweights: SOLID pushes toward structure and abstraction; these three push back against doing it where it isn\'t earned — good design balances the two', 'They only apply to scripts'], a: 2,
    ...
    w: { 0: 'Nothing is replaced — they coexist as force and counterforce.',
         1: 'They tension, not contradict — both sides are needed.',
         3: 'These habits scale from scripts to platforms.' },
    r: { id: 's9', label: 'Section 9 — when principles fight' } },
]

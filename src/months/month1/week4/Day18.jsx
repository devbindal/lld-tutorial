...
const QUESTIONS = [
  { q: 'The Law of Demeter says a method may talk only to…',
    o: ['Its parent class', 'Itself, its own fields, its parameters, and objects it creates — NOT to things returned by its friends\' getters', 'Classes in the same package', 'Static methods'], a: 1,
    ...
    w: { 0: 'Parents are fine (this covers inherited members) — strangers come from getters.',
         2: 'Packages are irrelevant to LoD.',
         3: 'Statics are usually counted as allowed too — but they\'re not the defining list.' },
    r: { id: 's1', label: 'Section 1 — the friend list' } },
  { q: 'Why is order.getCustomer().getAddress().getCity() expensive, precisely?',
    o: ['The caller is coupled to the internal STRUCTURE of three other classes — any rename, restructure, or null in the chain breaks every caller', 'Three method calls are slow', 'Getters are deprecated', 'It uses too much memory'], a: 0,
    ...
    w: { 1: 'Three calls cost nanoseconds — speed is the wrong lens entirely.',
         2: 'Getters are fine; NAVIGATING through them is the issue.',
         3: 'Memory is untouched.' },
    r: { id: 's2', label: 'Section 2 — anatomy of a train wreck' } },
  { q: 'Is new StringBuilder().append("a").append("b").toString() a Demeter violation?',
    o: ['Only in Java 8+', 'Yes, unless wrapped in a helper', 'No — fluent chains return the SAME object (or fresh values); you never reach into another object\'s structure', 'Yes — three dots!'], a: 2,
    ...
    w: { 0: 'Java version is irrelevant.',
         1: 'No helper wrapper needed for legitimate fluent chains.',
         3: 'Dot-counting flags this falsely — every append returns the SAME object.' },
    r: { id: 's6', label: 'Section 6 — chains that are fine' } },
  { q: '"Tell, Don\'t Ask" means…',
    o: ['Ask the user for confirmation', 'Always use void methods', 'Never write getters', 'Instead of pulling data out and deciding outside (ask), tell the owning object what you WANT and let it use its own data (customer.pay(30))'], a: 3,
    ...
    w: { 0: 'It\'s about objects, not user prompts. 🙂',
         1: 'Return types are unrelated.',
         2: 'Getters aren\'t banned — outsourcing DECISIONS through them is.' },
    r: { id: 's4', label: 'Section 4 — Tell, Don\'t Ask' } },
  { q: 'Your unit test needs: a mock Order returning a mock Customer returning a mock Address. This signals…',
    o: ['Too few mocks', 'Excellent thorough testing', 'A JUnit bug', 'A Demeter violation in the code under test — the test must rebuild the whole structure chain the code digs through'], a: 3,
    ...
    w: { 0: 'MORE mocks is the disease, not the cure.',
         1: 'Mock depth is pain, not thoroughness — tests should mirror needs, not internal geography.',
         2: 'JUnit/Mockito work fine — the code under test is the patient.' },
    r: { id: 's2', label: 'Section 2 — test torture' } },
  { q: 'The two standard fixes for a train wreck are…',
    o: ['Add a delegate method on the friend (order.getShippingCity()) — or have the caller receive the needed value directly as a parameter', 'Use static imports', 'Make all fields public', 'Try/catch and logging'], a: 0,
    ...
    w: { 1: 'Static imports are syntax sugar, not structure medicine.',
         2: 'Public fields make digging EASIER — pure regression.',
         3: 'Try/catch handles failures — it cannot remove structural knowledge.' },
    r: { id: 's3', label: 'Section 3 — breaking the chain' } },
  { q: 'What is the "delegation tax" trap?',
    o: ['Delegation is slow', 'Mechanically wrapping EVERY chain creates dozens of pass-through getters, bloating Order into a fat facade — add delegates for real questions clients ask, not for every path', 'Delegates break the compiler', 'Java charges for delegates'], a: 1,
    ...
    w: { 0: 'Delegation is a design move, not a performance one.',
         2: 'Compilers love forwarders; READERS drown in them.',
         3: 'Java charges nothing. 🙂' },
    r: { id: 's8', label: 'Section 8 — the delegation tax' } },
  { q: 'In Day 17\'s language, the Law of Demeter is…',
    o: ['A naming convention', 'A performance rule', 'A coupling rule: it forbids the hidden ropes created when callers memorize other objects\' internal structure', 'A cohesion rule'], a: 2,
    ...
    w: { 0: 'It\'s a structural law, not naming.',
         1: 'Performance never enters the picture.',
         3: 'Cohesion is about insides of one box; LoD polices the ropes between boxes.' },
    r: { id: 's9', label: 'Section 9 — cheat sheet (force-language)' } },
]

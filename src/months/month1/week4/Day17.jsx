...
const QUESTIONS = [
  { q: 'Coupling and cohesion measure, respectively…',
    o: ['Speed and memory', 'How much modules depend on EACH OTHER, and how strongly things INSIDE one module belong together', 'Inheritance depth and interface count', 'Lines of code and file count'], a: 1,
    ...
    w: { 0: 'Both are structural measures, not performance ones.',
         2: 'Those are narrow metrics; coupling/cohesion are the general forces.',
         3: 'Counting artifacts misses the point — it\'s about RELATIONSHIPS.' },
    r: { id: 's1', label: 'Section 1 — the office floor' } },
  { q: 'The universal design goal is…',
    o: ['LOW coupling between modules, HIGH cohesion inside each module', 'Low coupling, low cohesion', 'Zero coupling everywhere', 'High coupling, high cohesion'], a: 0,
    ...
    w: { 1: 'Low cohesion = junk drawers everywhere.',
         2: 'Zero coupling = modules that can\'t even cooperate — a pile of hermits, not a system.',
         3: 'High coupling = every change ripples everywhere.' },
    r: { id: 's1', label: 'Section 1 — the slogan' } },
  { q: 'account.balance = account.balance - fee; (from another class) is which coupling level?',
    o: ['No coupling', 'Data coupling — it passes data', 'Content coupling — reaching into another object\'s internals, the worst level', 'Interface coupling'], a: 2,
    ...
    w: { 0: 'Very much coupling — the strongest, most invasive kind.',
         1: 'Data coupling passes simple VALUES through parameters — this reaches into organs.',
         3: 'Interface coupling is the ladder\'s TOP — this is its bottom.' },
    r: { id: 's3', label: 'Section 3 — the coupling ladder' } },
  { q: 'printer.print(report, true) — what is the smell, and its name?',
    o: ['Temporal cohesion', 'Control coupling: a flag steers the callee\'s internal logic, and the caller must know its insides to pick the flag', 'Content coupling', 'Too few arguments'], a: 1,
    ...
    w: { 0: 'Temporal cohesion is about grouping by time inside a module.',
         2: 'Content coupling touches internals directly — this steers them via a flag.',
         3: 'Argument count isn\'t the issue — MEANING is.' },
    r: { id: 's3', label: 'Section 3 — control coupling' } },
  { q: 'class Utils { formatDate(), calcTax(), shuffle() } exhibits which cohesion?',
    o: ['Temporal', 'Functional — they are all useful', 'Sequential', 'Coincidental — the members share nothing but an address; it\'s a junk drawer'], a: 3,
    ...
    w: { 0: 'Temporal = grouped by WHEN they run (init); these share nothing at all.',
         1: '"All useful" describes every method ever written — usefulness isn\'t glue.',
         2: 'Sequential = output feeds input; no such chain here.' },
  ...
  { q: 'The smell "one small business change forces edits in 9 files" is called ______, and signals ______.',
    o: ['Shotgun surgery; the knowledge for that change is scattered — cohesion failed for that concern', 'Dead code; YAGNI', 'Feature envy; good layering', 'Divergent change; high cohesion'], a: 0,
    ...
    w: { 1: 'Dead code and YAGNI are about unused things, not scattered edits.',
         2: 'Feature envy is a method coveting another object\'s data (Day 18).',
         3: 'Divergent change is the MIRROR image — one class, many reasons.' },
    r: { id: 's8', label: 'Section 8 — diagnosing symptoms' } },
  { q: 'How do SRP and DIP map onto today\'s two forces?',
    o: ['Both are cohesion rules', 'Neither relates', 'Both are coupling rules', 'SRP is a COHESION rule (one reason to change = strong glue); DIP is a COUPLING rule (point your ropes at stable abstractions)'], a: 3,
    ...
    w: { 0: 'DIP is the textbook COUPLING rule.',
         1: 'They map cleanly — that mapping is the day\'s big reveal.',
         2: 'SRP is the textbook COHESION rule.' },
  ...
  { q: 'In the layered system, NOBODY depended on DbImpl (blast radius 0), while StorePort had several dependents. Why is that the healthy shape?',
    o: ['It is a bug — impls should be depended on', 'Ports should depend on impls', 'Volatile details (impls) should have few dependents so they can change freely; many dependents should rest only on STABLE things (small interfaces)', 'Blast radius should be equal everywhere'], a: 2,
    ...
    w: { 0: 'Zero dependents on an impl is the GOAL, not a bug.',
         1: 'Ports depending on impls would re-invert the arrows the wrong way.',

...
const QUESTIONS = [
  { q: 'The core difference between an ENTITY and a VALUE OBJECT is…',
    o: ['Entities are bigger', 'Entities have identity (WHICH one matters, compared by id); value objects are interchangeable content (compared by value)', 'Value objects live in the database', 'Entities are immutable'], a: 1,
    ...
    w: { 0: 'Size is irrelevant — a tiny class can be an entity, a big one a value.',
  ...
  { q: 'Why must money never be a double?',
    o: ['Doubles are too small', 'Binary floating point cannot represent most decimal fractions exactly — 0.1 + 0.2 ≠ 0.3 — and bare numbers happily add dollars to rupees', 'Doubles cannot be printed', 'Java deprecated double'], a: 1,
    ...
    w: { 0: 'double\'s RANGE is huge — its decimal PRECISION is the problem.',
         2: 'Doubles print fine; they just print the wrong 17th decimal.',
  ...
  { q: 'Why must value objects be IMMUTABLE?',
    o: ['Immutable objects are faster', 'They are shared freely and used as map keys — if a shared/keyed value mutates, every holder changes behind their back and hash lookups break', 'Java requires it', 'To save memory'], a: 1,
    ...
    w: { 0: 'Performance is a side bonus, not the reason.',
         2: 'No language mandate exists — it\'s a design necessity.',
  ...
  { q: 'What does a Java record (record Money(BigDecimal amount, Currency currency)) give you for free?',
    o: ['Setters and a default constructor', 'Final fields, constructor, equals(), hashCode() and toString() — a value object in one line (you add validation in the compact constructor)', 'Database persistence', 'Inheritance from other records'], a: 1,
    ...
    w: { 0: 'Records have NO setters by design — that\'s the point.',
         2: 'Persistence is a separate concern.',
         3: 'Records can\'t extend classes (they extend Record) — only implement interfaces.' },
    r: { id: 's7', label: 'Section 7 — records' } },
  { q: '"Parse, don\'t validate" means…',
    o: ['Never check inputs', 'Convert raw input into a TYPE (new Email(s)) at the boundary — once constructed, the type is PROOF of validity, and downstream re-checks disappear', 'Use regex everywhere', 'Validate in every method, every time'], a: 1,
    ...
    w: { 0: 'Checks happen — ONCE, at construction, instead of never.',
         2: 'Regex may be the tool; the principle is WHERE validation lives.',
         3: 'Re-validating everywhere is exactly what the technique eliminates.' },
  ...
  { q: 'bookFlight(String from, String to, String date) was called as bookFlight(date, to, from). What happens, and what fixes it?',
    o: ['Compile error already', 'It compiles and books garbage — typed parameters (Airport, Airport, FlightDate) turn the swap into a compile error', 'Java reorders them', 'A unit test is the only fix'], a: 1,
    ...
    w: { 0: 'Identical types = no compile error — that\'s the entire trap.',
         2: 'Java never reorders arguments.',
         3: 'Tests help, but a compile error at the call site is cheaper and earlier.' },
    r: { id: 's2', label: 'Section 2 — primitive obsession' } },
  { q: 'Which is NOT a good reason to create a value object?',
    o: ['Validation rules exist (email format)', 'Units/currency could be confused', 'Several fields always travel together (lat+lon)', 'Wrapping every int and String in the codebase for "purity"'], a: 3,
    ...
    w: { 0: 'Email validation is the textbook wrap reason.',
         1: 'Unit confusion (Mars!) is the second textbook reason.',
         2: 'Fields traveling together (lat+lon) is the third.' },

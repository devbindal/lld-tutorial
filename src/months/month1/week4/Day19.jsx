...
const QUESTIONS = [
  { q: 'Why is private String status; (holding "PLACED", "PAID"…) worse than an enum?',
    o: ['Strings cannot be compared', 'Enums are faster to type', 'Strings use more memory', 'Typos compile fine, illegal values flow into the system, the valid set is documented nowhere, and switch can\'t warn about missed cases'], a: 3,
    ...
    w: { 0: 'Strings compare fine (with equals) — comparison isn\'t the disease.',
         1: 'Typing effort was never the argument.',
         2: 'Memory difference is negligible — correctness is the prize.' },
    r: { id: 's2', label: 'Section 2 — magic strings' } },
  { q: 'A Java enum is really…',
    o: ['A list of int constants like in C', 'A kind of String', 'A full class: fixed set of instances, but with fields, constructors and methods allowed', 'An annotation'], a: 2,
    ...
    w: { 0: 'That\'s C — Java\'s enums are a different species: real objects.',
         1: 'name() RETURNS a String, but the constant itself is an object.',
  ...
  { q: 'Why is storing enum.ordinal() in the database a famous trap?',
    o: ['ordinal() is slow', 'Databases reject integers', 'ordinal() can be negative', 'ordinal() is the position in the declaration — reorder or insert a constant and every stored number silently means something else'], a: 3,
    ...
    w: { 0: 'Speed is fine; FRAGILITY is the issue.',
         1: 'Databases store ints happily — too happily, that\'s the trap.',
         2: 'ordinal() is always ≥ 0.' },
  ...
  { q: 'A switch over an enum that handles every constant (with no default) gives you what superpower?',
    o: ['Automatic logging', 'Faster execution', 'When someone adds a new constant, the compiler/IDE flags every switch that no longer covers all cases — a to-do list generated for free', 'Smaller bytecode'], a: 2,
    ...
    w: { 0: 'No logging is involved — the superpower is the compile-time TODO list.',
         1: 'Speed is identical.',
         3: 'Bytecode size is irrelevant.' },
    r: { id: 's4', label: 'Section 4 — exhaustive switch' } },
  { q: 'Which of these should probably NOT be an enum?',
    o: ['The list of countries your app ships to (changes with business deals)', 'Chess piece colors', 'Days of the week', 'Order statuses of your own system'], a: 0,
    ...
    w: { 1: 'Chess colors are eternally two.',
         2: 'The week is genuinely frozen — perfect enum.',
         3: 'YOUR statuses change only when YOUR code changes — enum fits.' },
    r: { id: 's4', label: 'Section 4 — when NOT enum' } },
  { q: 'Checked vs unchecked exceptions — the practical difference?',
    o: ['Unchecked cannot have messages', 'Checked (extends Exception) force callers to catch-or-declare at compile time; unchecked (extends RuntimeException) fly silently until caught', 'Checked are faster', 'Checked only exist in tests'], a: 1,
    ...
    w: { 0: 'Both kinds carry messages and causes.',
         2: 'No speed difference exists.',
  ...
  { q: 'catch (Exception e) { } — the empty catch — is deadly because…',
    o: ['The failure still happened, but all evidence is destroyed: no log, no alert, and the program continues in a possibly corrupted state', 'It catches nothing', 'It fails to compile', 'It is slow'], a: 0,
    ...
    w: { 1: 'It catches everything — and tells no one.',
         2: 'It compiles perfectly — that\'s what makes it dangerous.',
         3: 'It costs nothing in speed; it costs everything in evidence.' },

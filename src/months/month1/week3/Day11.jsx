...
const QUESTIONS = [
  { q: 'The official definition of the Single Responsibility Principle is: a class should have…',
    o: ['Only one method', 'Only one reason to change', 'Only one field', 'Less than 100 lines'], a: 1,
    ...
    w: { 0: 'One method per class would shred every design into confetti — size was never the rule.',
         2: 'Field count is equally irrelevant.',
  ...
  { q: 'The modern way to count "reasons to change" is to count…',
    o: ['The number of if statements', 'The number of imports only', 'The different people/teams (actors) who would request changes to the class', 'Lines of code'], a: 2,
    ...
    w: { 0: 'if-counting measures complexity, not responsibility.',
         1: 'Imports HINT at it (many worlds = many masters) but the actor count is the definition.',
         3: 'Lines measure size, not masters.' },
  ...
  { q: 'Invoice has calculateTotal(), printReport() and saveToDatabase(). The accountant changes a tax rule. What is the hidden danger in this design?',
    o: ['Java does not allow 3 methods', 'The edit happens in the same class as printing and saving, risking breakage, full retests, and merge conflicts with other teams', 'Nothing — methods are independent', 'The class becomes too fast'], a: 1,
    ...
    w: { 0: 'Three methods are perfectly legal Java — the danger is design-level.',
         2: 'Methods in one class share fields, helpers, and a release unit — independence is an illusion.',
  ...
  { q: 'Which class name is the strongest SRP smoke alarm?',
    o: ['InvoiceRepository', 'TaxCalculator', 'ReportPrinter', 'UtilityManagerHelper'], a: 3,
    ...
    w: { 0: 'Repository names one role: storage.',
         1: 'TaxCalculator names ONE job — exactly the kind of honest name SRP produces.',
         2: 'ReportPrinter: one job, one actor (the manager).' },
  ...
  { q: 'A Money class stores amount + currency AND has an add(other) method with currency checks. Is this an SRP violation?',
    o: ['No — protecting and operating on its own data is ONE responsibility (Information Expert)', 'Yes — data and behavior must be in separate classes', 'Yes — Money should extend Double', 'Only if amount is public'], a: 0,
    ...
    w: { 1: 'Splitting data from its guarding behavior is the ANEMIC anti-pattern — the opposite of OOP.',
  ...
  { q: 'EmailSender both opens an SMTP connection and sends the message. Two responsibilities?',
    o: ['Yes — SMTP is always a separate class', 'Yes — connection and sending must be separate classes', 'It depends on the file length', 'No — they are two STEPS of one job: delivering email'], a: 3,
    ...
    w: { 0: '"Always separate" rules ignore the actor test — the only test that matters.',
         1: 'Splitting every step makes confetti — steps of one job stay together.',
         2: 'File length is a symptom, never the criterion.' },
    r: { id: 's7', label: 'Section 7 — steps vs responsibilities' } },
  { q: 'What is the main danger of OVER-applying SRP?',
    o: ['The code becomes too fast', 'There is no danger; more classes is always better', 'Dozens of one-method classes where a simple change forces you to open ten files — complexity moves into the wiring', 'The compiler rejects small classes'], a: 2,
    ...
    w: { 0: 'Speed is unaffected by class count at this scale.',
         1: '"More classes always better" is how 10-file changes for one bugfix happen.',
         3: 'The compiler is indifferent to design quality.' },
    r: { id: 's9', label: 'Section 9 — Trap 2: confetti' } },
  { q: 'SRP also applies to methods. Which method most needs splitting?',
    o: ['processOrder() — validates, charges payment, updates stock, sends email, and prints', 'calculateGst(amount) — 12 lines of tax math', 'isEmpty() — checks the list', 'getName() — returns the name'], a: 0,
    ...
    w: { 1: '12 lines of ONE topic (tax math) is fine — cohesive length is not the issue.',
         2: 'isEmpty() does one tiny job.',
         3: 'getName() is as single-purpose as it gets.' },

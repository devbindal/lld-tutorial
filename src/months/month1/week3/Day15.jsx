...
const QUESTIONS = [
  { q: 'The Dependency Inversion Principle has two clauses. They are…',
    o: ['High-level modules should not depend on low-level modules — both depend on abstractions; and abstractions should not depend on details — details depend on abstractions', 'Use frameworks; avoid main()', 'Never use new; never use static', 'All classes need interfaces; all interfaces need factories'], a: 0,
    ...
    w: { 1: 'Frameworks and main() are implementation choices — not the principle.',
  ...
  { q: '"High-level" vs "low-level" modules means…',
    o: ['Fast vs slow code', 'Top vs bottom of the file', 'Business policy/rules (valuable, stable) vs mechanisms like DB, email, files (replaceable details)', 'Public vs private classes'], a: 2,
    ...
    w: { 0: 'Performance is not the axis either.',
         1: 'File position means nothing — it\'s an importance/stability axis.',
         3: 'Visibility is Day 1 vocabulary, unrelated.' },
    r: { id: 's2', label: 'Section 2 — policy vs detail' } },
  { q: 'What exactly gets "inverted" in dependency inversion?',
    o: ['The inheritance hierarchy', 'The class names', 'The direction of the source-code arrow: instead of policy → detail, the detail now points UP at an abstraction owned by the policy side', 'The order of method parameters'], a: 2,
    ...
    w: { 0: 'Inheritance trees aren\'t flipped — source-dependency arrows are.',
         1: 'Names don\'t move — dependencies do.',
         3: 'Parameter order is cosmetic.' },
    r: { id: 's3', label: 'Section 3 — the arrow flipper' } },
  { q: 'DIP vs dependency injection (DI) — what is the difference?',
    o: ['DI is the principle, DIP is the tool', 'DIP is the PRINCIPLE (depend on abstractions); DI is one TECHNIQUE for it (hand dependencies in from outside, e.g. via constructor)', 'DIP only works with Spring', 'They are the same thing'], a: 1,
    ...
    w: { 0: 'Backwards — DIP is the principle.',
         2: 'No framework needed; main() can wire everything by hand.',
         3: 'Same family, different jobs: principle vs technique.' },
    r: { id: 's6', label: 'Section 6 — DIP vs DI vs IoC' } },
  { q: 'The "composition root" is…',
    o: ['The root folder of the project', 'The database schema', 'The deepest class in the hierarchy', 'The one place at the edge of the app (e.g. main) where concrete classes are chosen, new-ed, and wired together'], a: 3,
    ...
    w: { 0: 'Folders don\'t wire objects.',
         1: 'Database schemas live elsewhere.',
         2: 'Depth in the hierarchy is unrelated — the root is at the EDGE of the app.' },
    r: { id: 's6', label: 'Section 6 — the composition root' } },
  { q: 'Why does DIP make unit testing dramatically easier?',
    o: ['Business classes accept interfaces, so tests inject tiny fakes — no real DB/SMTP server needed to test a rule', 'Tests run on the GPU', 'JUnit requires it', 'It removes the need for tests'], a: 0,
    ...
    w: { 1: 'GPUs are innocent here. 🙂',
         2: 'JUnit has no such requirement — good design does.',
         3: 'DIP makes testing EASIER, never unnecessary.' },
    r: { id: 's5', label: 'Section 5 — the test bench' } },
  { q: 'Where is DIP worth its cost?',
    o: ['Nowhere in small apps', 'At the boundaries where volatile details live: database, network, email, file system, clock, randomness', 'Only in getters', 'Everywhere — wrap String in an interface too'], a: 1,
    ...
    w: { 0: 'Small apps still need testable seams at their few doors.',
         2: 'Getters are not boundaries.',
         3: 'IString is the parody version — stable values need no socket.' },
    r: { id: 's8', label: 'Section 8 — where to invert' } },
  { q: 'How does DIP complete the rest of SOLID week?',
    o: ['It only matters for UML', 'It replaces SRP', 'It contradicts OCP', 'It supplies the abstractions that OCP\'s slots and Day 7\'s injection rely on — the final arrow-flip that makes the other principles practical'], a: 3,
    ...
    w: { 0: 'DIP shapes the whole architecture, far beyond diagrams.',
         1: 'Nothing replaces SRP — small pieces stay step one.',
         2: 'OCP and DIP are teammates: slots + arrow direction.' },

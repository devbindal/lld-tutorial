...
const QUESTIONS = [
  { q: 'The Interface Segregation Principle says…',
    o: ['Interfaces are better than classes', 'No client should be FORCED to depend on methods it does not use — prefer many small role interfaces over one fat one', 'Every class needs exactly one interface', 'Interfaces may have at most 3 methods'], a: 1,
    ...
    w: { 0: '"Better" is meaningless — interfaces and classes do different jobs.',
         2: 'No one-interface-per-class quota exists — roles drive the count.',
         3: 'No magic number; List has ~25 cohesive methods and is fine.' },
    r: { id: 's1', label: 'Section 1 — the mega gym contract' } },
  { q: 'What do classes do when a fat interface demands abilities they don\'t have?',
    o: ['They skip those methods', 'The compiler exempts them', 'Java auto-generates real implementations', 'They implement dummy methods — empty or throwing UnsupportedOperationException — i.e., they lie'], a: 3,
    ...
    w: { 0: 'Skipping methods is a compile error (unless the class goes abstract).',
         1: 'No exemptions exist — implements means ALL of it.',
         2: 'Java generates nothing — the lie must be hand-written.' },
    r: { id: 's2', label: 'Section 2 — the forced lies' } },
  { q: 'Besides forcing lies, what is the OTHER cost of a fat interface?',
    o: ['Interfaces cannot be tested', 'Every client is coupled to the whole contract: a change to fax() ripples into print-only and scan-only code that never faxes', 'It uses more RAM', 'It slows down the JVM'], a: 1,
    ...
    w: { 0: 'Interfaces test beautifully (fakes!) — when lean.',
         2: 'Memory is untouched — coupling is the cost.',
         3: 'JVM speed is identical.' },
    r: { id: 's4', label: 'Section 4 — client coupling' } },
  { q: 'The standard ISP refactor for Machine { print, scan, fax } is…',
    o: ['Renaming it IMachine', 'One abstract class with empty defaults', 'Making fax() static', 'Role interfaces — Printable, Scannable, Faxable — with each class implementing only what it truly does (and AllInOne implementing all three)'], a: 3,
    ...
    w: { 0: 'Renaming with an I changes nothing structural.',
         1: 'Empty defaults just hide the lie inside the contract.',
         2: 'static fax() removes polymorphism entirely.' },
    r: { id: 's6', label: 'Section 6 — role interfaces' } },
  { q: 'Which JDK design is ISP taken to its logical extreme?',
    o: ['System.out', 'The Object class', 'Functional interfaces like Runnable and Comparator — exactly one role, one method', 'java.util.Vector'], a: 2,
    ...
    w: { 0: 'System.out is a field holding a PrintStream.',
         1: 'Object is the root class, not an interface.',
         3: 'Vector is a CLASS — and a bloated legacy one at that.' },
    r: { id: 's7', label: 'Section 7 — ISP in the JDK' } },
  { q: 'A method only LOOPS over names it receives. Client-side ISP says its parameter should be…',
    o: ['Iterable<String> (or at most List<String>) — the narrowest type that supports what it actually does', 'ArrayList<String> — the concrete class', 'Object', 'String[] always'], a: 0,
    ...
    w: { 1: 'Concrete parameter types reject every other collection for zero benefit.',
         2: 'Object loses the element type entirely — too far the OTHER way.',
         3: 'Arrays force conversions on most callers.' },
    r: { id: 's6', label: 'Section 6 — the client side' } },
  { q: 'ISP\'s relationship to its SOLID siblings is best described as…',
    o: ['ISP is SRP applied to interfaces — and it prevents the fat promises that force LSP violations', 'ISP contradicts LSP', 'ISP replaces OCP', 'Unrelated rules'], a: 0,
    ...
    w: { 1: 'No contradiction — ISP PREVENTS the situations that break LSP.',
         2: 'Different jobs: OCP seals cores; ISP slims contracts.',
         3: 'They form one interlocking system — that\'s the week\'s arc.' },
    r: { id: 's7', label: 'Section 7 — family connections' } },
  { q: 'What is the danger of OVER-segregating?',
    o: ['None — smaller is always better', 'The compiler runs out of names', 'Interface confetti: one interface per method everywhere, so understanding one object means chasing ten files, with no cohesive roles left', 'It breaks the JVM'], a: 2,
    ...
    w: { 0: '"Smaller always better" produces interface confetti — cohesion is the test.',
         1: 'The compiler has infinite names. 🙂',
         3: 'The JVM doesn\'t care; readers do.' },

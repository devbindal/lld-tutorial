...
const QUESTIONS = [
  { q: 'The Interface Segregation Principle says…',
    ...
    w: { 0: 'No one-interface-per-class quota exists — roles drive the count.',
         2: '"Better" is meaningless — interfaces and classes do different jobs.',
  ...
  { q: 'What do classes do when a fat interface demands abilities they don\'t have?',
    ...
    w: { 0: 'No exemptions exist — implements means ALL of it.',
         2: 'Skipping methods is a compile error (unless the class goes abstract).',
         3: 'Java generates nothing — the lie must be hand-written.' },
  ...
  { q: 'Besides forcing lies, what is the OTHER cost of a fat interface?',
    ...
    w: { 0: 'Memory is untouched — coupling is the cost.',
         2: 'Interfaces test beautifully (fakes!) — when lean.',
  ...
  { q: 'The standard ISP refactor for Machine { print, scan, fax } is…',
    ...
    w: { 0: 'Empty defaults just hide the lie inside the contract.',
         2: 'Renaming with an I changes nothing structural.',
         3: 'static fax() removes polymorphism entirely.' },
  ...
  { q: 'Which JDK design is ISP taken to its logical extreme?',
    ...
    w: { 0: 'Vector is a CLASS — and a bloated legacy one at that.',
         2: 'Object is the root class, not an interface.',
         3: 'System.out is a field holding a PrintStream.' },
  ...
  { q: 'A method only LOOPS over names it receives. Client-side ISP says its parameter should be…',
    ...
    w: { 0: 'Concrete parameter types reject every other collection for zero benefit.',
         2: 'Arrays force conversions on most callers.',
         3: 'Object loses the element type entirely — too far the OTHER way.' },
  ...
  { q: 'ISP\'s relationship to its SOLID siblings is best described as…',
    ...
    w: { 0: 'They form one interlocking system — that\'s the week\'s arc.',
         2: 'No contradiction — ISP PREVENTS the situations that break LSP.',
         3: 'Different jobs: OCP seals cores; ISP slims contracts.' },
  ...
  { q: 'What is the danger of OVER-segregating?',
    ...
    w: { 0: '"Smaller always better" produces interface confetti — cohesion is the test.',
         2: 'The compiler has infinite names. 🙂',

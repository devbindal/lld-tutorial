...
const QUESTIONS = [
  { q: '"Open for extension, closed for modification" means…',
    o: ['Classes must be open-source', 'You should be able to add new behavior by ADDING code, without EDITING existing tested code', 'All classes must be final', 'Methods must be public'], a: 1,
    ...
    w: { 0: 'Open-source is licensing — different universe.',
         2: 'final classes are CLOSED to extension — nearly the opposite of the goal.',
  ...
  { q: 'Why is editing existing, working code considered risky?',
    o: ['It is slow to type', 'Every edit can break behavior that already worked, forces retesting everything around it, and risks merge conflicts', 'The compiler forbids it', 'Old code is read-only in Java'], a: 1,
    ...
    w: { 0: 'Typing time is trivial — re-validation time is the real bill.',
         2: 'The compiler happily lets you edit anything.',
         3: 'No such read-only rule exists — discipline does this, not the language.' },
    r: { id: 's2', label: 'Section 2 — the growing if-else' } },
  { q: 'Which Java mechanism is the engine that makes OCP possible?',
    o: ['Garbage collection', 'Polymorphism through interfaces/abstract classes (dynamic dispatch)', 'The static keyword', 'try/catch blocks'], a: 1,
    ...
    w: { 0: 'GC manages memory — it can\'t route method calls.',
         2: 'static is dispatch\'s OPPOSITE — bound at compile time.',
         3: 'Exceptions handle failures, not extension.' },
    r: { id: 's4', label: 'Section 4 — carving the slot' } },
  { q: 'In an OCP-compliant payment design, "add Crypto payments" means…',
    o: ['Adding an else-if to processPayment()', 'Creating class CryptoPayment implements PaymentMethod — and touching nothing else', 'Editing every payment class', 'Recompiling the JVM'], a: 1,
    ...
    w: { 0: 'The else-if IS the violation — reopening tested code per feature.',
         2: 'Other payment classes have no reason to know Crypto exists.',
         3: 'The JVM needs no recompiling for your features. 🙂' },
    r: { id: 's4', label: 'Section 4 — the fix' } },
  { q: 'A switch over a truly fixed set (DayOfWeek, CardinalDirection) inside one method is…',
    o: ['Always an OCP violation — switch is banned', 'Acceptable — OCP guards against LIKELY change, and these sets cannot grow', 'Only legal in C', 'A Liskov violation'], a: 1,
    ...
    w: { 0: 'No keyword is banned — only misapplied. Switch on a frozen set is honest code.',
         2: 'Java\'s switch is alive and well (and got better with expressions).',
         3: 'LSP is about substitution contracts — different principle.' },
    r: { id: 's7', label: 'Section 7 — the judgment call' } },
  { q: 'What is "speculative generality"?',
    o: ['A useful design pattern', 'Adding interfaces and plug points for changes that never come — paying OCP\'s cost with no benefit', 'A type of unit test', 'Using generics in Java'], a: 1,
    ...
    w: { 0: 'It\'s a SMELL from Fowler\'s catalog, not a pattern.',
         2: 'Unit tests are unrelated.',
         3: 'Generics are a tool — this smell is about unused flexibility of any kind.' },
  ...
  { q: 'You\'ve edited ReportExporter three times this quarter, each time adding an output format. The best response is…',
    o: ['Keep adding else-ifs; it works', 'Extract an Exporter interface and move each format into its own class — the edit history proves this axis changes often', 'Delete the feature', 'Make the method static'], a: 1,
    ...
    w: { 0: '"It works" ignores the trend — the fourth format is already scheduled somewhere.',
         2: 'Deleting features is rarely on the menu. 🙂',
         3: 'static changes nothing about the reopening problem.' },
    r: { id: 's7', label: 'Section 7 — evidence-based slots' } },
  { q: 'How do OCP and SRP (yesterday) reinforce each other?',
    o: ['They are unrelated', 'Small single-purpose classes are easier to close; and OCP\'s "new feature = new class" keeps each class single-purpose', 'OCP replaces SRP', 'SRP only applies to interfaces'], a: 1,
    ...
    w: { 0: 'They interlock tightly — SOLID is a system, not five islands.',
         2: 'Neither replaces the other; they work different angles (cohesion vs sealing).',
         3: 'SRP is about classes/modules primarily.' },

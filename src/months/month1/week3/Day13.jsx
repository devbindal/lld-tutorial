...
const QUESTIONS = [
  { q: 'The Liskov Substitution Principle says…',
    o: ['Anywhere the parent type is expected, ANY subtype must work without the client noticing or breaking', 'Subclasses must be smaller than parents', 'Inheritance is forbidden', 'All classes need interfaces'], a: 0,
    ...
    w: { 1: 'Size has nothing to do with substitution.',
         2: 'LSP restricts inheritance to honest cases — it doesn\'t ban it.',
         3: 'Interfaces are great but LSP is about BEHAVIOR under any parent type.' },
    r: { id: 's1', label: 'Section 1 — the stand-in rule' } },
  { q: 'class Penguin extends Bird overrides fly() to throw UnsupportedOperationException. What is the LSP-correct fix?',
    o: ['Catch the exception in every client', 'Restructure: Bird without fly(); FlyingBird extends Bird adds fly(); Penguin extends plain Bird', 'Make fly() static', 'Delete the Penguin class'], a: 1,
    ...
    w: { 0: 'Try-catch in every client = patching symptoms forever, and new clients will forget.',
         2: 'static fly() would break dispatch for the honest birds too.',
         3: 'Penguins are real birds — deleting the class dodges the modeling question.' },
  ...
  { q: 'Mathematically every square IS a rectangle. Why does Square extends Rectangle still fail LSP?',
    o: ['Squares have no area method', 'Rectangle\'s contract implies setWidth and setHeight are independent; Square must break that to stay square — behavior, not math, decides is-a', 'Rectangles are final', 'Java cannot model squares'], a: 1,
    ...
    w: { 0: 'area() exists via inheritance — and gives the WRONG answer, which is the point.',
         2: 'Nothing is final here; the math-vs-behavior gap is the lesson.',
         3: 'Java models squares fine — the conflict is contractual, not technical.' },
    r: { id: 's5', label: 'Section 5 — square–rectangle' } },
  { q: 'Which change is a child override ALLOWED to make?',
    o: ['Deliver less than the parent promised (weaken postcondition)', 'Demand stricter inputs than the parent (strengthen precondition)', 'Throw brand-new exceptions for valid inputs', 'Accept MORE inputs and/or deliver MORE than promised'], a: 3,
    ...
    w: { 0: 'Delivering less starves clients counting on the parent\'s promise.',
         1: 'Stricter inputs reject calls the parent accepted — existing clients explode.',
         2: 'New surprise exceptions are the penguin move — banned.' },
    r: { id: 's7', label: 'Section 7 — the contract rules' } },
  { q: 'Arrays.asList("a","b").add("c") throws UnsupportedOperationException at runtime. What does this illustrate?',
    o: ['A bug in Java', 'Correct use of generics', 'A real-world LSP violation: it claims to be a List but refuses part of List\'s contract', 'An OCP violation'], a: 2,
    ...
    w: { 0: 'It\'s documented, deliberate ("optional operations") — a design trade-off, not a bug.',
         1: 'Generics are fine here; the violation is behavioral.',
         3: 'OCP is about extension without edits — unrelated to this surprise.' },
    r: { id: 's9', label: 'Section 9 — JDK scars' } },
  { q: 'You find this in client code: if (account instanceof SavingsAccount) { …special handling… }. What does it usually signal?',
    o: ['An LSP smell: subtypes are NOT truly substitutable, so clients must check who they really got', 'Required Java syntax', 'Good defensive programming', 'A performance optimization'], a: 0,
    ...
    w: { 1: 'instanceof is legal syntax — its NECESSITY is the smell.',
         2: '"Defensive" instanceof is a confession that the type can\'t be trusted.',
         3: 'No performance story here.' },
    r: { id: 's9', label: 'Section 9 — instanceof smell' } },
  { q: 'A child overrides a parent method with an EMPTY body (the parent\'s version did real work). This is…',
    o: ['Fine — overriding allows anything', 'Recommended for performance', 'An LSP violation: the postcondition silently disappears, and clients believe work happened', 'Only a problem if it throws'], a: 2,
    ...
    w: { 0: 'Overriding ALLOWS anything syntactically — the contract forbids this semantically.',
         1: 'Speed gained by not doing the promised work is called lying.',
         3: 'Not-throwing makes it WORSE — at least exceptions are visible.' },
    r: { id: 's9', label: 'Section 9 — silent no-ops' } },
  { q: 'How does LSP connect to Day 7\'s inheritance checklist?',
    o: ['LSP replaces composition', 'LSP only applies to interfaces', 'It doesn\'t', 'LSP IS checklist item 2 — "substitutable everywhere" — the formal test for whether is-a is real; if it fails, use composition instead'], a: 3,
    ...
    w: { 0: 'LSP and composition are allies: failing LSP routes you TO composition.',
         1: 'implements is a substitutability claim too — LSP governs both.',
         2: 'Deeply connected — LSP IS Day 7\'s checklist item 2 formalized.' },
    r: { id: 's10', label: 'Section 10 — cheat sheet (the Day 7 tie)' } },
]

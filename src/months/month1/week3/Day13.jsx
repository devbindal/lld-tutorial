...
const QUESTIONS = [
  { q: 'The Liskov Substitution Principle says…',
    ...
    w: { 0: 'Size has nothing to do with substitution.',
         2: 'Interfaces are great but LSP is about BEHAVIOR under any parent type.',
         3: 'LSP restricts inheritance to honest cases — it doesn\'t ban it.' },
  ...
  { q: 'class Penguin extends Bird overrides fly() to throw UnsupportedOperationException. What is the LSP-correct fix?',
    ...
    w: { 0: 'Try-catch in every client = patching symptoms forever, and new clients will forget.',
         2: 'Penguins are real birds — deleting the class dodges the modeling question.',
         3: 'static fly() would break dispatch for the honest birds too.' },
  ...
  { q: 'Mathematically every square IS a rectangle. Why does Square extends Rectangle still fail LSP?',
    ...
    w: { 0: 'Java models squares fine — the conflict is contractual, not technical.',
         2: 'area() exists via inheritance — and gives the WRONG answer, which is the point.',
         3: 'Nothing is final here; the math-vs-behavior gap is the lesson.' },
  ...
  { q: 'Which change is a child override ALLOWED to make?',
    ...
    w: { 0: 'Stricter inputs reject calls the parent accepted — existing clients explode.',
         1: 'Delivering less starves clients counting on the parent\'s promise.',
         3: 'New surprise exceptions are the penguin move — banned.' },
  ...
  { q: 'Arrays.asList("a","b").add("c") throws UnsupportedOperationException at runtime. What does this illustrate?',
    ...
    w: { 0: 'It\'s documented, deliberate ("optional operations") — a design trade-off, not a bug.',
         2: 'Generics are fine here; the violation is behavioral.',
  ...
  { q: 'You find this in client code: if (account instanceof SavingsAccount) { …special handling… }. What does it usually signal?',
    ...
    w: { 0: '"Defensive" instanceof is a confession that the type can\'t be trusted.',
         2: 'No performance story here.',
         3: 'instanceof is legal syntax — its NECESSITY is the smell.' },
  ...
  { q: 'A child overrides a parent method with an EMPTY body (the parent\'s version did real work). This is…',
    ...
    w: { 0: 'Overriding ALLOWS anything syntactically — the contract forbids this semantically.',
         2: 'Speed gained by not doing the promised work is called lying.',
  ...
  { q: 'How does LSP connect to Day 7\'s inheritance checklist?',
    ...
    w: { 0: 'Deeply connected — LSP IS Day 7\'s checklist item 2 formalized.',
         2: 'LSP and composition are allies: failing LSP routes you TO composition.',
         3: 'implements is a substitutability claim too — LSP governs both.' },

...
const QUESTIONS = [
  { q: 'Inheritance is an "is-a" relationship. Association, aggregation and composition are all forms of…',
    ...
    w: { 0: 'is-a is ONE class being a kind of another (extends) — today\'s three bonds connect two separate living objects.',
         2: '"does-a" is not a relationship term — behavior lives in methods.',
         3: 'Same trap as A — extends is the only is-a.' },
  ...
  { q: 'In composition, what happens to the parts when the whole is destroyed?',
    ...
    w: { 0: 'Surviving parts describe AGGREGATION — the club, not the body.',
         2: 'No automatic re-homing exists — ownership transfer would have to be coded explicitly.',
         3: 'Deletion is silent reachability, never an exception.' },
  ...
  { q: 'A Team object receives already-created Player objects through its constructor. Players can join other teams later. This is…',
    ...
    w: { 0: 'Composition requires the team to CREATE and exclusively own players — these arrived from outside and can leave.',
         2: 'No extends anywhere — a team is not a kind of player.',
         3: 'They are STORED as fields — dependency never stores.' },
  ...
  { q: 'In UML, a FILLED diamond ◆ on a line means…',
    ...
    w: { 0: 'Two errors in one: filled = composition (not aggregation), and diamonds NEVER sit on the part.',
  ...
  { q: 'Which code line creates the STRONGEST bond between Car and Engine?',
    ...
    w: { 0: 'A method parameter used once = dependency, the WEAKEST bond.',
         1: 'Injected from outside = aggregation — strong, but the engine could be shared or outlive the car.',
         3: 'extends is technically the tightest coupling, but it is a FALSE is-a here — broken design, not a bond grade.' },
  ...
  { q: 'Student has a Teacher field, and Teacher has a List<Student> field. What is this called, and is it free?',
    ...
    w: { 1: 'No whole-part ownership here — just two peers pointing at each other.',
         2: 'No extends in sight.',
         3: 'Mutual references compile perfectly — the cost is maintenance, not compilation.' },
  ...
  { q: 'Delete a Playlist. Its Songs are still in the music library. Delete a House. Its Rooms are gone. Why the difference?',
    ...
    w: { 0: 'static has nothing to do with it — sharing vs exclusive ownership decides survival.',
         2: 'Java treats all references identically — the DESIGN differs, not the language.',
         3: 'Rooms are objects; primitiveness is irrelevant.' },
  ...
  { q: 'How does the Java garbage collector "delete the parts" in composition?',
    ...
    w: { 0: 'Java has no delete keyword at all — that\'s C++.',
         1: 'No destructor calls exist to insert — reachability does everything.',

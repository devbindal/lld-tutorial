...
const QUESTIONS = [
  { q: 'In noun–verb analysis of a requirement, nouns and verbs become (roughly)…',
    o: ['Nouns → candidate classes/attributes, verbs → candidate methods/relationships', 'Both become classes', 'Nouns → packages, verbs → interfaces', 'Nouns → methods, verbs → classes'], a: 0,
    ...
    w: { 1: 'If both became classes, verbs would have no home — behavior needs methods.',
         2: 'Packages come much later, from grouping classes — not from words.',
         3: 'Backwards — things are nouns, actions are verbs, in grammar and in modeling.' },
    r: { id: 's1', label: 'Section 1 — the five passes' } },
  { q: '"Each item has a name and a price." In your model, price should be…',
    o: ['A subclass of Item', 'A separate database table class', 'An attribute (field) of the item class', 'A Price class with its own file'], a: 2,
    ...
    w: { 0: 'Subclassing Item for a value would be a bizarre is-a ("price IS an item"?).',
         1: 'Persistence design comes after the object model — and not every attribute is a table.',
         3: 'A bare price has no rules or structure yet — a Price/Money CLASS is earned later, when currency and rounding rules appear (Day 20).' },
    r: { id: 's3', label: 'Section 3 — the filter' } },
  { q: 'A beginner creates one big class: SystemManager, with methods bookSeat(), payBill(), sendEmail(), addUser()… What is this called and why is it bad?',
    o: ['A god class — one class doing every job, impossible to maintain or test', 'A facade — it is good design', 'An entity class', 'A singleton — required for systems'], a: 0,
    ...
    w: { 1: 'A facade is a thin DOOR to a subsystem (Month 2) — this thing does all the work itself.',
         2: '"Entity class" describes domain objects like Customer — this monster is their opposite.',
         3: 'Singleton is about instance COUNT — and no, it isn\'t required either.' },
    r: { id: 's3', label: 'Section 3 — the god-class trap' } },
  { q: '"An order contains one or more items." The multiplicity on the Order ◆── OrderItem line, at the item end, is…',
    ...
    w: { 0: '1 means exactly one — but "one or MORE" was stated.',
         2: '* allows zero — close, but the requirement guarantees at least one.',
         3: '0..1 is "optional" — an order with zero items contradicts the text.' },
    r: { id: 's6', label: 'Section 6 — multiplicity from wording' } },
  { q: 'Ticket stores entryTime. ParkingLot stores floors. Who should own calculateFee(exitTime), and by what rule?',
    o: ['ParkingLot — it is the biggest class', 'A static FeeUtils class, always', 'Driver — drivers pay', 'Ticket — it holds the data the calculation needs (Information Expert)'], a: 3,
    ...
    w: { 0: 'Size is not a qualification — assigning by "biggest class" is how god classes breed.',
         1: 'A static FeeUtils must ASK Ticket for its data — feature envy, the very thing to avoid (Day 18 names it).',
         2: 'Drivers PAY fees; computing them needs data drivers don\'t hold.' },
    r: { id: 's5', label: 'Section 5 — Information Expert' } },
  { q: '"A spot is either COMPACT or LARGE." The cleanest model for this is…',
    o: ['Two subclasses: CompactSpot and LargeSpot, always', 'An enum: SpotSize { COMPACT, LARGE } as a field of Spot', 'A String field: size = "compact"', 'Two boolean fields: isCompact, isLarge'], a: 1,
    ...
    w: { 0: 'Two subclasses for a label-only difference is hierarchy overkill — wait until behavior truly diverges.',
         2: 'Strings compile with typos — the exact disease Day 19 cures.',
         3: 'isCompact + isLarge can BOTH be true — an illegal state the type system should forbid.' },
    r: { id: 's3', label: 'Section 3 — enums for fixed sets' } },
  { q: 'The requirement says "users can pay for the order" — nothing more. Your best FIRST move in an interview:',
    o: ['Silently design a full payment gateway with refunds and EMI', 'Skip payment entirely without saying anything', 'Ask clarifying questions: which payment methods? refunds in scope? what happens on failure?', 'Add an abstract PaymentFactory immediately'], a: 2,
    ...
    w: { 0: 'Silently building a gateway monolith burns interview time on unrequested scope.',
         1: 'Silently SKIPPING is as bad as silently building — say your assumption out loud.',
         3: 'Patterns before entities is cart-before-horse (Day 10\'s own trap list).' },
    r: { id: 's8', label: 'Section 8 — Trap 5: vague requirements' } },
  { q: 'Which noun from "The gym sends reminder emails to members" should you usually NOT turn into a class on day one?',
    o: ['member', 'Both should be classes immediately', 'Neither should ever be a class', 'the gym (the system itself)'], a: 3,
    ...
    w: { 0: 'Members have identity, data and behavior — textbook entity.',
         1: '"Both immediately" puts the system-as-class trap right back in.',
         2: 'Never-class is too strict — member must be one; only "the gym" is the trap.' },
    r: { id: 's3', label: 'Section 3 — the filter (ignore bucket)' } },
]

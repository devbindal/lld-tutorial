...
const QUESTIONS = [
  { q: 'In a UML class box, what do the three compartments contain, top to bottom?',
    o: ['Name, fields (attributes), methods (operations)', 'Methods, fields, name', 'Package, name, everything else', 'Name, constructors, fields'], a: 0,
    e: 'Always: ① class name, ② attributes/fields, ③ operations/methods. Empty compartments may be hidden, but the order never changes.',
    w: { 1: 'Methods-first feels natural to code readers, but UML fixed the order: name, data, behavior.',
         2: 'Packages are drawn as folders around classes — never inside the box.',
         3: 'Constructors (when shown) live INSIDE the methods compartment, not in their own shelf.' },
    r: { id: 's2', label: 'Section 2 — the class box' } },
  { q: 'What does the symbol # mean in front of a UML member?',
    o: ['public', 'private', 'static', 'protected'], a: 3,
    e: 'The four visibility marks: − private, + public, # protected, ~ package-private. (Same four levels you mastered on Day 1!)',
    w: { 0: 'public is the plus + ("open to all").',
         1: 'private is the minus sign − ("kept from you").',
         2: 'static is shown by UNDERLINING, not by a symbol.' },
    r: { id: 's2', label: 'Section 2 — visibility symbols' } },
  { q: 'A member name is UNDERLINED in a class box. This means…',
    o: ['It is final', 'It is abstract', 'It is static — owned by the class, not by objects', 'It is deprecated'], a: 2,
    e: 'Underline = static (class-level). Italics = abstract. Two easy-to-confuse marks — interviewers love asking the difference.',
    w: { 0: 'final has no standard UML text style (a {readOnly}/{leaf} property note is used).',
         1: 'Abstract is ITALICS — the eternal mix-up this question exists to cure.',
  ...
  { q: 'Both inheritance and interface realization use a hollow triangle ▷. How do you tell them apart?',
    o: ['Inheritance points at the child', 'Realization uses a filled triangle', 'Inheritance uses a solid line; realization uses a dashed line', 'You cannot — context decides'], a: 2,
    ...
    w: { 0: 'Triangles point UP the family tree, at the parent — never at the child.',
         1: 'Filled triangles don\'t exist in standard UML class diagrams.',
         3: 'You CAN tell — line style is the differentiator, and it\'s always graded.' },
    r: { id: 's5', label: 'Section 5 — the six arrows' } },
  { q: 'On which side of an aggregation/composition line does the diamond sit?',
    o: ['On the part (Player, Room)', 'On whichever side has multiplicity *', 'In the middle', 'On the whole/owner (Team, House)'], a: 3,
    ...
    w: { 0: 'Diamond-on-the-part reverses the ownership claim — the classic hand-slip.',
         1: 'Diamond side is fixed by OWNERSHIP, never by multiplicity.',
  ...
  { q: 'The multiplicity 1..* written near Player on the line Team ◇── 1..* Player means…',
    o: ['Each Team has 1 or more Players', 'Teams are optional', 'Each Player has 1 or more Teams', 'There can only be one Player ever'], a: 0,
    ...
    w: { 1: 'Optionality would be 0..1 or 0..* — 1..* demands at least one.',
         2: 'Reading it backwards (player\'s teams) is THE multiplicity trap — read the number at the far end.',
         3: 'Multiplicity counts instances per relationship, not a global object limit.' },
    r: { id: 's7', label: 'Section 7 — multiplicity' } },
  { q: 'An open arrow Student ──▶ Teacher on an association means…',
    o: ['Student creates Teacher', 'Student stores a reference to Teacher; Teacher does not know Student', 'They are the same package', 'Teacher is the parent class'], a: 1,
    ...
    w: { 0: 'Creation is a dashed «create» dependency — different arrow entirely.',
         2: 'Packages aren\'t shown on association lines.',
         3: 'Parenthood needs a hollow TRIANGLE — an open arrow is just "knows about".' },
  ...
  { q: 'In an interview, which of these should you usually LEAVE OUT of your class diagram?',
    o: ['Interface names', 'Every getter, setter and toString of every class', 'Multiplicities on important lines', 'Relationships between core classes'], a: 1,
    ...
    w: { 0: 'Interface names carry design meaning — keep them too. Only the accessor noise goes.',
         2: 'Multiplicities encode key decisions — keep them.',
         3: 'Relationships are the diagram\'s main cargo — never omit them.' },

...
const QUESTIONS = [
  { q: 'Class diagram vs sequence diagram — what is the core difference?',
    o: ['Class = structure (what exists & how it connects), sequence = behavior of ONE scenario over time', 'Sequence diagrams replace code', 'They show the same thing in different styles', 'Class = small systems, sequence = big systems'], a: 0,
    ...
    w: { 1: 'Diagrams plan and explain code — they never replace it.',
         2: 'They answer different questions: structure vs behavior-over-time.',
         3: 'System size is irrelevant — both diagrams serve systems of any size.' },
    r: { id: 's1', label: 'Section 1 — cast list vs script' } },
  { q: 'In which direction does time flow in a sequence diagram?',
    o: ['Right to left', 'Top to bottom', 'Left to right', 'Along the arrows only'], a: 1,
    e: 'Downward, always. A message lower on the page happens later. That is why you "read the scene" top to bottom.',
    w: { 0: 'Same — horizontal position is layout, not chronology.',
         2: 'Left-right is just participant placement — it carries no time meaning.',
  ...
  { q: 'What is the dashed vertical line below each participant called, and what does it mean?',
    o: ['A lock on the object', 'A return arrow drawn vertically', 'A dependency — the participant uses time', 'A lifeline — the participant exists during this period'], a: 3,
    ...
    w: { 0: 'Locks are a Month 4 concept, not sequence notation.',
         1: 'Returns are HORIZONTAL dashed arrows.',
         2: 'Dependencies are dashed arrows in CLASS diagrams — here dashed-vertical means existence.' },
    r: { id: 's2', label: 'Section 2 — lifelines & activation bars' } },
  { q: 'A solid arrow with a filled head vs a dashed arrow — which is which?',
    o: ['Solid = synchronous call, dashed = return (the reply)', 'Solid = return, dashed = call', 'Both are calls; dashed is slower', 'Dashed means static method'], a: 0,
    ...
    w: { 1: 'Exactly inverted — the most common memorization slip. Solid goes out, dashed comes home.',
  ...
  { q: 'Participants are usually written like "m: Member" and underlined. Why the colon and underline?',
    o: ['Decoration only', 'It means Member is abstract', 'It means the member is private', 'It marks an OBJECT (instance) named m of class Member — sequence diagrams star objects, not classes'], a: 3,
    ...
    w: { 0: 'Every mark in UML carries meaning — underline = instance is the same convention as object diagrams.',
         1: 'Abstract is italics, in class diagrams — and abstract types can\'t even be instantiated as participants.',
  ...
  { q: 'An arrow that loops from a lifeline back to the SAME lifeline means…',
    o: ['An infinite loop', 'A self-message: the object calls one of its own methods', 'A bug in the diagram', 'The object is being deleted'], a: 1,
    ...
    w: { 0: 'One loop-back = one call; recursion would be NESTED activation bars.',
         2: 'Perfectly standard notation, not an error.',
         3: 'Destruction is the ✕ at a lifeline\'s end.' },
    r: { id: 's3', label: 'Section 3 — self-messages' } },
  { q: 'You need to show "if paymentOk → ship, else → cancel". Which fragment frame do you use?',
    o: ['loop', 'opt', 'alt', 'ref'], a: 2,
    ...
    w: { 0: 'loop repeats; it doesn\'t choose between branches.',
         1: 'opt has ONE compartment — no else branch possible.',
  ...
  { q: 'In an interview, the best scope for one sequence diagram is…',
    o: ['Every feature of the system in one giant diagram', 'Only two objects, to keep it tiny', 'One scenario (e.g. "user books a ticket"), happy path first, 4–6 participants', 'All getters and setters included'], a: 2,
    ...
    w: { 0: 'Whole-system behavior in one sequence = unreadable spaghetti; that\'s what the class diagram is for.',
         1: 'Two objects is usually too little to show the design\'s flow.',

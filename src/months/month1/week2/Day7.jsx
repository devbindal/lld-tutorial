...
const QUESTIONS = [
  { q: 'OrderService receives a PdfWriter as a method parameter, uses it inside that one method, and never stores it. This relationship is…',
    o: ['Association', 'Aggregation', 'Dependency', 'Composition'], a: 2,
    ...
    w: { 0: 'Association requires a stored FIELD — this PdfWriter evaporates when the method returns.',
         1: 'Aggregation is whole-part with a field — neither applies here.',
  ...
  { q: 'Java\'s own java.util.Stack extends Vector. Why do experts call this a famous design mistake?',
    o: ['Vector is too slow', 'A Stack now inherits insert/remove-at-any-position methods, so code can break the "top only" rule', 'extends is deprecated', 'Stack should have extended ArrayList instead'], a: 1,
    ...
    w: { 0: 'Vector\'s speed (or slowness) is beside the point — the inherited API is the wound.',
         2: 'extends is alive and well — misusing it is the issue.',
         3: 'ArrayList would inherit the SAME rule-breaking methods — the fix is composition: Stack HAS a list.' },
  ...
  { q: 'You have 5 independent robot abilities, any mix allowed. Roughly how many classes does each approach need?',
    o: ['Inheritance: 5, Composition: 32', 'Inheritance: up to 32 subclasses, Composition: about 6 classes', 'Both need 32', 'Both need 6'], a: 1,
    ...
    w: { 0: 'Reversed — inheritance is the exploding one (2ⁿ), composition the linear one (n+1).',
         2: 'Composition never explodes: abilities are parts, combos are runtime wiring.',
         3: 'Inheritance can\'t stay at 6 — every COMBINATION must be its own class.' },
  ...
  { q: 'A subclass overrode addAll() AND add() to count insertions, but the count comes out double. The base class\'s addAll() secretly calls add() internally. This problem is called…',
    o: ['Class explosion', 'The fragile base class problem', 'Garbage collection', 'Method overloading'], a: 1,
    ...
    w: { 0: 'Class explosion is the 2ⁿ subclass-count disease — different problem.',
         2: 'GC manages memory, not double-counting.',
         3: 'Overloading is same-name-different-params — this is about internal self-calls leaking.' },
  ...
  { q: 'Which constructor gives Shop the LOOSEST coupling to its email mechanism?',
    o: ['Shop() { mail = new SmtpSender(); }', 'Shop(SmtpSender mail)', 'Shop(EmailSender mail)  // an interface', 'class Shop extends SmtpSender'], a: 2,
    ...
    w: { 0: 'new inside = welded: untestable and unswappable without editing Shop.',
         1: 'Injected but CONCRETE — better, yet still breaks when the vendor changes.',
         3: 'extends is the tightest coupling possible — and a false is-a on top.' },
    r: { id: 's3', label: 'Section 3 — the coupling thermometer' } },
  { q: 'When IS inheritance the right choice?',
    o: ['Whenever you want to reuse code from another class', 'When the subclass truly IS a kind of the parent and can be used anywhere the parent is expected', 'Never — it is deprecated', 'When you need more than one base class'], a: 1,
    ...
    w: { 0: 'Wanting code is the #1 WRONG reason — that\'s what has-a fields are for.',
         2: 'Far from deprecated — just reserved for true family relationships.',
         3: 'Java forbids multiple base classes entirely.' },

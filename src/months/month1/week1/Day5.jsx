...
const QUESTIONS = [
  { q: 'What is abstraction, in one line?',
    o: ['Copying methods between classes', 'Hiding data behind private fields', 'Making all methods static', 'Exposing WHAT something does while hiding HOW it does it'], a: 3,
    ...
    w: { 0: 'Copying is inheritance/cloning territory — unrelated.',
         1: 'That is encapsulation (Day 2) — the two are cousins, and confusing them is the classic exam trap.',
         2: 'static is ownership, not hiding.' },
    r: { id: 's1', label: 'Section 1 — what is abstraction' } },
  { q: 'Can you write: Animal a = new Animal(); if Animal is abstract?',
    o: ['Yes, abstract only affects methods', 'No — abstract classes cannot be instantiated, only extended', 'Yes, but the abstract methods return null', 'Only inside the Animal class itself'], a: 1,
    ...
    w: { 0: 'abstract on the CLASS forbids new — even with zero abstract methods.',
         2: 'Java never auto-fills missing bodies with null returns — it refuses to compile instead.',
         3: 'Not even inside itself — incompleteness doesn\'t care where the new is written.' },
    r: { id: 's2', label: 'Section 2 — abstract classes' } },
  { q: 'A class extends an abstract class but implements only some of its abstract methods. What must be true?',
    o: ['Java fills in the rest with empty bodies', 'It must itself be declared abstract', 'The missing methods throw exceptions at runtime', 'Compile error always'], a: 1,
    ...
    w: { 0: 'Java never writes bodies for you — silence is not implementation.',
         2: 'Abstract methods can\'t be reached at runtime because the incomplete class can\'t be instantiated.',
         3: 'It compiles FINE — as long as the class honestly declares itself abstract too.' },
    r: { id: 's2', label: 'Section 2 — abstract classes' } },
  { q: 'How many interfaces can one class implement, and how many classes can it extend?',
    o: ['Many interfaces, many classes', 'One interface, many classes', 'Many interfaces, one class', 'One interface, one class'], a: 2,
    ...
    w: { 0: 'Multiple class inheritance is exactly what Java banned (diamond problem with state).',
         1: 'Backwards on both counts — the memorable phrase is "one parent, many contracts".',
         3: 'Interfaces are unlimited — that\'s their superpower.' },
    r: { id: 's3', label: 'Section 3 — interfaces' } },
  { q: 'Fields declared in an interface are implicitly…',
    o: ['public static final (constants)', 'private', 'normal instance fields', 'protected'], a: 0,
    ...
    w: { 1: 'private fields would be state — interfaces have none (private METHODS exist since Java 9, fields never).',
         2: 'Instance fields are exactly what separates abstract classes FROM interfaces.',
         3: 'protected doesn\'t exist in interfaces at all.' },
    r: { id: 's3', label: 'Section 3 — interfaces' } },
  { q: 'What problem do default methods (Java 8+) in interfaces solve?',
    o: ['They replace abstract classes completely', 'They allow private fields in interfaces', 'You can add a new method to a published interface without breaking every existing implementer', 'They make interfaces faster'], a: 2,
    ...
    w: { 0: 'Abstract classes still own state + constructors; defaults narrowed the gap, didn\'t close it.',
         1: 'Still no fields — defaults bring behavior, never state.',
         3: 'Performance is untouched — this is an EVOLUTION feature.' },
    r: { id: 's5', label: 'Section 5 — default & static methods' } },
  { q: 'Two interfaces both give Duck the SAME default method sound(). What must Duck do?',
    o: ['Override sound() itself (it may call InterfaceName.super.sound() to choose one)', 'Rename one interface', 'It cannot implement both interfaces', 'Nothing, Java picks one randomly'], a: 0,
    ...
    w: { 1: 'Renaming third-party interfaces isn\'t a thing — the override-and-choose rule is.',
         2: 'Both can be implemented — Duck just has to break the tie explicitly.',
         3: 'Java NEVER picks silently — ambiguity is a compile error until you resolve it.' },
    r: { id: 's5', label: 'Section 5 — default methods & the diamond' } },
  { q: 'You need shared state (fields) and a constructor in the common parent. Which tool?',
    o: ['Either works the same', 'A static utility class', 'Interface — contracts are cleaner', 'Abstract class — interfaces cannot hold instance state or constructors'], a: 3,
    ...
    w: { 0: 'They differ on exactly this: state and constructors are abstract-class-only.',
         1: 'A utility class shares nothing per-object — it dodges the requirement instead of meeting it.',
         2: 'Cleaner, yes — but physically incapable of holding the fields you need.' },
    r: { id: 's6', label: 'Section 6 — the decision table' } },
]

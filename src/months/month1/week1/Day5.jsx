...
const QUESTIONS = [
  { q: 'What is abstraction, in one line?',
    ...
    w: { 0: 'That is encapsulation (Day 2) — the two are cousins, and confusing them is the classic exam trap.',
         2: 'Copying is inheritance/cloning territory — unrelated.',
         3: 'static is ownership, not hiding.' },
  ...
  { q: 'Can you write: Animal a = new Animal(); if Animal is abstract?',
    ...
    w: { 0: 'abstract on the CLASS forbids new — even with zero abstract methods.',
         2: 'Not even inside itself — incompleteness doesn\'t care where the new is written.',
         3: 'Java never auto-fills missing bodies with null returns — it refuses to compile instead.' },
  ...
  { q: 'A class extends an abstract class but implements only some of its abstract methods. What must be true?',
    ...
    w: { 0: 'Java never writes bodies for you — silence is not implementation.',
         2: 'It compiles FINE — as long as the class honestly declares itself abstract too.',
         3: 'Abstract methods can\'t be reached at runtime because the incomplete class can\'t be instantiated.' },
    r: { id: 's2', label: 'Section 2 — abstract classes' } },
  { q: 'How many interfaces can one class implement, and how many classes can it extend?',
    ...
    w: { 0: 'Interfaces are unlimited — that\'s their superpower.',
         1: 'Multiple class inheritance is exactly what Java banned (diamond problem with state).',
         3: 'Backwards on both counts — the memorable phrase is "one parent, many contracts".' },
  ...
  { q: 'Fields declared in an interface are implicitly…',
    ...
    w: { 0: 'private fields would be state — interfaces have none (private METHODS exist since Java 9, fields never).',
         1: 'protected doesn\'t exist in interfaces at all.',
         3: 'Instance fields are exactly what separates abstract classes FROM interfaces.' },
    r: { id: 's3', label: 'Section 3 — interfaces' } },
  { q: 'What problem do default methods (Java 8+) in interfaces solve?',
    ...
    w: { 0: 'Performance is untouched — this is an EVOLUTION feature.',
         2: 'Still no fields — defaults bring behavior, never state.',
         3: 'Abstract classes still own state + constructors; defaults narrowed the gap, didn\'t close it.' },
  ...
  { q: 'Two interfaces both give Duck the SAME default method sound(). What must Duck do?',
    ...
    w: { 0: 'Java NEVER picks silently — ambiguity is a compile error until you resolve it.',
         2: 'Both can be implemented — Duck just has to break the tie explicitly.',
         3: 'Renaming third-party interfaces isn\'t a thing — the override-and-choose rule is.' },
  ...
  { q: 'You need shared state (fields) and a constructor in the common parent. Which tool?',
    ...
    w: { 0: 'Cleaner, yes — but physically incapable of holding the fields you need.',
         2: 'They differ on exactly this: state and constructors are abstract-class-only.',
         3: 'A utility class shares nothing per-object — it dodges the requirement instead of meeting it.' },

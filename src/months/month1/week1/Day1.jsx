...
const QUESTIONS = [
  { q:'A class uses memory for its instance fields even before any object is created.',
    ...
    w:{ 0:'It feels true because the class itself IS loaded into memory — but that is the blueprint (code + static parts), not instance data. Per-object fields cost nothing until a house is actually built.' },
  ...
  { q:'Car c = new Car(); — what does the variable c actually store?',
    ...
    w:{ 0:'Natural to think, because we SAY "c is the car" — but c is a tiny variable holding only where the car lives.',
        2:'Java never copies objects on assignment — that would be expensive and surprising. Only addresses are copied.',
        3:'It is more than a name: it holds real data — the address — and can be reassigned or set to null.' },
  ...
  { q:'You wrote: public Car(String color) { ... } and nothing else. What happens to `new Car()`?',
    ...
    w:{ 0:'The free constructor exists ONLY while you have written none at all — writing one revokes the gift.',
        2:'For it to run with null, a no-arg constructor would have to exist — it does not, so this never compiles.',
        3:'Runtime exceptions need running code; this is stopped earlier, by the compiler.' },
  ...
  { q:'Inside which of these can you NOT use the keyword `this`?',
    ...
    w:{ 0:'Instance methods always run ON some object — this is exactly that object.',
        1:'A constructor is busy building the current object — this points to the half-built newcomer.',
  ...
  { q:'A field with NO access modifier written (e.g., `int speed;`) is visible to…',
    ...
    w:{ 0:'That is private — and private must be written explicitly; silence is NOT private in Java.',
        2:'That describes protected — default is strictly narrower (no subclasses outside the package).',
        3:'That is public. Default is the quiet middle level people forget exists.' },
  ...
  { q:'Car a = new Car("Red",0); Car b = a; b.color = "Black"; — what is a.color now?',
    ...
    w:{ 0:'Only true if b = a had created a second car. It did not — it photocopied the ADDRESS, so both names point at one car.',
        2:'Nothing was deleted or unset — the one shared object simply got repainted.',
  ...
  { q:'static int totalCars; — where does this variable live, and how many copies exist?',
    ...
    w:{ 0:'That is how INSTANCE fields work — static is the opposite: the keyword moves it from the objects to the class.',
        2:'The stack holds local variables of running methods — fields never live there.',
        3:'There is no per-object copy at all; every object reads the same one notice board.' },
    r:{ id:'s8', label:'Section 8 — static vs instance' } },
  { q:'Why must main be static in Java?',
    ...
    w:{ 0:'Speed has nothing to do with it — it is about WHO can be called when no objects exist yet.',
        1:'C heritage influenced the NAME, but the static is a real technical necessity in Java.',
        3:'The return type is independent — a void instance method would still need an object to run on.' },
    r:{ id:'s8', label:'Section 8 — static vs instance' } },
]

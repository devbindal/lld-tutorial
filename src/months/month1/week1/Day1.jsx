...
const QUESTIONS = [
  { q:'A class uses memory for its instance fields even before any object is created.',
    ...
    w:{ 1:'It feels true because the class itself IS loaded into memory — but that is the blueprint (code + static parts), not instance data. Per-object fields cost nothing until a house is actually built.' },
    r:{ id:'s1', label:'Section 1 — class vs object' } },
  { q:'Car c = new Car(); — what does the variable c actually store?',
    o:['The whole Car object',"Nothing, it's just a name",'The address (reference) of the object','A copy of the object'], a:2,
    e:'c lives on the stack and stores the heap address of the object. The object itself lives in the heap.',
    w:{ 0:'Natural to think, because we SAY "c is the car" — but c is a tiny variable holding only where the car lives.',
        1:'It is more than a name: it holds real data — the address — and can be reassigned or set to null.',
        3:'Java never copies objects on assignment — that would be expensive and surprising. Only addresses are copied.' },
    r:{ id:'s4', label:'Section 4 — stack vs heap' } },
  { q:'You wrote: public Car(String color) { ... } and nothing else. What happens to `new Car()`?',
    o:['Throws NullPointerException at runtime','Works fine, Java keeps the default constructor','Compile error — the free default constructor is gone','Runs with color = null'], a:2,
    ...
    w:{ 0:'Runtime exceptions need running code; this is stopped earlier, by the compiler.',
        1:'The free constructor exists ONLY while you have written none at all — writing one revokes the gift.',
        3:'For it to run with null, a no-arg constructor would have to exist — it does not, so this never compiles.' },
    r:{ id:'s5', label:'Section 5 — constructors' } },
  { q:'Inside which of these can you NOT use the keyword `this`?',
    o:['A constructor','A static method','An instance method','All of them allow `this`'], a:1,
    e:"Static methods belong to the class, not to any object, so there is no 'current object' for `this` to point to.",
    w:{ 0:'A constructor is busy building the current object — this points to the half-built newcomer.',
        2:'Instance methods always run ON some object — this is exactly that object.',
        3:'Three of them do; the static one is the odd member out — no object, no this.' },
    r:{ id:'s8', label:'Section 8 — static vs instance' } },
  { q:'A field with NO access modifier written (e.g., `int speed;`) is visible to…',
    o:['Everyone','The same package','Only the same class','Same package + all subclasses everywhere'], a:1,
    ...
    w:{ 0:'That is public. Default is the quiet middle level people forget exists.',
        2:'That is private — and private must be written explicitly; silence is NOT private in Java.',
        3:'That describes protected — default is strictly narrower (no subclasses outside the package).' },
    r:{ id:'s7', label:'Section 7 — access modifiers' } },
  { q:'Car a = new Car("Red",0); Car b = a; b.color = "Black"; — what is a.color now?',
    o:['"Black"','null','"Red"','Compile error'], a:0,
    e:'`b = a` copies the address, not the object. a and b point to the SAME object, so changing through b changes what a sees too.',
    w:{ 1:'Nothing was deleted or unset — the one shared object simply got repainted.',
        2:'Only true if b = a had created a second car. It did not — it photocopied the ADDRESS, so both names point at one car.',
        3:'Perfectly legal Java — which is exactly what makes this bug so common.' },
    r:{ id:'s9', label:'Section 9 — Trap 1: two references, one object' } },
  { q:'static int totalCars; — where does this variable live, and how many copies exist?',
    o:['On the stack; one per method call','In every object; one copy per object','Two copies: one per object plus one on the class','On the class; exactly one shared copy'], a:3,
    e:'Static members are stored once with the class itself and shared by all objects — the society notice board.',
    w:{ 0:'The stack holds local variables of running methods — fields never live there.',
        1:'That is how INSTANCE fields work — static is the opposite: the keyword moves it from the objects to the class.',
        2:'There is no per-object copy at all; every object reads the same one notice board.' },
    r:{ id:'s8', label:'Section 8 — static vs instance' } },
  { q:'Why must main be static in Java?',
    o:['JVM must call it before any object exists, so it cannot depend on an object','Tradition from C language only','Because main returns void','Because static methods run faster'], a:0,
    e:'At program start, zero objects exist. The JVM needs an entry point it can call directly on the class — hence static.',
    w:{ 1:'C heritage influenced the NAME, but the static is a real technical necessity in Java.',
        2:'The return type is independent — a void instance method would still need an object to run on.',
        3:'Speed has nothing to do with it — it is about WHO can be called when no objects exist yet.' },
    r:{ id:'s8', label:'Section 8 — static vs instance' } },
]

...
const QUESTIONS = [
  { q: 'Inheritance models which relationship?',
    ...
    w: { 0: 'has-a is composition — a Car HAS an engine; it is not a kind of engine.',
         2: 'uses-a is dependency (Day 7) — borrowed briefly, not a family bond.',
         3: 'made-of suggests composition too — parts, not kinds.' },
  ...
  { q: 'Which members of a parent are NOT inherited by a child?',
    ...
    w: { 0: 'public members are exactly what inheritance hands down.',
         1: 'protected exists precisely TO be visible to children.',
  ...
  { q: 'To override a method correctly, the child method must…',
    ...
    w: { 0: 'A different name is just a new method — nothing gets replaced.',
         2: 'static methods are HIDDEN, never overridden (no dynamic dispatch).',
         3: 'The return type must match (or be a subtype — covariant); changing it freely breaks the contract.' },
  ...
  { q: 'Inside Dog, what does super.makeSound() do?',
    ...
    w: { 0: 'That would be this.makeSound() inside itself — super explicitly escapes upward.',
         2: 'super never allocates — it reaches into the SAME object\'s parent layer.',
  ...
  { q: 'When you write new Dog(), in what order do the constructor BODIES finish running?',
    ...
    w: { 0: 'You CALLED Dog first, but its first act is climbing up — bodies complete top-down.',
         2: 'Constructors run strictly sequentially, never in parallel.',
         3: 'Every ancestor constructor runs — the parent layers must exist before the child decorates them.' },
  ...
  { q: 'You write a constructor in Animal that takes arguments, and Dog’s constructor has no super(...) call. What happens?',
    ...
    w: { 0: 'Java never invents argument values — it only inserts an empty super().',
  ...
  { q: 'Every class in Java ultimately extends which class?',
    ...
    w: { 0: 'Main is just your class with the entry method — nothing special in the type system.',
         1: 'System is a utility class (System.out) — not a root.',
         3: 'The extends Object is invisible but always there — that\'s why every object has toString().' },
  ...
  { q: 'Why are very deep inheritance hierarchies discouraged in LLD?',
    ...
    w: { 0: 'Dispatch cost is negligible — the price is paid in CHANGE, not speed.',
         2: 'No such limit exists — which is why 7-level towers get built and regretted.',

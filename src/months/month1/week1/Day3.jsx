...
const QUESTIONS = [
  { q: 'Inheritance models which relationship?',
    o: ['uses-a', 'made-of', 'is-a', 'has-a'], a: 2,
    ...
    w: { 0: 'uses-a is dependency (Day 7) — borrowed briefly, not a family bond.',
         1: 'made-of suggests composition too — parts, not kinds.',
         3: 'has-a is composition — a Car HAS an engine; it is not a kind of engine.' },
    r: { id: 's1', label: 'Section 1 — the is-a idea' } },
  { q: 'Which members of a parent are NOT inherited by a child?',
    o: ['public methods', 'private members and constructors', 'protected fields', 'static fields'], a: 1,
    ...
    w: { 0: 'public members are exactly what inheritance hands down.',
         2: 'protected exists precisely TO be visible to children.',
         3: 'static members are accessible through the child (though shared, not copied).' },
    r: { id: 's2', label: 'Section 2 — what gets inherited' } },
  { q: 'To override a method correctly, the child method must…',
    o: ['have the SAME name and parameter list as the parent method', 'return a different type', 'be static', 'have a different name'], a: 0,
    ...
    w: { 1: 'The return type must match (or be a subtype — covariant); changing it freely breaks the contract.',
         2: 'static methods are HIDDEN, never overridden (no dynamic dispatch).',
         3: 'A different name is just a new method — nothing gets replaced.' },
    r: { id: 's3', label: 'Section 3 — method overriding' } },
  { q: 'Inside Dog, what does super.makeSound() do?',
    o: ['Creates a new Animal object', 'Calls the parent Animal’s version of makeSound', 'Calls Dog’s own makeSound again (infinite loop)', 'Is a compile error'], a: 1,
    e: 'super.x() runs the PARENT’s version. It lets a child add to the parent’s behavior instead of fully replacing it.',
    w: { 0: 'super never allocates — it reaches into the SAME object\'s parent layer.',
         2: 'That would be this.makeSound() inside itself — super explicitly escapes upward.',
         3: 'Fully legal — and the standard way to extend (not replace) parent behavior.' },
    r: { id: 's4', label: 'Section 4 — the super keyword' } },
  { q: 'When you write new Dog(), in what order do the constructor BODIES finish running?',
    o: ['Only Dog runs', 'All at the same time', 'Dog, then Animal, then Object', 'Object, then Animal, then Dog'], a: 3,
    ...
    w: { 0: 'Every ancestor constructor runs — the parent layers must exist before the child decorates them.',
         1: 'Constructors run strictly sequentially, never in parallel.',
         2: 'You CALLED Dog first, but its first act is climbing up — bodies complete top-down.' },
    r: { id: 's5', label: 'Section 5 — constructor chaining' } },
  { q: 'You write a constructor in Animal that takes arguments, and Dog’s constructor has no super(...) call. What happens?',
    o: ['Compile error — Java tries to call super() with no args, which doesn’t exist', 'It works — Java guesses the arguments', 'Dog skips Animal entirely', 'Animal’s fields stay null forever'], a: 0,
    ...
    w: { 1: 'Java never invents argument values — it only inserts an empty super().',
         2: 'Skipping the parent is impossible — every child construction passes through it.',
         3: 'It never compiles, so no runtime state exists at all.' },
    r: { id: 's5', label: 'Section 5 — constructor chaining' } },
  { q: 'Every class in Java ultimately extends which class?',
    o: ['nothing — top classes extend nothing', 'Main', 'System', 'Object'], a: 3,
    ...
    w: { 0: 'The extends Object is invisible but always there — that\'s why every object has toString().',
         1: 'Main is just your class with the entry method — nothing special in the type system.',
         2: 'System is a utility class (System.out) — not a root.' },
    r: { id: 's6', label: 'Section 6 — the Object class' } },
  { q: 'Why are very deep inheritance hierarchies discouraged in LLD?',
    o: ['Java limits depth to 3', 'They run slower', 'They are tightly coupled and fragile — a change high up can break many children (the fragile base class problem)', 'They use too much memory'], a: 2,
    ...
    w: { 0: 'No such limit exists — which is why 7-level towers get built and regretted.',
         1: 'Dispatch cost is negligible — the price is paid in CHANGE, not speed.',
         3: 'Memory is irrelevant; fragility and coupling are the costs.' },
    r: { id: 's7', label: 'Section 7 — when NOT to inherit' } },
]

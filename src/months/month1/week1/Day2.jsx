...
const QUESTIONS = [
  { q: 'What is encapsulation, in one line?',
    ...
    w: { 0: 'That is just file organization — encapsulation is about hiding STATE, not grouping classes.',
         2: 'That is copying/cloning (Day 25\'s topic) — unrelated to protection.',
         3: 'static changes ownership (class vs object), not visibility or safety.' },
  ...
  { q: 'Why are public fields considered bad in LLD?',
    ...
    w: { 0: 'Visibility costs zero memory — the danger is about ACCESS, not size.',
         2: 'Field access speed is identical regardless of modifier — performance is a red herring here.',
         3: 'Java happily allows them — which is exactly why the discipline must come from you.' },
  ...
  { q: 'A setter with no validation inside it (just this.x = x;) is…',
    ...
    w: { 0: 'Too extreme — a bare setter at least gives you a single place to ADD the guard later.',
         2: 'It is a method call, so if anything marginally slower — speed was never the point.',
         3: 'Java requires no setters at all; JavaBeans CONVENTION expects them, which is different.' },
  ...
  { q: 'You write: private final List<String> items = new ArrayList<>(); Can the list still be changed?',
    ...
    w: { 0: 'The #1 final misconception: final freezes the ARROW (reference), never the box it points at.',
         ...
         3: 'Perfectly legal Java — initializing a final field at declaration is standard.' },
  ...
  { q: 'A getter returns the object’s real internal list directly. What is the risk?',
    ...
    w: { 0: 'It IS efficient — and that efficiency hands your private organs to strangers. Wrong trade.',
         2: 'No exception fires; the danger is silent mutation later, far away.',
  ...
  { q: 'Which set of rules makes a class immutable?',
    ...
    w: { 0: 'static methods remove the object entirely — that is utility design, not immutability.',
         2: 'public final fields still leak mutable objects (a final List can be cleared by anyone).',
         3: 'A copy constructor copies; it does nothing to stop mutation of the original.' },
    r: { id: 's6', label: 'Section 6 — immutability & final' } },
  { q: 'Why is an immutable object easy to use across threads?',
    ...
    w: { 0: 'Size is unchanged — safety comes from absence of WRITES, not bytes.',
         2: 'Immutability and static are unrelated dimensions.',
         3: 'No locking happens or is needed — that is the beauty: nothing to lock.' },
    r: { id: 's6', label: 'Section 6 — immutability & final' } },
  { q: 'Best practice for object fields in everyday LLD?',
    ...
    w: { 0: 'Public fields = no guards — the day\'s central crime.',
         2: 'protected leaks internals to every future subclass — use sparingly, never as default.',
         3: 'Avoiding objects defeats OOP entirely; static state is shared global state (Day 17).' },

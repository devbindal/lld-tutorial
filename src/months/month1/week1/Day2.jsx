...
const QUESTIONS = [
  { q: 'What is encapsulation, in one line?',
    o: ['Hiding the internal data and exposing only safe methods', 'Copying one object into another', 'Making everything static', 'Putting many classes in one file'], a: 0,
    ...
    w: { 1: 'That is copying/cloning (Day 25\'s topic) — unrelated to protection.',
         2: 'static changes ownership (class vs object), not visibility or safety.',
         3: 'That is just file organization — encapsulation is about hiding STATE, not grouping classes.' },
    r: { id: 's1', label: 'Section 1 — what is encapsulation' } },
  { q: 'Why are public fields considered bad in LLD?',
    o: ['They are slower to read', 'They use more memory', 'Java forbids them', 'Anyone can write any value directly, so the object can be corrupted with no validation'], a: 3,
    ...
    w: { 0: 'Field access speed is identical regardless of modifier — performance is a red herring here.',
         1: 'Visibility costs zero memory — the danger is about ACCESS, not size.',
         2: 'Java happily allows them — which is exactly why the discipline must come from you.' },
    r: { id: 's2', label: 'Section 2 — why public fields are dangerous' } },
  { q: 'A setter with no validation inside it (just this.x = x;) is…',
    o: ['Required by Java', 'Basically a public field with extra typing — it adds little protection', 'Always wrong — never write one', 'Faster than a public field'], a: 1,
    ...
    w: { 0: 'Java requires no setters at all; JavaBeans CONVENTION expects them, which is different.',
         2: 'Too extreme — a bare setter at least gives you a single place to ADD the guard later.',
         3: 'It is a method call, so if anything marginally slower — speed was never the point.' },
    r: { id: 's4', label: 'Section 4 — getters and setters' } },
  { q: 'You write: private final List<String> items = new ArrayList<>(); Can the list still be changed?',
    o: ['No, final makes the list read-only', 'Java will not compile this', 'Only if it is static', 'Yes — final only stops items from pointing to a NEW list; the existing list can still be added to / cleared'], a: 3,
    ...
    w: { 0: 'The #1 final misconception: final freezes the ARROW (reference), never the box it points at.',
         1: 'Perfectly legal Java — initializing a final field at declaration is standard.',
         2: 'static is about ownership, irrelevant to mutability.' },
    r: { id: 's6', label: 'Section 6 — immutability & final' } },
  { q: 'A getter returns the object’s real internal list directly. What is the risk?',
    o: ['It throws a NullPointerException', 'The caller now holds a live reference and can mutate your private data from outside', 'None, it is efficient', 'The list becomes static'], a: 1,
    ...
    w: { 0: 'No exception fires; the danger is silent mutation later, far away.',
         2: 'It IS efficient — and that efficiency hands your private organs to strangers. Wrong trade.',
         3: 'Returning a reference changes nothing about static-ness.' },
    r: { id: 's7', label: 'Section 7 — defensive copying' } },
  { q: 'Which set of rules makes a class immutable?',
    o: ['Add a copy constructor', 'Make every field public final', 'final class, all fields private final, no setters, set everything in the constructor, and defensively copy any mutable field in and out', 'Mark every method static'], a: 2,
    ...
    w: { 0: 'A copy constructor copies; it does nothing to stop mutation of the original.',
         1: 'public final fields still leak mutable objects (a final List can be cleared by anyone).',
         3: 'static methods remove the object entirely — that is utility design, not immutability.' },
    r: { id: 's6', label: 'Section 6 — immutability & final' } },
  { q: 'Why is an immutable object easy to use across threads?',
    o: ['The JVM locks it automatically', 'It is smaller', 'Its state never changes after creation, so there is nothing to corrupt by two threads at once', 'It is always static'], a: 2,
    ...
    w: { 0: 'No locking happens or is needed — that is the beauty: nothing to lock.',
         1: 'Size is unchanged — safety comes from absence of WRITES, not bytes.',
         3: 'Immutability and static are unrelated dimensions.' },
    r: { id: 's6', label: 'Section 6 — immutability & final' } },
  { q: 'Best practice for object fields in everyday LLD?',
    o: ['private fields, and public methods only for behavior outsiders truly need', 'static fields to avoid making objects', 'public fields, public methods', 'protected fields so subclasses can reach them'], a: 0,
    ...
    w: { 1: 'Avoiding objects defeats OOP entirely; static state is shared global state (Day 17).',
         2: 'Public fields = no guards — the day\'s central crime.',
         3: 'protected leaks internals to every future subclass — use sparingly, never as default.' },
    r: { id: 's9', label: 'Section 9 — the mindset' } },
]

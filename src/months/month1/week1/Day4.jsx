...
const QUESTIONS = [
  { q: 'Polymorphism literally means…',
    o: ['many objects', 'many classes', 'many constructors', 'many forms — one name, many behaviors'], a: 3,
    e: 'Poly = many, morph = form. One method name (makeSound) takes many forms (Woof, Meow, Quack) depending on who answers.',
    w: { 0: 'Many objects of one class all behave identically — that\'s not it either.',
         1: 'You can have many classes with no polymorphism at all — the magic is one NAME, many behaviors.',
         2: 'Many constructors is overloading-of-constructors — a sliver of the idea, not its meaning.' },
    r: { id: 's1', label: 'Section 1 — what is polymorphism' } },
  { q: 'Overloading is resolved at ____ ; overriding is resolved at ____.',
    o: ['runtime / compile time', 'compile time / compile time', 'compile time / runtime', 'runtime / runtime'], a: 2,
    ...
    w: { 0: 'Exactly backwards — the most common way to misremember it. Compile/run, in that order.',
         1: 'If overriding were compile-time, Animal a = new Dog() could never bark.',
         3: 'Overload choice is baked into the bytecode before the program ever runs.' },
    r: { id: 's2', label: 'Section 2 — overloading (compile-time)' } },
  { q: 'Animal a = new Dog(); a.makeSound(); — whose makeSound runs?',
    o: ['Dog’s, because the heap object is a Dog', 'Animal’s, because the reference is Animal', 'Compile error', 'Both run'], a: 0,
    ...
    w: { 1: 'The reference type is just the lens — the OBJECT behind it answers the call.',
         2: 'Perfectly legal — upcasting plus an inherited method is everyday Java.',
         3: 'One call, one winner — unless Dog\'s version itself calls super.makeSound().' },
    r: { id: 's4', label: 'Section 4 — dynamic dispatch' } },
  { q: 'Animal a = new Dog(); a.fetch(); — fetch() exists only in Dog. What happens?',
    o: ['Runs fine, the object is a Dog', 'Runtime exception', 'Runs Animal’s fetch', 'Compile error — the Animal reference only “sees” Animal’s methods'], a: 3,
    ...
    w: { 0: 'The object COULD fetch — but the compiler only trusts the reference\'s menu, and Animal\'s menu has no fetch.',
         1: 'It never gets far enough to run — this dies at compile time.',
         2: 'Animal has no fetch at all — that\'s the entire problem.' },
    r: { id: 's3', label: 'Section 3 — upcasting' } },
  { q: 'What does the JVM use under the hood to find the right overridden method at runtime?',
    o: ['The garbage collector', 'A virtual method table (vtable) attached to each class', 'Reflection on every call', 'It scans all classes alphabetically'], a: 1,
    ...
    w: { 0: 'The GC manages memory, not method calls.',
         2: 'Reflection is the slow, explicit API — normal dispatch never touches it.',
         3: 'No searching happens — the slot index is precomputed; dispatch is one array jump.' },
  ...
  { q: 'Which call CANNOT be polymorphic (no dynamic dispatch)?',
    o: ['An interface method', 'A static method', 'A protected overridden method', 'A public overridden method'], a: 1,
    ...
    w: { 0: 'Interface methods are maximally polymorphic — any implementer can answer.',
         2: 'Visibility doesn\'t matter — protected overrides dispatch like public ones.',
         3: 'public overridden methods are the TEXTBOOK case of dispatch.' },
    r: { id: 's5', label: 'Section 5 — vtables (and what skips them)' } },
  { q: 'Why is "Animal a = new Dog()" (upcasting) so common in good LLD?',
    o: ['Code written against the general type works for every current and future subclass', 'It makes methods run faster', 'It saves memory', 'Java requires it'], a: 0,
    ...
    w: { 1: 'Speed is unchanged; FLEXIBILITY is the prize.',
         2: 'The object is identical either way — only the lens changes.',

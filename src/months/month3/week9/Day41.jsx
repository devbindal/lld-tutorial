...
const QUESTIONS = [
  { q: 'The FIRST thing you do when the interviewer says "design a parking lot"?',
    o: ['Recite patterns', 'Write the Vehicle class', 'Start drawing classes', 'Ask 4–6 design-shaping clarifying questions (floors? vehicle types? pricing? gates? displays?) and agree scope OUT LOUD — including what is out of scope'], a: 3,
    ...
    w: { 0: 'Pattern recitation without a problem is noise.',
         1: 'Whose vehicles? Carrying what types? You don\'t know yet.',
         2: 'Classes drawn before scope = classes redrawn at minute 20.' },
    r: { id: 's2', label: 'Section 2 — the ritual' } },
  { q: 'Bike/Car/Truck with no differing behavior — model them as…',
    o: ['One Vehicle class with enum VehicleType — subclass only when behavior diverges; the enum drives spot-compatibility data', 'Three subclasses of Vehicle', 'String type field', 'Three unrelated classes'], a: 0,
    ...
    w: { 1: 'Subclasses earn their files only with real behavioral differences.',
         2: 'Day 19 called this the typo machine.',
         3: 'Unrelated classes can\'t share Ticket/Spot handling.' },
    r: { id: 's5', label: 'Section 5 — debate #1' } },
  { q: 'Spot allocation policy belongs in…',
    o: ['The database', 'A big if-else inside ParkingLot.park()', 'An injected SpotAllocationStrategy interface — NearestFirst now; future policies (EV-near-chargers, handicapped-priority) become new classes, lot untouched', 'The Vehicle class'], a: 2,
    ...
    w: { 0: 'Storage stores; policy decides.',
         1: 'Every policy tweak reopens the tested lot.',
         3: 'Vehicles choosing spots = feature envy of lot data (Day 18).' },
    r: { id: 's5', label: 'Section 5 — debate #3' } },
  { q: 'The Lot→Floor→Spot relationship and the "247 free spots" rollup are…',
    o: ['Composition (◆ — floors die with the lot, Day 6) shaped like Day 28\'s Composite: availableCount() on the lot sums floors, floors sum spots — recursion by structure', 'Inheritance', 'A flat global list', 'Three unrelated classes'], a: 0,
    ...
    w: { 1: 'A floor is not a kind of lot (Day 3\'s is-a test).',
         2: 'Flat lists lose per-floor boards and future per-floor rules.',
         3: 'They form the problem\'s backbone hierarchy.' },
  ...
  { q: 'Floor display boards should update via…',
    o: ['Polling the lot every second', 'Observer: the lot publishes SpotOccupied/SpotFreed; boards (and later the mobile app) subscribe — the lot never knows its audience', 'Manual refresh', 'Spots calling board.update() directly'], a: 1,
    ...
    w: { 0: '86,400 wasted checks per day per board.',
         2: 'It\'s 2026. 🙂',
         3: 'Parking logic importing display classes inverts DIP the wrong way.' },
    r: { id: 's5', label: 'Section 5 — debate #5' } },
  { q: 'The Ticket should record…',
    o: ['The fee, precomputed at entry', 'Nothing; tickets are decorative', 'Only the vehicle number', 'An id, the vehicle, the assigned spot, and an IMMUTABLE entryTime — the fee computation (Part 2) needs exactly this data, making Ticket its natural home (Information Expert)'], a: 3,
    ...
    w: { 0: 'You\'d be billing the future.',
         1: 'The ticket IS the parking session\'s identity.',
         2: 'Then exit can\'t find the spot or compute duration.' },
  ...
  { q: 'Two entry gates, one remaining spot, two cars at the same instant — your interview answer?',
    o: ['Cannot happen', 'First gate alphabetically wins', 'Flag it: spot assignment must be ATOMIC (synchronized allocation or a tryOccupy compare-and-set on the spot); naming the race and a fix is expected even before Month 4\'s deep dive', 'Add more spots'], a: 2,
    ...
    w: { 0: 'It happens every rush hour.',
         1: 'The scheduler doesn\'t alphabetize. 🙂',

...
const QUESTIONS = [
  { q: 'The drill clock starts. Minutes 0–5 are for…',
    o: ['Clarifying questions + scope said out loud — including exclusions and rule variants; the questions are graded, and their answers shape everything after', 'Setting up the project and IDE', 'Typing the entity classes while ideas are fresh', 'Drawing full UML for every class'], a: 0,
    ...
    w: { 1: 'Project setup happens before the timer starts — that\'s part of drill discipline.',
         2: 'Entities before scope = entities for the wrong problem.',
  ...
  { q: 'Connect Four: win = 4 in a row on a 6×7 board. Your Day 43 tally scoreboard…',
    o: ['Works exactly as-is', 'Just needs bigger arrays', 'Is INVALID here — the win length (4) is SHORTER than the lines, so tallies count non-consecutive pieces; you need the directional scan: 4 directions from the landed cell, walk ≤3 steps each way, count the run', 'Only fails on the diagonals'], a: 2,
    ...
    w: { 0: 'Four scattered pieces in one row would be declared a win.',
         1: 'Array size was never the issue — consecutiveness is.',
  ...
  { q: 'In the bike-rental track, the fee is computed…',
    o: ['At rent-out, stored on the ticket', 'By a static PriceUtil', 'Inside the Bike class', 'At RETURN, by an injected PricingStrategy reading the ticket\'s immutable start time — the parking-lot shape (resource + ticket + exit-time pricing), re-skinned'], a: 3,
    ...
    w: { 0: 'The duration doesn\'t exist yet — Day 42\'s fee-at-entry trap, relocated.',
         1: 'Static utils are unswappable — weekend rates would mean editing it (Day 12).',
  ...
  { q: 'Drill rule: "compile every ~8 minutes." Which build order makes that painless?',
    o: ['Order doesn\'t matter with a good IDE', 'Bottom-up: bricks first (enums, records, Board) — every layer only calls code that already exists, so the project is green at every checkpoint', 'The biggest class first, to de-risk it', 'Top-down from main()'], a: 1,
    ...
    w: { 0: 'The IDE finds errors; the order determines whether there\'s anything runnable to check.',
         2: 'The biggest class depends on everything; it compiles LAST by nature.',
         3: 'main() wires classes that don\'t exist yet — you\'d stub for ten minutes.' },
    r: { id: 's1', label: 'Section 1 — the protocol' } },
  { q: 'You scored 10/15, all misses in Extensibility. The review loop says…',
    o: ['Restudy ONLY the flagged material (Day 42\'s seams and composition root), then re-drill the OTHER track at full length', 'Redo all of Weeks 1–9', 'Lower the rubric to 10 items', 'Move on to Week 10 — passing score'], a: 0,
    ...
    w: { 1: 'Untargeted re-study is how drills turn into rereading — train the gap, not the syllabus.',
         2: 'Adjusting the measuring stick is the one move that guarantees no improvement. 🙂',
         3: '9–12 means the gap is real; carrying it into Week 10 compounds it.' },
    r: { id: 's7', label: 'Section 7 — the review loop' } },
  { q: 'Which of these clarifying questions is the clock-burner?',
    o: ['"Is pricing hourly, daily, or both?"', '"What color is the rental ticket printed in?" — zero design impact; Day 41\'s lesson at drill speed', '"One shop, or multiple branches someday?"', '"Do electric bikes price differently?"'], a: 1,
    ...
    w: { 0: 'That answer shapes the entire PricingStrategy family.',
         2: 'That answer decides whether a composition root or a lot/shop registry is needed.',
         3: 'That question creates the surcharge-wrapper decision — pure design fuel.' },
    r: { id: 's4', label: 'Section 4 — Track A' } },
  { q: 'Why talk out loud even when drilling alone at your desk?',
    o: ['To memorize a fixed script word-for-word', 'No reason — it\'s embarrassing', 'Narration is a separate muscle: citing the principle AS you decide ("strategy here, pricing varies") must be automatic by interview day — silent reps train silent interviews', 'Because interviewers can\'t read code'], a: 2,
    ...
    w: { 0: 'Not a script — a habit of attaching the WHY to each decision, in your own words.',
         1: 'Mild embarrassment now, or silence in the round that counts.',
  ...
  { q: 'public char[][] getGrid() { return grid; } — the smell, and where you learned it?',
    o: ['A naming-convention issue', 'A missing null check', 'A performance problem', 'An aliasing leak (Days 2 & 43): the caller holds the real array and can write cells past every guard — expose at(r,c) or a defensive copy'], a: 3,
    ...
    w: { 0: 'Rename it anything; the leak remains.',
         1: 'grid is never null here; the danger is what callers can DO with it.',
         2: 'Returning a reference is fast — that was never the issue.' },

...
const QUESTIONS = [
  { q: 'Snake and Ladder: two subclasses of an abstract Jumper with polymorphic apply()?',
    o: ['Yes — model the real-world hierarchy', 'No — they have IDENTICAL behavior (move from → to); they differ only in data (direction). One Jumper record + one Map<from,to>; subclass only when behavior diverges (Day 41\'s rule, again)', 'Yes, plus a JumperFactory', 'Use inheritance: Snake extends Ladder'], a: 1,
    ...
    w: { 0: 'Real-world nouns are candidates, not commandments — behavior decides.',
         2: 'A factory for two data rows is pattern-itis (Day 22\'s warning).',
         3: 'A snake is not a kind of ladder by any test, least of all Day 3\'s is-a.' },
    r: { id: 's3', label: 'Section 3 — the jumper insight' } },
  { q: 'How should dice be modeled?',
    o: ['Math.random() inline in the game loop', 'A Dice interface with roll(); FairDice takes an INJECTED Random — and ScriptedDice replays a fixed sequence, making a whole game a deterministic unit test (Day 15\'s Random boundary)', 'A static utility: DiceUtil.roll()', 'An enum of all six faces'], a: 1,
    ...
    w: { 0: 'Untestable: every test run plays a different game.',
         2: 'A static call is just inline randomness wearing a tie — still unswappable, untestable.',
  ...
  { q: 'Player on 98 rolls a 4 (board of 100). What happens?',
    o: ['Move to 100 and win', 'Depends on a rule you must ASK about: classic "exact landing" = stay at 98 (or bounce to 98); the answer changes endgames — and whichever it is, the rule lives in ONE place in the move pipeline', 'Move to 102', 'The roll is re-thrown'], a: 1,
    ...
    w: { 0: 'That\'s a third variant (first-past-the-post) — fine, IF agreed out loud.',
         2: 'Position 102 on a 100-cell board is the bug version, not a variant.',
         3: 'No standard variant re-rolls — and inventing rules silently is the real error.' },
  ...
  { q: 'A ladder drops you on a snake\'s head. Your resolve() follows it. What MUST the code have?',
    o: ['Nothing special — while loops are fine', 'A cycle guard — best as a constructor-time validation that walks every chain once, so a broken board fails at BUILD time instead of hanging a live game mid-move', 'A recursion depth of 3', 'Thread.sleep between hops'], a: 1,
    ...
    w: { 0: 'while (jumps.containsKey(cell)) + a cyclic config = a game that never returns.',
         ...
         3: '🙂 Sleeping through an infinite loop just makes it a slow infinite loop.' },
    r: { id: 's3', label: 'Section 3 — validate at construction' } },
  { q: 'Between tic-tac-toe and snake & ladder, the game-loop skeleton changes…',
    o: ['Completely — different games, different loops', 'In exactly two slots: HOW the move is obtained (chosen cell vs forced dice roll) and WHAT the rules do (guards+tally vs overshoot+jumpers) — the queue, status enum and rotation are IDENTICAL', 'Only the variable names', 'Snake & ladder needs no loop'], a: 1,
    ...
    w: { 0: 'Run the two loops side by side — four of the six lines match word for word.',
         2: 'Two slots genuinely differ in KIND: input source and rule set.',
  ...
  { q: 'playTurn() returns a TurnResult record instead of printing the move. Why?',
    o: ['Records are fashionable', 'The Game produces FACTS (who, rolled what, from, to); rendering them is the runner\'s job — so the same Game drives a console, a GUI, or a JUnit assert with zero changes (Day 11)', 'Printing is slow', 'So the result can be modified later'], a: 1,
    ...
    w: { 0: 'The shape (record) is taste; the separation (facts vs presentation) is the design point.',
         2: 'Speed is irrelevant; coupling is the cost.',
         3: 'Records are immutable — and a result that can be "adjusted" later is a lie waiting to happen.' },
  ...
  { q: 'Two jumpers configured to start on cell 40, or a ladder starting on cell 100 — where is this caught?',
    o: ['In resolve(), during the game', 'In the Board CONSTRUCTOR: reject duplicate starts, jumpers on the final cell, and from==to before the board ever exists — an object that constructs successfully is a VALID board (Day 20: parse, don\'t validate)', 'By the dice', 'It cannot be caught'], a: 1,
    ...
    w: { 0: 'Mid-game is the worst possible moment to learn your setup was broken.',
         2: 'Dice produce numbers; they know nothing of boards.',
         3: 'Every rule listed is a cheap constructor check.' },
    r: { id: 's3', label: 'Section 3 — validate at construction' } },
  { q: '"Rolling a 6 grants another turn." Where does that rule live?',
    o: ['Inside LoadedDice', 'In the TURN policy of the loop — after applying the move, skip the queue rotation when the roll was 6; dice produce numbers, they do not know about turns', 'Inside Player', 'Inside the Board'], a: 1,
    ...
    w: { 0: 'A die that grants turns has absorbed a game rule — now every game with that die has the rule.',
         2: 'Players don\'t control turn order; the queue does.',

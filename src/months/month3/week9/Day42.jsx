...
const QUESTIONS = [
  { q: 'Why can the fee only be computed at the EXIT gate?',
    o: ['Entry gates are too busy to do math', 'The fee depends on DURATION, unknown until exit — so price(ticket, exitTime) runs at exit, fed by the immutable entryTime the Ticket has carried since minute zero', 'Java cannot subtract timestamps', 'Because payment was scoped out in Part 1'], a: 1,
    ...
    w: { 0: 'It is not about workload — entry-time pricing is logically impossible, not slow.',
         2: 'Duration.between(entry, exit) does exactly this.',
         3: 'Scoping out payment skipped the card machine, not the arithmetic.' },
    r: { id: 's5', label: 'Section 5 — pricing is an exit-time question' } },
  { q: 'The fee should be represented as…',
    o: ['double — fast and easy', 'Money (BigDecimal + currency — Day 20\'s value object): exact decimal math, explicit rounding, and no mixing ₹ with $ by accident', 'float', 'String, formatted nicely'], a: 1,
    ...
    w: { 0: '0.1 + 0.2 != 0.3 in double — the Mars-orbiter lesson at parking-lot scale.',
         2: 'float is double\'s less precise sibling — worse, not better.',
         3: 'Strings can be displayed but not added, compared, or rounded safely.' },
  ...
  { q: 'Minute 40: "weekend rates are double." Your move?',
    o: ['Add if(weekend) inside ParkingLot', 'A new PricingStrategy (e.g. WeekendAwarePricing wrapping the weekday one) + one changed line in the composition root — lot and gates stay closed (Day 12)', 'Edit ExitGate with a calendar check', 'Recompute already-issued receipts'], a: 1,
    ...
    w: { 0: 'The lot does not price; opening it for a price change breaks the separation that earned points.',
         2: 'Same leak, different room — gates stay pricing-ignorant.',
         3: 'Old receipts are history; policies apply at exit time.' },
    r: { id: 's8', label: 'Section 8 — one line in main()' } },
  { q: 'EV charging cost on top of normal parking — best structure?',
    o: ['Copy HourlyPricing into EvHourlyPricing and edit the copy', 'A surcharge strategy that WRAPS any base PricingStrategy and adds energy cost — composition (Day 27\'s move) keeps hourly logic in exactly one place', 'An isEv boolean inside HourlyPricing', 'Charge the energy fee at entry'], a: 1,
    ...
    w: { 0: 'Two copies of ceilHours() = the next bug is fixed in only one of them.',
         2: 'Today a boolean, next month three booleans — the flag-monster from Day 27.',
         3: 'Energy used is unknown at entry, just like duration.' },
    r: { id: 's5', label: 'Section 5 — the EV wrapper' } },
  { q: 'Why do the lot and gates take a Clock in the constructor?',
    o: ['Java requires a Clock for Instant.now()', 'So tests can use a FIXED clock: park at t₀, "exit" at t₀ + 150 min, assert ₹60 — no Thread.sleep, no flaky tests (Day 15\'s Clock boundary)', 'Performance — system calls are slow', 'To keep the two gates\' clocks synchronized'], a: 1,
    ...
    w: { 0: 'Instant.now() works fine without one — that is exactly the untestable trap.',
         2: 'Irrelevant at gate speeds; testability is the real win.',
         3: 'A nice side-effect, not the reason.' },
    r: { id: 's3', label: 'Section 3 — the lot core' } },
  { q: 'In the exit flow, why compute the fee BEFORE spot.free(), and close the ticket LAST?',
    o: ['The order is arbitrary', 'Free the spot first and a new car can claim it while payment is still settling; close the ticket last so a double-scanned ticket THROWS instead of billing twice', 'The fee formula needs the Spot object', 'Observers require it'], a: 1,
    ...
    w: { 0: 'Reorder it and you get phantom double-parking or double-billing — very not arbitrary.',
         2: 'The fee needs entryTime, exit time and vehicle type — all on the Ticket.',
         3: 'Observers just hear spotFreed whenever it happens.' },

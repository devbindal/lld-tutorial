...
const QUESTIONS = [
  { q: 'The classic beginner win-check is 8 hardcoded if-conditions. The REAL problem with it?',
    o: ['Too slow for 3×3', 'It is welded to 3×3 — the first follow-up ("make it N×N") forces a rewrite; loops keyed on board size absorb the same follow-up with ZERO new lines', 'It uses too much memory', 'Java forbids long boolean expressions'], a: 1,
    ...
    w: { 0: 'At 9 cells, even the ugliest check is instantaneous — performance is not the lesson here.',
         2: 'It uses less memory than any alternative; memory is not the cost.',
         3: 'It compiles fine — the compiler is happier with it than the next maintainer.' },
    r: { id: 's5', label: 'Section 5 — the win-check ladder' } },
  { q: 'X just played (1,2). Which lines can POSSIBLY have become a win this move?',
    o: ['All 2n+2 lines on the board', 'Only the lines THROUGH (1,2): row 1, column 2, and a diagonal only if (1,2) sits on one — at most 4 line checks', 'Only row 1', 'Every cell must be re-read'], a: 1,
    ...
    w: { 0: 'Lines that don\'t touch (1,2) are exactly as won or unwon as they were last turn.',
         2: 'Column 2 also gained a mark — and a diagonal might have.',
         3: 'That is the full scan you were just freed from.' },
    r: { id: 's5', label: 'Section 5 — lines through the last move' } },
  { q: 'The O(1) tally trick (row count hits n ⇒ win) silently BREAKS when…',
    o: ['The board is larger than 9×9', 'The win length K is SHORTER than the board (gomoku-style "5 in a row on 15×15"): tallies count marks ANYWHERE in the line, not consecutive ones — you must scan ±(K−1) around the last move instead', 'There are three players', 'You add undo'], a: 1,
    ...
    w: { 0: 'Size is irrelevant — tallies scale linearly and stay O(1) per move.',
         2: 'Tallies are per player; three players just means three tally sets.',
         3: 'Undo works fine — decrement the same four tallies (which is why we increment all four without short-circuiting).' },
  ...
  { q: 'The final move fills the board AND completes 3-in-a-row. Correct referee behavior?',
    o: ['DRAW — the board is full', 'Check WIN before DRAW: a winning move that happens to fill the board is a WIN; swap the order and your referee steals victories exactly once per full game', 'Throw an exception — ambiguous state', 'Ask the players'], a: 1,
    ...
    w: { 0: 'That IS the bug — the player completed a line and your code shrugged.',
  ...
  { q: 'Turn management that survives "now add a third player"?',
    o: ['boolean isXTurn', 'A rotating queue: take the mover from the front, put them at the back — 2 or 10 players, the same one line of code', 'Three booleans', 'A switch on the player count'], a: 1,
    ...
    w: { 0: 'A boolean can hold exactly two states; the third player has nowhere to live.',
         2: 'Three booleans whose consistency you maintain by hand — a bug waiting for a flip you forgot.',
  ...
  { q: 'Board.getGrid() returning the private char[][] directly is…',
    o: ['Fine — convenient for rendering', 'An aliasing leak: callers can now write cells without any guard running; expose at(r,c) or a defensive copy (Day 2), so every write stays behind place()', 'OK if the field is final', 'Better, because it is faster'], a: 1,
    ...
    w: { 0: 'Convenient for rendering AND for corrupting — the same reference does both.',
         2: 'final stops reassigning the field; it does nothing to stop editing the array\'s contents.',
         3: 'Rendering reads nine cells; this was never the bottleneck.' },
    r: { id: 's3', label: 'Section 3 — the board' } },
  { q: '"Add an unbeatable AI." The design answer?',
    o: ['if (aiMode) blocks inside the game loop', 'A MoveStrategy on the Player (human-input / random / minimax) — the referee and the loop never change, because HOW a player picks moves was a strategy axis all along (Day 31)', 'Subclass Game as AiGame', 'Rewrite the project in Python'], a: 1,
    ...
    w: { 0: 'The growing-if disease (Day 12): next comes if(easyMode), if(networkPlayer)…',
         2: 'Game = the rules. The rules of tic-tac-toe did not change — wrong axis to subclass.',
         3: 'Tempting. Still no. 🙂' },
    r: { id: 's9', label: 'Section 9 — pluggable brains' } },
  { q: 'Players are X and O today — a third symbol may join tomorrow. Model the symbol as…',
    o: ['enum Mark { X, O }', 'Data on the player (record Player(name, symbol, brain)): the player set is OPEN, and Day 19 said enums are for CLOSED sets the compiler should enforce', 'Public String constants', 'Integers 1 and 2'], a: 1,
    ...
    w: { 0: 'Every new player would mean recompiling an enum — the wrong tool for an open set.',
         2: 'Stringly-typed: nothing stops two players claiming "X".',
         3: 'Magic numbers (Day 19) — and printing the board becomes a lookup chore.' },

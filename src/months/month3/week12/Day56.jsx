...
const QUESTIONS = [
  { q: 'The defining constraint of an LRU cache that rules out the obvious solutions?',
    o: ['It must persist to disk', 'It must be thread-safe', 'It must be generic', 'BOTH get and put must be O(1) — so you cannot scan a list to find the least-recently-used item or shift an array on every access; that constraint forces the HashMap + doubly-linked-list combo'], a: 3,
    ...
    w: { 0: 'Persistence is out of scope for the classic in-memory LRU.',
         1: 'Thread-safety is a real follow-up, but the core constraint is the O(1) time bound.',
         2: 'Generics are nice but not the constraint that shapes the data structure.' },
    r: { id: 's1', label: 'Section 1 — the O(1) requirement' } },
  { q: 'Why a HashMap alone is not enough for LRU?',
    o: ['HashMaps cannot store objects', 'A HashMap gives O(1) lookup but has NO notion of order/recency — you still need to know which key is least-recently-used in O(1), which the map cannot tell you', 'HashMaps are too slow', 'HashMaps have a size limit'], a: 1,
    ...
    w: { 0: 'Maps store any objects fine.',
         2: 'HashMap lookup is O(1) — speed isn\'t the gap; ordering is.',
  ...
  { q: 'Why a DOUBLY-linked list and not a singly-linked list?',
    o: ['Singly-linked lists cannot hold values', 'Double uses less memory', 'To remove a node in O(1) you need its predecessor; a singly-linked list would require an O(n) scan to find prev, but a doubly-linked node carries node.prev directly', 'Double is faster to traverse'], a: 2,
    ...
    w: { 0: 'Both can hold values; that\'s irrelevant.',
         1: 'Doubly-linked uses MORE memory (extra pointer) — the trade buys O(1) removal.',
         3: 'Traversal speed is the same; it\'s about O(1) removal of a known node.' },
    r: { id: 's4', label: 'Section 4 — why doubly-linked' } },
  { q: 'What are the dummy head/tail SENTINEL nodes for?',
    o: ['To store extra data', 'To make the list circular', 'To eliminate null/edge-case checks: with sentinels, every real node always has a non-null prev and next, so insert/remove never special-case the empty list or the first/last node', 'To count the size'], a: 2,
    ...
    w: { 0: 'They hold no real data — that\'s the point.',
         1: 'They bound the list; they don\'t make it circular.',
  ...
  { q: 'On get(key) for a key that IS present, what happens?',
    o: ['Increment a counter only', 'Return the value AND move its node to the front (most-recently-used) — a get is an access, so it must update recency, or your LRU evicts recently-read items', 'Move it to the back', 'Just return the value'], a: 1,
    ...
    w: { 0: 'Counting frequency is LFU; LRU promotes on access.',
         2: 'The back is the eviction end — that would make reads MORE likely to be evicted.',
         3: 'Returning without promoting breaks LRU semantics — reads are accesses.' },
    r: { id: 's6', label: 'Section 6 — get & put' } },
  { q: 'On eviction, beyond unlinking the tail node, what must NOT be forgotten?',
    o: ['Remove the evicted key from the HashMap too — the map and the list must stay in sync, or the map keeps a dangling entry pointing to a node no longer in the list (a leak + wrong lookups)', 'Re-sort the list', 'Nothing else', 'Resize the cache'], a: 0,
    ...
    w: { 1: 'The list never needs sorting — it\'s maintained in order by every operation.',
         2: 'Forgetting the map entry is the classic bug — it\'s definitely something.',
         3: 'Capacity is fixed; eviction keeps you at capacity.' },
  ...
  { q: 'The interviewer says "now make it LFU instead." The clean design that anticipated this?',
    o: ['Subclass the cache per policy', 'Rewrite the whole cache', 'Add an if(isLFU) everywhere', 'An EvictionPolicy abstraction (which item to evict + how to record access): LRU, LFU, FIFO become swappable strategies, and the cache\'s get/put logic stays the same (Day 31)'], a: 3,
    ...
    w: { 0: 'Subclassing per policy duplicates the cache mechanics; compose a policy instead.',
         1: 'A rewrite means the policy wasn\'t isolated — the smell the abstraction removes.',
  ...
  { q: 'Java already ships an LRU primitive. Which, and the catch worth mentioning?',
    o: ['LinkedHashMap with accessOrder=true (override removeEldestEntry) gives LRU out of the box — but it is NOT thread-safe, so concurrent use needs synchronization (or a different structure)', 'HashMap', 'TreeMap', 'ArrayList'], a: 0,
    ...
    w: { 1: 'Plain HashMap has no ordering — that\'s the whole problem.',

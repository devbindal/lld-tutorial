...
const QUESTIONS = [
  { q: 'A Driver has a current availability AND a separate trip has its own progress. How are these modeled?',
    o: ['Status on the Rider', 'A boolean isBusy', 'One big status field on Driver', 'TWO small state machines: Driver status (AVAILABLE / ON_TRIP / OFFLINE) and Trip lifecycle (REQUESTED → ASSIGNED → ARRIVED → IN_PROGRESS → COMPLETED/CANCELLED) — they\'re different concerns that change at different times'], a: 3,
    ...
    w: { 0: 'Riders don\'t hold driver/trip state — wrong owner.',
         1: 'A boolean collapses ON_TRIP, OFFLINE, and AVAILABLE into two values.',
         2: 'One field can\'t hold both "driver is available" and "trip is in progress" cleanly — they\'re orthogonal.' },
    r: { id: 's5', label: 'Section 5 — the state machines' } },
  { q: 'Finding the nearest available driver by scanning every driver is…',
    o: ['Impossible', 'O(N) per request — acceptable for a demo but fatal at city scale (hundreds of thousands of drivers); you need a SPATIAL INDEX (grid/geohash/quadtree) so a search only examines nearby drivers', 'Fine — it\'s simple', 'Best done in the database with ORDER BY'], a: 1,
    ...
    w: { 0: 'It\'s very possible — just slow; the fix is a spatial index.',
         2: 'It\'s simple AND it melts under real load — the scale is the point.',
  ...
  { q: 'Two riders request a ride at the same instant and the SAME driver is nearest to both. What must happen?',
    o: ['The driver chooses', 'Exactly one gets the driver — assignment is an ATOMIC claim (compare-and-set the driver to ON_TRIP); the loser is re-matched to the next-nearest. It\'s the same check-then-act race as Day 21', 'Both get the driver', 'The faster network wins by luck'], a: 1,
    ...
    w: { 0: 'Driver choice is a different feature (accept/reject); the SYSTEM must still claim atomically.',
         2: 'One driver can\'t serve two trips — that\'s the double-booking bug in a cab.',
  ...
  { q: 'Drivers send their GPS location every few seconds. How should the system consume these and update apps?',
    o: ['Each rider polls every driver\'s location', 'Store pings and query on demand only', 'Location pings update the spatial index AND publish to subscribers (the matched rider\'s app) via push/Observer — never poll; the rider subscribes to their driver\'s live position (Day 32)', 'Drivers call each rider directly'], a: 2,
    ...
    w: { 0: 'Polling every driver is the wasteful anti-pattern (Day 32) at massive scale.',
         1: 'On-demand-only loses the live tracking the rider expects.',
  ...
  { q: 'When a trip reaches COMPLETED (or CANCELLED), what happens to the driver?',
    o: ['The driver flips back to AVAILABLE and re-enters the matching pool — the two state machines are coupled: a trip ending frees its driver', 'Nothing — the driver stays ON_TRIP', 'The driver goes OFFLINE', 'The driver is deleted'], a: 0,
    ...
    w: { 1: 'Leaving them ON_TRIP means they never get another ride — a stuck driver.',
  ...
  { q: 'A grid-based spatial index uses a fixed cell size. The tuning trade-off?',
    o: ['Smaller is always better', 'Cell size doesn\'t matter', 'Cells too BIG → each holds many drivers (back toward O(N) per cell); too SMALL → a search must scan many cells to cover the radius — pick a size near the typical search radius (geohash/quadtree adapt to density)', 'Bigger is always better'], a: 2,
    ...
    w: { 0: 'Too small means scanning a huge neighborhood of empty cells.',
         1: 'It\'s the central tuning knob of a grid index.',
  ...
  { q: 'The trip lifecycle should refuse "start trip" while still in DRIVER_ASSIGNED. Why model this strictly?',
    o: ['States must not be skipped: starting before the driver ARRIVED would bill a rider who isn\'t in the car — guarded transitions (State pattern / enum guards, Days 34/48) make illegal jumps impossible', 'To slow down the code', 'It doesn\'t matter', 'Because the database requires it'], a: 0,
    ...
    w: { 1: 'Guards are about correctness, not slowing things down.',
         2: 'Skipping ARRIVED → starting means charging for a ride that hasn\'t begun.',
  ...
  { q: 'WHICH driver to pick (nearest now vs best ETA vs batch a few requests) is a decision that should be…',
    o: ['Hardcoded as "always nearest"', 'Random', 'Decided by the driver', 'A pluggable matching/dispatch STRATEGY (Day 31) — nearest-distance today, ETA-aware or batched dispatch tomorrow, as new classes; the trip flow never changes (and Part 2 builds these)'], a: 3,
    ...
    w: { 0: 'Hardcoding nearest blocks ETA/surge-aware dispatch the business will want.',
         1: 'Random ignores distance/ETA and gives terrible matches.',

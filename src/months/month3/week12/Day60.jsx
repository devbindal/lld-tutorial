...
const QUESTIONS = [
  { q: 'A data-structure design prompt (cache, store, leaderboard). After scope, the FIRST thing to nail?',
    o: ['The class diagram', 'The TIME-COMPLEXITY target — it dictates the data structure(s) (O(1) get/put forces HashMap + DLL; top-K forces a heap). Name it before coding', 'The database', 'The REST API'], a: 1,
    ...
    w: { 0: 'The structure choice comes from the complexity, drawn after.', 2: 'In-memory DS, not storage.', 3: 'API is the surface; the structure is the test.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'Track A is a TTL cache (expiring key-value store). The two mechanisms it needs beyond a HashMap?',
    o: ['Just a bigger map', 'EXPIRY (lazy: treat expired entries as misses on read + a background SWEEPER; or a TTL) AND an EVICTION policy when it hits max size (LRU/LFU as a strategy) — Days 56 + 52', 'Encryption', 'A second server'], a: 1,
    ...
    w: { 0: 'A map alone has no expiry or eviction.', 2: 'Irrelevant.', 3: 'It\'s in-memory.' },
  ...
  { q: 'Track B is a webhook delivery service. The planted trap in delivering events to subscriber URLs?',
    o: ['Using too much memory', 'At-least-once delivery + retries means a subscriber can receive DUPLICATES — deliver async with retry/backoff/dead-letter, and give each event an id so subscribers can DEDUP (idempotency); never block the producer on a slow subscriber (Days 57, 59)', 'Slow JSON parsing', 'Missing validation'], a: 1,
    ...
    w: { 0: 'Not memory.', 2: 'Not parsing speed.', 3: 'The concurrency/duplication issue is the trap.' },
    r: { id: 's5', label: 'Section 5 — Track B' } },
  { q: 'Both tracks involve background work and external calls. The shared reliability toolkit?',
    o: ['Bigger timeouts', 'Async (don\'t block the caller) + retry with exponential backoff + dead-letter queue + idempotency — plus a deterministic test using fakes and a fixed clock', 'Synchronous everything', 'Hope'], a: 1,
    ...
    w: { 0: 'Timeouts just delay failure.', 2: 'Sync blocks the hot path.', 3: '🙂 No.' },
    r: { id: 's2', label: 'Section 2 — the week\'s skills' } },
  { q: 'You scored 12/17, all misses in Reliability. The review loop says…',
    o: ['Move on — passing', 'Restudy ONLY the flagged material (async + retry + idempotency from Days 57/59), then re-drill the OTHER track at full length', 'Redo all of Weeks 1–12', 'Lower the rubric'], a: 1,
    ...
    w: { 0: 'A real gap; carrying it forward compounds it.', 2: 'Untargeted re-study is rereading.', 3: 'Adjusting the stick guarantees no improvement. 🙂' },
    r: { id: 's7', label: 'Section 7 — review loop' } },
  { q: 'LRU\'s get must move the node to the front because…',
    o: ['It looks nice', 'A read is a USE — without promoting it, recently-read keys drift to the tail and get evicted wrongly; both lookup and move-to-front are O(1) (HashMap + DLL)', 'It frees memory', 'The map requires it'], a: 1,
    ...
    w: { 0: 'Correctness, not aesthetics.', 2: 'It doesn\'t free memory.', 3: 'It\'s an LRU rule, not a map rule.' },
    r: { id: 's6', label: 'Section 6 — smell hunt' } },
  { q: 'Across Month 3, "a scary big system" should now feel like…',
    o: ['A unique puzzle each time', 'A SHAPE you recognize in minute one — resource+atomic-claim, scheduling/dispatch, state machine, money ledger, pipeline, or game loop — decomposed into patterns you already own', 'Something to memorize', 'Mostly database design'], a: 1,
    ...
    w: { 0: 'They reduce to a handful of shapes.', 2: 'Recognition beats memorization.', 3: 'It grades modeling/concurrency, not schema.' },
  ...
  { q: 'Why narrate "O(1) because HashMap + DLL" or "async because external I/O" while drilling alone?',
    o: ['Unnecessary', 'Citing the structure/pattern AND the constraint it serves must be automatic by interview day — and saying it forces you to verify the choice actually fits, catching mis-design early', 'To memorize a script', 'Interviewers can\'t read code'], a: 1,
    ...
    w: { 0: 'It\'s half the skill being drilled.', 2: 'A habit, not a script.', 3: 'They grade whether you can justify it.' },

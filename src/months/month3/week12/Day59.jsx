...
const QUESTIONS = [
  { q: 'The trip fare is computed…',
    o: ['At request time, stored on the trip', 'At trip END, by a FareStrategy reading the trip\'s distance + duration (from the Part-1 timestamps) — base + per-km + per-min, as Money; the fare can\'t exist until the trip is over', 'By the driver', 'In the Rider class'], a: 1,
    ...
    w: { 0: 'Distance/duration are unknown at request time.', 2: 'Drivers don\'t set rates.', 3: 'Riders carry no pricing policy.' },
    r: { id: 's2', label: 'Section 2 — fare' } },
  { q: 'Surge pricing is best added as…',
    o: ['An if(surge) inside the fare calculator', 'A wrapper around the base FareStrategy (Decorator/composition): fare = base.price(trip) × surgeFactor(area, time) — surge is a separate concern that multiplies the base, so base rates and the flow never change', 'A new Trip subclass', 'A hardcoded constant'], a: 1,
    ...
    w: { 0: 'An if-branch scatters pricing policy and reopens the calculator.', 2: 'Pricing isn\'t a trip subtype.', 3: 'Surge varies with demand — not constant.' },
    r: { id: 's4', label: 'Section 4 — surge' } },
  { q: 'Surge multiplier is driven by…',
    o: ['The time of day only', 'The demand/supply ratio (riders requesting vs drivers available) in an area — more demand or fewer drivers → higher surge (capped); it incentivizes more drivers and rations scarce supply', 'The driver\'s mood', 'The fare tier'], a: 1,
    ...
    w: { 0: 'Time correlates but the real driver is supply vs demand.', 2: '🙂 No.', 3: 'Tier sets the base rate, not surge.' },
    r: { id: 's4', label: 'Section 4 — surge' } },
  { q: 'A driver 2 km away in heavy traffic vs one 4 km away on a clear road. A good dispatcher picks…',
    o: ['The 2 km driver (nearest by distance)', 'The one with the lowest ETA (the 4 km driver arrives sooner) — dispatch should optimize TIME, not raw distance; matching policy is a swappable MatchingStrategy', 'Random', 'The cheaper one'], a: 1,
    ...
    w: { 0: 'Nearest by distance can be slowest by time.', 2: 'Random ignores ETA.', 3: 'Cheapness isn\'t the matching goal.' },
    r: { id: 's6', label: 'Section 6 — dispatch' } },
  { q: 'Dispatch policy (nearest / lowest-ETA / batched pooling) should be…',
    o: ['Hardcoded as nearest', 'A pluggable MatchingStrategy — each policy is a class; the trip/request flow never changes when you swap it (the elevator\'s scheduling lesson, reused)', 'Chosen by the rider', 'In the database'], a: 1,
    ...
    w: { 0: 'Hardcoding blocks ETA/pooling dispatch.', 2: 'The system matches; riders don\'t pick the algorithm.', 3: 'Storage stores; policy decides.' },
    r: { id: 's6', label: 'Section 6 — dispatch' } },
  { q: 'Ratings are two-sided (rider rates driver AND driver rates rider). Where does a driver\'s rating live and how is it used?',
    o: ['Computed by scanning all trips each time', 'A running average maintained per user (updated after each trip) — used for matching eligibility / display; not recomputed over the whole trip history on every read', 'Only on the trip', 'Nowhere'], a: 1,
    ...
    w: { 0: 'Re-scanning all history per read doesn\'t scale.', 2: 'A trip holds one rating; the AVERAGE lives on the user.', 3: 'It\'s a core feature.' },
    r: { id: 's8', label: 'Section 8 — ratings' } },
  { q: 'A ride-pool (shared) request — how does it fit the design?',
    o: ['A total rewrite', 'A different MatchingStrategy that assigns multiple compatible trips to one car along a route, plus a fare-split rule — the trip lifecycle and atomic driver claim are unchanged; it\'s an additive policy', 'Impossible', 'A new app'], a: 1,
    ...
    w: { 0: 'It reuses the core; only policies change.', 2: 'It\'s a known extension.', 3: 'Same system, new strategy.' },
    r: { id: 's9', label: 'Section 9 — pooling' } },
  { q: 'Payment for the completed trip should go through…',
    o: ['A hardcoded payment vendor', 'An injected PaymentGateway port (with an idempotency key so a retried charge doesn\'t double-bill) — the same ports + idempotency lesson as BookMyShow and the ATM', 'Cash only', 'The driver\'s bank app'], a: 1,
    ...
    w: { 0: 'Hardcoding blocks failover and testing.', 2: 'Real systems are cashless-first.', 3: 'Payment is a system concern, behind a port.' },

...
const QUESTIONS = [
  { q: 'Email, SMS, and push should be sent through…',
    o: ['One NotificationChannel interface (send), implemented by EmailChannel / SmsChannel / PushChannel — a Strategy family, so a new channel (WhatsApp) is a new class, not an edit (Day 31)', 'A giant switch in the service', 'Three separate send methods with if-else on the type', 'One method that takes a "type" string'], a: 0,
    ...
    w: { 1: 'A switch on channel type scatters the same coupling.',
         2: 'Three methods + if-else is the growing-switch smell (Day 12) — every new channel reopens it.',
  ...
  { q: 'The event that triggers a notification should know…',
    o: ['The user\'s phone number', 'Exactly which channels and providers to use', 'How to render HTML email', 'Nothing about delivery — it fires a channel-agnostic event/request (who, type, data); preferences and templates decide channels and wording downstream (Observer decoupling, Day 32)'], a: 3,
    ...
    w: { 0: 'The event carries a user reference; contact details are resolved later.',
         1: 'Baking channels into the event couples every producer to delivery details.',
         2: 'Rendering is the template engine\'s job, not the event\'s.' },
  ...
  { q: 'The same "order shipped" message is a subject+HTML for email but a short title+line for push. The clean design?',
    o: ['Send the raw data and let the client format it', 'One hardcoded string reused everywhere', 'A template rendered PER (notification type, channel): the template owns wording, the channel owns format — so each channel gets an appropriate rendering of the same event', 'Let each channel write its own copy'], a: 2,
    ...
    w: { 0: 'Pushing formatting to clients scatters presentation and breaks consistency.',
         1: 'One string can\'t be both a 160-char SMS and an HTML email.',
         3: 'Per-channel ad-hoc copy duplicates and drifts; centralize in templates.' },
    r: { id: 's5', label: 'Section 5 — templates' } },
  { q: 'A user has SMS off for marketing but on for OTP. Where does this live?',
    o: ['In the sending code as if-checks', 'In user PREFERENCES resolved per (user, notification type) → the set of channels to use; the dispatch loop just iterates the resolved channels, with no per-type if-logic in the senders', 'It cannot be supported', 'In each channel class'], a: 1,
    ...
    w: { 0: 'if-checks in sending code scatter a cross-cutting policy everywhere.',
         2: 'It\'s a core feature — preferences are a first-class lookup.',
         3: 'A channel sends; it shouldn\'t know a user\'s per-type opt-outs.' },
    r: { id: 's6', label: 'Section 6 — user preferences' } },
  { q: 'Sending is on a network and providers fail. Reliable delivery needs…',
    o: ['Synchronous sends so you know immediately', 'Async delivery with retries + exponential backoff, provider FAILOVER (try a secondary), and a dead-letter queue when all retries are exhausted — never block the caller, never silently drop (echoes logging async, Day 49)', 'Send once and hope', 'A bigger timeout'], a: 1,
    ...
    w: { 0: 'Synchronous sends block the request thread on slow third-party APIs.',
         2: 'Send-once loses messages on any transient blip.',
         3: 'A longer timeout just makes the caller wait longer to fail.' },
    r: { id: 's8', label: 'Section 8 — reliability' } },
  { q: 'SMS can go through Twilio OR MSG91. How are providers modeled?',
    o: ['A new channel per provider', 'Hardcode Twilio in SmsChannel', 'Each provider is an Adapter (Day 26) behind a common SmsProvider port; the SmsChannel tries the primary and FAILS OVER to a secondary — vendors are swappable and testable with a fake', 'Pick one provider forever'], a: 2,
    ...
    w: { 0: 'Provider is orthogonal to channel — Twilio and MSG91 are both SMS.',
         1: 'Hardcoding one vendor makes failover and testing impossible.',
  ...
  { q: 'A transient failure causes a retry, but the first send actually succeeded. The risk and the fix?',
    o: ['The message is lost', 'No risk — retries are safe', 'The provider charges double only', 'Duplicate notifications (the user gets two OTPs); fix with an idempotency key per notification so a retry of an already-sent message is a no-op (same lesson as Days 52/54)'], a: 3,
    ...
    w: { 0: 'The opposite — it\'s sent twice, not lost.',
         1: 'Retries are NOT inherently safe — that\'s why idempotency exists.',
         2: 'Double-charge is a symptom; the user-facing bug is the duplicate message.' },
    r: { id: 's8', label: 'Section 8 — idempotency' } },
  { q: 'An OTP and a marketing blast hit the system at once. A good design treats them…',
    o: ['By PRIORITY: OTP is high-priority/low-latency (send now, bypass quiet hours and batching), marketing is low-priority/batchable and respects quiet hours — priority is a property of the notification type', 'Identically, first-come-first-served', 'Marketing first, it has more volume', 'Randomly'], a: 0,
    ...
    w: { 1: 'FIFO lets a marketing blast delay someone\'s login OTP — unacceptable.',

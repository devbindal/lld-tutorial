...
const QUESTIONS = [
  { q: 'An ATM and a vending machine (Day 48) are both flagship problems for which pattern, and why?',
    o: ['Observer — they notify users', 'State pattern: the SAME inputs (keypad, card, buttons) mean different things in each state, and every state gives every event a different answer — so one class per state beats a giant switch (Day 48)', 'Strategy — they swap algorithms', 'Singleton — there is one ATM'], a: 1,
    ...
    w: { 0: 'Observer is about notifying subscribers, not modeling modes.',
         2: 'Cash-dispensing is a strategy-ish sub-part, but the overall structure is a state machine.',
         3: 'Oneness is incidental and is wiring (Day 21), not the core modeling.' },
  ...
  { q: 'In the AUTHENTICATED state, who decides that "withdraw" is allowed but "enter PIN" is not?',
    o: ['A central switch in the ATM controller', 'The state object itself: each state exposes/handles only its legal actions and names its successor — so illegal actions are refused BY the state, with no scattered if-checks in the controller', 'The keypad hardware', 'The bank server'], a: 1,
    ...
    w: { 0: 'A central switch is the switch-grid disease the State pattern cures (Day 48).',
         2: 'Hardware reads keys; it doesn\'t know which actions are legal in which state.',
         3: 'The bank authorizes accounts; it doesn\'t drive the ATM\'s UI state.' },
    r: { id: 's3', label: 'Section 3 — states own transitions' } },
  { q: 'Why model CardReader, CashDispenser, Keypad, and Screen as interfaces (ports)?',
    o: ['Java requires interfaces', 'Dependency inversion (Day 15): the ATM logic depends on abstractions, so real hardware drivers OR fakes can be injected — making the entire flow testable without a physical machine', 'Interfaces are faster', 'To hide the hardware from the bank'], a: 1,
    ...
    w: { 0: 'Nothing requires it; it\'s a testability/DIP choice.',
         2: 'Indirection doesn\'t speed up hardware I/O.',
         3: 'The point is swappable drivers + fakes, not hiding from the bank.' },
    r: { id: 's5', label: 'Section 5 — hardware behind ports' } },
  { q: 'Dispensing ₹3700 from {₹2000, ₹500, ₹100} notes is fundamentally…',
    o: ['A unique problem', 'The change-making problem again (Day 48): greedy biggest-note-first works for standard denominations, but with INVENTORY limits it can fail — so feasibility must be checked before any money moves', 'A sorting problem', 'Impossible without exact notes'], a: 1,
    ...
    w: { 0: 'It\'s the same shape as vending-machine change (Day 48), not new.',
         2: 'Sorting denominations is a step, not the problem.',
         3: 'Many amounts ARE dispensable; the issue is checking feasibility, not impossibility.' },
    r: { id: 's6', label: 'Section 6 — cash dispensing' } },
  { q: 'The ATM can\'t make the requested amount with its current notes. Correct behavior?',
    o: ['Dispense the closest amount it can', 'Refuse the withdrawal up front (before any debit) and tell the user — never debit the account or hand out a different amount than requested', 'Debit anyway and owe the user', 'Round down to the nearest ₹500'], a: 1,
    ...
    w: { 0: 'Handing out a different amount than requested is a correctness and trust failure.',
         2: 'Debiting without dispensing is the cardinal ATM sin — money lost.',
         3: 'Silently changing the amount cheats the user\'s intent.' },
  ...
  { q: 'A withdrawal touches the account balance AND the ATM cash inventory. The non-negotiable rule?',
    o: ['Debit first, dispense whenever', 'They must change together or not at all (atomicity): the account is debited IF AND ONLY IF the cash is dispensed — if dispensing fails after a debit, roll the debit back', 'Dispense first, debit later', 'Keep them independent for speed'], a: 1,
    ...
    w: { 0: 'Debit-then-maybe-dispense risks debiting without dispensing (a jam).',
         2: 'Dispense-then-maybe-debit risks giving cash for free.',
         3: 'Independence is exactly what causes lost money — they must be coupled.' },
    r: { id: 's8', label: 'Section 8 — transaction atomicity' } },
  { q: 'The dispenser JAMS after the account was already debited. What must happen?',
    o: ['Keep the debit — it is the bank\'s policy', 'Roll back the debit so the balance is restored — the failed dispense means the transaction did not complete, and the account must reflect that no cash left', 'Retry forever', 'Charge a jam fee'], a: 1,
    ...
    w: { 0: 'Keeping a debit for undelivered cash robs the customer.',
  ...
  { q: 'Three wrong PINs in a row should…',
    o: ['Let the user keep trying', 'Trigger a state transition to a terminal/locked outcome (retain card or block, → IDLE) — the strike limit is a guarded transition in the state machine, not an ad-hoc counter check sprinkled in the controller', 'Email the user', 'Lower the withdrawal limit'], a: 1,
    ...
    w: { 0: 'Unlimited PIN attempts is a brute-force hole.',
         2: 'Email is a side effect, not the security transition.',
         3: 'The response to bad PINs is lockout/retention, not a limit change.' },

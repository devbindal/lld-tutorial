...
const QUESTIONS = [
  { q: 'Where does a seat\'s AVAILABLE / BOOKED status live in the model?',
    o: ['On the physical Seat object', 'On a ShowSeat(show, seat, state): the SAME seat has different availability for each show, so status is a property of the seat-FOR-A-SHOW, not the seat itself', 'On the Movie', 'In a global boolean array'], a: 1,
    ...
    w: { 0: 'Put status on Seat and the 10 AM booking would mark C4 taken for the 6 PM show too — same furniture, wrong scope.',
         2: 'A movie has many shows across many screens; it can\'t hold one seat\'s status.',
         3: 'A global array loses which show/screen and can\'t scale past one screen.' },
    r: { id: 's3', label: 'Section 3 — Seat vs ShowSeat' } },
  { q: 'Movie vs Show — the relationship is…',
    o: ['They\'re the same thing', 'A Show is one screening: a Movie playing on a specific Screen at a specific time. One Movie has MANY Shows; a Show references one Movie and one Screen (Day 10\'s noun analysis)', 'A Movie is a kind of Show', 'A Show contains many Movies'], a: 1,
    ...
    w: { 0: 'Collapse them and you can\'t represent the same film at 10 AM and 2 PM.',
         2: 'A movie is-a show fails Day 3\'s is-a test — they\'re different concepts.',
         3: 'A single screening plays one film, not many.' },
  ...
  { q: 'Two users click seat C4 for the same show at the same instant. With a naive read-then-write, what happens?',
    o: ['The database prevents it automatically', 'Both read "AVAILABLE", both write "BOOKED", both get a ticket — a DOUBLE BOOKING; check-then-act on shared state is the same race as Day 21, and it is THE defining problem of this system', 'The second user gets an error', 'One booking is randomly dropped'], a: 1,
    ...
    w: { 0: 'A plain table write does NOT prevent this without a constraint or lock — that\'s the work you must design.',
         2: 'Nothing tells the second user "taken" — both reads happened before either write.',
         3: 'Neither is dropped; both succeed, which is the disaster.' },
  ...
  { q: 'The standard fix that lets a user pick seats, then pay, without others grabbing them meanwhile?',
    o: ['Book instantly on click, refund if they don\'t pay', 'A temporary HOLD/LOCK with expiry: AVAILABLE →(atomic)→ HELD (5 min) → BOOKED on payment, or back to AVAILABLE if the timer expires — a state machine with a TIMED transition', 'Make users pay before seeing seats', 'Disable other users while one is booking'], a: 1,
    ...
    w: { 0: 'Instant-book-then-refund pollutes the system with phantom bookings and refund churn.',
         2: 'Forcing payment before seat selection is a terrible UX and still races on the seat.',
         3: 'You can\'t freeze all other users — thousands book concurrently.' },
    r: { id: 's5', label: 'Section 5 — the seat lifecycle' } },
  { q: 'An atomic hold is best expressed as which operation?',
    o: ['if (seat.isAvailable()) seat.hold();', 'A single compare-and-set: "set state to HELD only if it is currently AVAILABLE", which succeeds for exactly one racer and fails for the other — the check and the set are ONE indivisible step', 'A sleep then a write', 'A try-catch around the write'], a: 1,
    ...
    w: { 0: 'That\'s TWO operations with a gap between them — the exact race, just in code.',
         2: 'Sleeping changes timing but the race still exists after the sleep.',
         3: 'A try-catch reacts to failures; it doesn\'t make the check-and-set atomic.' },
    r: { id: 's7', label: 'Section 7 — atomic holds' } },
  { q: 'Pessimistic vs optimistic locking for seat booking — the honest framing?',
    o: ['Optimistic is always better', 'A trade-off: pessimistic (lock the row while held) is simple and safe under high contention but holds locks; optimistic (version/CAS, retry on conflict) scales when conflicts are rare — name the trade and pick per expected contention', 'Pessimistic is obsolete', 'They produce different final data'], a: 1,
    ...
    w: { 0: 'Optimistic thrashes under high contention (constant retries) — not always better.',
  ...
  { q: 'Search: "Inception in Bengaluru on Saturday." What does this query traverse?',
    o: ['All seats in the country', 'City → Theatres in that city → Screens → Shows on that date whose Movie is Inception — the City◆Theatre◆Screen◆Show hierarchy is exactly what makes this query efficient', 'Every Movie ever made', 'The Booking table'], a: 1,
    ...
    w: { 0: 'Scanning all seats nationwide ignores the hierarchy that scopes the query.',
         2: 'The movie catalog isn\'t filtered by city/date; shows are.',
         3: 'Bookings are the result of choosing a show, not the search input.' },
    r: { id: 's4', label: 'Section 4 — the entity model' } },
  { q: 'A held seat\'s 5-minute timer expires before payment. What must happen, and whose job is it?',
    o: ['Nothing — it stays held forever', 'It auto-releases HELD → AVAILABLE so others can book it — a background expiry mechanism (scheduled sweep or TTL) owns this; without it, abandoned holds permanently leak seats', 'The user is charged anyway', 'The seat becomes BOOKED'], a: 1,
    ...
    w: { 0: 'A hold that never expires is a permanent leak — the sold-out screen that\'s actually empty.',
         2: 'Charging without a completed payment is fraud, not expiry handling.',
         3: 'Expiry means the user did NOT pay — it must free the seat, not book it.' },
    r: { id: 's5', label: 'Section 5 — the seat lifecycle' } },
]

...
const QUESTIONS = [
  { q: 'EQUAL, EXACT, and PERCENTAGE splits — the clean way to model them?',
    o: ['An if-else on a splitType string inside addExpense()', 'A SplitStrategy interface with one method (compute shares for an amount + participants); EqualSplit / ExactSplit / PercentSplit are classes, and a new split type is a new class, not an edit (Day 31)', 'Three separate addExpense methods', 'A boolean isEqual'], a: 1,
    ...
    w: { 0: 'The if-else is the growing-switch smell (Day 12) — every new split type reopens addExpense.',
         2: 'Three methods duplicate the surrounding flow and don\'t compose.',
  ...
  { q: 'Every split type shares one validation rule. What is it?',
    o: ['Everyone must pay the same', 'The shares must sum EXACTLY to the expense total — equal, exact, or percentage, the parts always add up to the whole, or the split is rejected', 'There must be at least 3 people', 'The payer must be in the group'], a: 1,
    ...
    w: { 0: 'Equal is one strategy; exact/percentage are deliberately unequal.',
         2: 'Two people can split a bill fine.',
         3: 'True but unrelated — the universal rule is shares summing to the total.' },
    r: { id: 's3', label: 'Section 3 — the sum rule' } },
  { q: 'Split ₹100 equally three ways. 100/3 = 33.333… How should the system handle it?',
    o: ['Round each to ₹33.33 and lose a penny', 'Work in the smallest unit (paise), give each the floor, and distribute the leftover paise deterministically — 33.34 + 33.33 + 33.33 = exactly ₹100; money is conserved and the rule is repeatable', 'Use a double and ignore it', 'Refuse non-divisible amounts'], a: 1,
    ...
    w: { 0: '33.33 × 3 = 99.99 — a penny vanishes, and balances stop reconciling over time.',
         2: 'double drifts (Day 20) — 0.1 isn\'t representable; money math needs exact units.',
         3: 'Refusing ₹100/3 would reject most real bills — handle the remainder instead.' },
    r: { id: 's4', label: 'Section 4 — the penny problem' } },
  { q: 'To answer "what does Bob owe Alice?", the system should…',
    o: ['Scan every expense ever and recompute', 'Maintain running NET balances (pairwise or per-user), updated as each expense is added — the answer is a lookup, not a recomputation over a transaction log', 'Ask Alice', 'Store only the latest expense'], a: 1,
    ...
    w: { 0: 'Re-scanning all history per query is O(n) and pointless when you can maintain the net.',
         2: '🙂 Not a data structure.',
         3: 'You need the full balance, not just the last expense.' },
  ...
  { q: 'Alice paid ₹600 for a dinner split equally among Alice, Bob, Carol. The balance update?',
    o: ['Everyone owes Alice ₹200', 'Bob owes Alice ₹200 and Carol owes Alice ₹200 — the payer\'s OWN share (₹200) is not a debt; only the other participants\' shares become what they owe the payer', 'Alice owes ₹600', 'Bob and Carol owe ₹300 each'], a: 1,
    ...
    w: { 0: 'Alice doesn\'t owe herself — her share is already covered by paying.',
         2: 'Alice is owed, not owing — she fronted the money.',
         3: '₹300 would be splitting two ways; it\'s three participants at ₹200.' },
    r: { id: 's7', label: 'Section 7 — adding an expense' } },
  { q: '"Settle up with the fewest payments" given a tangle of debts is fundamentally…',
    o: ['A sorting problem', 'A graph / min-cash-flow problem: net everyone out, then match creditors to debtors (greedy: biggest creditor ↔ biggest debtor) — the structure that turns a chain A→B→C into one A→C payment', 'A search problem', 'Impossible to optimize'], a: 1,
    ...
    w: { 0: 'Sorting is a step, not the problem; the problem is minimizing transfers.',
         2: 'It\'s not a lookup/search; it\'s netting and matching flows.',
         3: 'It\'s very optimizable — netting alone collapses chains and cycles.' },
    r: { id: 's8', label: 'Section 8 — debt simplification' } },
  { q: 'Three debts form a cycle: A→B ₹50, B→C ₹50, C→A ₹50. After simplification…',
    o: ['Everyone still pays ₹50', 'Zero payments — each person\'s net is 0, so the cycle cancels completely; the three "debts" represent nobody actually owing anything', 'A pays ₹150', 'It can\'t be simplified'], a: 1,
    ...
    w: { 0: 'Paying the raw debts moves ₹150 around pointlessly — everyone ends where they started.',
         2: 'A\'s net is zero; A owes nothing.',
         3: 'It simplifies to the cleanest possible result: nothing.' },
    r: { id: 's8', label: 'Section 8 — cycles cancel' } },
  { q: 'A subtle downside of "simplify debts" that a thoughtful design should surface?',
    o: ['It loses money', 'It can ask you to pay someone you never directly transacted with (Carol → Alice, though Carol\'s debt was to Bob) — mathematically correct, but some users want to settle only with people they actually owe, so make simplification optional/explicit', 'It is always slower', 'It double-counts'], a: 1,
    ...
    w: { 0: 'Money is conserved — netting never loses any.',
         2: 'It\'s fast (netting + greedy match), not slow.',

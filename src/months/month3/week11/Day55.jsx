...
const QUESTIONS = [
  { q: 'You get a big-system prompt (booking, payments, inventory). After scope, the FIRST thing to nail?',
    o: ['The class diagram', 'The REST endpoints', 'The database schema', 'The DEFINING problem — usually a shared resource claimed concurrently or money that must stay consistent — name it out loud, because the whole design bends around it'], a: 3,
    ...
    w: { 0: 'A class diagram without the hard problem identified is boxes around the wrong center.',
         1: 'Endpoints are the surface; the concurrency/atomicity core is the test.',
  ...
  { q: 'Track A is a hotel booking system. The entity split that mirrors Seat ≠ ShowSeat?',
    o: ['One Room object per guest', 'Room (physical) ≠ RoomNight (a room\'s availability for a specific date) — the same room has different availability per night, so you claim a RoomNight, not a Room (Day 51)', 'Room and Hotel are the same', 'Room extends Booking'], a: 1,
    ...
    w: { 0: 'A room is reused across guests on different nights; not one per guest.',
         2: 'A hotel HAS rooms; collapsing them loses per-night availability.',
         3: 'A room is-a booking fails the is-a test — different concepts.' },
    r: { id: 's4', label: 'Section 4 — Track A' } },
  { q: 'Track B is a digital wallet. The planted trap in a money transfer is…',
    o: ['Missing input validation', 'Lost-update / non-atomic transfer (Days 21, 54): read-modify-write on a balance loses concurrent updates, and a debit without a matching credit creates or destroys money — use an atomic conditional update and make debit+credit one unit', 'Slow database queries', 'Using too much memory'], a: 1,
    ...
    w: { 0: 'Validation matters but isn\'t the planted concurrency/atomicity trap.',
         2: 'Speed is secondary to never losing money.',
         3: 'Memory isn\'t the issue; correctness under concurrency is.' },
    r: { id: 's5', label: 'Section 5 — Track B\'s planted trap' } },
  { q: 'Across Week 11, four different problems shared ONE underlying bug. Which?',
    o: ['Null pointer exceptions', 'Memory leaks', 'Check-then-act / lost-update races on shared state — seat booking, wallet balance, account debit, inventory claim are all the same race in different costumes (Day 21)', 'Off-by-one errors'], a: 2,
    ...
    w: { 0: 'NPEs are generic; the recurring bug is the concurrency race specifically.',
         1: 'Memory leaks weren\'t the recurring issue.',
         3: 'Off-by-one is unrelated to the shared-state theme.' },
    r: { id: 's6', label: 'Section 6 — the smell hunt' } },
  { q: 'Both tracks need a deterministic test of the hard part. The shared technique?',
    o: ['Run it in production and watch', 'Test only the happy path', 'Drive the race/atomicity case explicitly: for concurrency, fire two claims and assert exactly one wins; for atomicity, inject a fake that fails mid-transaction and assert the rollback — fakes + controlled inputs, no real money, no flakiness', 'Add lots of logging'], a: 2,
    ...
    w: { 0: 'Production is the worst place to first learn your atomicity is broken.',
         1: 'The happy path is the entry ticket; the race/rollback is the grade.',
         3: 'Logs describe; they don\'t assert or fail a build.' },
  ...
  { q: 'A transfer request times out — the client retries. Without protection, what happens, and the fix?',
    o: ['It can double-charge (the first request actually succeeded); fix with an IDEMPOTENCY key so a retry with the same key returns the original result instead of transferring again (Days 52, 54)', 'The user is logged out', 'Nothing — retries are safe', 'The database rejects it automatically'], a: 0,
    ...
    w: { 1: 'Logout is unrelated to transfer idempotency.',
         2: 'Money operations are NOT naturally safe to retry — that\'s the whole point.',
         3: 'Nothing rejects a second valid-looking request without an idempotency check.' },
  ...
  { q: 'You scored 12/17, all misses in Code (the atomic-claim and rollback rows). The review loop says…',
    o: ['Restudy ONLY the flagged material (the atomic hold in Day 52, the rollback in Day 54), then re-drill the OTHER track at full length', 'Lower the rubric', 'Redo all of Weeks 1–11', 'Move on — passing score'], a: 0,
    ...
    w: { 1: 'Adjusting the measuring stick guarantees no improvement. 🙂',
         2: 'Untargeted re-study is rereading — train the gap, not the syllabus.',
         3: '11–14 means a real gap in the most important rows; carrying it forward compounds it.' },
    r: { id: 's7', label: 'Section 7 — the review loop' } },
  { q: 'It\'s the end of Month 3. What should a "classic LLD problem" feel like now?',
    o: ['Something to memorize per problem', 'A unique puzzle each time', 'Mostly about the database', 'A SHAPE you recognize: resource+reservation+atomic-claim, money ledger, state machine — plus the defining problem named in minute one, the patterns reached for by name, and the race/atomicity tested deliberately'], a: 3,
    ...
    w: { 0: 'Memorizing solutions fails on a twist; recognizing shapes generalizes.',
         1: 'They look unique but reduce to a handful of shapes — that\'s the payoff.',
         2: 'The LLD round grades modeling and concurrency, not schema design.' },

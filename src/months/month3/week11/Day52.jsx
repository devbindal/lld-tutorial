...
const QUESTIONS = [
  { q: 'The in-memory atomic hold is written as status.compareAndSet(AVAILABLE, HELD). Why not if (status == AVAILABLE) status = HELD?',
    o: ['compareAndSet is faster', 'compareAndSet is shorter', 'They behave identically', 'The if-then is TWO steps with a gap — two threads can both pass the check before either assigns; compareAndSet does the test-and-set as ONE indivisible operation, so exactly one of two racers wins'], a: 3,
    ...
    w: { 0: 'Speed isn\'t why — correctness under concurrency is.',
         1: 'Brevity is irrelevant; atomicity is the point.',
         2: 'They behave the SAME single-threaded but differently under a race — which is exactly the case that matters.' },
  ...
  { q: 'A user requests seats C4, C5, C6 but C5 is already held by someone else. tryHoldAll should…',
    o: ['Hold all three anyway', 'All-or-nothing: release the holds it already acquired (C4) and fail the whole request — never leave the user with a partial selection or a partial charge', 'Hold C4 and C6, skip C5', 'Wait for C5 to free up'], a: 1,
    ...
    w: { 0: 'You can\'t hold C5 — it\'s taken; pretending otherwise is the double-booking bug.',
         2: 'Partial holds leave the user with 2 of 3 seats and a confusing charge.',
  ...
  { q: 'tryHoldAll acquires seats in a CONSISTENT order (e.g. sorted by seat id). Why?',
    o: ['To make the UI consistent', 'Deadlock avoidance: if user A holds C4 and waits for C5 while user B holds C5 and waits for C4, neither proceeds — acquiring in the same global order means they contend in the same sequence and one cleanly fails', 'It looks neater', 'Sorting is required by Java'], a: 1,
    ...
    w: { 0: 'The UI is unaffected by acquisition order.',
         2: 'Neatness is a side effect; deadlock prevention is the reason.',
         3: 'Java doesn\'t require sorting — it\'s a design choice.' },
  ...
  { q: 'The Booking starts PENDING and only becomes CONFIRMED after payment. Why not create it CONFIRMED immediately?',
    o: ['CONFIRMED bookings are slower to save', 'PENDING is just a label', 'The database requires PENDING first', 'Booking and payment are separate steps: a CONFIRMED-on-creation booking that then fails payment is a phantom ticket — PENDING lets a failed charge cleanly become FAILED (and release holds) with no confirmed ticket ever existing'], a: 3,
    ...
    w: { 0: 'Save speed is irrelevant; consistency is the reason.',
         1: 'It\'s a real state with real transitions, not decoration.',
         2: 'No DB rule forces this — it\'s a correctness design.' },
    r: { id: 's5', label: 'Section 5 — the booking lifecycle' } },
  { q: 'payment.charge(...) goes through a PaymentGateway interface. The main benefits?',
    o: ['It makes charging faster', 'It\'s required by Java', 'A port (Day 26/15): Stripe/Razorpay are swappable adapters wired at the root, AND a FakePaymentGateway makes the whole booking flow deterministically testable (force success/decline/timeout) with no real money', 'It hides the amount from the gateway'], a: 2,
    ...
    w: { 0: 'Indirection doesn\'t speed up a network charge.',
         1: 'Nothing requires it; it\'s a Dependency-Inversion choice.',
  ...
  { q: 'A user holds seats and closes their laptop. What reclaims those seats?',
    o: ['A background expiry sweeper (a scheduled job) releases holds whose heldUntil has passed, HELD → AVAILABLE — OR the store enforces a TTL and treats expired holds as free on read; without it, abandoned holds leak seats permanently', 'A manual admin task', 'The next user who tries that seat', 'Nothing — they\'re lost'], a: 0,
    ...
    w: { 1: 'Manual cleanup can\'t keep up with thousands of abandoned holds.',
         2: 'Relying on a later user to trigger cleanup means popular-but-untried seats never free up.',
         3: 'Lost-forever holds are the seat leak that empties a "sold out" show.' },
  ...
  { q: 'The server crashes after saving a PENDING booking but before charging. On restart, recovery should…',
    o: ['Charge the user now', 'Delete the user\'s account', 'Find PENDING bookings older than the hold window and mark them EXPIRED/FAILED, releasing their holds — persisting PENDING before charging is exactly what makes this recovery possible', 'Confirm the booking'], a: 2,
    ...
    w: { 0: 'Charging on restart bills someone who may have given up — and you have no payment authorization.',
         1: '🙂 Obviously not — clean up the booking, not the customer.',
         3: 'It was never paid — confirming it issues a free ticket.' },
  ...
  { q: 'confirm() is also written as a compare-and-set: status.compareAndSet(HELD, BOOKED) guarded by the holder. Why guard it?',
    o: ['Defense in depth: by the time payment returns, the hold could have expired and the seat been re-held by someone else — confirming only if WE still hold it (HELD→BOOKED, same user) prevents charging then failing to seat them', 'BOOKED must come from HELD syntactically', 'Belt and suspenders for no reason', 'To update the timestamp'], a: 0,
    ...
    w: { 1: 'The transition is a design choice, not a syntax rule.',
         2: 'It guards a real race — slow payment vs hold expiry — not nothing.',

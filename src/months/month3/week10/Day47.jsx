...
const QUESTIONS = [
  { q: 'In the controller\'s step() loop, who decides the next floor and who carries out the motion?',
    o: ['step() computes the next floor with if-else and moves the car', 'The strategy moves the car directly', 'The car decides and the controller watches', 'step() ASKS the injected strategy for the next stop (a decision) then tells the car to execute guarded transitions — the loop is plumbing, the brain is swappable, the body is dumb'], a: 3,
    ...
    w: { 0: 'An if-else inside step() welds one policy into the loop — the exact thing the strategy interface removes.',
         1: 'A strategy that moves the car has grabbed the body\'s job and lost its testability (it can no longer be a pure function).',
  ...
  { q: 'Why does the loop advance exactly ONE floor per tick instead of jumping straight to the target?',
    o: ['To save memory', 'So the brain re-decides every floor — a newly-pressed in-between floor (e.g. 6↑ while sweeping 5→9) gets folded into THIS sweep instead of forcing a back-track later', 'Because Java can\'t skip array indices', 'It looks smoother in an animation'], a: 1,
    ...
    w: { 0: 'Memory is irrelevant; correctness of in-flight pickups is the reason.',
         2: 'It could jump fine — the point is to give the strategy a decision point each floor.',
         3: 'The animation is a side benefit; the design reason is re-evaluation granularity.' },
  ...
  { q: 'FCFS and LOOK are both SchedulingStrategy implementations. To switch the whole building from one to the other you…',
    o: ['Add an if(useLook) branch in step()', 'Subclass the controller', 'Rewrite the controller', 'Change ONE line at the composition root: new ElevatorController(car, new Look()) — the controller, car, observers and tests are untouched (Day 12 + Day 31 paying out)'], a: 3,
    ...
    w: { 0: 'A flag branch is the growing-switch smell creeping back in (Day 12).',
         1: 'Subclassing the controller to vary the brain is exactly what composition replaced.',
         2: 'The controller never mentions a concrete strategy — there\'s nothing in it to rewrite.' },
  ...
  { q: 'How do you write a deterministic test that "a SCAN car serves {9,2,8,3,7} from floor 1 in 8 floors"?',
    o: ['You can\'t unit-test scheduling', 'Run a real elevator and time it', 'Drive the controller with a scripted clock/tick: feed the pending calls, call step() until idle, assert the visit ORDER and the floor count — no threads, no sleeps, fully repeatable (Days 42/44 seam, again)', 'Use Thread.sleep to simulate travel time'], a: 2,
    ...
    w: { 0: 'Scheduling is a pure function of (snapshot, pending) — it\'s among the MOST testable parts.',
         1: 'Physical timing is the opposite of a unit test.',
         3: 'Thread.sleep makes tests slow AND flaky — the thing the tick model exists to avoid.' },
    r: { id: 's5', label: 'Section 5 — the simulation harness' } },
  { q: 'Multi-car dispatch is best modeled as…',
    o: ['Random assignment', 'A Dispatcher that scores each car\'s COST for a hall call (ETA-style, with a wrong-direction penalty) and assigns the cheapest — a strategy ONE LEVEL UP; each car keeps its own single-car brain', 'Copy the controller code N times with if-else to pick a car', 'One giant strategy that controls all cars at once'], a: 1,
    ...
    w: { 0: 'Random "works" but ignores the cost information that makes dispatch good — and it\'s still a strategy slot, so do it properly.',
         2: 'Copy-paste-with-if-else is the parallel-hierarchy / shotgun-surgery smell at fleet scale.',
         3: 'A monolith brain throws away the clean single-car design you already have.' },
  ...
  { q: 'A car is MOVING_UP and committed past floor 3. A new 3▼ call arrives. A good cost function…',
    o: ['Always sends the nearest car by distance', 'Refuses the call', 'Adds a large wrong-direction penalty: the car must finish its upward sweep and come back, so "close" by distance is far by TIME — cost models ETA, not raw distance', 'Treats it as cheap because floor 3 is close'], a: 2,
    ...
    w: { 0: 'Nearest-by-distance is the naive version this question is warning against.',
         1: 'Refusing a valid call is a bug, not a policy.',
         3: 'Distance-close but it has to travel up, turn around, and come down — slow in practice.' },
    r: { id: 's7', label: 'Section 7 — the cost function' } },
  { q: 'Where does the doors-closed-before-motion invariant live in the Part 2 code?',
    o: ['In BOTH: step() closes doors before planning a move (loop discipline) AND car.startMoving() throws if doors are open (defense in depth) — the safety rule is enforced at the body even if the brain misbehaves', 'Only in the controller loop', 'Only in the strategy', 'In the test suite'], a: 0,
    ...
    w: { 1: 'Loop-only means a strategy or refactor bug could bypass it — too fragile for a safety rule.',
  ...
  { q: 'The cleanest reason nextStop(snapshot, hallCalls, cabinCalls) takes a snapshot instead of the live car?',
    o: ['It keeps the strategy a PURE function of its inputs — it can read the car\'s floor/direction but cannot move it or mutate state, which makes every strategy trivially unit-testable and impossible to misuse', 'To hide the car\'s floor from the strategy', 'Snapshots are faster to copy', 'Java requires immutable parameters'], a: 0,
    ...
    w: { 1: 'The strategy NEEDS the floor — it\'s exposed by the snapshot, just read-only.',
         2: 'Copy speed is negligible; the design reason is purity and safety.',
         3: 'Java allows mutable params; the discipline is a choice, not a rule.' },

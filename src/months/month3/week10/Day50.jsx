...
const QUESTIONS = [
  { q: 'You\'re given a state-machine prompt. The FIRST modeling artifact to produce (out loud)?',
    o: ['The class diagram', 'The states and the events, then the state × event grid — what each event does in each state, including which are illegal — because that grid IS the design and exposes missing states immediately', 'The database schema', 'The main() method'], a: 1,
    ...
    w: { 0: 'A class diagram without the state grid is boxes with no behaviour — draw the grid first.',
         2: 'Schema is persistence; the behaviour model comes first.',
         3: 'main() wires things that don\'t exist yet — bottom-up, not top-down.' },
    r: { id: 's1', label: 'Section 1 — the protocol' } },
  { q: 'Track A is a traffic-signal controller. Timing plans (rush-hour vs night) should be…',
    o: ['if (isRushHour) hardcoded in the controller', 'An injected SignalTimingStrategy — the state machine drives transitions; HOW LONG each state lasts is a swappable policy, so a new plan is a new class, controller untouched (Days 46–47)', 'A separate controller subclass per plan', 'Stored as magic numbers'], a: 1,
    ...
    w: { 0: 'if(isRushHour) is the growing-switch smell (Day 12) — next comes if(isWeekend), if(holiday).',
         2: 'A subclass per plan explodes combinatorially and welds timing into type.',
         3: 'Magic numbers scattered in the controller are Day 19\'s trap and unswappable.' },
  ...
  { q: 'Track B is a request middleware pipeline. A handler that processes a request but forgets next.handle(r)…',
    o: ['Throws a compile error', 'Silently drops every downstream handler — the deleted-link gap (Day 36): valid requests vanish with no error, the single nastiest bug in chain-of-responsibility code', 'Is automatically retried', 'Logs a warning'], a: 1,
    ...
    w: { 0: 'The compiler can\'t know you MEANT to forward — it sees a valid void method.',
         ...
         3: 'No warning fires — the absence of a call is invisible at runtime.' },
  ...
  { q: 'In the pipeline, ORDER matters: auth, logging, rate-limit, routing. The defensible ordering and why?',
    o: ['Any order works', 'Rate-limit and auth BEFORE expensive work (routing/handling): reject early to protect resources, and authenticate before logging user-specific data — ordering is a design decision to STATE, not stumble into', 'Alphabetical', 'Routing first, it\'s most important'], a: 1,
    ...
    w: { 0: 'Order changes behaviour and cost — logging unauthenticated noise, or routing before rate-limiting, are real bugs.',
         2: 'Alphabetical is arbitrary; order must follow cost/dependency logic.',
  ...
  { q: 'You scored 11/17, all misses in Extensibility. The review loop says…',
    o: ['Move on to Week 11 — passing score', 'Restudy ONLY the flagged material (the strategy/handler seams from Days 47–49), then re-drill the OTHER track at full length', 'Redo all of Weeks 1–10', 'Lower the rubric to 11 items'], a: 1,
    ...
    w: { 0: '10–13 means the gap is real; carrying it into Week 11 compounds it.',
         2: 'Untargeted re-study turns a drill into rereading — train the gap, not the syllabus.',
  ...
  { q: 'Both tracks need a deterministic test. The shared technique?',
    o: ['Run it and eyeball the output', 'Drive the machine with explicit ticks/events and a fixed clock, then assert on recorded state/output — no Thread.sleep, no real time, fully repeatable (the elevator harness + ScriptedDice idea)', 'Use lots of print statements', 'Test only the happy path manually'], a: 1,
    ...
    w: { 0: 'Eyeballing isn\'t a test — it can\'t catch a regression next week.',
         2: 'Prints are debugging, not assertions; nothing fails automatically.',
  ...
  { q: 'A traffic light completes RED → GREEN → YELLOW → RED. Who decides the next state?',
    o: ['The controller with a big if-else', 'Each state names its successor (State pattern) OR an enum encodes next() — transitions live with the state, so adding a flashing-RED or all-RED state is additive, not a controller rewrite (Day 48)', 'A central StateManager switch', 'The timing strategy'], a: 1,
    ...
    w: { 0: 'A controller if-else over states is the switch-grid you\'re escaping (Day 48).',
         2: 'A central manager switch is the same scattered knowledge in a different class.',
  ...
  { q: 'Why narrate "this is the State pattern because every event differs per state" while coding alone in a drill?',
    o: ['It\'s not necessary', 'Citing the pattern AND its trigger as you decide must be automatic by interview day — and it forces you to verify you actually picked the right rung (vs enum+guards), catching mis-modeling early', 'To memorize a script', 'Because the interviewer can\'t read code'], a: 1,
    ...
    w: { 0: 'Silent reps train silent interviews — the narration is half the skill being drilled.',
         2: 'Not a script — a habit of attaching the WHY (and verifying it) to each choice.',

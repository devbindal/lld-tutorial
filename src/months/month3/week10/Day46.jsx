...
const QUESTIONS = [
  { q: 'The ▲ button on floor 5 and the "5" button inside the cabin should create…',
    o: ['Two different types: HallCall(5, Direction.UP) — a pickup whose DIRECTION matters to the scheduler — and CabinCall(5) — a committed stop for this specific car', 'The same Request(5) — both mean floor 5', 'Nothing; the car polls the floors', 'A shared Button object'], a: 0,
    ...
    w: { 1: 'Direction died — a down-sweeping car will now stop for up-bound riders.',
         2: 'Polling floors is Day 32\'s anti-pattern wearing an elevator suit.',
         3: 'Buttons are UI; the REQUEST they create is the domain object.' },
    r: { id: 's3', label: 'Section 3 — the two request types' } },
  { q: 'The single most important guard in the car\'s state machine?',
    o: ['No more than 10 requests', 'Floors must be visited in order', 'startMoving() throws while DOORS_OPEN — motion with open doors is the safety invariant; it must be structurally impossible, not just "unlikely if everyone is careful"', 'No moving on weekends'], a: 2,
    ...
    w: { 0: 'A capacity worry, and it limits the queue, not the state machine.',
         1: 'Visiting order is exactly what the strategy is FREE to choose — that\'s scheduling, not safety.',
         3: '🙂 Policy at best — and not the machine\'s concern.' },
  ...
  { q: 'Enum + guarded transitions vs full State-pattern classes for the car — the honest answer?',
    o: ['Always booleans isMoving + isOpen', 'Start with the enum + guards (4 states, thin behavior) and SAY the trade out loud: Day 34\'s ladder says climb to State classes when per-state behavior multiplies — the vending machine (Day 48) is the problem that genuinely needs the climb', 'Strings: "IDLE", "MOVING"', 'Always full State classes — GoF says so'], a: 1,
    ...
    w: { 0: 'Two booleans = four combinations, including isMoving && isOpen. The invariant just became representable.',
         2: 'Day 19\'s typo machine — "MOVNIG" compiles fine.',
         3: 'Pattern-by-reflex is Day 22\'s factory-itis cousin — ceremony before need.' },
  ...
  { q: 'FCFS served [9,2,8,3,7] from floor 1 in 30 floors; SCAN did it in 8. The design conclusion?',
    o: ['"Which floor next" is a varying ALGORITHM with huge outcome differences → a SchedulingStrategy interface, injected; FCFS, SCAN, LOOK become swappable classes and the controller never changes (Day 31)', 'Buy faster elevators', 'FCFS is a bug', 'Always hardcode SCAN'], a: 0,
    ...
    w: { 1: 'A 4× algorithmic gap outruns any motor upgrade.',
         2: 'FCFS is correct and even has virtues (no starvation!) — it\'s a trade-off, not a defect.',
         3: 'Hardcoding SCAN repeats the mistake one level up — the next building wants destination dispatch.' },
    r: { id: 's7', label: 'Section 7 — the scheduling problem' } },
  { q: 'WHO decides the next stop, and WHO executes the motion?',
    o: ['Whoever pressed first', 'The building decides', 'The controller asks an injected SchedulingStrategy to DECIDE; the car only EXECUTES guarded transitions — brain and body separated, so brains are swappable and the body is simple enough to trust', 'The car decides and moves — it\'s the elevator after all'], a: 2,
    ...
    w: { 0: 'That\'s FCFS smuggled in as physics. It\'s a strategy CHOICE.',
         1: '"The building" is the noun trap from Day 10 — it\'s a label, not a decider.',
         3: 'The god-car: scheduling policy welded into the machine that should only obey guards.' },
    r: { id: 's7', label: 'Section 7 — brain vs body' } },
  { q: 'Floor displays and arrival chimes should update via…',
    o: ['Riders looking at the car', 'The car calling display.update() directly', 'Each display polling the car every second', 'Observer (Day 32): the car publishes arrived/departed events; displays, chimes and the building app subscribe — the car never knows its audience'], a: 3,
    ...
    w: { 0: '🙂 Charming, but the app in your pocket disagrees.',
         1: 'The car importing display classes couples machinery to UI — DIP inverted the wrong way.',
         2: '86,400 wasted polls per display per day, plus lag.' },
  ...
  { q: 'A button press becomes an object (HallCall) that\'s queued, inspected, and executed later. That framing is…',
    o: ['Flyweight', 'Decorator', 'Observer', 'Command (Day 33): the request reifies an action, decoupling the press (invoker) from the ride (execution) — which is exactly why the controller can reorder stops at all'], a: 3,
    ...
    w: { 0: 'Nothing heavy is being shared.',
         1: 'Nothing is being wrapped with extra behavior.',
         2: 'Observer broadcasts what HAPPENED; Command packages what\'s WANTED.' },

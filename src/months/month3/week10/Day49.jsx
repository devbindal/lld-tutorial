...
const QUESTIONS = [
  { q: 'Log levels (TRACE < DEBUG < INFO < WARN < ERROR) are best modelled as…',
    o: ['Strings you compare with .equals()', 'An ORDERED enum (Day 19): each level has a rank, and an event is emitted when event.level.rank ≥ threshold.rank — the ordering IS the filter, with no magic strings and compiler-checked names', 'Integer constants 1–5', 'A boolean isError per message'], a: 1,
    ...
    w: { 0: '"WARN".equals(level) is the magic-string typo machine (Day 19) with no ordering.',
         2: 'Bare ints lose names and meaning — what does level 3 mean at the call site?',
         3: 'A boolean can\'t express five ordered levels, let alone filter by threshold.' },
  ...
  { q: 'A logger has no explicit level set. How is its effective threshold determined?',
    o: ['It defaults to TRACE (log everything)', 'It INHERITS from the nearest ancestor in the logger NAME hierarchy that has an explicit level — com.shop.payment.X borrows com.shop.payment\'s, falling back to ROOT — so you configure a whole package with one line', 'It throws an exception', 'It logs nothing'], a: 1,
    ...
    w: { 0: 'Defaulting to TRACE would flood every app that forgot to configure a logger.',
         2: 'A missing level is normal and expected, not an error — that\'s the point of inheritance.',
         3: 'Silently logging nothing would hide events; inheritance ensures a sensible threshold always exists (ROOT).' },
  ...
  { q: 'Where the event travels up the logger tree and each ancestor\'s appenders get a chance to handle it — that structure is…',
    o: ['Observer', 'Chain of Responsibility (Day 36): the event propagates up the parent chain, each logger optionally passing to its appenders and on to its parent (additivity), until ROOT or a logger that stops propagation', 'Decorator', 'Visitor'], a: 1,
    ...
    w: { 0: 'Observer is fan-out to listeners with no chain/ordering up a hierarchy — close, but the parent-chain propagation is CoR.',
         ...
         3: 'No operation is visiting a structure.' },
  ...
  { q: 'Console wants readable text, the file wants timestamps, the network wants JSON. The clean design?',
    o: ['if(appender==console) format simple; else if…', 'Each Appender owns a Layout (formatter): the logger hands the same LogEvent to every appender, and each appender\'s Layout formats it independently — add a sink or a format as a new class, never an edit (Day 31 + Day 12)', 'Format once in the logger and send the string everywhere', 'Three separate logger classes'], a: 1,
    ...
    w: { 0: 'The if-else chain is the growing-switch smell (Day 12) — every new sink reopens it.',
         2: 'Formatting once forces ONE format on all sinks — JSON to the console, unreadable.',
         3: 'Per-sink logger classes duplicate everything and explode combinatorially.' },
  ...
  { q: 'Why is log.debug("user " + id + " cart " + serialize(cart)) a performance trap even when the level is INFO?',
    ...
    w: { 0: 'String ops aren\'t the issue — doing them needlessly, millions of times, is.',
         2: 'Synchronization isn\'t the cost here; eager argument evaluation is.',
         3: 'No logger is allocated per call; the wasted work is the message construction.' },
  ...
  { q: 'SLF4J vs Logback/Log4j2 — what\'s the relationship, and why does it matter for design?',
    o: ['They\'re competitors; pick one', 'SLF4J is a FACADE/API (Day 29) you code against; Logback/Log4j2 are implementations bound at deploy time — depending on the abstraction (Day 15) lets you swap the logging backend without changing a single log call', 'SLF4J is older and deprecated', 'Logback is just SLF4J renamed'], a: 1,
    ...
    w: { 0: 'They\'re layers, not competitors — you use SLF4J WITH Logback.',
         2: 'SLF4J is actively the standard facade, not deprecated.',
         3: 'They\'re distinct: an API vs an implementation of it.' },
  ...
  { q: 'The framework is used by many threads and you want one shared configuration. The right approach for the LoggerFactory?',
    o: ['A new Logger per call', 'getLogger(name) returns a cached, shared Logger per name (often via a registry) and configuration is centralized — but reach it through the FACTORY, not a hand-rolled global; thread-safety on the shared structures is the real concern (Day 21 + Month 4)', 'A thread-local logger', 'A brand new config object per log call'], a: 1,
    ...
    w: { 0: 'A fresh Logger per call loses cached config and wastes allocation.',
         2: 'Thread-local loggers fragment configuration and defeat sharing.',
         3: 'Re-reading config per call is enormous needless work on the hot path.' },
  ...
  { q: 'Logging is on the hot path of every request. The flag to plant for high-throughput systems?',
    o: ['Logging can never be slow', 'Synchronous appenders block the calling thread on I/O (disk/network) — for high throughput use an ASYNC appender: the call enqueues the event and a background thread writes it; flag the trade-off (possible event loss on crash, ordering, backpressure) — deep-dived in Month 4', 'Always log to /dev/null', 'Disable logging in production'], a: 1,
    ...
    w: { 0: 'A blocking network appender absolutely can be slow — that\'s the whole concern.',
         2: 'Discarding logs throws away your production observability.',
         3: 'Production is where you need logs MOST; the fix is async, not off.' },

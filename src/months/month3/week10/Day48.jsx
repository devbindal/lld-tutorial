...
const QUESTIONS = [
  { q: 'The elevator (Day 46) stayed at the enum-and-guards rung. Why does the vending machine CLIMB to full State classes?',
    ...
    w: { 0: 'Pattern-by-reflex is factory-itis\'s cousin — the enum rung was right for the elevator.',
         2: 'Money lives in the context (the machine), not the state — and enums could hold it fine.',
         3: 'Importance is irrelevant; the amount of per-state behaviour is the deciding factor.' },
  ...
  { q: 'The disease the State pattern cures, shown by "add a MAINTENANCE state" to a switch-based machine?',
    o: ['Slow performance', 'Compile errors', 'Shotgun surgery: one new state means new branches scattered across EVERY event\'s switch block — miss one and the bug is silent; with State classes you ADD one class and touch nothing existing (Day 12)', 'Too much memory'], a: 2,
    ...
    w: { 0: 'Both designs are instant at runtime; maintainability is the cost.',
         1: 'A forgotten branch compiles fine and fails silently — worse than a compile error.',
         3: 'Memory is identical; the cost is scattered edits.' },
    r: { id: 's3', label: 'Section 3 — switch-grid disease' } },
  { q: 'In the State pattern, who decides the NEXT state after handling an event?',
    o: ['The client code', 'The context (VendingMachine) with a big if-else', 'A central StateManager', 'The current state object itself: e.g. DispensingState.dispense() calls machine.setState(soldOut ? SOLD_OUT : IDLE) — states name their own successors, keeping each transition with the behaviour that triggers it'], a: 3,
    ...
    w: { 0: 'Clients press buttons (fire events); they must not know the state graph.',
         1: 'Putting transitions back in the context recreates the switch-grid you were escaping.',
  ...
  { q: 'selectItem() is pressed while the machine is in IDLE (no money). The cleanest design?',
    o: ['Disable the buttons in IDLE', 'IdleState.selectItem() itself rejects it ("insert money first") — every state implements every event, so illegal moves are refused BY the state, with no outer guards in the context', 'The context checks if(balance==0) before calling the state', 'Throw a generic exception from one place'], a: 1,
    ...
    w: { 0: 'UI disabling is a nice extra, but the MODEL must still refuse illegal events authoritatively.',
         2: 'An if(balance==0) in the context is the switch-grid leaking back in.',
         3: 'One central throw still needs to KNOW the state — same coupling.' },
  ...
  { q: 'Where do the balance, inventory, and coin reserves live — in the states or the context?',
    o: ['In the context (the VendingMachine): states are usually stateLESS behaviour holders that READ and MUTATE the context\'s data — so they can be shared/singletons, and switching state doesn\'t lose the balance', 'In each state object', 'In a global', 'In the client'], a: 0,
    ...
    w: { 1: 'Per-state data would be lost on every transition and forces a new object each time.',
         2: 'A global is the singleton-as-global-variable anti-pattern (Day 21).',
         3: 'The client triggers events; it owns none of the machine\'s data.' },
  ...
  { q: '"Exact change only" — the machine can\'t make change for a sale. Correct handling?',
    o: ['Refuse the sale UP FRONT (refund the coins) — "can\'t make change" is a real pre-condition checked before dispensing, often surfaced as an "exact change only" state/flag, never a crash after the coins are swallowed', 'Dispense anyway and keep the extra', 'Round to the nearest coin', 'Crash'], a: 0,
    ...
    w: { 1: 'Keeping the customer\'s extra money is theft, not a design choice.',
         2: 'Silently rounding cheats someone every time — money math (Day 20) forbids it.',
         3: 'A predictable shortfall must be handled gracefully, not crashed on.' },
  ...
  { q: 'Why is the State pattern often paired with making each state a flyweight singleton?',
    o: ['To make them thread-safe automatically', 'Singletons are required by the pattern', 'Because states are stateLESS behaviour (all mutable data is in the context), so ONE IdleState/HasMoneyState instance can be shared across the machine — no reason to allocate a new state object per transition (Day 30 flyweight)', 'To save typing'], a: 2,
    ...
    w: { 0: 'Sharing stateless objects is incidentally safe, but singleton-ness doesn\'t MAKE anything thread-safe.',
         1: 'The pattern works with fresh instances too; flyweight is an optimization, not a rule.',
         3: 'It\'s about avoiding needless allocation, not typing.' },
  ...
  { q: 'State pattern vs Strategy — both inject behaviour behind an interface. The real difference?',
    o: ['State is faster', 'Strategy uses interfaces, State uses classes', 'They\'re identical', 'WHO swaps it and WHEN: a Strategy is chosen ONCE (usually by the client/wiring) and stays; States swap THEMSELVES at runtime in response to events, forming a graph where each state picks the next — same structure, opposite control of transitions'], a: 3,
    ...
    w: { 0: 'Performance is identical; intent differs.',
         1: 'Both use interfaces with concrete implementations.',
         2: 'Structurally similar, but transition control differs sharply.' },

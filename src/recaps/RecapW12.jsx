...
const QUESTIONS = [
  { q: 'Week 12 mixes a data-structure problem with two systems. The throughline?',
    o: ['Unrelated topics', 'Pick the right STRUCTURE for the complexity (LRU), keep varying concerns behind STRATEGIES/ports (Notification, Cab), and add a RELIABILITY layer for external I/O — the everyday engineering toolkit', 'Only caching', 'Databases'], a: 1,
    ...
    w: { 0: 'A clear engineering throughline.', 2: 'Caching is one of three.', 3: 'In-memory/system design, not storage.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'LRU: the two correctness rules beyond "use a HashMap + DLL"?',
    o: ['None', '(1) a get HIT must moveToFront (a read is a use); (2) every structural change updates BOTH structures (insert → map.put + addFront; evict → remove + map.remove)', 'Sort on every access', 'Use recursion'], a: 1,
    ...
    w: { 0: 'Two real rules.', 2: 'No sorting — order is maintained by every op.', 3: 'It\'s pointer rewiring, not recursion.' },
    r: { id: 's2', label: 'Section 2 — LRU' } },
  { q: 'Notification: the five axes that must vary independently?',
    o: ['Just "send"', 'CHANNEL (Strategy) · TEMPLATE (per type+channel) · PREFERENCE (per user) · PROVIDER (Adapter+failover) · RELIABILITY (async/retry/DLQ/idempotency) — keep them apart or every change touches everything', 'Channel + message', 'Sender + receiver'], a: 1,
    ...
    w: { 0: 'Five distinct concerns.', 2: 'Too coarse.', 3: 'Too coarse.' },
  ...
  { q: 'Notification/webhook: log.debug or a send that constructs its payload eagerly, plus at-least-once delivery, imply…',
    o: ['Nothing special', 'Defer eager work (parameterized logging) AND expect DUPLICATES under retries — at-least-once means consumers must dedup via an event/idempotency id; exactly-once is dedup at the edge', 'Use synchronous sends', 'Drop failures silently'], a: 1,
    ...
    w: { 0: 'Two real lessons.', 2: 'Sync blocks the caller.', 3: 'Failures go to a DLQ, not /dev/null.' },
    r: { id: 's2', label: 'Section 2 — Notification' } },
  { q: 'Cab booking: the matching problem (nearest available driver) at city scale needs…',
    o: ['A scan of all drivers', 'A SPATIAL INDEX (grid/geohash/quadtree) so a query examines only nearby cells (~O(1) for the nearby set), with O(1) re-bucket on move — a linear scan is O(N) per request and melts', 'A database ORDER BY', 'Nothing'], a: 1,
    ...
    w: { 0: 'O(N) per request is the trap.', 2: 'Needs a spatial index/extension to avoid a full scan.', 3: 'Scale demands a structure.' },
  ...
  { q: 'Cab booking: two riders match the same nearest driver at once. The fix (and where you\'ve seen it)?',
    o: ['Both get the driver', 'An ATOMIC claim — compareAndSet(driver, AVAILABLE, ON_TRIP) — exactly one wins; the loser re-matches. It\'s the SAME check-then-act race as parking/BookMyShow/ATM (Day 21)', 'First packet wins', 'The driver chooses'], a: 1,
    ...
    w: { 0: 'One driver can\'t serve two trips.', 2: 'Relying on timing is the unguarded race.', 3: 'Accept/decline is a separate step; the SYSTEM claims atomically.' },
    r: { id: 's2', label: 'Section 2 — Cab Booking' } },
  { q: 'Cab booking is called the capstone because…',
    o: ['It is the hardest to type', 'It is every Month-3 SHAPE at once: resource+atomic-claim (driver), scheduling/dispatch (Strategy), state machines (trip+driver), money/pricing (fare+surge), ratings (running aggregate), real-time (spatial index + Observer)', 'It needs the most code', 'It uses a new pattern'], a: 1,
    ...
    w: { 0: 'Not about typing volume.', 2: 'Not about LOC.', 3: 'It reuses known patterns.' },
    r: { id: 's2', label: 'Section 2 — Cab Booking' } },
  { q: 'Java ships an LRU primitive. Which, with what caveat?',
    o: ['HashMap', 'LinkedHashMap with accessOrder=true (+ override removeEldestEntry) = LRU out of the box — but it is NOT thread-safe, so concurrent use needs synchronization', 'TreeMap', 'ArrayList'], a: 1,
    ...
    w: { 0: 'Plain HashMap has no ordering.', 2: 'TreeMap sorts by key, not access.', 3: 'ArrayList has no O(1) lookup.' },
    r: { id: 's2', label: 'Section 2 — LRU' } },
]

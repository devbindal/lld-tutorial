...
const QUESTIONS = [
  { q: 'Week 10\'s theme?',
    o: ['Random problems', 'Machines & frameworks: state machines (Elevator, Vending Machine) and a framework (Logging) — heavy on State, Strategy, Observer, and Chain of Responsibility', 'Only the State pattern', 'Databases'], a: 1,
    ...
    w: { 0: 'A clear theme.', 2: 'State is central but not alone.', 3: 'Not about storage.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'Elevator: brain vs body?',
    o: ['The car does everything', 'The controller ASKS an injected SchedulingStrategy for the next stop (decide); the car only EXECUTES guarded transitions (do) — so brains are swappable and the body is simple enough to trust', 'The building decides', 'Whoever pressed first'], a: 1,
    ...
    w: { 0: 'The god-car is the anti-pattern.', 2: '"The building" is a god-noun.', 3: 'That\'s FCFS smuggled in as physics.' },
    r: { id: 's2', label: 'Section 2 — Elevator' } },
  { q: 'Elevator scheduling: why is the SchedulingStrategy a PURE function of a snapshot?',
    o: ['For speed', 'It reads a read-only CarSnapshot + pending calls and RETURNS a floor — it cannot move the car or mutate state, which keeps every strategy trivially unit-testable and the brain/body line clean', 'Java requires it', 'To hide the floor'], a: 1,
    ...
    w: { 0: 'Testability, not speed.', 2: 'A design choice.', 3: 'It exposes the floor read-only.' },
    r: { id: 's2', label: 'Section 2 — Elevator' } },
  { q: 'Why does the vending machine CLIMB to full State classes when the elevator stayed at enum+guards?',
    o: ['State classes are always better', 'Per-state BEHAVIOR multiplies: the same events (insert/select/dispense/refund) do completely different things in each state — that\'s the ladder\'s climbing condition (Day 34)', 'Vending is more important', 'Enums can\'t hold money'], a: 1,
    ...
    w: { 0: 'Pattern-by-reflex is a smell — the enum rung was right for the elevator.', 2: 'Importance is irrelevant.', 3: 'Money lives in the context.' },
  ...
  { q: 'Logging: the four nouns to keep separate?',
    o: ['Just a Logger', 'LogEvent (WHAT happened, immutable) · Logger (filters + routes) · Appender (WHERE it goes) · Layout (HOW it\'s formatted) — separating them makes new sinks/formats additive', 'Logger + String', 'Level + Message'], a: 1,
    ...
    w: { 0: 'Four distinct concerns.', 2: 'Too coarse.', 3: 'Those live on the LogEvent.' },
    r: { id: 's2', label: 'Section 2 — Logging' } },
  { q: 'Logging: log.debug("x=" + serialize(big)) is a performance trap because…',
    o: ['Strings are slow', 'The argument is BUILT before debug() can check the level, so the expensive serialize runs even when DEBUG is filtered out — use parameterized logging (log.debug("x={}", big)) to defer it', 'debug() is synchronized', 'It allocates a logger'], a: 1,
    ...
    w: { 0: 'Needless work, not string speed.', 2: 'Not synchronization.', 3: 'No logger allocated.' },
    r: { id: 's2', label: 'Section 2 — Logging' } },
  { q: 'SLF4J vs Logback/Log4j2?',
    o: ['Competitors — pick one', 'SLF4J is a FACADE/API you code against; Logback/Log4j2 are implementations bound at deploy time — depend on the abstraction (Day 15) and swap the backend without changing a single log call', 'SLF4J is deprecated', 'Logback is SLF4J renamed'], a: 1,
    ...
    w: { 0: 'They\'re layers, used together.', 2: 'It\'s the standard facade.', 3: 'Distinct: an API vs an implementation.' },
    r: { id: 's2', label: 'Section 2 — Logging' } },
]

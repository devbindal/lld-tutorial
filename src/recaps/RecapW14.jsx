...
const QUESTIONS = [
  { q: 'Chess: adding a new piece type (Amazon piece — moves like Queen + Knight) requires…',
    o: ['Editing the switch in Game.java and Board.java', 'Extending the Board class', 'A new enum constant and a new case in all switches', 'Adding ONE file (Amazon.java extends Piece, overrides legalMoves) + one wiring line — zero other edits'],
    a: 3,
    ...
    w: { 0: 'There is no switch — that was the smell replaced by polymorphism.', 1: 'Board knows about squares, not piece types.', 2: 'Enum is for state/constants; piece movement lives in each piece class.' },
    r: { id: 's1', label: 'Section 1 — the week arc' } },
  { q: 'In Food Delivery, the delivery agent is matched to…',
    o: ['The customer\'s address (shortest final delivery)', 'Whichever address the routing algorithm prefers', 'The midpoint between restaurant and customer', 'The RESTAURANT\'s location — the agent must pick up food there first; nearest to the restaurant minimises pickup time'],
    a: 3,
    ...
    w: { 0: 'Shorter last-mile doesn\'t help if the agent arrives at the restaurant late.', 1: 'The domain logic constrains the choice: pickup first, then deliver.', 2: 'Midpoint is an arbitrary heuristic not used in practice.' },
  ...
  { q: 'Amazon Locker: two couriers submit packages simultaneously. The same slot passes both filters. What prevents double-assignment?',
    o: ['A synchronized method on LockerService', 'A database unique constraint', 'CAS: compareAndSet(AVAILABLE, OCCUPIED) — exactly one wins atomically, the loser gets false and re-searches', 'The slot is marked PENDING first by the filter'],
    a: 2,
    ...
    w: { 0: 'A synchronized method works but CAS is the lightweight lock-free version — both are correct.', 1: 'A DB constraint is correct at the persistence layer but doesn\'t prevent the race in memory.', 3: 'The filter doesn\'t mark slots — it only reads state, so a race remains.' },
  ...
  { q: 'A Pub-Sub consumer commits the offset, then calls process(event). The process() call throws. What breaks?',
    o: ['At-most-once (data loss) — the offset says "done" but the event was never handled; restarting skips it forever', 'The consumer retries from the committed offset, re-processing correctly', 'The dead-letter queue catches the exception', 'Nothing — the offset is saved so the consumer can resume'],
    a: 0,
    ...
    w: { 1: 'Restarting from the committed offset means restarting AFTER the lost event.', 2: 'DLQ only catches events that entered process() and threw; here process() was never called.', 3: 'The event was not processed — silence is the problem, not recovery.' },
  ...
  { q: 'An OrderItem stores a reference to MenuItem and computes lineTotal() = menuItem.price() * qty. The problem?',
    o: ['qty should be a Money type', 'Price not snapshotted — if the menu price changes later, every existing order\'s total changes retroactively; capture unitPrice: Money at order creation', 'lineTotal() should be static', 'The reference should be an interface'],
    a: 1,
    ...
    w: { 0: 'qty is a count; Money is for prices.', 2: 'A static lineTotal() couldn\'t access instance data.', 3: 'Using an interface changes the coupling but not the snapshot problem.' },
    r: { id: 's2', label: 'Section 2 — Day 67' } },
  { q: 'The 4-stage Chess validation pipeline is: geometry → path → destination → ???',
    o: ['piece count', 'king safety (simulate move on a copy of the board; if the king is in check after, the move is illegal)', 'capture rules', 'turn order'],
    a: 1,
    ...
    w: { 0: 'Piece count is not part of move validation.', 2: 'Capture rules are covered in stages 1–3 (destination = opponent\'s piece or empty).', 3: 'Turn order is enforced by the game loop before validation.' },
  ...
  { q: 'Consumer group: 3 instances of the OrderService subscribe to the "orders" channel in the same group. Message M arrives. How many instances process M?',
    o: ['Exactly 1 — the group routes M to one instance (load balancing); the others skip it', '0 until one instance polls', '2 — majority delivery', 'All 3 — each gets a copy'],
    a: 0,
    ...
    w: { 1: 'The message is delivered; one consumer picks it up on poll.', 2: '2-of-3 is not how consumer groups work — it\'s 1-of-N.', 3: 'That is broadcast (multiple independent subscribers or consumer groups).' },
    r: { id: 's2', label: 'Section 2 — Day 69' } },
  { q: 'Greedy smallest-fitting locker slot. The enum sizes are SMALL, MEDIUM, LARGE. The correct ordering field is…',
    o: ['name().compareTo() alphabetically', 'A static comparator outside the enum', 'An explicit int rank field (SMALL=1, MEDIUM=2, LARGE=3) — stable and explicit, immune to enum reordering', 'ordinal() (SMALL=0, MEDIUM=1, LARGE=2)'],
    a: 2,
    ...
    w: { 0: 'Alphabetical gives L < M < S, reversed from smallest-to-largest.', 1: 'A static comparator is better than ordinal/alpha but still external; the rank field is self-contained.', 3: 'ordinal() breaks on reordering — the Day 19 ordinal trap.' },
    r: { id: 's2', label: 'Section 2 — Day 68' } },
]

...
const QUESTIONS = [
  { q: 'SOLID in one breath?',
    o: ['Five unrelated rules', 'S: one reason to change · O: extend without modifying · L: subtypes substitute safely · I: small focused interfaces · D: depend on abstractions — together they make code changeable without ripple', 'Always use interfaces', 'Rules only for big teams'], a: 1,
    ...
    w: { 0: 'They reinforce each other toward changeability.', 2: 'Interfaces are a tool, not the principle set.', 3: 'They help any size codebase.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'The precise definition of a "responsibility" in SRP?',
    o: ['A method', 'A reason to change, i.e. a single ACTOR/stakeholder — group code that changes for the same reason and for the same people', 'A class', 'A line of code'], a: 1,
    ...
    w: { 0: 'A method is too granular.', 2: 'A class can legitimately hold one responsibility.', 3: 'Far too granular.' },
    r: { id: 's2', label: 'Section 2 — Day 11' } },
  { q: 'OCP\'s "closed for modification" is achieved by…',
    o: ['Never editing code', 'Polymorphism: a stable abstraction (interface/abstract method) that new variants EXTEND as new classes, so adding behavior doesn\'t reopen tested code', 'Comments', 'final on everything'], a: 1,
    ...
    w: { 0: 'You still write code — new classes, not edits to old ones.', 2: 'Comments don\'t close anything.', 3: 'final restricts, it doesn\'t enable extension.' },
    r: { id: 's2', label: 'Section 2 — Day 12' } },
  { q: 'LSP\'s contract rules (a subtype may…)?',
    o: ['Do anything', 'Demand LESS and deliver MORE: weaken preconditions, strengthen postconditions, preserve invariants, and not throw new checked exceptions — so callers of the base never break', 'Strengthen preconditions', 'Throw any exception'], a: 1,
    ...
    w: { 0: 'It\'s tightly constrained by the base contract.', 2: 'Strengthening preconditions breaks callers.', 3: 'New checked exceptions break callers.' },
  ...
  { q: 'A class is forced to implement interface methods it can\'t support (empty or throwing). The principle + fix?',
    o: ['SRP; merge classes', 'ISP; split the fat interface into small role interfaces (Printable/Scannable/…) so clients depend only on what they use', 'LSP; add instanceof', 'OCP; add a switch'], a: 1,
    ...
    w: { 0: 'Merging makes it fatter.', 2: 'instanceof is an LSP smell, not a fix here.', 3: 'A switch doesn\'t address the forced lie.' },
    r: { id: 's2', label: 'Section 2 — Day 14' } },
  { q: 'DIP says high-level modules and low-level modules should both depend on…',
    o: ['Each other', 'ABSTRACTIONS (and abstractions shouldn\'t depend on details) — invert the arrow so the policy owns the interface and the detail implements it, wired at a composition root', 'The database', 'Concrete classes'], a: 1,
    ...
    w: { 0: 'Mutual concrete dependence is the tight coupling DIP removes.', 2: 'The DB is a detail.', 3: 'Concrete dependence is exactly what to avoid.' },
    r: { id: 's2', label: 'Section 2 — Day 15' } },
  { q: 'Why does DIP make code testable?',
    o: ['It doesn\'t', 'Swappability: depending on an interface lets a test inject a FAKE (FakeClock, FakeRepository, FakePaymentGateway) — testability and swappability are the same property', 'It adds logging', 'It runs faster'], a: 1,
    ...
    w: { 0: 'It directly enables it.', 2: 'Unrelated.', 3: 'Speed isn\'t the point.' },
    r: { id: 's2', label: 'Section 2 — Day 15' } },
  { q: 'The SOLID smells you should recognize instantly?',
    o: ['Long methods only', 'A class doing X "and" Y (SRP) · a growing switch on a type (OCP) · instanceof / UnsupportedOperationException in overrides (LSP) · forced empty interface methods (ISP) · new ConcreteClass() inside high-level code (DIP)', 'Comments', 'Short variable names'], a: 1,
    ...
    w: { 0: 'Length is a different smell.', 2: 'Comments aren\'t SOLID smells.', 3: 'Naming is unrelated.' },

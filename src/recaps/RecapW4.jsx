...
const QUESTIONS = [
  { q: 'The theme of Week 4?',
    o: ['More rules for the sake of rules', 'Keep code small, honest, and loosely connected: DRY/KISS/YAGNI as counterweights, low coupling + high cohesion, Law of Demeter, proper enums & exceptions, and value objects', 'Everything from Week 3 again', 'Only about exceptions'], a: 1,
    ...
    w: { 0: 'Each earns its place by reducing change-cost.', 2: 'It builds beyond SOLID.', 3: 'Exceptions are one of five topics.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'DRY is really about…',
    o: ['Never writing similar-looking code', 'A single source of truth for each piece of KNOWLEDGE — beware FALSE duplication (same code, different actors) and apply on the third real repeat (rule of three)', 'Short files', 'Copy-paste being illegal'], a: 1,
    ...
    w: { 0: 'Similar-looking ≠ same knowledge.', 2: 'File length is unrelated.', 3: 'Some duplication is correct.' },
    r: { id: 's2', label: 'Section 2 — Day 16' } },
  { q: 'KISS and YAGNI together push you to…',
    o: ['Add every feature now', 'Prefer the simplest thing that works and DON\'T build for speculative future needs — add the seam when a real second case appears, not before', 'Use the cleverest solution', 'Write more abstractions upfront'], a: 1,
    ...
    w: { 0: 'YAGNI says the opposite.', 2: 'Clever is a KISS violation.', 3: 'Upfront abstractions are the speculation YAGNI warns against.' },
    r: { id: 's2', label: 'Section 2 — Day 16' } },
  { q: 'Coupling and cohesion — the goal?',
    o: ['High coupling, low cohesion', 'LOW coupling (modules depend on each other minimally, via data/interfaces) + HIGH cohesion (each module\'s parts serve one clear purpose)', 'Both high', 'Both low'], a: 1,
    ...
    w: { 0: 'That\'s the worst case.', 2: 'High coupling is bad.', 3: 'Low cohesion is bad.' },
    r: { id: 's2', label: 'Section 2 — Day 17' } },
  { q: 'The Law of Demeter says a method should only talk to…',
    o: ['Any object it can reach', 'Its immediate "friends": this, its own fields, its parameters, and objects it creates — not strangers reached by chaining through others', 'Only static methods', 'The database'], a: 1,
    ...
    w: { 0: 'That\'s exactly the train-wreck it forbids.', 2: 'Unrelated.', 3: 'Unrelated.' },
    r: { id: 's2', label: 'Section 2 — Day 18' } },
  { q: 'Why model a fixed set (order statuses, directions) as an enum, not strings/ints?',
    o: ['Enums are shorter', 'A closed, compiler-checked domain: no typos, exhaustive switches, and Java enums can carry per-constant behavior/state — but never persist ordinal() (reordering breaks it)', 'Strings are faster', 'Ints use less memory'], a: 1,
    ...
    w: { 0: 'Brevity isn\'t the win — safety is.', 2: 'Speed is irrelevant.', 3: 'Memory is irrelevant.' },
    r: { id: 's2', label: 'Section 2 — Day 19' } },
  { q: 'An Entity vs a Value Object — the distinction?',
    o: ['No difference', 'An Entity has identity that persists through change (a Customer #42); a Value Object is defined entirely by its values and is immutable (Money ₹50 == any other ₹50) with equals/hashCode by value', 'VOs are bigger', 'Entities are immutable'], a: 1,
    ...
    w: { 0: 'They model different things.', 2: 'VOs are typically small.', 3: 'Entities usually change over time.' },
    r: { id: 's2', label: 'Section 2 — Day 20' } },
  { q: 'Checked vs unchecked exceptions — the guidance?',
    o: ['Always checked', 'Checked for recoverable conditions the caller should handle; unchecked (RuntimeException) for programming errors/bugs — and always fail fast with a clear, context-rich domain exception', 'Always unchecked', 'Never throw'], a: 1,
    ...
    w: { 0: 'Checked-everywhere is noisy and often wrong for bugs.', 2: 'Unchecked-everywhere loses recoverable signaling.', 3: 'Failures must be signaled.' },
    r: { id: 's2', label: 'Section 2 — Day 19' } },
]

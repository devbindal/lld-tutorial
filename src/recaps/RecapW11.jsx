...
const QUESTIONS = [
  { q: 'Week 11\'s unifying lesson?',
    o: ['They are about UI', 'They share one recurring problem — a contended resource or money claimed concurrently — solved by an ATOMIC claim, plus money correctness; BookMyShow, Splitwise, ATM are variations on it', 'Each big system is unique', 'They need new patterns'], a: 1,
    ...
    w: { 0: 'They\'re about concurrency/consistency.', 2: 'They share a deep structure.', 3: 'They reuse known tools.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'BookMyShow: the two entity splits that separate a clean design from a tangle?',
    o: ['Seat extends Show', 'None needed', 'Movie ≠ Show (the film vs a screening) and Seat ≠ ShowSeat (the physical seat vs its availability for one show) — booking attaches to a Show via ShowSeats', 'Just one big Booking class'], a: 2,
    ...
    w: { 0: 'A seat is not a kind of show.', 1: 'These splits are the design.', 3: 'A god class loses the structure.' },
    r: { id: 's2', label: 'Section 2 — BookMyShow' } },
  { q: 'BookMyShow: the seat lifecycle and why its expiry edge matters?',
    o: ['AVAILABLE → HELD → BOOKED, with a TIMED HELD → AVAILABLE expiry — without it, abandoned checkouts leak seats forever (a "sold out" empty hall)', 'BOOKED → AVAILABLE on a timer', 'AVAILABLE → BOOKED only', 'It never changes'], a: 0,
    ...
    w: { 1: 'BOOKED is terminal (until cancel).', 2: 'You need the HELD phase for the pay window.', 3: 'The timed edge is load-bearing.' },
    r: { id: 's2', label: 'Section 2 — BookMyShow' } },
  { q: 'A user books 3 seats but one is taken. The booking should be…',
    o: ['Queue and wait', 'Partial — hold the 2 it can', 'Hold all 3 anyway', 'All-or-nothing: release the holds it acquired and fail cleanly; acquire seats in a CONSISTENT (sorted) order to avoid deadlock'], a: 3,
    ...
    w: { 0: 'Blocking can deadlock.', 1: 'Partial leaves a confusing charge.', 2: 'You can\'t hold a taken seat.' },
    r: { id: 's2', label: 'Section 2 — BookMyShow' } },
  { q: 'Splitwise: splitting ₹100 equally three ways — handle the remainder how?',
    o: ['Compute in paise: floor each share, distribute the leftover paise deterministically so shares sum to exactly ₹100; money is BigDecimal/Money, never double', 'Use a double', 'Refuse non-divisible amounts', '33.33 each (lose a paisa)'], a: 0,
    ...
    w: { 1: 'double drifts (Day 20).', 2: 'Real bills aren\'t divisible; handle the remainder.', 3: '33.33×3 = 99.99 — a vanishing paisa.' },
    r: { id: 's2', label: 'Section 2 — Splitwise' } },
  { q: 'Splitwise: split types (equal/exact/percentage) are modeled as…',
    o: ['A boolean', 'Three addExpense methods', 'A SplitStrategy interface (one class per type), with the universal invariant that shares sum to the total — a new split type is a new class', 'if-else on a type string'], a: 2,
    ...
    w: { 0: 'Can\'t express three+ types.', 1: 'Duplicates the flow.', 3: 'The growing switch reopens addExpense.' },
    r: { id: 's2', label: 'Section 2 — Splitwise' } },
  { q: 'ATM: why model the hardware (CardReader, CashDispenser, Keypad) as interfaces?',
    o: ['Java requires it', 'To hide it from the bank', 'For speed', 'Dependency inversion (Day 15): the ATM logic depends on abstractions, so real drivers OR fakes can be injected — a FakeCashDispenser that "jams" makes the rollback path testable with no hardware'], a: 3,
    ...
    w: { 0: 'A design choice, not required.', 1: 'About swappable drivers/fakes.', 2: 'Testability, not speed.' },
    r: { id: 's2', label: 'Section 2 — ATM' } },
  { q: 'The "six shapes" Month 3 reduces to — name the family of BookMyShow/parking/hotel?',
    o: ['Each is unique', 'Resource + reservation + ATOMIC CLAIM ("two users, one X") — alongside scheduling/dispatch, state machine, money ledger, pipeline, and turn-based game loop', 'Money ledger', 'State machine'], a: 1,

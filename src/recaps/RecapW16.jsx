...
const QUESTIONS = [
  {
    ...
    w: { 0: 'volatile makes the latest write visible, but both threads can still read AVAILABLE before either writes CHECKED_OUT. Visibility is not atomicity.', 2: 'AtomicBoolean CAS works but requires careful rollback if findAvailableCopy() is a separate step outside the CAS. synchronized is simpler here.', 3: 'ConcurrentModificationException is for structural modification of collections during iteration — not for field updates.' },
  ...
  },
  {
    ...
    w: { 0: 'Per-save grading is an information leak — the student can probe all correct answers before submitting.', 2: 'MCQ auto-grading requires no human involvement — waiting for the teacher is wasteful and creates delay.', 3: 'Pre-scoring is impossible without the student answers.' },
  ...
  },
  {
    ...
    w: { 0: 'The broker has no way to know the consumer processed it — it only knows about committed offsets.', 2: 'Restart from the beginning would happen only if no committed offset exists at all (new consumer group). Existing committed offsets are preserved across restarts.', 3: 'Brokers do not detect consumer crashes at the message level — they rely on offset commits to know progress.' },
  ...
  },
  {
    ...
    w: { 0: 'More classes signals over-engineering or poor grouping — not thoroughness. An interviewer who sees 11 classes will ask "do you really need all of these?" and expect a good answer for each.', 2: 'Interviewers absolutely note whether the entity count is reasonable for the time allotted.', 3: 'Adding more to an already over-large model makes the implementation even harder to complete.' },
  ...
  },
  {
    ...
    w: { 0: 'Token counts are integers, not doubles. Rounding is not the issue.', 2: 'ConcurrentHashMap makes individual map operations atomic — but here the issue is a compound read-modify-write on the counter itself.', 3: 'The timer is for refilling — a separate concern. The atomicity issue is on the take (decrement) path.' },
  ...
  },
  {
    ...
    w: { 0: 'Erasing and rewriting at minute 28 loses 5 minutes and breaks flow. The session may not recover.', 2: 'Stopping to ask for time is not an option in a real interview. Work within the constraint.', 3: 'Interviewers notice every signature mismatch. Not addressing it signals you missed it — worse than flagging it.' },
  ...
  },
  {
    ...
    w: { 0: 'Creating families of related objects describes Abstract Factory (Day 23) — not Visitor.', 2: 'Notifying multiple objects on state change describes Observer (Day 32) — not Visitor.', 3: 'Providing a simplified interface describes Facade (Day 29) — not Visitor.' },

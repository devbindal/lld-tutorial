...
const QUESTIONS = [
  { q: 'What do STRUCTURAL patterns do?',
    o: ['Create objects', 'Compose objects/classes into larger structures — adapting interfaces, adding behavior, building trees, simplifying or controlling access, sharing state, splitting hierarchies', 'Define algorithms', 'Manage threads'], a: 1,
    ...
    w: { 0: 'That\'s creational.', 2: 'That\'s behavioral.', 3: 'Unrelated.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'Adapter vs Decorator vs Facade vs Proxy — the one-line distinction?',
    o: ['All the same wrapper', 'Adapter CHANGES the interface; Decorator ADDS behavior (same interface); Facade SIMPLIFIES a subsystem (new, smaller interface); Proxy CONTROLS access (same interface)', 'They differ only in name', 'Only Adapter wraps'], a: 1,
    ...
    w: { 0: 'Same shape, different intent.', 2: 'Intent differs.', 3: 'All four wrap something.' },
    r: { id: 's5', label: 'Section 5 — disambiguation' } },
  { q: 'Object adapter vs class adapter?',
    o: ['Identical', 'Object adapter uses COMPOSITION (holds the adaptee) — flexible, preferred; class adapter uses INHERITANCE (extends adaptee) — single-inheritance limited', 'Class adapter is better', 'Object adapter needs reflection'], a: 1,
    ...
    w: { 0: 'They differ in mechanism.', 2: 'Object adapter is usually preferred.', 3: 'No reflection needed.' },
    r: { id: 's2', label: 'Section 2 — Day 26' } },
  { q: 'In java.io, new BufferedReader(new InputStreamReader(in)) is which two patterns?',
    o: ['Two adapters', 'InputStreamReader is an ADAPTER (bytes→chars, changing the interface); BufferedReader is a DECORATOR (adds buffering, same Reader interface)', 'Two decorators', 'Composite + Proxy'], a: 1,
    ...
    w: { 0: 'Only one changes the interface.', 2: 'InputStreamReader changes the interface (adapter).', 3: 'Neither is a tree/access-control.' },
    r: { id: 's2', label: 'Section 2 — Days 26-27' } },
  { q: 'Composite\'s "transparency vs safety" trade-off?',
    o: ['No trade-off', 'Transparent (GoF): add/remove live on the Component so leaves and composites share one type, but leaves must reject child ops; Safe: child ops only on Composite, so you must downcast/instanceof to add children', 'Safety is always chosen', 'Composite needs no Component type'], a: 1,
    ...
    w: { 0: 'It\'s a genuine trade.', 2: 'GoF leans transparent.', 3: 'The Component type is the whole point.' },
    r: { id: 's2', label: 'Section 2 — Day 28' } },
  { q: 'Facade vs Proxy — both can sit in front of something. The difference?',
    o: ['Identical', 'Facade SIMPLIFIES a whole subsystem behind a new, smaller interface (and is non-exclusive); Proxy has the SAME interface as ONE subject and CONTROLS access to it (lazy/security/remote)', 'Facade controls access', 'Proxy simplifies subsystems'], a: 1,
    ...
    w: { 0: 'Different intent.', 2: 'That\'s Proxy.', 3: 'That\'s Facade.' },
    r: { id: 's2', label: 'Section 2 — Day 29' } },
  { q: 'Flyweight splits state into…',
    o: ['Public and private', 'INTRINSIC (shared, context-free — stored in the flyweight, e.g. glyph shape) and EXTRINSIC (context-dependent — passed in by the client, e.g. position/color)', 'Static and instance', 'Big and small'], a: 1,
    ...
    w: { 0: 'Not about visibility.', 2: 'Not about static.', 3: 'Not about size.' },
    r: { id: 's2', label: 'Section 2 — Day 30' } },
  { q: 'Bridge solves which problem, and how?',
    o: ['Wrong interface', 'A class hierarchy exploding along TWO independent axes (m shapes × n renderers): split abstraction from implementation into TWO hierarchies joined by composition, turning m×n into m+n', 'Adding behavior at runtime', 'Sharing memory'], a: 1,
    ...
    w: { 0: 'That\'s Adapter.', 2: 'That\'s Decorator.', 3: 'That\'s Flyweight.' },
    r: { id: 's2', label: 'Section 2 — Day 30' } },
]

...
const QUESTIONS = [
  { q: 'Hotel reservations A: Mon–Thu and B: Thu–Sat use half-open intervals. Do they overlap?',
    o: ['Only if the same room is involved', 'Yes — both include Thursday', 'It depends on check-out time policy', 'No — A = [Mon, Thu): Thursday is excluded; B = [Thu, Sat): the intervals share no night; back-to-back bookings are allowed'],
    a: 3,
    ...
    w: { 0: 'The overlap formula applies to any resource; the same room is a separate filter.', 1: 'A closed interval [Mon, Thu] would include Thursday, causing a conflict. Half-open is the design that makes back-to-back bookings work.', 2: 'In this design check-out time is the exclusive endpoint; time-of-day is not modelled.' },
  ...
  { q: 'Two members simultaneously call gymClass.book() and both see bookings.size() = 9 (capacity = 10). What prevents the race?',
    o: ['The List.add() method is inherently thread-safe', 'A synchronized block covering both the size check and the booking insert — collapsing two steps into one atomic operation', 'Checking capacity in a separate pre-validation step', 'AtomicInteger on the booking count with a CAS loop'],
    a: 1,
    ...
    w: { 0: 'ArrayList.add() is not thread-safe; even Vector/synchronizedList only makes individual method calls atomic, not the check+add pair.', 2: 'A pre-validation step outside the lock still has the race between validation and insert.', 3: 'An AtomicInteger CAS works but requires more careful rollback logic; synchronized is simpler here.' },
    r: { id: 's2', label: 'Section 2 — Day 71' } },
  { q: 'An online auction has received 5 bids. The seller calls cancel(). What should the system do?',
    o: ['Cancel immediately — the seller owns the auction', 'Cancel silently and remove bid history', 'Reject or require refund policy — after bids arrive, cancellation may be disallowed or must trigger refunds to all bidders; the state machine encodes the policy', 'Cancel and notify the highest bidder only'],
    a: 2,
    ...
    w: { 0: 'Ownership does not override business rules; cancelling after bids arrive harms bidders who made decisions based on the auction.', 1: 'Bid history must never be deleted — it is a financial audit trail.', 3: 'All bidders who placed bids are affected, not just the highest.' },
  ...
  { q: 'A social feed shows posts ordered by recency. After scrolling to page 3, a new viral post arrives. Using offset pagination, what does page 4 show?',
    o: ['Page 4 re-shows the last post of page 3 as a duplicate — the new post shifted all positions by 1', 'Page 4 shows the next 20 posts after page 3 correctly', 'Page 4 shows the new viral post at the top', 'Page 4 is empty because the offset recalculates'],
    a: 0,
    ...
    w: { 1: 'Offset is calculated from the current list length, which changed when the new post arrived.', 2: 'The new post appears at the top of page 1 on next refresh, not in mid-feed pagination.', 3: 'Offset never recalculates automatically — it blindly skips N items from the current list.' },
  ...
  { q: 'A PR has all assigned reviewers marked APPROVED. One comment thread is still open. What should mergePR() do?',
    o: ['Merge with a warning to the author', 'Merge — all reviewers approved', 'Ask the reviewer to close the comment automatically', 'Block merge — the gate is a conjunction: ALL approvals AND NO open comments; one unresolved comment blocks regardless of approvals'],
    a: 3,
    ...
    w: { 0: 'A warning that allows merge is no gate at all — the open comment would be silently buried.', 1: 'Approvals cover code correctness; comments cover specific questions or required changes — they are independent gates.', 2: 'The reviewer is a human; the system cannot close their comments automatically.' },
    r: { id: 's2', label: 'Section 2 — Day 74' } },
  { q: 'Why must bids in an online auction be append-only?',
    o: ['To make queries faster with an indexed list', 'Because the auction state machine requires it', 'Bids are a financial audit log — a bid changes the market signal other bidders respond to; deleting it enables fraud and corrupts the auction history', 'To save database space'],
    a: 2,
    ...
    w: { 0: 'Append-only may actually require more storage and can be slower to query — but correctness is the constraint.', 1: 'The state machine constrains state transitions, not the data model of individual bids.', 3: 'Append-only logs are typically larger than mutable tables — space saving is not the reason.' },
    r: { id: 's2', label: 'Section 2 — Day 72' } },
  { q: 'A user moves a task, then moves it again, then clicks Undo twice. Where does the task end up?',
    o: ['Back at the original position — two Undos reverse two moves', 'At the position after the first move — one Undo reverses the second move, then another Undo reverses the first move, landing at the original position', 'It depends on whether the redo stack was cleared', 'At the position after the second move — Undo only reverts the most recent move'],
    a: 0,
    ...
    w: { 1: 'This describes the same outcome as option A — two Undos from the second-move position lands at the original position. Both A and B describe "original position" — the question tests whether you understand the stack pop mechanism. The stack pops most-recent-first.', 2: 'The redo stack is cleared by new moves, not by Undo — so after only moves and undos, it contains the undone moves.', 3: 'Each Undo reverses one move; two Undos reverse two moves.' },
    r: { id: 's2', label: 'Section 2 — Day 74' } },
  { q: 'Celebrity threshold in a social feed: a user with 5M followers posts. What delivery strategy is used?',
    o: ['No delivery — celebrity posts require a separate subscription', 'Pull-on-read — the post is stored once; follower feeds merge celebrity posts at query time', 'Fan-out-on-write — write to all 5M follower feeds immediately', 'Push to the first 1M followers, pull for the rest'],
    a: 1,
    ...
    w: { 0: 'Celebrity posts are visible to all followers; the difference is delivery mechanism, not access.', 2: '5M writes per post for every celebrity post would saturate the write capacity of the feed service.', 3: 'Partial fan-out creates inconsistency — some followers get push, others get pull, with different latencies.' },
    r: { id: 's2', label: 'Section 2 — Day 73' } },
]

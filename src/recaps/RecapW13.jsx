...
const QUESTIONS = [
  { q: 'The arc of Week 13?',
    o: ['Only locks', 'Only the JMM', 'DIAGNOSE the disease (races: atomicity/visibility/ordering) → CURE it (immutable/atomics/locks/volatile) → COORDINATE (producer-consumer) → CHOOSE a locking strategy → APPLY it all (rate limiter)', 'Random concurrency facts'], a: 2,
    ...
    w: { 0: 'Locks are one of several tools.', 1: 'The JMM is the foundation, not the whole.', 3: 'It\'s a deliberate build.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'The three concurrency hazards (the Java Memory Model)?',
    o: ['Speed, memory, disk', 'Only races', 'Only deadlock', 'ATOMICITY (a compound op interrupted), VISIBILITY (a write not seen by other threads), ORDERING (instructions reordered) — synchronization establishes happens-before to address them'], a: 3,
    ...
    w: { 0: 'Those are performance, not correctness.', 1: 'Races are a symptom of the three.', 2: 'Deadlock is a liveness failure, separate.' },
    r: { id: 's2', label: 'Section 2 — Day 61' } },
  { q: 'The cure hierarchy (cheapest correct tool first)?',
    o: ['Always a lock', 'Always synchronized', 'volatile everywhere', 'immutable > confined > atomic (one var) > volatile (visibility-only flag) > synchronized/Lock (compound) > concurrent collection'], a: 3,
    ...
    w: { 0: 'Locks are a last resort, not first.', 1: 'Over-locking is slow and deadlock-prone.', 2: 'volatile isn\'t atomic.' },
    r: { id: 's2', label: 'Section 2 — Day 62' } },
  { q: 'synchronized vs volatile vs AtomicInteger — pick correctly:',
    o: ['synchronized = atomicity + visibility for a COMPOUND section; volatile = visibility for a single FLAG (not atomic); AtomicInteger = lock-free atomic for a single VARIABLE (CAS)', 'They are interchangeable', 'AtomicInteger needs a lock', 'volatile is for counters'], a: 0,
    ...
    w: { 1: 'They solve different problems.', 2: 'Atomics are lock-free.', 3: 'volatile count++ races.' },
    r: { id: 's2', label: 'Section 2 — Day 62' } },
  { q: 'Producer–consumer the RIGHT way?',
    o: ['A bounded BlockingQueue (put blocks when full, take blocks when empty) — and a thread pool IS this pattern (submit = produce, workers = consume)', 'A plain list', 'volatile array', 'Hand-rolled wait/notify'], a: 0,
    ...
    w: { 1: 'Not thread-safe/blocking.', 2: 'Doesn\'t coordinate put/take.', 3: 'Use the library unless you must hand-roll.' },
    r: { id: 's2', label: 'Section 2 — Day 63' } },
  { q: 'wait() must be called inside a WHILE loop because…',
    o: ['Style', 'To RE-CHECK the condition on wake — spurious wakeups happen, and another thread may have taken the item; an if would proceed on a false condition', 'if won\'t compile', 'while is faster'], a: 1,
    ...
    w: { 0: 'A correctness rule.', 2: 'Both compile; if is wrong.', 3: 'Speed is irrelevant.' },
    r: { id: 's2', label: 'Section 2 — Day 63' } },
  { q: 'Optimistic vs pessimistic locking — the deciding axis (and the third option)?',
    o: ['They are the same', 'Optimistic is always faster', 'CONTENTION decides: low → optimistic (version/CAS, no lock, retry); high → pessimistic (lock & serialize). For operations spanning user think-time, use a LEASE (atomic claim + TTL)', 'Pessimistic is always safer'], a: 2,
    ...
    w: { 0: 'Opposite bets.', 1: 'Optimistic thrashes under contention.', 3: 'Pessimistic over-serializes when conflicts are rare.' },
    r: { id: 's2', label: 'Section 2 — Day 64' } },
  { q: 'A rate limiter\'s single hardest requirement?',
    o: ['Caching', 'THREAD-SAFETY: it is shared mutable state hit by every request; refill+take is check-then-act and must be atomic (per-client lock or CAS) — pick the algorithm (token bucket/sliding window), but the concurrency is the crux', 'Logging 429s', 'Choosing the algorithm'], a: 1,
    ...
    w: { 0: 'Different concern.', 2: 'Observability, not the crux.', 3: 'Important, but the concurrency is harder.' },

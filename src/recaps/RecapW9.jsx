...
const QUESTIONS = [
  { q: 'Week 9 starts Month 3. Beyond the specific problems, what is it really teaching?',
    o: ['New patterns', 'The interview RITUAL (clarify → entities → decisions out loud → diagram + one flow → follow-ups) applied to real problems, reusing everything from Months 1–2', 'How to draw UML', 'Faster typing'], a: 1,
    ...
    w: { 0: 'No new patterns — it applies known ones.', 2: 'UML is a sub-skill, not the point.', 3: 'Method, not speed.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'Parking Lot — the patterns deployed?',
    o: ['Just classes', 'Lot ◆ Floor ◆ Spot (composition); VehicleType/SpotType enums + fit rules; guarded enum SpotState; injected SpotAllocationStrategy; Observer display boards; Ticket{spot,vehicle,entryTime}; exit-time pricing Strategy (Part 2)', 'Inheritance hierarchy of vehicles', 'A single god class'], a: 1,
    ...
    w: { 0: 'It\'s a deliberate pattern showcase.', 2: 'Vehicles are enum-typed (data), not subclassed.', 3: 'Responsibilities are distributed.' },
    r: { id: 's2', label: 'Section 2 — Parking Lot' } },
  { q: 'Bike/Car/Truck with no differing behavior — model as…',
    o: ['Three subclasses of Vehicle', 'One Vehicle class with an enum VehicleType; the enum drives spot-compatibility data — subclass only when behavior diverges', 'Three unrelated classes', 'A String type field'], a: 1,
    ...
    w: { 0: 'Empty subclasses earn nothing.', 2: 'Unrelated classes can\'t share Ticket/Spot handling.', 3: 'Stringly-typed = typo machine.' },
    r: { id: 's2', label: 'Section 2 — Parking Lot' } },
  { q: 'Tic-tac-toe: how to support "now add a third player"?',
    o: ['boolean isXTurn', 'A rotating turn QUEUE (Deque): take the front mover, put them at the back — 2 or 10 players, same one line', 'Three booleans', 'A switch on player count'], a: 1,
    ...
    w: { 0: 'A boolean holds two states only.', 2: 'Three booleans to hand-sync = bugs.', 3: 'A growing switch is the Day-12 smell.' },
  ...
  { q: 'The reusable turn-based game-loop skeleton (Snake & Ladder, Tic-Tac-Toe) has how many varying slots?',
    o: ['None — every game is different', 'TWO: (1) how a move is obtained (human choice vs forced dice roll) and (2) what the rules do — the queue, status enum, and rotation are identical', 'Five', 'It is unique per game'], a: 1,
    ...
    w: { 0: 'They share most of the loop.', 2: 'Only two slots genuinely vary.', 3: 'The skeleton generalizes.' },
  ...
  { q: 'Snake & Ladder: a player-supplied jumper config could be 24→4→...→24. Where do you catch it?',
    o: ['During the game in resolve()', 'In the Board CONSTRUCTOR — walk every chain once and reject cycles (parse, don\'t validate); then resolve() can be a simple loop', 'Never — let it loop', 'In the dice'], a: 1,
    ...
    w: { 0: 'Mid-game is the worst time to discover a broken config.', 2: 'That hangs the game.', 3: 'Dice produce numbers, not board validation.' },
    r: { id: 's2', label: 'Section 2 — Snake & Ladder' } },
  { q: 'Where should a parking fee be computed, and why?',
    o: ['At entry, stored on the ticket', 'At EXIT, by a pricing Strategy reading the ticket\'s immutable entryTime — duration is unknown until exit; money is a Money value object, never double', 'In the Vehicle class', 'By a static PriceUtil'], a: 1,
    ...
    w: { 0: 'Duration is unknown at entry.', 2: 'Vehicles carry data, not billing policy.', 3: 'Static utils are unswappable (Day 12).' },
    r: { id: 's2', label: 'Section 2 — Parking Lot' } },
  { q: 'The Week 9 machine-coding drill\'s core discipline?',
    o: ['Code as fast as possible', 'A timed dress rehearsal: clarify+scope → entity table → code BOTTOM-UP (compile every ~8 min) → happy path + one edge → self-grade against a rubric and re-drill the gaps', 'Memorize solutions', 'Skip testing'], a: 1,
    ...
    w: { 0: 'Method beats raw speed.', 2: 'Recognizing shapes beats memorizing.', 3: 'The one edge case is part of the grade.' },

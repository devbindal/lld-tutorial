...
const QUESTIONS = [
  { q: 'What do BEHAVIORAL patterns address?',
    o: ['Object creation', 'How objects INTERACT and distribute responsibility — algorithms, notifications, requests, state-driven behavior, and reusable skeletons', 'Compilation', 'Memory layout'], a: 1,
  ...
  { q: 'Strategy\'s division of labor?',
    o: ['Strategy does everything', 'The CONTEXT decides WHEN to act and holds the data; the STRATEGY decides HOW (the algorithm). Swap strategies without touching the context (OCP + DIP)', 'They are the same object', 'The context picks the algorithm with if-else'], a: 1,
    ...
    w: { 0: 'It only knows HOW, not WHEN.', 2: 'They\'re separate roles.', 3: 'That if-else is the anti-pattern.' },
    r: { id: 's2', label: 'Section 2 — Day 31' } },
  { q: 'Observer: push vs pull, and the classic hazards?',
    o: ['Always poll', 'Observers must know each other', 'No hazards', 'Push sends the changed data in the event; pull lets observers query the subject. Hazards: one observer\'s exception breaking the rest (isolate), lapsed-listener memory leaks (unsubscribe), CME during iteration (CopyOnWriteArrayList), and reentrancy'], a: 3,
    ...
    w: { 0: 'Polling is the anti-pattern Observer replaces.', 1: 'Observers are decoupled from each other.', 2: 'Several real hazards.' },
    r: { id: 's2', label: 'Section 2 — Day 32' } },
  { q: 'Command\'s four roles?',
    o: ['Just a function', 'Context + strategy', 'Command (the request object) · Receiver (does the work) · Invoker (triggers it, stays ignorant) · Client (wires command→receiver) — enabling queues, macros, and undo/redo', 'Subject + observers'], a: 2,
    ...
    w: { 0: 'It has explicit roles enabling undo/queue/macro.', 1: 'That\'s Strategy.', 3: 'That\'s Observer.' },
    r: { id: 's2', label: 'Section 2 — Day 33' } },
  { q: 'State pattern: who decides the next state?',
    o: ['Each state object names its successor (calls context.setState(...)) — one class per state, every state handles every event; the context just delegates, no switch-grid', 'A central manager', 'The client', 'The context with a switch'], a: 0,
    ...
    w: { 1: 'A manager is the switch in disguise.', 2: 'Clients fire events; they don\'t know the graph.', 3: 'A switch in the context is the disease State cures.' },
    r: { id: 's2', label: 'Section 2 — Day 34' } },
  { q: 'Template Method vs Strategy?',
    o: ['Strategy uses inheritance', 'Template Method uses composition', 'Identical', 'Template Method varies steps via INHERITANCE (subclass overrides hook methods inside a fixed skeleton); Strategy varies the whole algorithm via COMPOSITION (inject a different object)'], a: 3,
    ...
    w: { 0: 'Strategy uses composition.', 1: 'It uses inheritance.', 2: 'Different mechanism.' },
    r: { id: 's2', label: 'Section 2 — Day 35' } },
  { q: 'The Template Method "constructor trap"?',
    o: ['If the base constructor calls an overridable step, it runs against a HALF-BUILT subclass (subclass fields not yet initialized) — don\'t call overridable methods from constructors', 'It needs reflection', 'Templates can\'t have constructors', 'Constructors are banned'], a: 0,
    ...
    w: { 1: 'No reflection involved.', 2: 'They can.', 3: 'Constructors are fine, just don\'t call overridable steps from them.' },
    r: { id: 's2', label: 'Section 2 — Day 35' } },
  { q: 'You see a growing switch(currentMode) repeated inside several methods. Which pattern fixes it?',
    o: ['Strategy', 'Template Method', 'State — one class per state replaces the scattered switches; adding a mode becomes adding a class, not editing every method', 'Observer'], a: 2,
    ...
    w: { 0: 'Strategy is set once externally; this is mode-driven self-transition.', 1: 'Template Method is a fixed skeleton.', 3: 'Observer is notification.' },

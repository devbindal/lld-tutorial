...
const QUESTIONS = [
  { q: 'The single best one-line summary of Week 1\'s arc?',
    o: ['Objects are the same as classes', 'Always use inheritance', 'Define a class (state + behavior), HIDE its state behind methods (encapsulation), then REUSE/EXTEND via inheritance, vary behavior via polymorphism, and program to ABSTRACTIONS', 'Make everything public for convenience'], a: 2,
    ...
    w: { 0: 'A class is the blueprint; an object is the built instance (Day 1).',
         1: 'Inheritance is one tool, often beaten by composition (Day 7) — not an "always".',
         3: 'Public fields destroy encapsulation and invariants (Day 2).' },
  ...
  { q: 'A setter that just assigns the field (no checks) misses the main point of encapsulation, which is…',
    o: ['VALIDATION and invariants — a setter should reject invalid state (e.g. negative age), and immutable fields need no setter at all', 'Making fields public', 'Faster access', 'Shorter code'], a: 0,
    ...
    w: { 1: 'Public fields are the opposite of encapsulation.',
         2: 'Access speed is irrelevant — correctness/invariants are the point.',
         3: 'A pass-through setter is barely better than a public field.' },
    r: { id: 's2', label: 'Section 2 — Day 2' } },
  { q: 'In a subclass constructor, what happens before the subclass body runs?',
    o: ['The parent constructor runs first (constructor chaining UP) — explicitly via super(...) or implicitly via super()', 'Nothing special', 'Object\'s equals() is called', 'The subclass fields are set first'], a: 0,
    ...
    w: { 1: 'There IS a defined order — parent first, always.',
         2: 'equals() is unrelated to construction.',
         3: 'Parent construction precedes the child\'s own field init/body.' },
    r: { id: 's2', label: 'Section 2 — Day 3' } },
  { q: 'Animal a = new Dog(); a.speak(); — Dog overrides speak(). Which runs, and why?',
    o: ['Compile error', 'Dog.speak() — dynamic dispatch picks the method of the ACTUAL object type at runtime (the vtable)', 'Animal.speak() — a is an Animal reference', 'Both run'], a: 1,
    ...
    w: { 0: 'It compiles fine — speak() exists on Animal.',
         2: 'The reference type gates the API, but the override is chosen by the real type.',
         3: 'Exactly one method runs — Dog\'s override.' },
    r: { id: 's2', label: 'Section 2 — Day 4' } },
  { q: 'Downcasting (Animal → Dog) is safe only when…',
    o: ['Never', 'Always', 'The object is actually a Dog — guard with instanceof, or risk a ClassCastException', 'The reference is final'], a: 2,
    ...
    w: { 0: 'It\'s safe when guarded — not never.',
         1: 'Blind downcasts crash when the object isn\'t that subtype.',
         3: 'final affects reassignment, not cast safety.' },
    r: { id: 's2', label: 'Section 2 — Day 4' } },
  { q: 'Interface vs abstract class — the cleanest decision rule?',
    o: ['Always use abstract classes', 'Always use interfaces', 'They\'re interchangeable', 'Interface = a capability many unrelated types can have (implement many); abstract class = a shared base WITH state and common code (extend one). Need shared fields/implementation → abstract class; need a contract / multiple types → interface'], a: 3,
    ...
    w: { 0: 'Abstract-only blocks multiple-capability modeling.',
         1: 'Interface-only can\'t hold shared mutable state.',
         2: 'They overlap but differ in state, multiplicity, and intent.' },
    r: { id: 's2', label: 'Section 2 — Day 5' } },
  { q: 'You override equals() but not hashCode(). What breaks?',
    o: ['Nothing', 'The code won\'t compile', 'toString() breaks', 'Hash-based collections (HashMap/HashSet) misbehave — equal objects can land in different buckets, so lookups/dedup fail; equals and hashCode must be consistent'], a: 3,
    ...
    w: { 0: 'It silently corrupts hash-based collection behavior.',
         1: 'It compiles — the bug is at runtime.',
         2: 'toString is independent of the equals/hashCode contract.' },

...
const QUESTIONS = [
  { q: 'What do all five CREATIONAL patterns share?',
    o: ['They make code faster', 'They replace constructors entirely', 'They are the same pattern', 'They control HOW objects are created — hiding/centralizing construction so client code depends on abstractions, not on new ConcreteClass()'], a: 3,
    ...
    w: { 0: 'They\'re about structure/coupling, not speed.', 1: 'They wrap construction, not abolish it.', 2: 'Five distinct intents.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'The safest, simplest Singleton implementations?',
    o: ['The holder idiom (lazy, thread-safe via class loading) and the ENUM singleton (also reflection- and serialization-proof) — and double-checked locking REQUIRES volatile', 'Eager static field only', 'Plain lazy init', 'A global variable'], a: 0,
    ...
    w: { 1: 'Eager works but isn\'t lazy; not the "safest+lazy" answer.', 2: 'Plain lazy is racy.', 3: 'A global is the anti-pattern Singleton is criticized for becoming.' },
    r: { id: 's2', label: 'Section 2 — Day 21' } },
  { q: 'Factory Method vs Abstract Factory?',
    o: ['Factory Method makes families', 'Same thing', 'Factory Method = ONE product via an overridable hook (subclass decides the concrete type); Abstract Factory = a FAMILY of related products via a kit of factory methods (consistency by construction)', 'Abstract Factory is older'], a: 2,
    ...
    w: { 0: 'That\'s Abstract Factory\'s job.', 1: 'Different scope.', 3: 'Age is irrelevant.' },
    r: { id: 's2', label: 'Section 2 — Days 22-23' } },
  { q: 'Abstract Factory\'s key trade-off?',
    o: ['Adding a new family MEMBER (theme) is cheap; adding a new product TYPE is expensive — every factory must gain a method', 'It is always free to extend', 'It can\'t make families', 'It needs reflection'], a: 0,
    ...
    w: { 1: 'One axis is expensive.', 2: 'Families are its purpose.', 3: 'No reflection needed.' },
    r: { id: 's2', label: 'Section 2 — Day 23' } },
  { q: 'The Effective Java Builder beats telescoping constructors AND JavaBeans setters because…',
    o: ['It is shorter', 'It avoids all constructors', 'It is readable for many (optional) params AND produces an IMMUTABLE product — no half-built object (JavaBeans) and no positional/boolean-swap bugs (telescoping)', 'It is required by records'], a: 2,
    ...
    w: { 0: 'It\'s often longer — the win is safety/readability.', 1: 'It uses a private constructor.', 3: 'Records and builders are complementary.' },
    r: { id: 's2', label: 'Section 2 — Day 24' } },
  { q: 'Why prefer a copy constructor over Cloneable in Java?',
    o: ['Cloneable is faster', 'They are identical', 'Copy constructors are required', 'Cloneable is a broken marker interface: clone() runs NO constructor, is shallow by default, and is awkward to override correctly — a copy constructor (new Foo(other)) is explicit and safe'], a: 3,
    ...
    w: { 0: 'Speed isn\'t the issue — correctness/clarity is.', 1: 'They differ sharply in safety.', 2: 'Neither is required.' },
    r: { id: 's2', label: 'Section 2 — Day 25' } },
  { q: 'Shallow vs deep copy matters when the object holds…',
    o: ['Nothing', 'Mutable REFERENCE fields — a shallow copy shares them (both copies see mutations); a deep copy clones them too. Immutable fields can be shared safely', 'A name', 'Only primitives'], a: 1,
    ...
    w: { 0: 'It matters with mutable refs.', 2: 'Strings are immutable.', 3: 'Primitives copy by value — no issue.' },
    r: { id: 's2', label: 'Section 2 — Day 25' } },
  { q: 'The recurring criticism of Singleton (and the modern alternative)?',
    o: ['It is too fast', 'It is global mutable state — hard to test, hidden coupling; prefer wiring ONE instance via dependency injection at the composition root instead of getInstance() everywhere', 'It can\'t be lazy', 'It uses too much memory'], a: 1,
    ...
    w: { 0: 'Speed isn\'t the criticism.', 2: 'It can be lazy (holder idiom).', 3: 'One object isn\'t a memory problem.' },
    r: { id: 's2', label: 'Section 2 — Day 21' } },
]

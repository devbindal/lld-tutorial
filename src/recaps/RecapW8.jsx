...
const QUESTIONS = [
  { q: 'Behavioral II completes the GoF catalog. What ties these five together?',
    o: ['They create objects', 'They manage interaction & responsibility in specific ways: pass requests along (Chain), traverse (Iterator), centralize talk (Mediator), capture/restore state (Memento), add operations over structures (Visitor)', 'They are structural', 'They are the same pattern'], a: 1,
    ...
    w: { 0: 'That\'s creational.', 2: 'They\'re behavioral.', 3: 'Five distinct intents.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'Chain of Responsibility: what must every passing handler do?',
    o: ['Always handle the request', 'Either handle it OR forward to the next handler — forgetting the forward silently drops the request (the deleted-link gap)', 'Throw an exception', 'Return null'], a: 1,
    ...
    w: { 0: 'Only the right handler handles; others forward.', 2: 'It forwards, not throws.', 3: 'It forwards, not returns null.' },
    r: { id: 's2', label: 'Section 2 — Day 36' } },
  { q: 'Fail-fast vs weakly-consistent iterators?',
    o: ['Same thing', 'Fail-fast (ArrayList/HashMap) throws ConcurrentModificationException if the collection is structurally modified during iteration (via modCount); weakly-consistent (ConcurrentHashMap, CopyOnWriteArrayList) tolerate concurrent modification without throwing', 'Both throw CME', 'Neither detects changes'], a: 1,
    ...
    w: { 0: 'Opposite behaviors.', 2: 'Weakly-consistent don\'t throw.', 3: 'Fail-fast detects.' },
    r: { id: 's2', label: 'Section 2 — Day 37' } },
  { q: 'Mediator vs Observer?',
    o: ['Identical', 'Mediator CENTRALIZES many-to-many interaction (colleagues coordinate THROUGH a hub that knows them); Observer is one-to-many BROADCAST (subject doesn\'t know/coordinate observers)', 'Mediator broadcasts', 'Observer centralizes'], a: 1,
    ...
    w: { 0: 'Different intent.', 2: 'That\'s Observer.', 3: 'That\'s Mediator.' },
    r: { id: 's2', label: 'Section 2 — Day 38' } },
  { q: 'Memento keeps the saved state OPAQUE to the Caretaker because…',
    o: ['It is encrypted', 'Only the Originator can read/write the memento\'s contents (e.g. via a narrow interface / inner class); the Caretaker just stores and returns it — preserving encapsulation', 'It is compressed', 'The Caretaker is trusted'], a: 1,
    ...
    w: { 0: 'Not encryption — access scoping.', 2: 'Not compression.', 3: 'The Caretaker is deliberately kept ignorant.' },
    r: { id: 's2', label: 'Section 2 — Day 39' } },
  { q: 'Visitor and the "Expression Problem"?',
    o: ['It has no trade-off', 'Visitor makes adding new OPERATIONS easy (one new visitor) but adding new element TYPES hard (every visitor must gain a method) — the two-axis Expression Problem; the reverse of a normal class hierarchy', 'It makes both easy', 'It makes both hard'], a: 1,
    ...
    w: { 0: 'It has a sharp two-axis trade-off.', 2: 'Types are hard with Visitor.', 3: 'Operations are easy with Visitor.' },
    r: { id: 's2', label: 'Section 2 — Day 40' } },
  { q: 'Why does Visitor need accept()/visit() (double dispatch) instead of just overloading?',
    o: ['For speed', 'Java overload resolution is by STATIC type, so visit(element) wouldn\'t pick the right overload for a Component reference — element.accept(visitor) dispatches on the element\'s real type, then visitor.visit(this) dispatches on the visitor: two dynamic dispatches', 'For memory', 'It does not need it'], a: 1,
    ...
    w: { 0: 'Not performance.', 2: 'Not memory.', 3: 'It genuinely needs it in Java.' },
    r: { id: 's2', label: 'Section 2 — Day 40' } },
  { q: 'A servlet FilterChain calling chain.doFilter(req, res) is which pattern, in which flavor?',
    o: ['Mediator', 'Chain of Responsibility — the PIPELINE flavor (every filter processes and passes along, rather than exactly one handling it)', 'Iterator', 'Visitor'], a: 1,
    ...
    w: { 0: 'No central hub.', 2: 'Not a traversal cursor.', 3: 'Not operations over a hierarchy.' },

...
const QUESTIONS = [
  { q: 'The one-line arc of Week 2?',
    o: ['UML is mandatory in all code', 'Model how objects RELATE (association/aggregation/composition), prefer composition over inheritance, DRAW it (class + sequence UML), and DERIVE entities from requirements via noun-verb analysis', 'Inheritance solves everything', 'Draw lots of diagrams'], a: 1,
    ...
    w: { 0: 'UML is a communication aid, used as needed.', 2: 'Composition usually beats inheritance (Day 7).', 3: 'Diagrams are a tool for thinking/communicating, not the goal.' },
    r: { id: 's1', label: 'Section 1' } },
  { q: 'Aggregation vs composition — the deciding test?',
    o: ['Composition has no multiplicity', 'Does the PART share/outlive the whole (aggregation, hollow ◇) or is it owned and die with the whole (composition, filled ◆)?', 'They are identical', 'Aggregation is bigger'], a: 1,
    ...
    w: { 0: 'Both carry multiplicities.', 2: 'They model different ownership.', 3: 'Size is irrelevant — lifecycle ownership decides.' },
    r: { id: 's2', label: 'Section 2 — Day 6' } },
  { q: 'The coupling ladder, loosest to tightest?',
    o: ['Composition is tightest', 'Inheritance < composition < dependency', 'Dependency (uses-a) < composition/delegation < inheritance (is-a) — inheritance is the TIGHTEST coupling', 'They are all equal'], a: 2,
    ...
    w: { 0: 'Inheritance, not composition, is tightest.', 1: 'Reversed — inheritance is tightest.', 3: 'They differ sharply.' },
  ...
  { q: 'The six UML class-diagram arrows include inheritance and realization. How do they differ visually?',
    o: ['Inheritance is dashed', 'They look the same', 'Realization uses a diamond', 'Both use a hollow triangle head, but inheritance (extends) is a SOLID line and realization (implements) is a DASHED line'], a: 3,
    ...
    w: { 0: 'Inheritance is solid.', 1: 'The line style distinguishes them.', 2: 'Diamonds are for aggregation/composition.' },
    r: { id: 's2', label: 'Section 2 — Day 8' } },
  { q: 'A sequence diagram\'s "alt" fragment represents…',
    o: ['An if/else branch (alternative paths based on a guard)', 'A loop', 'Object creation', 'A return'], a: 0,
    ...
    w: { 1: 'That\'s loop.', 2: 'That\'s «create».', 3: 'Returns are dashed arrows.' },
    r: { id: 's2', label: 'Section 2 — Day 9' } },
  { q: 'In noun-verb analysis, "Information Expert" means…',
    o: ['Make everything static', 'Hire an expert', 'Assign a responsibility to the class that already HAS the data needed to fulfill it', 'Add a Manager class'], a: 2,
    ...
    w: { 0: 'Static has nothing to do with it.', 1: '🙂 It\'s a design heuristic.', 3: 'Manager/god classes are the trap it avoids.' },
    r: { id: 's2', label: 'Section 2 — Day 10' } },
  { q: 'A bidirectional association (both sides reference each other) mainly costs you…',
    o: ['Nothing', 'Memory only', 'Compile errors', 'Consistency maintenance — both references must be kept in sync, and it tightens coupling; add it only when a real operation needs both directions'], a: 3,
    ...
    w: { 0: 'It has a real maintenance/coupling cost.', 1: 'The bug risk (drift), not memory, is the cost.', 2: 'It compiles fine.' },
    r: { id: 's2', label: 'Section 2 — Day 6' } },
  { q: 'The biggest requirements-modeling trap?',
    o: ['The GOD CLASS (a "Manager/System" that does everything) and over-modeling (YAGNI) — model the nouns the problem actually needs, with behavior distributed by Information Expert', 'Drawing UML', 'Using enums', 'Too few classes'], a: 0,
    ...
    w: { 1: 'UML helps, not hurts.', 2: 'Enums are good for fixed sets.', 3: 'Too FEW well-chosen classes is fine; the trap is one doing everything.' },
